...
import net.daporkchop.lib.primitive.lambda.function.bi.ShortByteToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of short => byte mappings. Implementing classes should be
 * thread safe and concurrent (calling advance() should throw {@link IteratorCompleteException} instead of (short) 0,
 ...
 * @author DaPorkchop_
 */
public interface ShortByteConcurrentIterator extends ShortByteIterator   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ShortByteToBooleanFunction function)    {
        try {
            while (this.hasNext())  {
                ShortByteTuple tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<ShortByteTuple> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<ShortByteTuple> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ShortByteConsumer consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<ShortByteTuple> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull ShortByteToBooleanFunction function)    {
        this.concurrentIterate(function);
    }
}
