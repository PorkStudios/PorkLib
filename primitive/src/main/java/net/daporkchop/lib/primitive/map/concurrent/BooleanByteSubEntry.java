...
/**
 * A single subentry for a {@link BooleanByteConcurrentTreeHashMap}.
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
class BooleanByteSubEntry    {
    public final AtomicInteger size = new AtomicInteger(0);
    public final BooleanByteConcurrentTreeHashMap parent;
    ...
    public BooleanByteSubEntry(BooleanByteConcurrentTreeHashMap parent)   {
        this.parent = parent;
        this.rootPointer = this.parent.subHashLength == 0 ? new BooleanByteSubEntry.ValueSector(null, -1) : new BooleanByteSubEntry.PointerSector(null, -1);
    }
    ...
    public void forEachKey(BooleanConsumer consumer)   {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof BooleanByteSubEntry.PointerSector)    {
    ...
    public boolean advanceKey(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext)   {
        ...
        for (int i = sector.states.nextSetBit(curr.get() + 1); i != -1; i = sector.states.nextSetBit(i + 1)) {
            curr.set(i);
            if (sector instanceof BooleanByteSubEntry.ValueSector)  {
    ...
    public void advanceEntry(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext, BooleanByteMutableTuple tuple)   {
        Sector sector = sectorRef.get();
        if (sector == null) {
            sectorRef.set(sector = this.rootPointer);
        }
    ...
    public Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.parent.subHashLength; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.parent.subHashLength ? new BooleanByteSubEntry.ValueSector(curr, off) : new BooleanByteSubEntry.PointerSector(curr, off);
    ...
    public abstract class Sector  {
        ...
        public byte setEntry(int index, boolean key, byte value)   {
            throw new UnsupportedOperationException();
        }
    ...
    public class ValueSector extends Sector   {
        ...
        public byte setEntry(int index, boolean key, byte value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                BooleanByteSubEntry.this.size.incrementAndGet();
                BooleanByteSubEntry.this.parent.size.incrementAndGet();
                return (byte) 0;
            }
        }
        ...
        public byte removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                BooleanByteSubEntry.this.size.decrementAndGet();
                BooleanByteSubEntry.this.parent.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
