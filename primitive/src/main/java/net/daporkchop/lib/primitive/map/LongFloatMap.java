...
import net.daporkchop.lib.primitive.lambda.function.LongToFloatFunction;
...
/**
 * A map, using a key type of long and a value type of float.
 * <p>
...
public interface LongFloatMap   {
     ...
     */
    default void putAll(@NonNull long[] keys, @NonNull float[] values)    {
     ...
     */
    default void putAll(@NonNull LongFloatMap map)    {
     ...
     * @param consumer the function to run
     */
    void forEachEntry(LongFloatConsumer consumer);
     ...
     */
    LongFloatIterator entryIterator();
     ...
     */
    default float getOrDefault(long key, float def)   {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
            this.put(key, def);
     ...
     */
    default float computeIfAbsent(long key, @NonNull LongToFloatFunction func) {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
