...
/**
 * A hash map, using a key type of byte and a value type of boolean.
 ...
 * @author DaPorkchop_
 */
public class ByteBooleanHashMap implements ByteBooleanMap    {
    protected byte[] keys;
    ...
    public ByteBooleanHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public ByteBooleanHashMap(ByteToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public ByteBooleanHashMap(int baseSize, ByteToIntegerFunction keyHash)    {
    ...
    protected boolean reHash(boolean[] values, byte[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
    ...
    private class KeyIterator implements ByteIterator {
        ...
        public KeyIterator()    {
            this.size = ByteBooleanHashMap.this.size;
            this.next = ByteBooleanHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public byte get()   {
            return ByteBooleanHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = ByteBooleanHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            ByteBooleanHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements BooleanIterator {
        ...
        @SuppressWarnings("unchecked")
        public boolean get()   {
            return ByteBooleanHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements ByteBooleanIterator {
        private int size;
        private int index = -1;
        private int next;
        private ByteBooleanTuple tuple;
        ...
        @Override
        public ByteBooleanTuple advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new ByteBooleanImmutableTuple( ByteBooleanHashMap.this.keys[this.index],  ByteBooleanHashMap.this.values[this.index]);
        }
