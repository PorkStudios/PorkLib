...
import net.daporkchop.lib.primitive.lambda.function.bi.ShortCharacterToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of short => char mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface ShortCharacterConcurrentIterator extends ShortCharacterIterator   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ShortCharacterToBooleanFunction function)    {
        try {
            while (this.hasNext())  {
                ShortCharacterTuple tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<ShortCharacterTuple> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<ShortCharacterTuple> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ShortCharacterConsumer consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<ShortCharacterTuple> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull ShortCharacterToBooleanFunction function)    {
        this.concurrentIterate(function);
    }
}
