...
/**
 * A tree map, using a key type of float and a value type of short.
 ...
 * Although this map WORKS, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.FloatShortIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class FloatShortTreeMap implements FloatShortMap    {
    ...
    @SuppressWarnings("unchecked")
    protected short remove(float key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return (short) 0; //no mapping
    ...
    public FloatIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.FloatShortIndexedTreeMap} for an implementation of this method
        return null;
    }
