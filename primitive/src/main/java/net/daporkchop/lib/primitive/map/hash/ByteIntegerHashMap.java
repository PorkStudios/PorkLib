...
import net.daporkchop.lib.primitive.iterator.bi.ByteIntegerIterator;
...
/**
 * A hash map, using a key type of byte and a value type of int.
 ...
 * @author DaPorkchop_
 */
public class ByteIntegerHashMap implements ByteIntegerMap    {
    protected byte[] keys;
    ...
    protected int mask;
    protected int size;
    protected final ByteToIntegerFunction keyHash;

    public ByteIntegerHashMap()    {
        this(16384);
    }

    public ByteIntegerHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public ByteIntegerHashMap(ByteToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public ByteIntegerHashMap(int baseSize, ByteToIntegerFunction keyHash)    {
    ...
    @Override
    public void forEachEntry(@NonNull ByteIntegerConsumer consumer)   {
    ...
    public ByteIntegerIterator entryIterator() {
    ...
    protected boolean reHash(int[] values, byte[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
    ...
    private class KeyIterator implements ByteIterator {
        private int size;
        private int index = -1;
        private int next;

        public KeyIterator()    {
            this.size = ByteIntegerHashMap.this.size;
            this.next = ByteIntegerHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public byte get()   {
            return ByteIntegerHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = ByteIntegerHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            ByteIntegerHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements IntegerIterator {
        ...
        @SuppressWarnings("unchecked")
        public int get()   {
            return ByteIntegerHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements ByteIntegerIterator {
        private int size;
        private int index = -1;
        private int next;
        private ByteIntegerTuple tuple;
        ...
        @Override
        public ByteIntegerTuple advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new ByteIntegerImmutableTuple( ByteIntegerHashMap.this.keys[this.index],  ByteIntegerHashMap.this.values[this.index]);
        }
