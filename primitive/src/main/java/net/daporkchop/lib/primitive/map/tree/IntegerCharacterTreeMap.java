 ...
 * Although this map WORKS, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.IntegerCharacterIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class IntegerCharacterTreeMap implements IntegerCharacterMap    {
    protected int[] keys;
    ...
    public IntegerCharacterTreeMap()    {
        this.clear();
    }
    ...
    public char get(int key)    {
        ...
        if (!containsKeyHash(hash)) return (char) 0; //no mapping
    ...
    @SuppressWarnings("unchecked")
    protected char remove(int key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return (char) 0; //no mapping
    ...
    public IntegerIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.IntegerCharacterIndexedTreeMap} for an implementation of this method
        return null;
    }
