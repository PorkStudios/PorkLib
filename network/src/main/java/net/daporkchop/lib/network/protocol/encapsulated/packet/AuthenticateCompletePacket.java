 ...
 * @author DaPorkchop_
 */
public class AuthenticateCompletePacket implements EncapsulatedPacket, EncapsulatedConstants {
    @Override
    ...
    @Override
    public byte getId() {
        return ID_AUTHENTICATECOMPLETE;
    }

    public static class AuthenticateCompleteHandler implements PacketHandler<AuthenticateCompletePacket, EncapsulatedSession> {
        @Override
        public void handle(AuthenticateCompletePacket packet, EncapsulatedSession session) {
            ...
            ((Client) session.getEndpoint()).getCompletableFuture().complete(SessionData.PROTOCOL_SESSION.get(session.getSession()));
        }
    }
}
