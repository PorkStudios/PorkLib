 ...
 * Although this map WORKS, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.LongDoubleIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class LongDoubleTreeMap implements LongDoubleMap    {
    protected long[] keys;
    ...
    @SuppressWarnings("unchecked")
    protected double remove(long key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return 0D; //no mapping
    ...
    public LongIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.LongDoubleIndexedTreeMap} for an implementation of this method
        return null;
    }
