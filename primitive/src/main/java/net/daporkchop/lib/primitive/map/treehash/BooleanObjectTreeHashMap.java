 ...
 * overhead, and can even be made concurrent with little extra work (see {@link net.daporkchop.lib.primitive.map.concurrent.BooleanObjectConcurrentTreeHashMap})
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
public class BooleanObjectTreeHashMap<V extends Object> implements BooleanObjectMap<V>    {
    ...
    public BooleanObjectTreeHashMap()    {
        this(null, 0);
    }

    public BooleanObjectTreeHashMap(BooleanToLongFunction hashFunc, int hashLen)    {
        ...
        this.rootPointer = this.hashLen == 0 ? new BooleanObjectTreeHashMap<V>.ValueSector(null, -1) : new BooleanObjectTreeHashMap<V>.PointerSector(null, -1);
    }
    ...
    @Override
    public void clear() {
        this.rootPointer = this.hashLen == 0 ? new BooleanObjectTreeHashMap.ValueSector(null, -1) : new BooleanObjectTreeHashMap.PointerSector(null, -1);
    }
    ...
    public void forEachKey(@NonNull BooleanConsumer consumer)  {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof BooleanObjectTreeHashMap.PointerSector)    {
    ...
    private Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.hashLen; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.hashLen ? new BooleanObjectTreeHashMap<V>.ValueSector(curr, off) : new BooleanObjectTreeHashMap<V>.PointerSector(curr, off);
    ...
    private class ValueSector extends Sector   {
        ...
        public Object setEntry(int index, boolean key, Object value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                BooleanObjectTreeHashMap.this.size.incrementAndGet();
                return null;
            }
        }
        ...
        public Object removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                BooleanObjectTreeHashMap.this.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
