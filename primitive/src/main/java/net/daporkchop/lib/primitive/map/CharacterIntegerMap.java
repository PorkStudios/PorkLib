 ...
 * @author DaPorkchop_
 */
public interface CharacterIntegerMap   {
    /**
     ...
     */
    default void putAll(@NonNull char[] keys, @NonNull int[] values)    {
     ...
     */
    default void putAll(@NonNull CharacterIntegerMap map)    {
     ...
     * @param consumer the function to run
     */
    void forEachEntry(CharacterIntegerConsumer consumer);
     ...
     */
    CharacterIntegerIterator entryIterator();
     ...
     */
    default int getOrDefault(char key, int def)   {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
            this.put(key, def);
     ...
     */
    default int computeIfAbsent(char key, @NonNull CharacterToIntegerFunction func) {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
