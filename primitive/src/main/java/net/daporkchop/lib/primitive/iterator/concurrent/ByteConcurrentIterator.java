...
/**
 * Allows parallel iteration over an iterator of byte values. Implementing classes should be thread safe
 * and concurrent (calling advance() should throw {@link IteratorCompleteException} instead of (byte) 0,
 ...
 * @author DaPorkchop_
 */
public interface ByteConcurrentIterator extends ByteIterator   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ByteToBooleanFunction function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ByteConsumer consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ByteToBooleanFunction function)   {
