...
public class BlowFishKeyGen {
    ...
    public static BlowFishKey gen(byte[] data) {
        byte[] b = new byte[BlockCipherType.BLOWFISH.blockSize];
        int i = 0;
        do {
        ...
        data = b;
        return new BlowFishKey(new SecretKeySpec(data, "aaa"), KeyRandom.getBytes(BlockCipherType.BLOWFISH.blockSize));
    }

    public static BlowFishKey gen() {
        byte[] key = new byte[BlockCipherType.BLOWFISH.blockSize];
