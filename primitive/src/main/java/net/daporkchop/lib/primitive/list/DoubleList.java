...
public interface DoubleList   {
     ...
     * @return the element at to the given index, or 0D if no such mapping exists
     */
     ...
     */
    default void addAll(@NonNull double[] values)    {
        for (double p : values)   {
            this.add(p);
        }
    }
     ...
     */
    default void addAll(@NonNull DoubleList list)    {
     ...
     */
    default void removeAll(@NonNull Collection<Double> values) {
