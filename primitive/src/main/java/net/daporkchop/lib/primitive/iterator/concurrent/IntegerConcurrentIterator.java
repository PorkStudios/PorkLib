...
/**
 * Allows parallel iteration over an iterator of int values. Implementing classes should be thread safe
 * and concurrent (calling advance() should throw {@link IteratorCompleteException} instead of 0,
 ...
 * @author DaPorkchop_
 */
public interface IntegerConcurrentIterator extends IntegerIterator   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull IntegerToBooleanFunction function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull IntegerConsumer consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull IntegerToBooleanFunction function)   {
