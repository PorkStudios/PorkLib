...
import net.daporkchop.lib.primitive.tuple.LongByteImmutableTuple;
...
/**
 * A hash map, using a key type of long and a value type of byte.
 ...
 * @author DaPorkchop_
 */
public class LongByteHashMap implements LongByteMap    {
    protected long[] keys;
    ...
    public LongByteHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public LongByteHashMap(LongToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public LongByteHashMap(int baseSize, LongToIntegerFunction keyHash)    {
    ...
    protected boolean reHash(byte[] values, long[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
    ...
    private class KeyIterator implements LongIterator {
        ...
        public KeyIterator()    {
            this.size = LongByteHashMap.this.size;
            this.next = LongByteHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public long get()   {
            return LongByteHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = LongByteHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            LongByteHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements ByteIterator {
        ...
        @SuppressWarnings("unchecked")
        public byte get()   {
            return LongByteHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements LongByteIterator {
        private int size;
        private int index = -1;
        private int next;
        private LongByteTuple tuple;
        ...
        @Override
        public LongByteTuple advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new LongByteImmutableTuple( LongByteHashMap.this.keys[this.index],  LongByteHashMap.this.values[this.index]);
        }
