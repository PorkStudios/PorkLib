 ...
 * overhead, and can even be made concurrent with little extra work (see {@link net.daporkchop.lib.primitive.map.concurrent.ByteObjectConcurrentTreeHashMap})
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
public class ByteObjectTreeHashMap<V extends Object> implements ByteObjectMap<V>    {
    ...
    public ByteObjectTreeHashMap()    {
        this(null, 0);
    }

    public ByteObjectTreeHashMap(ByteToLongFunction hashFunc, int hashLen)    {
        ...
        this.rootPointer = this.hashLen == 0 ? new ByteObjectTreeHashMap<V>.ValueSector(null, -1) : new ByteObjectTreeHashMap<V>.PointerSector(null, -1);
    }
    ...
    @Override
    public void clear() {
        this.rootPointer = this.hashLen == 0 ? new ByteObjectTreeHashMap.ValueSector(null, -1) : new ByteObjectTreeHashMap.PointerSector(null, -1);
    }
    ...
    public void forEachKey(@NonNull ByteConsumer consumer)  {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof ByteObjectTreeHashMap.PointerSector)    {
    ...
    private Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.hashLen; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.hashLen ? new ByteObjectTreeHashMap<V>.ValueSector(curr, off) : new ByteObjectTreeHashMap<V>.PointerSector(curr, off);
    ...
    private class ValueSector extends Sector   {
        ...
        public Object setEntry(int index, byte key, Object value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                ByteObjectTreeHashMap.this.size.incrementAndGet();
                return null;
            }
        }
        ...
        public Object removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                ByteObjectTreeHashMap.this.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
