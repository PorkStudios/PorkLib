 ...
 * Although this map 𝙒𝙊𝙍𝙆𝙎, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.IntegerShortIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class IntegerShortTreeMap implements IntegerShortMap    {
    protected int[] keys;
    ...
    @SuppressWarnings("unchecked")
    protected short remove(int key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return (short) 0; //no mapping
    ...
    public IntegerIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.IntegerShortIndexedTreeMap} for an implementation of this method
        return null;
    }
