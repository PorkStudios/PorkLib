...
/**
 * A single subentry for a {@link IntegerDoubleConcurrentTreeHashMap}.
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
class IntegerDoubleSubEntry    {
    public final AtomicInteger size = new AtomicInteger(0);
    public final IntegerDoubleConcurrentTreeHashMap parent;
    ...
    public IntegerDoubleSubEntry(IntegerDoubleConcurrentTreeHashMap parent)   {
        this.parent = parent;
        this.rootPointer = this.parent.subHashLength == 0 ? new IntegerDoubleSubEntry.ValueSector(null, -1) : new IntegerDoubleSubEntry.PointerSector(null, -1);
    }
    ...
    public void forEachKey(IntegerConsumer consumer)   {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof IntegerDoubleSubEntry.PointerSector)    {
    ...
    public int advanceKey(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext)   {
        ...
        for (int i = sector.states.nextSetBit(curr.get() + 1); i != -1; i = sector.states.nextSetBit(i + 1)) {
            curr.set(i);
            if (sector instanceof IntegerDoubleSubEntry.ValueSector)  {
    ...
    public void advanceEntry(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext, IntegerDoubleMutableTuple tuple)   {
        Sector sector = sectorRef.get();
        if (sector == null) {
            sectorRef.set(sector = this.rootPointer);
        }
    ...
    public Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.parent.subHashLength; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.parent.subHashLength ? new IntegerDoubleSubEntry.ValueSector(curr, off) : new IntegerDoubleSubEntry.PointerSector(curr, off);
    ...
    public abstract class Sector  {
        ...
        public double setEntry(int index, int key, double value)   {
            throw new UnsupportedOperationException();
        }
    ...
    public class ValueSector extends Sector   {
        ...
        public double setEntry(int index, int key, double value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                IntegerDoubleSubEntry.this.size.incrementAndGet();
                IntegerDoubleSubEntry.this.parent.size.incrementAndGet();
                return 0D;
            }
        }
        ...
        public double removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                IntegerDoubleSubEntry.this.size.decrementAndGet();
                IntegerDoubleSubEntry.this.parent.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
