...
import net.daporkchop.lib.primitive.map.tree.ObjectFloatTreeMap;
...
/**
 * A tree map, using a key type of K and a value type of float.
 ...
 * @author DaPorkchop_
 */
public class ObjectFloatIndexedTreeMap<K extends Object> extends ObjectFloatTreeMap<K>    {
    protected boolean[] index;

    public ObjectFloatIndexedTreeMap()    {
        super();
    }
    ...
    public void forEachKey(@NonNull ObjectConsumer<K> consumer)   {
        ...
        for (int i = 0; i < index.length; i++)  {
            if (index[i]) consumer.accept( (K) keys[i]);
        }
    }
    ...
    public void forEachEntry(@NonNull ObjectFloatConsumer<K> consumer)   {
        ...
        //scan through the index until -1 is reached
        for (int i = 0; i < index.length; i++)  {
            if (index[i]) consumer.accept( (K) keys[i], values[i]);
        }
    }
    ...
    private class KeyIterator implements ObjectIterator<K>   {
        ...
        private final ObjectFloatIndexedTreeMap this_ = ObjectFloatIndexedTreeMap.this;
        ...
        @Override
        public K get()  {
            return (K) this_.keys[this.index];
        }
    ...
    private class EntryIterator implements ObjectFloatIterator<K>   {
        ...
        public ObjectFloatTuple<K> advance()   {
            if (this.hasNext()) {
                this.findNext();
                return this.tuple = new ObjectFloatImmutableTuple<K>( (K) this_.keys[this.index],  this_.values[this.index]);
            } else {
                throw new ArrayIndexOutOfBoundsException();
            }
        }
