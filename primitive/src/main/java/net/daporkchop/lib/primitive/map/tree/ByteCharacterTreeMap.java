 ...
 * Although this map WORKS, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.ByteCharacterIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class ByteCharacterTreeMap implements ByteCharacterMap    {
    protected byte[] keys;
    ...
    @SuppressWarnings("unchecked")
    protected char remove(byte key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return (char) 0; //no mapping
    ...
    public ByteIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.ByteCharacterIndexedTreeMap} for an implementation of this method
        return null;
    }
