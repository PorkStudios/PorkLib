...
import net.daporkchop.lib.primitive.lambda.function.bi.LongByteToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of long => byte mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface LongByteConcurrentIterator extends LongByteIterator   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull LongByteToBooleanFunction function)    {
        try {
            while (this.hasNext())  {
                LongByteTuple tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<LongByteTuple> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<LongByteTuple> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull LongByteConsumer consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<LongByteTuple> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull LongByteToBooleanFunction function)    {
        this.concurrentIterate(function);
    }
}
