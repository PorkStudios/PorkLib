...
/**
 * A hash map, using a key type of float and a value type of short.
 ...
 * @author DaPorkchop_
 */
public class FloatShortHashMap implements FloatShortMap    {
    ...
    public FloatShortHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public FloatShortHashMap(FloatToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public FloatShortHashMap(int baseSize, FloatToIntegerFunction keyHash)    {
    ...
    protected boolean reHash(short[] values, float[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
    ...
    private class KeyIterator implements FloatIterator {
        ...
        public KeyIterator()    {
            this.size = FloatShortHashMap.this.size;
            this.next = FloatShortHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public float get()   {
            return FloatShortHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = FloatShortHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            FloatShortHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements ShortIterator {
        ...
        @SuppressWarnings("unchecked")
        public short get()   {
            return FloatShortHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements FloatShortIterator {
        private int size;
        private int index = -1;
        private int next;
        private FloatShortTuple tuple;
        ...
        @Override
        public FloatShortTuple advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new FloatShortImmutableTuple( FloatShortHashMap.this.keys[this.index],  FloatShortHashMap.this.values[this.index]);
        }
