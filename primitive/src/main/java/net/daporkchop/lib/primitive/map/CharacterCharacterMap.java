...
import net.daporkchop.lib.primitive.lambda.function.CharacterToCharacterFunction;
...
/**
 * A map, using a key type of char and a value type of char.
 * <p>
...
public interface CharacterCharacterMap   {
     ...
     */
    default void putAll(@NonNull char[] keys, @NonNull char[] values)    {
     ...
     */
    default void putAll(@NonNull CharacterCharacterMap map)    {
     ...
     * @param consumer the function to run
     */
    void forEachEntry(CharacterCharacterConsumer consumer);
     ...
     */
    CharacterCharacterIterator entryIterator();
     ...
     */
    default char getOrDefault(char key, char def)   {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
            this.put(key, def);
     ...
     */
    default char computeIfAbsent(char key, @NonNull CharacterToCharacterFunction func) {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
