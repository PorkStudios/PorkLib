 ...
 * Although this map 𝙒𝙊𝙍𝙆𝙎, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.FloatFloatIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class FloatFloatTreeMap implements FloatFloatMap    {
    ...
    @SuppressWarnings("unchecked")
    protected float remove(float key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return 0F; //no mapping
    ...
    public FloatIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.FloatFloatIndexedTreeMap} for an implementation of this method
        return null;
    }
