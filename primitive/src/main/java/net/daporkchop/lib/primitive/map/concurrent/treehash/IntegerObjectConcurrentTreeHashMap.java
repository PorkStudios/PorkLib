...
import net.daporkchop.lib.primitive.iterator.concurrent.IntegerObjectConcurrentIterator;
import net.daporkchop.lib.primitive.map.IntegerObjectMap;
import net.daporkchop.lib.primitive.tuple.IntegerObjectTuple;
import net.daporkchop.lib.primitive.tuple.IntegerObjectImmutableTuple;
import net.daporkchop.lib.primitive.tuple.IntegerObjectMutableTuple;
...
/**
 * A tree hash map, using a key type of int and a value type of V.
 ...
 * @author DaPorkchop_
 */
public class IntegerObjectConcurrentTreeHashMap<V extends Object> implements IntegerObjectMap<V>    {
    final AtomicInteger size = new AtomicInteger(0);

    private final IntegerObjectSubEntry[] entries = new IntegerObjectSubEntry[256];
    private final IntegerToIntegerFunction keyHash;
    final IntegerToLongFunction subKeyHash;
    final int subHashLength;

    public IntegerObjectConcurrentTreeHashMap()    {
        this(null, null);
    }

    public IntegerObjectConcurrentTreeHashMap(IntegerToIntegerFunction keyHash, IntegerToLongFunction subKeyHash)    {
        this(keyHash, subKeyHash, 3);
    }

    public IntegerObjectConcurrentTreeHashMap(IntegerToIntegerFunction keyHash, IntegerToLongFunction subKeyHash, int subHashLength)    {
    ...
    public V get(int key)   {
        IntegerObjectSubEntry<V> entry = this.getSubentry(this.keyHash.apply(key) & 0xFF, false);
        if (entry == null)  {
            return null;
        } else {
    ...
    public V put(int key, V value)   {
        IntegerObjectSubEntry<V> entry = this.getSubentry(this.keyHash.apply(key) & 0xFF, true);
    ...
    public V remove(int key)    {
        ...
        IntegerObjectSubEntry<V> entry = this.getSubentry(hash, false);
        if (entry == null)  {
            return null;
        } else {
    ...
    public void clear() {
        synchronized (this.entries) {
            for (int i = 0; i < 256; i++) {
                IntegerObjectSubEntry<V> entry = this.entries[i];
                if (entry != null)  {
                    entry.clear();
                    this.entries[i] = null;
                }
    ...
    public void forEachKey(IntegerConsumer consumer)   {
        for (int i = 0; i < 256; i++) {
            IntegerObjectSubEntry<V> entry = this.getSubentry(i, false);
            if (entry != null)  {
    ...
    @Override
    public void forEachEntry(IntegerObjectConsumer<V> consumer) {
        for (int i = 0; i < 256; i++) {
            IntegerObjectSubEntry<V> entry = this.getSubentry(i, false);
            if (entry != null)  {
    ...
    public void forEachEntry(IntegerObjectConsumer<V> consumer, VoidFunction complete) {
        for (int i = 0; i < 256; i++) {
            IntegerObjectSubEntry<V> entry = this.getSubentry(i, false);
            if (entry != null)  {
                entry.forEachEntry(consumer);
    ...
    public IntegerConcurrentIterator keyIterator()   {
        return new IntegerConcurrentIterator()  {
            private final IntegerObjectConcurrentTreeHashMap this_ = IntegerObjectConcurrentTreeHashMap.this;
            ...
            private final AtomicReference<IntegerObjectSubEntry<V>.Sector> currSector = new AtomicReference(null);
            private final AtomicBoolean hasNextSub = new AtomicBoolean(true);
            private volatile boolean hasNext = true;
            private volatile int subEntryIndex = 0;
            private volatile int next;
            private volatile IntegerObjectSubEntry<V> entry;
            ...
            private void nextSub()   {
                ...
                while (this.subEntryIndex < 256)    {
                    IntegerObjectSubEntry<V> entry = getSubentry(this.subEntryIndex++, false);
                    if (entry != null)  {
                        this.entry = entry;
            ...
            class ThreadState   {
                public int key;
                public IntegerObjectSubEntry entry;
                private final IntegerConcurrentIterator this_;
                private final IntegerObjectConcurrentTreeHashMap this__;

                public ThreadState(IntegerConcurrentIterator this_, IntegerObjectConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
    ...
    public ObjectConcurrentIterator<V> valueIterator() {
        return new ObjectConcurrentIterator<V>()  {
            ...
            private final AtomicReference<IntegerObjectSubEntry<V>.Sector> currSector = new AtomicReference(null);
            private final AtomicBoolean hasNextSub = new AtomicBoolean(true);
            private final IntegerObjectMutableTuple<V> next = new IntegerObjectMutableTuple<V>();
            private volatile boolean hasNext = true;
            private volatile int subEntryIndex = 0;
            private volatile IntegerObjectSubEntry<V> entry;
            ...
            class ThreadState   {
                ...
                public ThreadState(ObjectConcurrentIterator<V> this_, IntegerObjectConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
    ...
    @Override
    public IntegerObjectConcurrentIterator<V> entryIterator() {
        return new IntegerObjectConcurrentIterator<V>()  {
            private final IntegerObjectConcurrentTreeHashMap this_ = IntegerObjectConcurrentTreeHashMap.this;
            //private final ReentrantLock lock = new ReentrantLock();
            private final ThreadLocal<ThreadState> tl = ThreadLocal.withInitial(() -> new ThreadState(this, this.this_));
            private final AtomicInteger sectorOffset = new AtomicInteger(0);
            ...
            @Override
            public IntegerObjectTuple<V> get()   {
                ThreadState state = this.tl.get();
                if (state.entry == null)    {
                    throw new IllegalStateException();
                } else {
            ...
            @Override
            public synchronized IntegerObjectTuple<V> advance()   {
                if (!this.hasNext())    {
                    throw new IteratorCompleteException();
                }
                ThreadState state = this.tl.get();
                IntegerObjectTuple<V> current = state.tuple = new IntegerObjectMutableTuple<V>(this.next.k, this.next.v);
                state.entry = this.entry;
                this.findNextValue();
                return current;
            }
            ...
            class ThreadState   {
                public IntegerObjectTuple<V> tuple;
                public IntegerObjectSubEntry entry;
                private final IntegerObjectConcurrentIterator<V> this_;
                private final IntegerObjectConcurrentTreeHashMap this__;

                public ThreadState(IntegerObjectConcurrentIterator<V> this_, IntegerObjectConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
     ...
     */
    public IntegerObjectConcurrentIterator<V> fastEntryIterator() {
        return new IntegerObjectConcurrentIterator<V>()  {
            private final IntegerObjectConcurrentTreeHashMap this_ = IntegerObjectConcurrentTreeHashMap.this;
            //private final ReentrantLock lock = new ReentrantLock();
            private final ThreadLocal<ThreadState> tl = ThreadLocal.withInitial(() -> new ThreadState(this, this.this_));
            ...
            private final AtomicReference<IntegerObjectSubEntry<V>.Sector> currSector = new AtomicReference(null);
            private final AtomicBoolean hasNextSub = new AtomicBoolean(true);
            private volatile IntegerObjectMutableTuple<V> next = new IntegerObjectMutableTuple<V>(0, null);
            private volatile boolean hasNext = true;
            private volatile int subEntryIndex = 0;
            private volatile IntegerObjectSubEntry<V> entry;
            ...
            @Override
            public IntegerObjectMutableTuple<V> get()   {
                ThreadState state = this.tl.get();
                if (state.entry == null)    {
                    throw new IllegalStateException();
                } else {
            ...
            @Override
            public synchronized IntegerObjectMutableTuple<V> advance()   {
                if (!this.hasNext())    {
                    throw new IteratorCompleteException();
                }
                ThreadState state = this.tl.get();
                IntegerObjectMutableTuple<V> a = state.tuple;
            ...
            class ThreadState   {
                public IntegerObjectMutableTuple<V> tuple = new IntegerObjectMutableTuple<V>(0, null);
                public IntegerObjectSubEntry entry;
                private final IntegerObjectConcurrentIterator<V> this_;
                private final IntegerObjectConcurrentTreeHashMap this__;
    ...
    private IntegerObjectSubEntry<V> getSubentry(int index, boolean create)    {
        assert index == (index & 0xFF);

        synchronized (this.entries) {
            IntegerObjectSubEntry<V> entry = (IntegerObjectSubEntry<V>) this.entries[index];
            if (entry == null && create)    {
                this.entries[index] = entry = new IntegerObjectSubEntry<V>(this);
            }
            return entry;
        }
    }
