 ...
 * Although this map WORKS, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.CharacterDoubleIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class CharacterDoubleTreeMap implements CharacterDoubleMap    {
    protected char[] keys;
    ...
    @SuppressWarnings("unchecked")
    protected double remove(char key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return 0D; //no mapping
    ...
    public CharacterIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.CharacterDoubleIndexedTreeMap} for an implementation of this method
        return null;
    }
