...
/**
 * A single subentry for a {@link IntegerIntegerConcurrentTreeHashMap}.
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
class IntegerIntegerSubEntry    {
    public final AtomicInteger size = new AtomicInteger(0);
    public final IntegerIntegerConcurrentTreeHashMap parent;
    ...
    public IntegerIntegerSubEntry(IntegerIntegerConcurrentTreeHashMap parent)   {
        this.parent = parent;
        this.rootPointer = this.parent.subHashLength == 0 ? new IntegerIntegerSubEntry.ValueSector(null, -1) : new IntegerIntegerSubEntry.PointerSector(null, -1);
    }
    ...
    public void forEachKey(IntegerConsumer consumer)   {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof IntegerIntegerSubEntry.PointerSector)    {
    ...
    public int advanceKey(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext)   {
        ...
        for (int i = sector.states.nextSetBit(curr.get() + 1); i != -1; i = sector.states.nextSetBit(i + 1)) {
            curr.set(i);
            if (sector instanceof IntegerIntegerSubEntry.ValueSector)  {
    ...
    public void advanceEntry(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext, IntegerIntegerMutableTuple tuple)   {
        Sector sector = sectorRef.get();
        if (sector == null) {
            sectorRef.set(sector = this.rootPointer);
        }
    ...
    public Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.parent.subHashLength; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.parent.subHashLength ? new IntegerIntegerSubEntry.ValueSector(curr, off) : new IntegerIntegerSubEntry.PointerSector(curr, off);
    ...
    public abstract class Sector  {
        ...
        public int setEntry(int index, int key, int value)   {
            throw new UnsupportedOperationException();
        }
    ...
    public class ValueSector extends Sector   {
        ...
        public int setEntry(int index, int key, int value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                IntegerIntegerSubEntry.this.size.incrementAndGet();
                IntegerIntegerSubEntry.this.parent.size.incrementAndGet();
                return 0;
            }
        }
        ...
        public int removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                IntegerIntegerSubEntry.this.size.decrementAndGet();
                IntegerIntegerSubEntry.this.parent.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
