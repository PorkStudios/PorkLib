...
import net.daporkchop.lib.primitive.lambda.function.bi.CharacterObjectToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of char => V mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface CharacterObjectConcurrentIterator<V extends Object> extends CharacterObjectIterator<V>   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull CharacterObjectToBooleanFunction<V> function)    {
        try {
            while (this.hasNext())  {
                CharacterObjectTuple<V> tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<CharacterObjectTuple<V>> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<CharacterObjectTuple<V>> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull CharacterObjectConsumer<V> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<CharacterObjectTuple<V>> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull CharacterObjectToBooleanFunction<V> function)    {
        this.concurrentIterate(function);
    }
}
