...
/**
 * A hash map, using a key type of float and a value type of char.
 ...
 * @author DaPorkchop_
 */
public class FloatCharacterHashMap implements FloatCharacterMap    {
    ...
    public FloatCharacterHashMap()    {
        this(16384);
    }

    public FloatCharacterHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public FloatCharacterHashMap(FloatToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public FloatCharacterHashMap(int baseSize, FloatToIntegerFunction keyHash)    {
    ...
    protected boolean reHash(char[] values, float[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
    ...
    private class KeyIterator implements FloatIterator {
        ...
        public KeyIterator()    {
            this.size = FloatCharacterHashMap.this.size;
            this.next = FloatCharacterHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public float get()   {
            return FloatCharacterHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = FloatCharacterHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            FloatCharacterHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements CharacterIterator {
        ...
        @SuppressWarnings("unchecked")
        public char get()   {
            return FloatCharacterHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements FloatCharacterIterator {
        private int size;
        private int index = -1;
        private int next;
        private FloatCharacterTuple tuple;
        ...
        @Override
        public FloatCharacterTuple advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new FloatCharacterImmutableTuple( FloatCharacterHashMap.this.keys[this.index],  FloatCharacterHashMap.this.values[this.index]);
        }
