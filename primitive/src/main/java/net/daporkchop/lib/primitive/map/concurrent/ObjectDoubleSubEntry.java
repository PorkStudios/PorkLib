...
/**
 * A single subentry for a {@link ObjectDoubleConcurrentTreeHashMap}.
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
class ObjectDoubleSubEntry<K extends Object>    {
    public final AtomicInteger size = new AtomicInteger(0);
    public final ObjectDoubleConcurrentTreeHashMap parent;
    ...
    public ObjectDoubleSubEntry(ObjectDoubleConcurrentTreeHashMap parent)   {
        this.parent = parent;
        this.rootPointer = this.parent.subHashLength == 0 ? new ObjectDoubleSubEntry<K>.ValueSector(null, -1) : new ObjectDoubleSubEntry<K>.PointerSector(null, -1);
    }
    ...
    public void clear() {
        this.lock.lock();
        try {
            this.rootPointer = this.parent.subHashLength == 0 ? new ObjectDoubleSubEntry.ValueSector(null, -1) : new ObjectDoubleSubEntry.PointerSector(null, -1);
    ...
    public void forEachKey(ObjectConsumer<K> consumer)   {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof ObjectDoubleSubEntry.PointerSector)    {
    ...
    public K advanceKey(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext)   {
        ...
        for (int i = sector.states.nextSetBit(curr.get() + 1); i != -1; i = sector.states.nextSetBit(i + 1)) {
            curr.set(i);
            if (sector instanceof ObjectDoubleSubEntry.ValueSector)  {
    ...
    public void advanceEntry(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext, ObjectDoubleMutableTuple<K> tuple)   {
        Sector sector = sectorRef.get();
        if (sector == null) {
            sectorRef.set(sector = this.rootPointer);
        }
    ...
    public Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.parent.subHashLength; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.parent.subHashLength ? new ObjectDoubleSubEntry<K>.ValueSector(curr, off) : new ObjectDoubleSubEntry<K>.PointerSector(curr, off);
    ...
    public abstract class Sector  {
        ...
        public double setEntry(int index, Object key, double value)   {
            throw new UnsupportedOperationException();
        }
        ...
        public void forEachEntry(ObjectDoubleConsumer consumer)   {
            throw new UnsupportedOperationException();
        }
    ...
    public class ValueSector extends Sector   {
        ...
        public double setEntry(int index, Object key, double value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                ObjectDoubleSubEntry.this.size.incrementAndGet();
                ObjectDoubleSubEntry.this.parent.size.incrementAndGet();
                return 0D;
            }
        }
        ...
        public double removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                ObjectDoubleSubEntry.this.size.decrementAndGet();
                ObjectDoubleSubEntry.this.parent.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
