 ...
 * overhead, and can even be made concurrent with little extra work (see {@link net.daporkchop.lib.primitive.map.concurrent.IntegerDoubleConcurrentTreeHashMap})
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
public class IntegerDoubleTreeHashMap implements IntegerDoubleMap    {
    ...
    public IntegerDoubleTreeHashMap()    {
        this(null, 0);
    }

    public IntegerDoubleTreeHashMap(IntegerToLongFunction hashFunc, int hashLen)    {
        ...
        this.rootPointer = this.hashLen == 0 ? new IntegerDoubleTreeHashMap.ValueSector(null, -1) : new IntegerDoubleTreeHashMap.PointerSector(null, -1);
    }
    ...
    public void forEachKey(@NonNull IntegerConsumer consumer)  {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof IntegerDoubleTreeHashMap.PointerSector)    {
    ...
    private Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.hashLen; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.hashLen ? new IntegerDoubleTreeHashMap.ValueSector(curr, off) : new IntegerDoubleTreeHashMap.PointerSector(curr, off);
    ...
    private class ValueSector extends Sector   {
        ...
        public double setEntry(int index, int key, double value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                IntegerDoubleTreeHashMap.this.size.incrementAndGet();
                return 0D;
            }
        }
        ...
        public double removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                IntegerDoubleTreeHashMap.this.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
