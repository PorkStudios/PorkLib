...
/**
 * A tree map, using a key type of float and a value type of char.
 ...
 * Although this map WORKS, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.FloatCharacterIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class FloatCharacterTreeMap implements FloatCharacterMap    {
    ...
    @SuppressWarnings("unchecked")
    protected char remove(float key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return (char) 0; //no mapping
    ...
    public FloatIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.FloatCharacterIndexedTreeMap} for an implementation of this method
        return null;
    }
