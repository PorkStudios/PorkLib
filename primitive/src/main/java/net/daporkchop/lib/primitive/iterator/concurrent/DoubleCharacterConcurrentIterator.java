...
import net.daporkchop.lib.primitive.lambda.function.bi.DoubleCharacterToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of double => char mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface DoubleCharacterConcurrentIterator extends DoubleCharacterIterator   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull DoubleCharacterToBooleanFunction function)    {
        try {
            while (this.hasNext())  {
                DoubleCharacterTuple tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<DoubleCharacterTuple> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<DoubleCharacterTuple> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull DoubleCharacterConsumer consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<DoubleCharacterTuple> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull DoubleCharacterToBooleanFunction function)    {
        this.concurrentIterate(function);
    }
}
