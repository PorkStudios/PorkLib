...
/**
 * A single subentry for a {@link ObjectFloatConcurrentTreeHashMap}.
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
class ObjectFloatSubEntry<K extends Object>    {
    public final AtomicInteger size = new AtomicInteger(0);
    public final ObjectFloatConcurrentTreeHashMap parent;
    ...
    public ObjectFloatSubEntry(ObjectFloatConcurrentTreeHashMap parent)   {
        this.parent = parent;
        this.rootPointer = this.parent.subHashLength == 0 ? new ObjectFloatSubEntry<K>.ValueSector(null, -1) : new ObjectFloatSubEntry<K>.PointerSector(null, -1);
    }
    ...
    public void clear() {
        this.lock.lock();
        try {
            this.rootPointer = this.parent.subHashLength == 0 ? new ObjectFloatSubEntry.ValueSector(null, -1) : new ObjectFloatSubEntry.PointerSector(null, -1);
    ...
    public void forEachKey(ObjectConsumer<K> consumer)   {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof ObjectFloatSubEntry.PointerSector)    {
    ...
    public K advanceKey(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext)   {
        ...
        for (int i = sector.states.nextSetBit(curr.get() + 1); i != -1; i = sector.states.nextSetBit(i + 1)) {
            curr.set(i);
            if (sector instanceof ObjectFloatSubEntry.ValueSector)  {
    ...
    public void advanceEntry(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext, ObjectFloatMutableTuple<K> tuple)   {
        Sector sector = sectorRef.get();
        if (sector == null) {
            sectorRef.set(sector = this.rootPointer);
        }
    ...
    public Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.parent.subHashLength; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.parent.subHashLength ? new ObjectFloatSubEntry<K>.ValueSector(curr, off) : new ObjectFloatSubEntry<K>.PointerSector(curr, off);
    ...
    public abstract class Sector  {
        ...
        public float setEntry(int index, Object key, float value)   {
            throw new UnsupportedOperationException();
        }
        ...
        public void forEachEntry(ObjectFloatConsumer consumer)   {
            throw new UnsupportedOperationException();
        }
    ...
    public class ValueSector extends Sector   {
        ...
        public float setEntry(int index, Object key, float value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                ObjectFloatSubEntry.this.size.incrementAndGet();
                ObjectFloatSubEntry.this.parent.size.incrementAndGet();
                return 0F;
            }
        }
        ...
        public float removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                ObjectFloatSubEntry.this.size.decrementAndGet();
                ObjectFloatSubEntry.this.parent.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
