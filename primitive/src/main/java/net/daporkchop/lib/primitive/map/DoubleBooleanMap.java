...
import net.daporkchop.lib.primitive.lambda.function.DoubleToBooleanFunction;
...
/**
 * A map, using a key type of double and a value type of boolean.
 * <p>
...
public interface DoubleBooleanMap   {
     ...
     */
    default void putAll(@NonNull double[] keys, @NonNull boolean[] values)    {
     ...
     */
    default void putAll(@NonNull DoubleBooleanMap map)    {
     ...
     * @param consumer the function to run
     */
    void forEachEntry(DoubleBooleanConsumer consumer);
     ...
     */
    DoubleBooleanIterator entryIterator();
     ...
     */
    default boolean getOrDefault(double key, boolean def)   {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
            this.put(key, def);
     ...
     */
    default boolean computeIfAbsent(double key, @NonNull DoubleToBooleanFunction func) {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
