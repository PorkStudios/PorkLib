...
import net.daporkchop.lib.primitive.iterator.bi.ObjectIntegerIterator;
...
/**
 * A hash map, using a key type of K and a value type of int.
 ...
 * @author DaPorkchop_
 */
public class ObjectIntegerHashMap<K extends Object> implements ObjectIntegerMap<K>    {
    ...
    public ObjectIntegerHashMap()    {
        this(16384);
    }

    public ObjectIntegerHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public ObjectIntegerHashMap(ObjectToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public ObjectIntegerHashMap(int baseSize, ObjectToIntegerFunction keyHash)    {
    ...
    @Override
    public void forEachEntry(@NonNull ObjectIntegerConsumer<K> consumer)   {
    ...
    public ObjectIntegerIterator<K> entryIterator() {
    ...
    protected boolean reHash(int[] values, Object[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
            K key = (K) this.keys[i];
            int j = getIndex(key);
            if (states.get(j))  {
    ...
    private class KeyIterator implements ObjectIterator<K> {
        ...
        public KeyIterator()    {
            this.size = ObjectIntegerHashMap.this.size;
            this.next = ObjectIntegerHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public K get()   {
            return (K) ObjectIntegerHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = ObjectIntegerHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            ObjectIntegerHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements IntegerIterator {
        ...
        @SuppressWarnings("unchecked")
        public int get()   {
            return ObjectIntegerHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements ObjectIntegerIterator<K> {
        private int size;
        private int index = -1;
        private int next;
        private ObjectIntegerTuple<K> tuple;
        ...
        @Override
        public ObjectIntegerTuple<K> advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new ObjectIntegerImmutableTuple<K>( (K) ObjectIntegerHashMap.this.keys[this.index],  ObjectIntegerHashMap.this.values[this.index]);
        }
