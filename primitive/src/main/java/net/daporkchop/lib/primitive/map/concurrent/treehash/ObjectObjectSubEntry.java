...
/**
 * A single subentry for a {@link ObjectObjectConcurrentTreeHashMap}.
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
class ObjectObjectSubEntry<K extends Object, V extends Object>    {
    public final AtomicInteger size = new AtomicInteger(0);
    public final ObjectObjectConcurrentTreeHashMap parent;
    ...
    public ObjectObjectSubEntry(ObjectObjectConcurrentTreeHashMap parent)   {
        this.parent = parent;
        this.rootPointer = this.parent.subHashLength == 0 ? new ObjectObjectSubEntry<K, V>.ValueSector(null, -1) : new ObjectObjectSubEntry<K, V>.PointerSector(null, -1);
    }
    ...
    public void clear() {
        this.lock.lock();
        try {
            this.rootPointer = this.parent.subHashLength == 0 ? new ObjectObjectSubEntry.ValueSector(null, -1) : new ObjectObjectSubEntry.PointerSector(null, -1);
    ...
    public void forEachKey(ObjectConsumer<K> consumer)   {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof ObjectObjectSubEntry.PointerSector)    {
    ...
    public void forEachEntry(ObjectObjectConsumer<K, V> consumer) {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof ObjectObjectSubEntry.PointerSector)    {
                        ...
                        sector.forEachEntry((k, v) -> consumer.accept( (K) k,  (V) v));
                    }
                };
        recursiveHack = c;
        this.rootPointer.forEachSector(c);
    ...
    public K advanceKey(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext)   {
        ...
        for (int i = sector.states.nextSetBit(curr.get() + 1); i != -1; i = sector.states.nextSetBit(i + 1)) {
            curr.set(i);
            if (sector instanceof ObjectObjectSubEntry.ValueSector)  {
    ...
    public void advanceEntry(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext, ObjectObjectMutableTuple<K, V> tuple)   {
        Sector sector = sectorRef.get();
        if (sector == null) {
            sectorRef.set(sector = this.rootPointer);
        }
    ...
    public Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.parent.subHashLength; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.parent.subHashLength ? new ObjectObjectSubEntry<K, V>.ValueSector(curr, off) : new ObjectObjectSubEntry<K, V>.PointerSector(curr, off);
    ...
    public abstract class Sector  {
        ...
        public Object setEntry(int index, Object key, Object value)   {
            throw new UnsupportedOperationException();
        }
        ...
        public void forEachEntry(ObjectObjectConsumer consumer)   {
            throw new UnsupportedOperationException();
        }
    ...
    public class ValueSector extends Sector   {
        ...
        public Object setEntry(int index, Object key, Object value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                ObjectObjectSubEntry.this.size.incrementAndGet();
                ObjectObjectSubEntry.this.parent.size.incrementAndGet();
                return null;
            }
        }
        ...
        public Object removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                ObjectObjectSubEntry.this.size.decrementAndGet();
                ObjectObjectSubEntry.this.parent.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
