 ...
 * Although this map 𝙒𝙊𝙍𝙆𝙎, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.ByteFloatIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class ByteFloatTreeMap implements ByteFloatMap    {
    protected byte[] keys;
    ...
    @SuppressWarnings("unchecked")
    protected float remove(byte key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return 0F; //no mapping
    ...
    public ByteIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.ByteFloatIndexedTreeMap} for an implementation of this method
        return null;
    }
