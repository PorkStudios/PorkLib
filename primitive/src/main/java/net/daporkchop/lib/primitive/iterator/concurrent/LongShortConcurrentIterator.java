...
import net.daporkchop.lib.primitive.lambda.function.bi.LongShortToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of long => short mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface LongShortConcurrentIterator extends LongShortIterator   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull LongShortToBooleanFunction function)    {
        try {
            while (this.hasNext())  {
                LongShortTuple tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<LongShortTuple> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<LongShortTuple> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull LongShortConsumer consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<LongShortTuple> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull LongShortToBooleanFunction function)    {
        this.concurrentIterate(function);
    }
}
