...
public class ARIAKeyGen {
    static {
        BouncyCastleInit.loadClass();
    }

    public static ARIAKey gen(byte[] data) {
        byte[] b = new byte[BlockCipherType.ARIA.blockSize];
        int i = 0;
        do {
        ...
        data = b;
        return new ARIAKey(new SecretKeySpec(data, "aaa"), KeyRandom.getBytes(BlockCipherType.ARIA.blockSize));
    }

    public static ARIAKey gen() {
        byte[] key = new byte[BlockCipherType.ARIA.blockSize];
