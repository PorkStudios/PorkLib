...
import net.daporkchop.lib.primitive.lambda.function.bi.IntegerIntegerToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of int => int mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface IntegerIntegerConcurrentIterator extends IntegerIntegerIterator   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull IntegerIntegerToBooleanFunction function)    {
        try {
            while (this.hasNext())  {
                IntegerIntegerTuple tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<IntegerIntegerTuple> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<IntegerIntegerTuple> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull IntegerIntegerConsumer consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<IntegerIntegerTuple> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull IntegerIntegerToBooleanFunction function)    {
        this.concurrentIterate(function);
    }
}
