...
/**
 * A hash map, using a key type of short and a value type of V.
...
public class ShortObjectHashMap<V extends Object> implements ShortObjectMap<V>    {
    ...
    public ShortObjectHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public ShortObjectHashMap(ShortToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public ShortObjectHashMap(int baseSize, ShortToIntegerFunction keyHash)    {
    ...
    protected boolean reHash(Object[] values, short[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
    ...
    private class KeyIterator implements ShortIterator {
        ...
        public KeyIterator()    {
            this.size = ShortObjectHashMap.this.size;
            this.next = ShortObjectHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public short get()   {
            return ShortObjectHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = ShortObjectHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            ShortObjectHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements ObjectIterator<V> {
        ...
        @SuppressWarnings("unchecked")
        public V get()   {
            return (V) ShortObjectHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements ShortObjectIterator<V> {
        ...
        public ShortObjectTuple<V> advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new ShortObjectImmutableTuple<V>( ShortObjectHashMap.this.keys[this.index],  (V) ShortObjectHashMap.this.values[this.index]);
        }
