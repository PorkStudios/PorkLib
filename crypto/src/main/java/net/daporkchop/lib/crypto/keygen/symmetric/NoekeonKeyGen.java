...
public class NoekeonKeyGen {
    static {
        BouncyCastleInit.loadClass();
    }

    public static NoekeonKey gen(byte[] data) {
        byte[] b = new byte[BlockCipherType.NOEKEON.blockSize];
        int i = 0;
        do {
        ...
        data = b;
        return new NoekeonKey(new SecretKeySpec(data, "aaa"), KeyRandom.getBytes(BlockCipherType.NOEKEON.blockSize));
    }

    public static NoekeonKey gen() {
        byte[] key = new byte[BlockCipherType.NOEKEON.blockSize];
