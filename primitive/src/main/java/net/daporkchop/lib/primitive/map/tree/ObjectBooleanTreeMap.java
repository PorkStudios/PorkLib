 ...
 * Although this map WORKS, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.ObjectBooleanIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class ObjectBooleanTreeMap<K extends Object> implements ObjectBooleanMap<K>    {
    ...
    @SuppressWarnings("unchecked")
    protected boolean remove(K key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return false; //no mapping
    ...
    public ObjectIterator<K> keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.ObjectBooleanIndexedTreeMap} for an implementation of this method
        return null;
    }
