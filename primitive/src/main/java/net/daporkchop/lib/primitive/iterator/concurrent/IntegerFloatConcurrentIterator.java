...
import net.daporkchop.lib.primitive.lambda.function.bi.IntegerFloatToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of int => float mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface IntegerFloatConcurrentIterator extends IntegerFloatIterator   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull IntegerFloatToBooleanFunction function)    {
        try {
            while (this.hasNext())  {
                IntegerFloatTuple tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<IntegerFloatTuple> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<IntegerFloatTuple> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull IntegerFloatConsumer consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<IntegerFloatTuple> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull IntegerFloatToBooleanFunction function)    {
        this.concurrentIterate(function);
    }
}
