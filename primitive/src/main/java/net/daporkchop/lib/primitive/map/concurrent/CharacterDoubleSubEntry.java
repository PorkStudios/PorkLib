...
/**
 * A single subentry for a {@link CharacterDoubleConcurrentTreeHashMap}.
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
class CharacterDoubleSubEntry    {
    public final AtomicInteger size = new AtomicInteger(0);
    public final CharacterDoubleConcurrentTreeHashMap parent;
    ...
    public CharacterDoubleSubEntry(CharacterDoubleConcurrentTreeHashMap parent)   {
        this.parent = parent;
        this.rootPointer = this.parent.subHashLength == 0 ? new CharacterDoubleSubEntry.ValueSector(null, -1) : new CharacterDoubleSubEntry.PointerSector(null, -1);
    }
    ...
    public void forEachKey(CharacterConsumer consumer)   {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof CharacterDoubleSubEntry.PointerSector)    {
    ...
    public char advanceKey(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext)   {
        ...
        for (int i = sector.states.nextSetBit(curr.get() + 1); i != -1; i = sector.states.nextSetBit(i + 1)) {
            curr.set(i);
            if (sector instanceof CharacterDoubleSubEntry.ValueSector)  {
    ...
    public void advanceEntry(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext, CharacterDoubleMutableTuple tuple)   {
        Sector sector = sectorRef.get();
        if (sector == null) {
            sectorRef.set(sector = this.rootPointer);
        }
    ...
    public Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.parent.subHashLength; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.parent.subHashLength ? new CharacterDoubleSubEntry.ValueSector(curr, off) : new CharacterDoubleSubEntry.PointerSector(curr, off);
    ...
    public abstract class Sector  {
        ...
        public double setEntry(int index, char key, double value)   {
            throw new UnsupportedOperationException();
        }
    ...
    public class ValueSector extends Sector   {
        ...
        public double setEntry(int index, char key, double value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                CharacterDoubleSubEntry.this.size.incrementAndGet();
                CharacterDoubleSubEntry.this.parent.size.incrementAndGet();
                return 0D;
            }
        }
        ...
        public double removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                CharacterDoubleSubEntry.this.size.decrementAndGet();
                CharacterDoubleSubEntry.this.parent.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
