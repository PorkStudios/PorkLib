...
import net.daporkchop.lib.primitive.lambda.function.bi.FloatCharacterToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of float => char mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface FloatCharacterConcurrentIterator extends FloatCharacterIterator   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull FloatCharacterToBooleanFunction function)    {
        try {
            while (this.hasNext())  {
                FloatCharacterTuple tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<FloatCharacterTuple> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<FloatCharacterTuple> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull FloatCharacterConsumer consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<FloatCharacterTuple> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull FloatCharacterToBooleanFunction function)    {
        this.concurrentIterate(function);
    }
}
