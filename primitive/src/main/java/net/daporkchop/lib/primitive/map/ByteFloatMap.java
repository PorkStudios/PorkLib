...
import net.daporkchop.lib.primitive.lambda.function.ByteToFloatFunction;
...
/**
 * A map, using a key type of byte and a value type of float.
 * <p>
...
public interface ByteFloatMap   {
     ...
     */
    default void putAll(@NonNull byte[] keys, @NonNull float[] values)    {
     ...
     */
    default void putAll(@NonNull ByteFloatMap map)    {
     ...
     * @param consumer the function to run
     */
    void forEachEntry(ByteFloatConsumer consumer);
     ...
     */
    ByteFloatIterator entryIterator();
     ...
     */
    default float getOrDefault(byte key, float def)   {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
            this.put(key, def);
     ...
     */
    default float computeIfAbsent(byte key, @NonNull ByteToFloatFunction func) {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
