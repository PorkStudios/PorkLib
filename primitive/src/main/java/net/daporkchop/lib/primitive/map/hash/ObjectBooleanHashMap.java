...
/**
 * A hash map, using a key type of K and a value type of boolean.
 ...
 * @author DaPorkchop_
 */
public class ObjectBooleanHashMap<K extends Object> implements ObjectBooleanMap<K>    {
    ...
    public ObjectBooleanHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public ObjectBooleanHashMap(ObjectToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public ObjectBooleanHashMap(int baseSize, ObjectToIntegerFunction keyHash)    {
    ...
    protected boolean reHash(boolean[] values, Object[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
            K key = (K) this.keys[i];
            int j = getIndex(key);
            if (states.get(j))  {
    ...
    private class KeyIterator implements ObjectIterator<K> {
        ...
        public KeyIterator()    {
            this.size = ObjectBooleanHashMap.this.size;
            this.next = ObjectBooleanHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public K get()   {
            return (K) ObjectBooleanHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = ObjectBooleanHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            ObjectBooleanHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements BooleanIterator {
        ...
        @SuppressWarnings("unchecked")
        public boolean get()   {
            return ObjectBooleanHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements ObjectBooleanIterator<K> {
        private int size;
        private int index = -1;
        private int next;
        private ObjectBooleanTuple<K> tuple;
        ...
        @Override
        public ObjectBooleanTuple<K> advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new ObjectBooleanImmutableTuple<K>( (K) ObjectBooleanHashMap.this.keys[this.index],  ObjectBooleanHashMap.this.values[this.index]);
        }
