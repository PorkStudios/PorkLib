 ...
 * @author DaPorkchop_
 */
public abstract class AbstractDirectBitmapABW extends AbstractDirectBitmap {
    public AbstractDirectBitmapABW(int width, int height) {
        super(width, height);
    }
    ...
    public int getRGB(int x, int y) throws BitmapCoordinatesOutOfBoundsException {
        return ColorFormatRGB.fromARGB(ColorFormatABW.toARGB(PUnsafe.getChar(this.addr(x, y))));
    }

    @Override
    public int getARGB(int x, int y) throws BitmapCoordinatesOutOfBoundsException {
        return ColorFormatABW.toARGB(PUnsafe.getChar(this.addr(x, y)));
    }

    @Override
    public int getBW(int x, int y) throws BitmapCoordinatesOutOfBoundsException {
        return PUnsafe.getChar(this.addr(x, y)) & 0xFF;
    }
    ...
    protected long addr(int x, int y) {
        return this.ptr + (super.addr(x, y) << 1L);
    }
}
