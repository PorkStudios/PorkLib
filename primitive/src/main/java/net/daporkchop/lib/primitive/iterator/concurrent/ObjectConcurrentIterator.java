...
/**
 * Allows parallel iteration over an iterator of K values. Implementing classes should be thread safe
 * and concurrent (calling advance() should throw {@link IteratorCompleteException} instead of null,
 ...
 * @author DaPorkchop_
 */
public interface ObjectConcurrentIterator<K extends Object> extends ObjectIterator<K>   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<K> function)    {
        try {
            while (this.hasNext())  {
                K val = this.advance();
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<K> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<K> function)   {
