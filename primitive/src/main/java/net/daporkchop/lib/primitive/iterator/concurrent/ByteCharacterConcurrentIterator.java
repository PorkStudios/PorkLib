...
import net.daporkchop.lib.primitive.lambda.function.bi.ByteCharacterToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of byte => char mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface ByteCharacterConcurrentIterator extends ByteCharacterIterator   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ByteCharacterToBooleanFunction function)    {
        try {
            while (this.hasNext())  {
                ByteCharacterTuple tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<ByteCharacterTuple> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<ByteCharacterTuple> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ByteCharacterConsumer consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<ByteCharacterTuple> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull ByteCharacterToBooleanFunction function)    {
        this.concurrentIterate(function);
    }
}
