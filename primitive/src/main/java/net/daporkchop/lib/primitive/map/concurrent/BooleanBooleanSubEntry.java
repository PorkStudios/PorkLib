...
/**
 * A single subentry for a {@link BooleanBooleanConcurrentTreeHashMap}.
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
class BooleanBooleanSubEntry    {
    public final AtomicInteger size = new AtomicInteger(0);
    public final BooleanBooleanConcurrentTreeHashMap parent;
    ...
    public BooleanBooleanSubEntry(BooleanBooleanConcurrentTreeHashMap parent)   {
        this.parent = parent;
        this.rootPointer = this.parent.subHashLength == 0 ? new BooleanBooleanSubEntry.ValueSector(null, -1) : new BooleanBooleanSubEntry.PointerSector(null, -1);
    }
    ...
    public void forEachKey(BooleanConsumer consumer)   {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof BooleanBooleanSubEntry.PointerSector)    {
    ...
    public boolean advanceKey(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext)   {
        ...
        for (int i = sector.states.nextSetBit(curr.get() + 1); i != -1; i = sector.states.nextSetBit(i + 1)) {
            curr.set(i);
            if (sector instanceof BooleanBooleanSubEntry.ValueSector)  {
    ...
    public void advanceEntry(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext, BooleanBooleanMutableTuple tuple)   {
        Sector sector = sectorRef.get();
        if (sector == null) {
            sectorRef.set(sector = this.rootPointer);
        }
    ...
    public Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.parent.subHashLength; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.parent.subHashLength ? new BooleanBooleanSubEntry.ValueSector(curr, off) : new BooleanBooleanSubEntry.PointerSector(curr, off);
    ...
    public class ValueSector extends Sector   {
        ...
        public boolean setEntry(int index, boolean key, boolean value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                BooleanBooleanSubEntry.this.size.incrementAndGet();
                BooleanBooleanSubEntry.this.parent.size.incrementAndGet();
                return false;
            }
        }
        ...
        public boolean removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                BooleanBooleanSubEntry.this.size.decrementAndGet();
                BooleanBooleanSubEntry.this.parent.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
