...
/**
 * A hash map, using a key type of char and a value type of short.
 ...
 * @author DaPorkchop_
 */
public class CharacterShortHashMap implements CharacterShortMap    {
    protected char[] keys;
    ...
    public CharacterShortHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public CharacterShortHashMap(CharacterToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public CharacterShortHashMap(int baseSize, CharacterToIntegerFunction keyHash)    {
    ...
    protected boolean reHash(short[] values, char[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
    ...
    private class KeyIterator implements CharacterIterator {
        ...
        public KeyIterator()    {
            this.size = CharacterShortHashMap.this.size;
            this.next = CharacterShortHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public char get()   {
            return CharacterShortHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = CharacterShortHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            CharacterShortHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements ShortIterator {
        ...
        @SuppressWarnings("unchecked")
        public short get()   {
            return CharacterShortHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements CharacterShortIterator {
        private int size;
        private int index = -1;
        private int next;
        private CharacterShortTuple tuple;
        ...
        @Override
        public CharacterShortTuple advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new CharacterShortImmutableTuple( CharacterShortHashMap.this.keys[this.index],  CharacterShortHashMap.this.values[this.index]);
        }
