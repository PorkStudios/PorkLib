...
/**
 * A hash map, using a key type of byte and a value type of char.
 ...
 * @author DaPorkchop_
 */
public class ByteCharacterHashMap implements ByteCharacterMap    {
    protected byte[] keys;
    ...
    public ByteCharacterHashMap()    {
        this(16384);
    }

    public ByteCharacterHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public ByteCharacterHashMap(ByteToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public ByteCharacterHashMap(int baseSize, ByteToIntegerFunction keyHash)    {
    ...
    protected boolean reHash(char[] values, byte[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
    ...
    private class KeyIterator implements ByteIterator {
        ...
        public KeyIterator()    {
            this.size = ByteCharacterHashMap.this.size;
            this.next = ByteCharacterHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public byte get()   {
            return ByteCharacterHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = ByteCharacterHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            ByteCharacterHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements CharacterIterator {
        ...
        @SuppressWarnings("unchecked")
        public char get()   {
            return ByteCharacterHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements ByteCharacterIterator {
        private int size;
        private int index = -1;
        private int next;
        private ByteCharacterTuple tuple;
        ...
        @Override
        public ByteCharacterTuple advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new ByteCharacterImmutableTuple( ByteCharacterHashMap.this.keys[this.index],  ByteCharacterHashMap.this.values[this.index]);
        }
