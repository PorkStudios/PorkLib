...
/**
 * A hash map, using a key type of float and a value type of double.
 ...
 * @author DaPorkchop_
 */
public class FloatDoubleHashMap implements FloatDoubleMap    {
    ...
    public FloatDoubleHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public FloatDoubleHashMap(FloatToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public FloatDoubleHashMap(int baseSize, FloatToIntegerFunction keyHash)    {
    ...
    @Override
    public void forEachEntry(@NonNull FloatDoubleConsumer consumer)   {
    ...
    public FloatDoubleIterator entryIterator() {
    ...
    protected boolean reHash(double[] values, float[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
    ...
    private class KeyIterator implements FloatIterator {
        ...
        public KeyIterator()    {
            this.size = FloatDoubleHashMap.this.size;
            this.next = FloatDoubleHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public float get()   {
            return FloatDoubleHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = FloatDoubleHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            FloatDoubleHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements DoubleIterator {
        ...
        @SuppressWarnings("unchecked")
        public double get()   {
            return FloatDoubleHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements FloatDoubleIterator {
        private int size;
        private int index = -1;
        private int next;
        private FloatDoubleTuple tuple;
        ...
        @Override
        public FloatDoubleTuple advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new FloatDoubleImmutableTuple( FloatDoubleHashMap.this.keys[this.index],  FloatDoubleHashMap.this.values[this.index]);
        }
