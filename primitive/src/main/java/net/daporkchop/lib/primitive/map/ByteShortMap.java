...
import net.daporkchop.lib.primitive.lambda.function.ByteToShortFunction;
...
/**
 * A map, using a key type of byte and a value type of short.
 * <p>
...
public interface ByteShortMap   {
     ...
     */
    default void putAll(@NonNull byte[] keys, @NonNull short[] values)    {
     ...
     */
    default void putAll(@NonNull ByteShortMap map)    {
     ...
     * @param consumer the function to run
     */
    void forEachEntry(ByteShortConsumer consumer);
     ...
     */
    ByteShortIterator entryIterator();
     ...
     */
    default short getOrDefault(byte key, short def)   {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
            this.put(key, def);
     ...
     */
    default short computeIfAbsent(byte key, @NonNull ByteToShortFunction func) {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
