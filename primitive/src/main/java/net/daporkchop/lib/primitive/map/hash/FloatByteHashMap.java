...
/**
 * A hash map, using a key type of float and a value type of byte.
 ...
 * @author DaPorkchop_
 */
public class FloatByteHashMap implements FloatByteMap    {
    ...
    public FloatByteHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public FloatByteHashMap(FloatToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public FloatByteHashMap(int baseSize, FloatToIntegerFunction keyHash)    {
    ...
    protected boolean reHash(byte[] values, float[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
    ...
    private class KeyIterator implements FloatIterator {
        ...
        public KeyIterator()    {
            this.size = FloatByteHashMap.this.size;
            this.next = FloatByteHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public float get()   {
            return FloatByteHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = FloatByteHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            FloatByteHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements ByteIterator {
        ...
        @SuppressWarnings("unchecked")
        public byte get()   {
            return FloatByteHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements FloatByteIterator {
        private int size;
        private int index = -1;
        private int next;
        private FloatByteTuple tuple;
        ...
        @Override
        public FloatByteTuple advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new FloatByteImmutableTuple( FloatByteHashMap.this.keys[this.index],  FloatByteHashMap.this.values[this.index]);
        }
