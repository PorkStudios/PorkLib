 ...
 * Although this map WORKS, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.ObjectByteIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class ObjectByteTreeMap<K extends Object> implements ObjectByteMap<K>    {
    ...
    @SuppressWarnings("unchecked")
    protected byte remove(K key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return (byte) 0; //no mapping
    ...
    public ObjectIterator<K> keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.ObjectByteIndexedTreeMap} for an implementation of this method
        return null;
    }
