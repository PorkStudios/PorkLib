 ...
 * Although this map 𝙒𝙊𝙍𝙆𝙎, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.ByteLongIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class ByteLongTreeMap implements ByteLongMap    {
    protected byte[] keys;
    ...
    @SuppressWarnings("unchecked")
    protected long remove(byte key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return 0L; //no mapping
    ...
    public ByteIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.ByteLongIndexedTreeMap} for an implementation of this method
        return null;
    }
