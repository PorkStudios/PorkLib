...
/**
 * A map, using a key type of float and a value type of long.
 * <p>
...
public interface FloatLongMap   {
     ...
     */
    default void putAll(@NonNull float[] keys, @NonNull long[] values)    {
     ...
     */
    default void putAll(@NonNull FloatLongMap map)    {
     ...
     * @param consumer the function to run
     */
    void forEachEntry(FloatLongConsumer consumer);
     ...
     */
    FloatLongIterator entryIterator();
     ...
     */
    default long getOrDefault(float key, long def)   {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
            this.put(key, def);
     ...
     */
    default long computeIfAbsent(float key, @NonNull FloatToLongFunction func) {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
