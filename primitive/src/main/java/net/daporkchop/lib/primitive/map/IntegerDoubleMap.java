...
import net.daporkchop.lib.primitive.lambda.function.IntegerToDoubleFunction;
...
/**
 * A map, using a key type of int and a value type of double.
 * <p>
...
public interface IntegerDoubleMap   {
     ...
     */
    default void putAll(@NonNull int[] keys, @NonNull double[] values)    {
     ...
     */
    default void putAll(@NonNull IntegerDoubleMap map)    {
     ...
     * @param consumer the function to run
     */
    void forEachEntry(IntegerDoubleConsumer consumer);
     ...
     */
    IntegerDoubleIterator entryIterator();
     ...
     */
    default double computeIfAbsent(int key, @NonNull IntegerToDoubleFunction func) {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
