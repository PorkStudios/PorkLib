...
/**
 * A hash map, using a key type of double and a value type of float.
 ...
 * @author DaPorkchop_
 */
public class DoubleFloatHashMap implements DoubleFloatMap    {
    ...
    public DoubleFloatHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public DoubleFloatHashMap(DoubleToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public DoubleFloatHashMap(int baseSize, DoubleToIntegerFunction keyHash)    {
    ...
    protected boolean reHash(float[] values, double[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
    ...
    private class KeyIterator implements DoubleIterator {
        ...
        public KeyIterator()    {
            this.size = DoubleFloatHashMap.this.size;
            this.next = DoubleFloatHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public double get()   {
            return DoubleFloatHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = DoubleFloatHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            DoubleFloatHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements FloatIterator {
        ...
        @SuppressWarnings("unchecked")
        public float get()   {
            return DoubleFloatHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements DoubleFloatIterator {
        private int size;
        private int index = -1;
        private int next;
        private DoubleFloatTuple tuple;
        ...
        @Override
        public DoubleFloatTuple advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new DoubleFloatImmutableTuple( DoubleFloatHashMap.this.keys[this.index],  DoubleFloatHashMap.this.values[this.index]);
        }
