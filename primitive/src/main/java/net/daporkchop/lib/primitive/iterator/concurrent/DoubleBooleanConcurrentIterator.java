...
import net.daporkchop.lib.primitive.lambda.function.bi.DoubleBooleanToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of double => boolean mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface DoubleBooleanConcurrentIterator extends DoubleBooleanIterator   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull DoubleBooleanToBooleanFunction function)    {
        try {
            while (this.hasNext())  {
                DoubleBooleanTuple tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<DoubleBooleanTuple> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<DoubleBooleanTuple> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull DoubleBooleanConsumer consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<DoubleBooleanTuple> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull DoubleBooleanToBooleanFunction function)    {
        this.concurrentIterate(function);
    }
}
