...
public class SM4KeyGen {
    ...
    public static SM4Key gen(byte[] data) {
        byte[] b = new byte[BlockCipherType.SM4.blockSize];
        int i = 0;
        do {
        ...
        data = b;
        return new SM4Key(new SecretKeySpec(data, "aaa"), KeyRandom.getBytes(BlockCipherType.SM4.blockSize));
    }

    public static SM4Key gen() {
        byte[] key = new byte[BlockCipherType.SM4.blockSize];
