...
/**
 * A single subentry for a {@link ObjectIntegerConcurrentTreeHashMap}.
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
class ObjectIntegerSubEntry<K extends Object>    {
    public final AtomicInteger size = new AtomicInteger(0);
    public final ObjectIntegerConcurrentTreeHashMap parent;
    ...
    public ObjectIntegerSubEntry(ObjectIntegerConcurrentTreeHashMap parent)   {
        this.parent = parent;
        this.rootPointer = this.parent.subHashLength == 0 ? new ObjectIntegerSubEntry<K>.ValueSector(null, -1) : new ObjectIntegerSubEntry<K>.PointerSector(null, -1);
    }
    ...
    public void clear() {
        this.lock.lock();
        try {
            this.rootPointer = this.parent.subHashLength == 0 ? new ObjectIntegerSubEntry.ValueSector(null, -1) : new ObjectIntegerSubEntry.PointerSector(null, -1);
    ...
    public void forEachKey(ObjectConsumer<K> consumer)   {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof ObjectIntegerSubEntry.PointerSector)    {
    ...
    public K advanceKey(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext)   {
        ...
        for (int i = sector.states.nextSetBit(curr.get() + 1); i != -1; i = sector.states.nextSetBit(i + 1)) {
            curr.set(i);
            if (sector instanceof ObjectIntegerSubEntry.ValueSector)  {
    ...
    public void advanceEntry(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext, ObjectIntegerMutableTuple<K> tuple)   {
        Sector sector = sectorRef.get();
        if (sector == null) {
            sectorRef.set(sector = this.rootPointer);
        }
    ...
    public Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.parent.subHashLength; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.parent.subHashLength ? new ObjectIntegerSubEntry<K>.ValueSector(curr, off) : new ObjectIntegerSubEntry<K>.PointerSector(curr, off);
    ...
    public abstract class Sector  {
        ...
        public int setEntry(int index, Object key, int value)   {
            throw new UnsupportedOperationException();
        }
        ...
        public void forEachEntry(ObjectIntegerConsumer consumer)   {
            throw new UnsupportedOperationException();
        }
    ...
    public class ValueSector extends Sector   {
        ...
        public int setEntry(int index, Object key, int value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                ObjectIntegerSubEntry.this.size.incrementAndGet();
                ObjectIntegerSubEntry.this.parent.size.incrementAndGet();
                return 0;
            }
        }
        ...
        public int removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                ObjectIntegerSubEntry.this.size.decrementAndGet();
                ObjectIntegerSubEntry.this.parent.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
