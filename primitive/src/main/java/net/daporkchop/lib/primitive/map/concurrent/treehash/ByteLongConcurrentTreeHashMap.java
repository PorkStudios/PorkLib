...
import net.daporkchop.lib.primitive.iterator.concurrent.ByteLongConcurrentIterator;
import net.daporkchop.lib.primitive.map.ByteLongMap;
import net.daporkchop.lib.primitive.tuple.ByteLongTuple;
import net.daporkchop.lib.primitive.tuple.ByteLongImmutableTuple;
import net.daporkchop.lib.primitive.tuple.ByteLongMutableTuple;
...
/**
 * A tree hash map, using a key type of byte and a value type of long.
 ...
 * @author DaPorkchop_
 */
public class ByteLongConcurrentTreeHashMap implements ByteLongMap    {
    final AtomicInteger size = new AtomicInteger(0);

    private final ByteLongSubEntry[] entries = new ByteLongSubEntry[256];
    private final ByteToIntegerFunction keyHash;
    final ByteToLongFunction subKeyHash;
    final int subHashLength;

    public ByteLongConcurrentTreeHashMap()    {
        this(null, null);
    }

    public ByteLongConcurrentTreeHashMap(ByteToIntegerFunction keyHash, ByteToLongFunction subKeyHash)    {
        this(keyHash, subKeyHash, 0);
    }

    public ByteLongConcurrentTreeHashMap(ByteToIntegerFunction keyHash, ByteToLongFunction subKeyHash, int subHashLength)    {
    ...
    public long get(byte key)   {
        ByteLongSubEntry entry = this.getSubentry(this.keyHash.apply(key) & 0xFF, false);
        if (entry == null)  {
            return 0L;
        } else {
    ...
    public long put(byte key, long value)   {
        ByteLongSubEntry entry = this.getSubentry(this.keyHash.apply(key) & 0xFF, true);
    ...
    public long remove(byte key)    {
        ...
        ByteLongSubEntry entry = this.getSubentry(hash, false);
        if (entry == null)  {
            return 0L;
        } else {
    ...
    public void clear() {
        synchronized (this.entries) {
            for (int i = 0; i < 256; i++) {
                ByteLongSubEntry entry = this.entries[i];
                if (entry != null)  {
                    entry.clear();
                    this.entries[i] = null;
                }
    ...
    public void forEachKey(ByteConsumer consumer)   {
        for (int i = 0; i < 256; i++) {
            ByteLongSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
    ...
    @Override
    public void forEachEntry(ByteLongConsumer consumer) {
        for (int i = 0; i < 256; i++) {
            ByteLongSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
    ...
    public void forEachEntry(ByteLongConsumer consumer, VoidFunction complete) {
        for (int i = 0; i < 256; i++) {
            ByteLongSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
                entry.forEachEntry(consumer);
    ...
    public ByteConcurrentIterator keyIterator()   {
        return new ByteConcurrentIterator()  {
            private final ByteLongConcurrentTreeHashMap this_ = ByteLongConcurrentTreeHashMap.this;
            ...
            private final AtomicReference<ByteLongSubEntry.Sector> currSector = new AtomicReference(null);
            ...
            private volatile ByteLongSubEntry entry;
            ...
            private void nextSub()   {
                ...
                while (this.subEntryIndex < 256)    {
                    ByteLongSubEntry entry = getSubentry(this.subEntryIndex++, false);
                    if (entry != null)  {
                        this.entry = entry;
            ...
            class ThreadState   {
                public byte key;
                public ByteLongSubEntry entry;
                private final ByteConcurrentIterator this_;
                private final ByteLongConcurrentTreeHashMap this__;

                public ThreadState(ByteConcurrentIterator this_, ByteLongConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
    ...
    public LongConcurrentIterator valueIterator() {
        return new LongConcurrentIterator()  {
            ...
            private final AtomicReference<ByteLongSubEntry.Sector> currSector = new AtomicReference(null);
            private final AtomicBoolean hasNextSub = new AtomicBoolean(true);
            private final ByteLongMutableTuple next = new ByteLongMutableTuple();
            private volatile boolean hasNext = true;
            private volatile int subEntryIndex = 0;
            private volatile ByteLongSubEntry entry;
            ...
            class ThreadState   {
                ...
                public ThreadState(LongConcurrentIterator this_, ByteLongConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
    ...
    @Override
    public ByteLongConcurrentIterator entryIterator() {
        return new ByteLongConcurrentIterator()  {
            private final ByteLongConcurrentTreeHashMap this_ = ByteLongConcurrentTreeHashMap.this;
            //private final ReentrantLock lock = new ReentrantLock();
            private final ThreadLocal<ThreadState> tl = ThreadLocal.withInitial(() -> new ThreadState(this, this.this_));
            private final AtomicInteger sectorOffset = new AtomicInteger(0);
            ...
            @Override
            public synchronized ByteLongTuple advance()   {
                if (!this.hasNext())    {
                    throw new IteratorCompleteException();
                }
                ThreadState state = this.tl.get();
                ByteLongTuple current = state.tuple = new ByteLongMutableTuple(this.next.k, this.next.v);
                state.entry = this.entry;
                this.findNextValue();
                return current;
            }
            ...
            class ThreadState   {
                public ByteLongTuple tuple;
                public ByteLongSubEntry entry;
                private final ByteLongConcurrentIterator this_;
                private final ByteLongConcurrentTreeHashMap this__;

                public ThreadState(ByteLongConcurrentIterator this_, ByteLongConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
     ...
     */
    public ByteLongConcurrentIterator fastEntryIterator() {
        return new ByteLongConcurrentIterator()  {
            private final ByteLongConcurrentTreeHashMap this_ = ByteLongConcurrentTreeHashMap.this;
            //private final ReentrantLock lock = new ReentrantLock();
            private final ThreadLocal<ThreadState> tl = ThreadLocal.withInitial(() -> new ThreadState(this, this.this_));
            ...
            private final AtomicReference<ByteLongSubEntry.Sector> currSector = new AtomicReference(null);
            private final AtomicBoolean hasNextSub = new AtomicBoolean(true);
            private volatile ByteLongMutableTuple next = new ByteLongMutableTuple((byte) 0, 0L);
            private volatile boolean hasNext = true;
            private volatile int subEntryIndex = 0;
            private volatile ByteLongSubEntry entry;
            ...
            @Override
            public ByteLongMutableTuple get()   {
                ThreadState state = this.tl.get();
                if (state.entry == null)    {
                    throw new IllegalStateException();
                } else {
            ...
            @Override
            public synchronized ByteLongMutableTuple advance()   {
                if (!this.hasNext())    {
                    throw new IteratorCompleteException();
                }
                ThreadState state = this.tl.get();
                ByteLongMutableTuple a = state.tuple;
            ...
            class ThreadState   {
                public ByteLongMutableTuple tuple = new ByteLongMutableTuple((byte) 0, 0L);
                public ByteLongSubEntry entry;
                private final ByteLongConcurrentIterator this_;
                private final ByteLongConcurrentTreeHashMap this__;
    ...
    private ByteLongSubEntry getSubentry(int index, boolean create)    {
        assert index == (index & 0xFF);

        synchronized (this.entries) {
            ByteLongSubEntry entry = (ByteLongSubEntry) this.entries[index];
            if (entry == null && create)    {
                this.entries[index] = entry = new ByteLongSubEntry(this);
            }
            return entry;
        }
    }
