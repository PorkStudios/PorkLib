...
/**
 * A tree map, using a key type of boolean and a value type of long.
 ...
 * Although this map WORKS, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.BooleanLongIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class BooleanLongTreeMap implements BooleanLongMap    {
    ...
    @SuppressWarnings("unchecked")
    protected long remove(boolean key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return 0L; //no mapping
    ...
    public BooleanIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.BooleanLongIndexedTreeMap} for an implementation of this method
        return null;
    }
