 ...
 * Although this map WORKS, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.DoubleIntegerIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class DoubleIntegerTreeMap implements DoubleIntegerMap    {
    ...
    @SuppressWarnings("unchecked")
    protected int remove(double key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return 0; //no mapping
    ...
    public DoubleIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.DoubleIntegerIndexedTreeMap} for an implementation of this method
        return null;
    }
