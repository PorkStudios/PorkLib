...
/**
 * A single subentry for a {@link LongFloatConcurrentTreeHashMap}.
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
class LongFloatSubEntry    {
    public final AtomicInteger size = new AtomicInteger(0);
    public final LongFloatConcurrentTreeHashMap parent;
    ...
    public LongFloatSubEntry(LongFloatConcurrentTreeHashMap parent)   {
        this.parent = parent;
        this.rootPointer = this.parent.subHashLength == 0 ? new LongFloatSubEntry.ValueSector(null, -1) : new LongFloatSubEntry.PointerSector(null, -1);
    }
    ...
    public void forEachKey(LongConsumer consumer)   {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof LongFloatSubEntry.PointerSector)    {
    ...
    public long advanceKey(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext)   {
        ...
        for (int i = sector.states.nextSetBit(curr.get() + 1); i != -1; i = sector.states.nextSetBit(i + 1)) {
            curr.set(i);
            if (sector instanceof LongFloatSubEntry.ValueSector)  {
    ...
    public void advanceEntry(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext, LongFloatMutableTuple tuple)   {
        Sector sector = sectorRef.get();
        if (sector == null) {
            sectorRef.set(sector = this.rootPointer);
        }
    ...
    public Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.parent.subHashLength; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.parent.subHashLength ? new LongFloatSubEntry.ValueSector(curr, off) : new LongFloatSubEntry.PointerSector(curr, off);
    ...
    public abstract class Sector  {
        ...
        public float setEntry(int index, long key, float value)   {
            throw new UnsupportedOperationException();
        }
    ...
    public class ValueSector extends Sector   {
        ...
        public float setEntry(int index, long key, float value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                LongFloatSubEntry.this.size.incrementAndGet();
                LongFloatSubEntry.this.parent.size.incrementAndGet();
                return 0F;
            }
        }
        ...
        public float removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                LongFloatSubEntry.this.size.decrementAndGet();
                LongFloatSubEntry.this.parent.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
