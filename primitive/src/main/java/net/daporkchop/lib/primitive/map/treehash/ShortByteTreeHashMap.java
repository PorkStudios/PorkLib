 ...
 * overhead, and can even be made concurrent with little extra work (see {@link net.daporkchop.lib.primitive.map.concurrent.ShortByteConcurrentTreeHashMap})
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
public class ShortByteTreeHashMap implements ShortByteMap    {
    ...
    public ShortByteTreeHashMap()    {
        this(null, 0);
    }

    public ShortByteTreeHashMap(ShortToLongFunction hashFunc, int hashLen)    {
        ...
        this.rootPointer = this.hashLen == 0 ? new ShortByteTreeHashMap.ValueSector(null, -1) : new ShortByteTreeHashMap.PointerSector(null, -1);
    }
    ...
    public void forEachKey(@NonNull ShortConsumer consumer)  {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof ShortByteTreeHashMap.PointerSector)    {
    ...
    private Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.hashLen; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.hashLen ? new ShortByteTreeHashMap.ValueSector(curr, off) : new ShortByteTreeHashMap.PointerSector(curr, off);
    ...
    private class ValueSector extends Sector   {
        ...
        public byte setEntry(int index, short key, byte value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                ShortByteTreeHashMap.this.size.incrementAndGet();
                return (byte) 0;
            }
        }
        ...
        public byte removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                ShortByteTreeHashMap.this.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
