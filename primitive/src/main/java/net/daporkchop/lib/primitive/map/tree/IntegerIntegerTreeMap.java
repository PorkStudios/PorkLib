 ...
 * Although this map WORKS, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.IntegerIntegerIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class IntegerIntegerTreeMap implements IntegerIntegerMap    {
    protected int[] keys;
    ...
    @SuppressWarnings("unchecked")
    protected int remove(int key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return 0; //no mapping
    ...
    public IntegerIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.IntegerIntegerIndexedTreeMap} for an implementation of this method
        return null;
    }
