 ...
 * overhead, and can even be made concurrent with little extra work (see {@link net.daporkchop.lib.primitive.map.concurrent.FloatByteConcurrentTreeHashMap})
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
public class FloatByteTreeHashMap implements FloatByteMap    {
    ...
    public FloatByteTreeHashMap()    {
        this(null, 0);
    }

    public FloatByteTreeHashMap(FloatToLongFunction hashFunc, int hashLen)    {
        ...
        this.rootPointer = this.hashLen == 0 ? new FloatByteTreeHashMap.ValueSector(null, -1) : new FloatByteTreeHashMap.PointerSector(null, -1);
    }
    ...
    public void forEachKey(@NonNull FloatConsumer consumer)  {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof FloatByteTreeHashMap.PointerSector)    {
    ...
    private Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.hashLen; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.hashLen ? new FloatByteTreeHashMap.ValueSector(curr, off) : new FloatByteTreeHashMap.PointerSector(curr, off);
    ...
    private class ValueSector extends Sector   {
        ...
        public byte setEntry(int index, float key, byte value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                FloatByteTreeHashMap.this.size.incrementAndGet();
                return (byte) 0;
            }
        }
        ...
        public byte removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                FloatByteTreeHashMap.this.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
