...
/**
 * A tree map, using a key type of byte and a value type of boolean.
 ...
 * Although this map WORKS, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.ByteBooleanIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class ByteBooleanTreeMap implements ByteBooleanMap    {
    protected byte[] keys;
    ...
    @SuppressWarnings("unchecked")
    protected boolean remove(byte key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return false; //no mapping
    ...
    public ByteIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.ByteBooleanIndexedTreeMap} for an implementation of this method
        return null;
    }
