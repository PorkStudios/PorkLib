...
import net.daporkchop.lib.primitive.tuple.ObjectShortImmutableTuple;
...
/**
 * A hash map, using a key type of K and a value type of short.
 ...
 * @author DaPorkchop_
 */
public class ObjectShortHashMap<K extends Object> implements ObjectShortMap<K>    {
    ...
    public ObjectShortHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public ObjectShortHashMap(ObjectToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public ObjectShortHashMap(int baseSize, ObjectToIntegerFunction keyHash)    {
    ...
    protected boolean reHash(short[] values, Object[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
            K key = (K) this.keys[i];
            int j = getIndex(key);
            if (states.get(j))  {
    ...
    private class KeyIterator implements ObjectIterator<K> {
        ...
        public KeyIterator()    {
            this.size = ObjectShortHashMap.this.size;
            this.next = ObjectShortHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public K get()   {
            return (K) ObjectShortHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = ObjectShortHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            ObjectShortHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements ShortIterator {
        ...
        @SuppressWarnings("unchecked")
        public short get()   {
            return ObjectShortHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements ObjectShortIterator<K> {
        private int size;
        private int index = -1;
        private int next;
        private ObjectShortTuple<K> tuple;
        ...
        @Override
        public ObjectShortTuple<K> advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new ObjectShortImmutableTuple<K>( (K) ObjectShortHashMap.this.keys[this.index],  ObjectShortHashMap.this.values[this.index]);
        }
