...
/**
 * A hash map, using a key type of double and a value type of char.
 ...
 * @author DaPorkchop_
 */
public class DoubleCharacterHashMap implements DoubleCharacterMap    {
    ...
    public DoubleCharacterHashMap()    {
        this(16384);
    }

    public DoubleCharacterHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public DoubleCharacterHashMap(DoubleToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public DoubleCharacterHashMap(int baseSize, DoubleToIntegerFunction keyHash)    {
    ...
    protected boolean reHash(char[] values, double[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
    ...
    private class KeyIterator implements DoubleIterator {
        ...
        public KeyIterator()    {
            this.size = DoubleCharacterHashMap.this.size;
            this.next = DoubleCharacterHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public double get()   {
            return DoubleCharacterHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = DoubleCharacterHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            DoubleCharacterHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements CharacterIterator {
        ...
        @SuppressWarnings("unchecked")
        public char get()   {
            return DoubleCharacterHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements DoubleCharacterIterator {
        private int size;
        private int index = -1;
        private int next;
        private DoubleCharacterTuple tuple;
        ...
        @Override
        public DoubleCharacterTuple advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new DoubleCharacterImmutableTuple( DoubleCharacterHashMap.this.keys[this.index],  DoubleCharacterHashMap.this.values[this.index]);
        }
