...
import net.daporkchop.lib.primitive.iterator.concurrent.FloatConcurrentIterator;
import net.daporkchop.lib.primitive.iterator.concurrent.FloatFloatConcurrentIterator;
import net.daporkchop.lib.primitive.map.FloatFloatMap;
import net.daporkchop.lib.primitive.tuple.FloatFloatTuple;
import net.daporkchop.lib.primitive.tuple.FloatFloatImmutableTuple;
import net.daporkchop.lib.primitive.tuple.FloatFloatMutableTuple;
...
/**
 * A tree hash map, using a key type of float and a value type of float.
 ...
 * @author DaPorkchop_
 */
public class FloatFloatConcurrentTreeHashMap implements FloatFloatMap    {
    final AtomicInteger size = new AtomicInteger(0);

    private final FloatFloatSubEntry[] entries = new FloatFloatSubEntry[256];
    private final FloatToIntegerFunction keyHash;
    final FloatToLongFunction subKeyHash;
    final int subHashLength;

    public FloatFloatConcurrentTreeHashMap()    {
        this(null, null);
    }

    public FloatFloatConcurrentTreeHashMap(FloatToIntegerFunction keyHash, FloatToLongFunction subKeyHash)    {
        this(keyHash, subKeyHash, 3);
    }

    public FloatFloatConcurrentTreeHashMap(FloatToIntegerFunction keyHash, FloatToLongFunction subKeyHash, int subHashLength)    {
    ...
    public float get(float key)   {
        FloatFloatSubEntry entry = this.getSubentry(this.keyHash.apply(key) & 0xFF, false);
        if (entry == null)  {
            return 0F;
        } else {
    ...
    public float put(float key, float value)   {
        FloatFloatSubEntry entry = this.getSubentry(this.keyHash.apply(key) & 0xFF, true);
    ...
    public float remove(float key)    {
        ...
        FloatFloatSubEntry entry = this.getSubentry(hash, false);
        if (entry == null)  {
            return 0F;
        } else {
    ...
    public void clear() {
        synchronized (this.entries) {
            for (int i = 0; i < 256; i++) {
                FloatFloatSubEntry entry = this.entries[i];
                if (entry != null)  {
                    entry.clear();
                    this.entries[i] = null;
                }
    ...
    public void forEachKey(FloatConsumer consumer)   {
        for (int i = 0; i < 256; i++) {
            FloatFloatSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
    ...
    @Override
    public void forEachEntry(FloatFloatConsumer consumer) {
        for (int i = 0; i < 256; i++) {
            FloatFloatSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
    ...
    public void forEachEntry(FloatFloatConsumer consumer, VoidFunction complete) {
        for (int i = 0; i < 256; i++) {
            FloatFloatSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
                entry.forEachEntry(consumer);
    ...
    public FloatConcurrentIterator keyIterator()   {
        return new FloatConcurrentIterator()  {
            private final FloatFloatConcurrentTreeHashMap this_ = FloatFloatConcurrentTreeHashMap.this;
            ...
            private final AtomicReference<FloatFloatSubEntry.Sector> currSector = new AtomicReference(null);
            ...
            private volatile FloatFloatSubEntry entry;
            ...
            private void nextSub()   {
                ...
                while (this.subEntryIndex < 256)    {
                    FloatFloatSubEntry entry = getSubentry(this.subEntryIndex++, false);
                    if (entry != null)  {
                        this.entry = entry;
            ...
            class ThreadState   {
                public float key;
                public FloatFloatSubEntry entry;
                private final FloatConcurrentIterator this_;
                private final FloatFloatConcurrentTreeHashMap this__;

                public ThreadState(FloatConcurrentIterator this_, FloatFloatConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
    ...
    public FloatConcurrentIterator valueIterator() {
        return new FloatConcurrentIterator()  {
            ...
            private final AtomicReference<FloatFloatSubEntry.Sector> currSector = new AtomicReference(null);
            private final AtomicBoolean hasNextSub = new AtomicBoolean(true);
            private final FloatFloatMutableTuple next = new FloatFloatMutableTuple();
            private volatile boolean hasNext = true;
            private volatile int subEntryIndex = 0;
            private volatile FloatFloatSubEntry entry;
    ...
    @Override
    public FloatFloatConcurrentIterator entryIterator() {
        return new FloatFloatConcurrentIterator()  {
            private final FloatFloatConcurrentTreeHashMap this_ = FloatFloatConcurrentTreeHashMap.this;
            //private final ReentrantLock lock = new ReentrantLock();
            private final ThreadLocal<ThreadState> tl = ThreadLocal.withInitial(() -> new ThreadState(this, this.this_));
            private final AtomicInteger sectorOffset = new AtomicInteger(0);
            ...
            @Override
            public synchronized FloatFloatTuple advance()   {
                if (!this.hasNext())    {
                    throw new IteratorCompleteException();
                }
                ThreadState state = this.tl.get();
                FloatFloatTuple current = state.tuple = new FloatFloatMutableTuple(this.next.k, this.next.v);
                state.entry = this.entry;
                this.findNextValue();
                return current;
            }
            ...
            class ThreadState   {
                public FloatFloatTuple tuple;
                public FloatFloatSubEntry entry;
                private final FloatFloatConcurrentIterator this_;
                private final FloatFloatConcurrentTreeHashMap this__;

                public ThreadState(FloatFloatConcurrentIterator this_, FloatFloatConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
     ...
     */
    public FloatFloatConcurrentIterator fastEntryIterator() {
        return new FloatFloatConcurrentIterator()  {
            private final FloatFloatConcurrentTreeHashMap this_ = FloatFloatConcurrentTreeHashMap.this;
            //private final ReentrantLock lock = new ReentrantLock();
            private final ThreadLocal<ThreadState> tl = ThreadLocal.withInitial(() -> new ThreadState(this, this.this_));
            ...
            private final AtomicReference<FloatFloatSubEntry.Sector> currSector = new AtomicReference(null);
            private final AtomicBoolean hasNextSub = new AtomicBoolean(true);
            private volatile FloatFloatMutableTuple next = new FloatFloatMutableTuple(0F, 0F);
            private volatile boolean hasNext = true;
            private volatile int subEntryIndex = 0;
            private volatile FloatFloatSubEntry entry;
            ...
            @Override
            public FloatFloatMutableTuple get()   {
                ThreadState state = this.tl.get();
                if (state.entry == null)    {
                    throw new IllegalStateException();
                } else {
            ...
            @Override
            public synchronized FloatFloatMutableTuple advance()   {
                if (!this.hasNext())    {
                    throw new IteratorCompleteException();
                }
                ThreadState state = this.tl.get();
                FloatFloatMutableTuple a = state.tuple;
            ...
            class ThreadState   {
                public FloatFloatMutableTuple tuple = new FloatFloatMutableTuple(0F, 0F);
                public FloatFloatSubEntry entry;
                private final FloatFloatConcurrentIterator this_;
                private final FloatFloatConcurrentTreeHashMap this__;
    ...
    private FloatFloatSubEntry getSubentry(int index, boolean create)    {
        assert index == (index & 0xFF);

        synchronized (this.entries) {
            FloatFloatSubEntry entry = (FloatFloatSubEntry) this.entries[index];
            if (entry == null && create)    {
                this.entries[index] = entry = new FloatFloatSubEntry(this);
            }
            return entry;
        }
    }
