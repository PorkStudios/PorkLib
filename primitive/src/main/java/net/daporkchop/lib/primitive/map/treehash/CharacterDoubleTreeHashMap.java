 ...
 * overhead, and can even be made concurrent with little extra work (see {@link net.daporkchop.lib.primitive.map.concurrent.CharacterDoubleConcurrentTreeHashMap})
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
public class CharacterDoubleTreeHashMap implements CharacterDoubleMap    {
    ...
    public CharacterDoubleTreeHashMap()    {
        this(null, 0);
    }

    public CharacterDoubleTreeHashMap(CharacterToLongFunction hashFunc, int hashLen)    {
        ...
        this.rootPointer = this.hashLen == 0 ? new CharacterDoubleTreeHashMap.ValueSector(null, -1) : new CharacterDoubleTreeHashMap.PointerSector(null, -1);
    }
    ...
    public void forEachKey(@NonNull CharacterConsumer consumer)  {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof CharacterDoubleTreeHashMap.PointerSector)    {
    ...
    private Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.hashLen; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.hashLen ? new CharacterDoubleTreeHashMap.ValueSector(curr, off) : new CharacterDoubleTreeHashMap.PointerSector(curr, off);
    ...
    private class ValueSector extends Sector   {
        ...
        public double setEntry(int index, char key, double value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                CharacterDoubleTreeHashMap.this.size.incrementAndGet();
                return 0D;
            }
        }
        ...
        public double removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                CharacterDoubleTreeHashMap.this.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
