...
/**
 * A hash map, using a key type of char and a value type of double.
 ...
 * @author DaPorkchop_
 */
public class CharacterDoubleHashMap implements CharacterDoubleMap    {
    protected char[] keys;
    ...
    public CharacterDoubleHashMap()    {
        this(16384);
    }

    public CharacterDoubleHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public CharacterDoubleHashMap(CharacterToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public CharacterDoubleHashMap(int baseSize, CharacterToIntegerFunction keyHash)    {
    ...
    protected boolean reHash(double[] values, char[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
    ...
    private class KeyIterator implements CharacterIterator {
        ...
        public KeyIterator()    {
            this.size = CharacterDoubleHashMap.this.size;
            this.next = CharacterDoubleHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public char get()   {
            return CharacterDoubleHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = CharacterDoubleHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            CharacterDoubleHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements DoubleIterator {
        ...
        @SuppressWarnings("unchecked")
        public double get()   {
            return CharacterDoubleHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements CharacterDoubleIterator {
        ...
        public CharacterDoubleTuple advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new CharacterDoubleImmutableTuple( CharacterDoubleHashMap.this.keys[this.index],  CharacterDoubleHashMap.this.values[this.index]);
        }
