...
/**
 * A tree map, using a key type of long and a value type of boolean.
 ...
 * Although this map 𝙒𝙊𝙍𝙆𝙎, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.LongBooleanIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class LongBooleanTreeMap implements LongBooleanMap    {
    protected long[] keys;
    ...
    @SuppressWarnings("unchecked")
    protected boolean remove(long key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return false; //no mapping
    ...
    public LongIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.LongBooleanIndexedTreeMap} for an implementation of this method
        return null;
    }
