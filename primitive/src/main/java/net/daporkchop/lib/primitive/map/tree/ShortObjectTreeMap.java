 ...
 * Although this map WORKS, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.ShortObjectIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class ShortObjectTreeMap<V extends Object> implements ShortObjectMap<V>    {
    ...
    @SuppressWarnings("unchecked")
    protected V remove(short key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return null; //no mapping
    ...
    public ShortIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.ShortObjectIndexedTreeMap} for an implementation of this method
        return null;
    }
