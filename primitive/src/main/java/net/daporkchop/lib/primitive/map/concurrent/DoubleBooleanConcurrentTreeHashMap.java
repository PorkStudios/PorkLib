...
import net.daporkchop.lib.primitive.iterator.concurrent.DoubleBooleanConcurrentIterator;
...
import net.daporkchop.lib.primitive.tuple.DoubleBooleanMutableTuple;
...
/**
 * A tree hash map, using a key type of double and a value type of boolean.
 ...
 * @author DaPorkchop_
 */
public class DoubleBooleanConcurrentTreeHashMap implements DoubleBooleanMap    {
    final AtomicInteger size = new AtomicInteger(0);

    private final DoubleBooleanSubEntry[] entries = new DoubleBooleanSubEntry[256];
    private final DoubleToIntegerFunction keyHash;
    final DoubleToLongFunction subKeyHash;
    final int subHashLength;

    public DoubleBooleanConcurrentTreeHashMap()    {
        this(null, null);
    }

    public DoubleBooleanConcurrentTreeHashMap(DoubleToIntegerFunction keyHash, DoubleToLongFunction subKeyHash)    {
        this(keyHash, subKeyHash, 7);
    }

    public DoubleBooleanConcurrentTreeHashMap(DoubleToIntegerFunction keyHash, DoubleToLongFunction subKeyHash, int subHashLength)    {
    ...
    public boolean get(double key)   {
        DoubleBooleanSubEntry entry = this.getSubentry(this.keyHash.apply(key) & 0xFF, false);
        if (entry == null)  {
            return false;
        } else {
    ...
    public boolean put(double key, boolean value)   {
        DoubleBooleanSubEntry entry = this.getSubentry(this.keyHash.apply(key) & 0xFF, true);
    ...
    public boolean remove(double key)    {
        ...
        DoubleBooleanSubEntry entry = this.getSubentry(hash, false);
        if (entry == null)  {
            return false;
        } else {
    ...
    public void clear() {
        synchronized (this.entries) {
            for (int i = 0; i < 256; i++) {
                DoubleBooleanSubEntry entry = this.entries[i];
                if (entry != null)  {
                    entry.clear();
                    this.entries[i] = null;
                }
    ...
    public void forEachKey(DoubleConsumer consumer)   {
        for (int i = 0; i < 256; i++) {
            DoubleBooleanSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
    ...
    @Override
    public void forEachEntry(DoubleBooleanConsumer consumer) {
        for (int i = 0; i < 256; i++) {
            DoubleBooleanSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
    ...
    public void forEachEntry(DoubleBooleanConsumer consumer, VoidFunction complete) {
        for (int i = 0; i < 256; i++) {
            DoubleBooleanSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
                entry.forEachEntry(consumer);
    ...
    public DoubleConcurrentIterator keyIterator()   {
        return new DoubleConcurrentIterator()  {
            private final DoubleBooleanConcurrentTreeHashMap this_ = DoubleBooleanConcurrentTreeHashMap.this;
            ...
            private final AtomicReference<DoubleBooleanSubEntry.Sector> currSector = new AtomicReference(null);
            ...
            private volatile DoubleBooleanSubEntry entry;
            ...
            private void nextSub()   {
                ...
                while (this.subEntryIndex < 256)    {
                    DoubleBooleanSubEntry entry = getSubentry(this.subEntryIndex++, false);
                    if (entry != null)  {
                        this.entry = entry;
            ...
            class ThreadState   {
                public double key;
                public DoubleBooleanSubEntry entry;
                private final DoubleConcurrentIterator this_;
                private final DoubleBooleanConcurrentTreeHashMap this__;

                public ThreadState(DoubleConcurrentIterator this_, DoubleBooleanConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
    ...
    public BooleanConcurrentIterator valueIterator() {
        return new BooleanConcurrentIterator()  {
            ...
            private final AtomicReference<DoubleBooleanSubEntry.Sector> currSector = new AtomicReference(null);
            private final AtomicBoolean hasNextSub = new AtomicBoolean(true);
            private final DoubleBooleanMutableTuple next = new DoubleBooleanMutableTuple();
            private volatile boolean hasNext = true;
            private volatile int subEntryIndex = 0;
            private volatile DoubleBooleanSubEntry entry;
            ...
            class ThreadState   {
                ...
                public ThreadState(BooleanConcurrentIterator this_, DoubleBooleanConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
    ...
    @Override
    public DoubleBooleanConcurrentIterator entryIterator() {
        return new DoubleBooleanConcurrentIterator()  {
            private final DoubleBooleanConcurrentTreeHashMap this_ = DoubleBooleanConcurrentTreeHashMap.this;
            //private final ReentrantLock lock = new ReentrantLock();
            private final ThreadLocal<ThreadState> tl = ThreadLocal.withInitial(() -> new ThreadState(this, this.this_));
            private final AtomicInteger sectorOffset = new AtomicInteger(0);
            ...
            @Override
            public synchronized DoubleBooleanTuple advance()   {
                if (!this.hasNext())    {
                    throw new IteratorCompleteException();
                }
                ThreadState state = this.tl.get();
                DoubleBooleanTuple current = state.tuple = new DoubleBooleanMutableTuple(this.next.k, this.next.v);
                state.entry = this.entry;
                this.findNextValue();
                return current;
            }
            ...
            class ThreadState   {
                public DoubleBooleanTuple tuple;
                public DoubleBooleanSubEntry entry;
                private final DoubleBooleanConcurrentIterator this_;
                private final DoubleBooleanConcurrentTreeHashMap this__;

                public ThreadState(DoubleBooleanConcurrentIterator this_, DoubleBooleanConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
     ...
     */
    public DoubleBooleanConcurrentIterator fastEntryIterator() {
        return new DoubleBooleanConcurrentIterator()  {
            private final DoubleBooleanConcurrentTreeHashMap this_ = DoubleBooleanConcurrentTreeHashMap.this;
            //private final ReentrantLock lock = new ReentrantLock();
            private final ThreadLocal<ThreadState> tl = ThreadLocal.withInitial(() -> new ThreadState(this, this.this_));
            ...
            private final AtomicReference<DoubleBooleanSubEntry.Sector> currSector = new AtomicReference(null);
            private final AtomicBoolean hasNextSub = new AtomicBoolean(true);
            private volatile DoubleBooleanMutableTuple next = new DoubleBooleanMutableTuple(0D, false);
            private volatile boolean hasNext = true;
            private volatile int subEntryIndex = 0;
            private volatile DoubleBooleanSubEntry entry;
            ...
            @Override
            public DoubleBooleanMutableTuple get()   {
                ThreadState state = this.tl.get();
                if (state.entry == null)    {
                    throw new IllegalStateException();
                } else {
            ...
            @Override
            public synchronized DoubleBooleanMutableTuple advance()   {
                if (!this.hasNext())    {
                    throw new IteratorCompleteException();
                }
                ThreadState state = this.tl.get();
                DoubleBooleanMutableTuple a = state.tuple;
            ...
            class ThreadState   {
                public DoubleBooleanMutableTuple tuple = new DoubleBooleanMutableTuple(0D, false);
                public DoubleBooleanSubEntry entry;
                private final DoubleBooleanConcurrentIterator this_;
                private final DoubleBooleanConcurrentTreeHashMap this__;
    ...
    private DoubleBooleanSubEntry getSubentry(int index, boolean create)    {
        assert index == (index & 0xFF);

        synchronized (this.entries) {
            DoubleBooleanSubEntry entry = (DoubleBooleanSubEntry) this.entries[index];
            if (entry == null && create)    {
                this.entries[index] = entry = new DoubleBooleanSubEntry(this);
            }
            return entry;
        }
    }
