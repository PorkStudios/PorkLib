...
import net.daporkchop.lib.primitive.tuple.LongDoubleImmutableTuple;
...
/**
 * A hash map, using a key type of long and a value type of double.
 ...
 * @author DaPorkchop_
 */
public class LongDoubleHashMap implements LongDoubleMap    {
    protected long[] keys;
    ...
    public LongDoubleHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public LongDoubleHashMap(LongToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public LongDoubleHashMap(int baseSize, LongToIntegerFunction keyHash)    {
    ...
    protected boolean reHash(double[] values, long[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
    ...
    private class KeyIterator implements LongIterator {
        ...
        public KeyIterator()    {
            this.size = LongDoubleHashMap.this.size;
            this.next = LongDoubleHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public long get()   {
            return LongDoubleHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = LongDoubleHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            LongDoubleHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements DoubleIterator {
        ...
        @SuppressWarnings("unchecked")
        public double get()   {
            return LongDoubleHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements LongDoubleIterator {
        private int size;
        private int index = -1;
        private int next;
        private LongDoubleTuple tuple;
        ...
        @Override
        public LongDoubleTuple advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new LongDoubleImmutableTuple( LongDoubleHashMap.this.keys[this.index],  LongDoubleHashMap.this.values[this.index]);
        }
