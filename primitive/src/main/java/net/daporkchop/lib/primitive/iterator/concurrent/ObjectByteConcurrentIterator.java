...
import net.daporkchop.lib.primitive.lambda.function.bi.ObjectByteToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of K => byte mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface ObjectByteConcurrentIterator<K extends Object> extends ObjectByteIterator<K>   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectByteToBooleanFunction<K> function)    {
        try {
            while (this.hasNext())  {
                ObjectByteTuple<K> tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<ObjectByteTuple<K>> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<ObjectByteTuple<K>> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectByteConsumer<K> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<ObjectByteTuple<K>> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull ObjectByteToBooleanFunction<K> function)    {
        this.concurrentIterate(function);
    }
}
