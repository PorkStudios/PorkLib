 ...
 * overhead, and can even be made concurrent with little extra work (see {@link net.daporkchop.lib.primitive.map.concurrent.LongFloatConcurrentTreeHashMap})
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
public class LongFloatTreeHashMap implements LongFloatMap    {
    ...
    public LongFloatTreeHashMap()    {
        this(null, 0);
    }

    public LongFloatTreeHashMap(LongToLongFunction hashFunc, int hashLen)    {
        ...
        this.rootPointer = this.hashLen == 0 ? new LongFloatTreeHashMap.ValueSector(null, -1) : new LongFloatTreeHashMap.PointerSector(null, -1);
    }
    ...
    public void forEachKey(@NonNull LongConsumer consumer)  {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof LongFloatTreeHashMap.PointerSector)    {
    ...
    private Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.hashLen; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.hashLen ? new LongFloatTreeHashMap.ValueSector(curr, off) : new LongFloatTreeHashMap.PointerSector(curr, off);
    ...
    private class ValueSector extends Sector   {
        ...
        public float setEntry(int index, long key, float value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                LongFloatTreeHashMap.this.size.incrementAndGet();
                return 0F;
            }
        }
        ...
        public float removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                LongFloatTreeHashMap.this.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
