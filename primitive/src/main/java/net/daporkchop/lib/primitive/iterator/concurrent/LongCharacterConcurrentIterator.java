...
import net.daporkchop.lib.primitive.lambda.function.bi.LongCharacterToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of long => char mappings. Implementing classes should be
 * thread safe and concurrent (calling advance() should throw {@link IteratorCompleteException} instead of 0L,
 ...
 * @author DaPorkchop_
 */
public interface LongCharacterConcurrentIterator extends LongCharacterIterator   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull LongCharacterToBooleanFunction function)    {
        try {
            while (this.hasNext())  {
                LongCharacterTuple tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<LongCharacterTuple> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<LongCharacterTuple> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull LongCharacterConsumer consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<LongCharacterTuple> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull LongCharacterToBooleanFunction function)    {
        this.concurrentIterate(function);
    }
}
