...
import net.daporkchop.lib.primitive.lambda.function.bi.FloatLongToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of float => long mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface FloatLongConcurrentIterator extends FloatLongIterator   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull FloatLongToBooleanFunction function)    {
        try {
            while (this.hasNext())  {
                FloatLongTuple tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<FloatLongTuple> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<FloatLongTuple> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull FloatLongConsumer consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<FloatLongTuple> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull FloatLongToBooleanFunction function)    {
        this.concurrentIterate(function);
    }
}
