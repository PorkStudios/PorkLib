...
/**
 * A single subentry for a {@link ObjectByteConcurrentTreeHashMap}.
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
class ObjectByteSubEntry<K extends Object>    {
    public final AtomicInteger size = new AtomicInteger(0);
    public final ObjectByteConcurrentTreeHashMap parent;
    ...
    public ObjectByteSubEntry(ObjectByteConcurrentTreeHashMap parent)   {
        this.parent = parent;
        this.rootPointer = this.parent.subHashLength == 0 ? new ObjectByteSubEntry<K>.ValueSector(null, -1) : new ObjectByteSubEntry<K>.PointerSector(null, -1);
    }
    ...
    public void clear() {
        this.lock.lock();
        try {
            this.rootPointer = this.parent.subHashLength == 0 ? new ObjectByteSubEntry.ValueSector(null, -1) : new ObjectByteSubEntry.PointerSector(null, -1);
    ...
    public void forEachKey(ObjectConsumer<K> consumer)   {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof ObjectByteSubEntry.PointerSector)    {
    ...
    public K advanceKey(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext)   {
        ...
        for (int i = sector.states.nextSetBit(curr.get() + 1); i != -1; i = sector.states.nextSetBit(i + 1)) {
            curr.set(i);
            if (sector instanceof ObjectByteSubEntry.ValueSector)  {
    ...
    public void advanceEntry(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext, ObjectByteMutableTuple<K> tuple)   {
        Sector sector = sectorRef.get();
        if (sector == null) {
            sectorRef.set(sector = this.rootPointer);
        }
    ...
    public Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.parent.subHashLength; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.parent.subHashLength ? new ObjectByteSubEntry<K>.ValueSector(curr, off) : new ObjectByteSubEntry<K>.PointerSector(curr, off);
    ...
    public class ValueSector extends Sector   {
        ...
        public byte setEntry(int index, Object key, byte value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                ObjectByteSubEntry.this.size.incrementAndGet();
                ObjectByteSubEntry.this.parent.size.incrementAndGet();
                return (byte) 0;
            }
        }
        ...
        public byte removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                ObjectByteSubEntry.this.size.decrementAndGet();
                ObjectByteSubEntry.this.parent.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
