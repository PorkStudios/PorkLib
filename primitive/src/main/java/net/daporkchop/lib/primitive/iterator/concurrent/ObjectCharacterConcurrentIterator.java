...
import net.daporkchop.lib.primitive.lambda.function.bi.ObjectCharacterToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of K => char mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface ObjectCharacterConcurrentIterator<K extends Object> extends ObjectCharacterIterator<K>   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectCharacterToBooleanFunction<K> function)    {
        try {
            while (this.hasNext())  {
                ObjectCharacterTuple<K> tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<ObjectCharacterTuple<K>> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<ObjectCharacterTuple<K>> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectCharacterConsumer<K> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<ObjectCharacterTuple<K>> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull ObjectCharacterToBooleanFunction<K> function)    {
        this.concurrentIterate(function);
    }
}
