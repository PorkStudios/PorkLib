...
/**
 * A hash map, using a key type of int and a value type of int.
 ...
 * @author DaPorkchop_
 */
public class IntegerIntegerHashMap implements IntegerIntegerMap    {
    protected int[] keys;
    ...
    public IntegerIntegerHashMap()    {
        this(16384);
    }

    public IntegerIntegerHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public IntegerIntegerHashMap(IntegerToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public IntegerIntegerHashMap(int baseSize, IntegerToIntegerFunction keyHash)    {
    ...
    protected boolean reHash(int[] values, int[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
            int key = this.keys[i];
            int j = getIndex(key);
            if (states.get(j))  {
    ...
    private class KeyIterator implements IntegerIterator {
        ...
        public KeyIterator()    {
            this.size = IntegerIntegerHashMap.this.size;
            this.next = IntegerIntegerHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public int get()   {
            return IntegerIntegerHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = IntegerIntegerHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            IntegerIntegerHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements IntegerIterator {
        ...
        @SuppressWarnings("unchecked")
        public int get()   {
            return IntegerIntegerHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements IntegerIntegerIterator {
        private int size;
        private int index = -1;
        private int next;
        private IntegerIntegerTuple tuple;
        ...
        @Override
        public IntegerIntegerTuple advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new IntegerIntegerImmutableTuple( IntegerIntegerHashMap.this.keys[this.index],  IntegerIntegerHashMap.this.values[this.index]);
        }
