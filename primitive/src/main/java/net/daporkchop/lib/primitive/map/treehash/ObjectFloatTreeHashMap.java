 ...
 * overhead, and can even be made concurrent with little extra work (see {@link net.daporkchop.lib.primitive.map.concurrent.ObjectFloatConcurrentTreeHashMap})
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
public class ObjectFloatTreeHashMap<K extends Object> implements ObjectFloatMap<K>    {
    ...
    public ObjectFloatTreeHashMap()    {
        this(null, 0);
    }

    public ObjectFloatTreeHashMap(ObjectToLongFunction hashFunc, int hashLen)    {
        ...
        this.rootPointer = this.hashLen == 0 ? new ObjectFloatTreeHashMap<K>.ValueSector(null, -1) : new ObjectFloatTreeHashMap<K>.PointerSector(null, -1);
    }
    ...
    @Override
    public void clear() {
        this.rootPointer = this.hashLen == 0 ? new ObjectFloatTreeHashMap.ValueSector(null, -1) : new ObjectFloatTreeHashMap.PointerSector(null, -1);
    }
    ...
    public void forEachKey(@NonNull ObjectConsumer<K> consumer)  {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof ObjectFloatTreeHashMap.PointerSector)    {
    ...
    private Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.hashLen; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.hashLen ? new ObjectFloatTreeHashMap<K>.ValueSector(curr, off) : new ObjectFloatTreeHashMap<K>.PointerSector(curr, off);
    ...
    private class ValueSector extends Sector   {
        ...
        public float setEntry(int index, Object key, float value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                ObjectFloatTreeHashMap.this.size.incrementAndGet();
                return 0F;
            }
        }
        ...
        public float removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                ObjectFloatTreeHashMap.this.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
