...
import net.daporkchop.lib.primitive.lambda.function.bi.ObjectObjectToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of K => V mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface ObjectObjectConcurrentIterator<K extends Object, V extends Object> extends ObjectObjectIterator<K, V>   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectObjectToBooleanFunction<K, V> function)    {
        try {
            while (this.hasNext())  {
                ObjectObjectTuple<K, V> tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<ObjectObjectTuple<K, V>> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<ObjectObjectTuple<K, V>> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectObjectConsumer<K, V> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<ObjectObjectTuple<K, V>> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull ObjectObjectToBooleanFunction<K, V> function)    {
        this.concurrentIterate(function);
    }
}
