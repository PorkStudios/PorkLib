...
/**
 * A tree map, using a key type of char and a value type of char.
 ...
 * Although this map WORKS, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.CharacterCharacterIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class CharacterCharacterTreeMap implements CharacterCharacterMap    {
    protected char[] keys;
    ...
    public CharacterCharacterTreeMap()    {
        this.clear();
    }
    ...
    @SuppressWarnings("unchecked")
    protected char remove(char key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return (char) 0; //no mapping
    ...
    public CharacterIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.CharacterCharacterIndexedTreeMap} for an implementation of this method
        return null;
    }
