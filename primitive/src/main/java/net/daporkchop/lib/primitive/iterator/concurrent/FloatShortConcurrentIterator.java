...
import net.daporkchop.lib.primitive.lambda.function.bi.FloatShortToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of float => short mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface FloatShortConcurrentIterator extends FloatShortIterator   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull FloatShortToBooleanFunction function)    {
        try {
            while (this.hasNext())  {
                FloatShortTuple tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<FloatShortTuple> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<FloatShortTuple> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull FloatShortConsumer consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<FloatShortTuple> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull FloatShortToBooleanFunction function)    {
        this.concurrentIterate(function);
    }
}
