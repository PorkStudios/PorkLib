...
/**
 * A tree map, using a key type of long and a value type of V.
 ...
 * Although this map 𝙒𝙊𝙍𝙆𝙎, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.LongObjectIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class LongObjectTreeMap<V extends Object> implements LongObjectMap<V>    {
    protected long[] keys;
    ...
    @SuppressWarnings("unchecked")
    protected V remove(long key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return null; //no mapping
    ...
    public LongIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.LongObjectIndexedTreeMap} for an implementation of this method
        return null;
    }
