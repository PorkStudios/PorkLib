...
/**
 * Allows parallel iteration over an iterator of float values. Implementing classes should be thread safe
 * and concurrent (calling advance() should throw {@link IteratorCompleteException} instead of 0F,
 ...
 * @author DaPorkchop_
 */
public interface FloatConcurrentIterator extends FloatIterator   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull FloatToBooleanFunction function)    {
        try {
            while (this.hasNext())  {
                float val = this.advance();
    ...
    @Override
    default void forEachRemaining(@NonNull FloatConsumer consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull FloatToBooleanFunction function)   {
