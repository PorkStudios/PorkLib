...
/**
 * A single subentry for a {@link LongObjectConcurrentTreeHashMap}.
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
class LongObjectSubEntry<V extends Object>    {
    public final AtomicInteger size = new AtomicInteger(0);
    public final LongObjectConcurrentTreeHashMap parent;
    ...
    public LongObjectSubEntry(LongObjectConcurrentTreeHashMap parent)   {
        this.parent = parent;
        this.rootPointer = this.parent.subHashLength == 0 ? new LongObjectSubEntry<V>.ValueSector(null, -1) : new LongObjectSubEntry<V>.PointerSector(null, -1);
    }
    ...
    public void clear() {
        this.lock.lock();
        try {
            this.rootPointer = this.parent.subHashLength == 0 ? new LongObjectSubEntry.ValueSector(null, -1) : new LongObjectSubEntry.PointerSector(null, -1);
    ...
    public void forEachKey(LongConsumer consumer)   {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof LongObjectSubEntry.PointerSector)    {
    ...
    public long advanceKey(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext)   {
        ...
        for (int i = sector.states.nextSetBit(curr.get() + 1); i != -1; i = sector.states.nextSetBit(i + 1)) {
            curr.set(i);
            if (sector instanceof LongObjectSubEntry.ValueSector)  {
    ...
    public void advanceEntry(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext, LongObjectMutableTuple<V> tuple)   {
        Sector sector = sectorRef.get();
        if (sector == null) {
            sectorRef.set(sector = this.rootPointer);
        }
    ...
    public Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.parent.subHashLength; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.parent.subHashLength ? new LongObjectSubEntry<V>.ValueSector(curr, off) : new LongObjectSubEntry<V>.PointerSector(curr, off);
    ...
    public abstract class Sector  {
        ...
        public Object setEntry(int index, long key, Object value)   {
            throw new UnsupportedOperationException();
        }
        ...
        public void forEachEntry(LongObjectConsumer consumer)   {
            throw new UnsupportedOperationException();
        }
    ...
    public class ValueSector extends Sector   {
        ...
        public Object setEntry(int index, long key, Object value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                LongObjectSubEntry.this.size.incrementAndGet();
                LongObjectSubEntry.this.parent.size.incrementAndGet();
                return null;
            }
        }
        ...
        public Object removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                LongObjectSubEntry.this.size.decrementAndGet();
                LongObjectSubEntry.this.parent.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
