...
/**
 * Allows parallel iteration over an iterator of short values. Implementing classes should be thread safe
 * and concurrent (calling advance() should throw {@link IteratorCompleteException} instead of (short) 0,
 ...
 * @author DaPorkchop_
 */
public interface ShortConcurrentIterator extends ShortIterator   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ShortToBooleanFunction function)    {
        try {
            while (this.hasNext())  {
                short val = this.advance();
    ...
    @Override
    default void forEachRemaining(@NonNull ShortConsumer consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ShortToBooleanFunction function)   {
