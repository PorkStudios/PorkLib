...
import net.daporkchop.lib.primitive.iterator.concurrent.DoubleByteConcurrentIterator;
import net.daporkchop.lib.primitive.map.DoubleByteMap;
import net.daporkchop.lib.primitive.tuple.DoubleByteTuple;
import net.daporkchop.lib.primitive.tuple.DoubleByteMutableTuple;
...
/**
 * A tree hash map, using a key type of double and a value type of byte.
 ...
 * @author DaPorkchop_
 */
public class DoubleByteConcurrentTreeHashMap implements DoubleByteMap    {
    final AtomicInteger size = new AtomicInteger(0);

    private final DoubleByteSubEntry[] entries = new DoubleByteSubEntry[256];
    private final DoubleToIntegerFunction keyHash;
    final DoubleToLongFunction subKeyHash;
    final int subHashLength;

    public DoubleByteConcurrentTreeHashMap()    {
        this(null, null);
    }

    public DoubleByteConcurrentTreeHashMap(DoubleToIntegerFunction keyHash, DoubleToLongFunction subKeyHash)    {
        this(keyHash, subKeyHash, 7);
    }

    public DoubleByteConcurrentTreeHashMap(DoubleToIntegerFunction keyHash, DoubleToLongFunction subKeyHash, int subHashLength)    {
    ...
    public byte get(double key)   {
        DoubleByteSubEntry entry = this.getSubentry(this.keyHash.apply(key) & 0xFF, false);
        if (entry == null)  {
            return (byte) 0;
        } else {
    ...
    public byte put(double key, byte value)   {
        DoubleByteSubEntry entry = this.getSubentry(this.keyHash.apply(key) & 0xFF, true);
    ...
    public byte remove(double key)    {
        ...
        DoubleByteSubEntry entry = this.getSubentry(hash, false);
        if (entry == null)  {
            return (byte) 0;
        } else {
    ...
    public void clear() {
        synchronized (this.entries) {
            for (int i = 0; i < 256; i++) {
                DoubleByteSubEntry entry = this.entries[i];
                if (entry != null)  {
                    entry.clear();
                    this.entries[i] = null;
                }
    ...
    public void forEachKey(DoubleConsumer consumer)   {
        for (int i = 0; i < 256; i++) {
            DoubleByteSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
    ...
    @Override
    public void forEachEntry(DoubleByteConsumer consumer) {
        for (int i = 0; i < 256; i++) {
            DoubleByteSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
    ...
    public void forEachEntry(DoubleByteConsumer consumer, VoidFunction complete) {
        for (int i = 0; i < 256; i++) {
            DoubleByteSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
                entry.forEachEntry(consumer);
    ...
    public DoubleConcurrentIterator keyIterator()   {
        return new DoubleConcurrentIterator()  {
            private final DoubleByteConcurrentTreeHashMap this_ = DoubleByteConcurrentTreeHashMap.this;
            ...
            private final AtomicReference<DoubleByteSubEntry.Sector> currSector = new AtomicReference(null);
            ...
            private volatile DoubleByteSubEntry entry;
            ...
            private void nextSub()   {
                ...
                while (this.subEntryIndex < 256)    {
                    DoubleByteSubEntry entry = getSubentry(this.subEntryIndex++, false);
                    if (entry != null)  {
                        this.entry = entry;
            ...
            class ThreadState   {
                public double key;
                public DoubleByteSubEntry entry;
                private final DoubleConcurrentIterator this_;
                private final DoubleByteConcurrentTreeHashMap this__;

                public ThreadState(DoubleConcurrentIterator this_, DoubleByteConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
    ...
    public ByteConcurrentIterator valueIterator() {
        return new ByteConcurrentIterator()  {
            ...
            private final AtomicReference<DoubleByteSubEntry.Sector> currSector = new AtomicReference(null);
            private final AtomicBoolean hasNextSub = new AtomicBoolean(true);
            private final DoubleByteMutableTuple next = new DoubleByteMutableTuple();
            private volatile boolean hasNext = true;
            private volatile int subEntryIndex = 0;
            private volatile DoubleByteSubEntry entry;
            ...
            class ThreadState   {
                ...
                public ThreadState(ByteConcurrentIterator this_, DoubleByteConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
    ...
    @Override
    public DoubleByteConcurrentIterator entryIterator() {
        return new DoubleByteConcurrentIterator()  {
            private final DoubleByteConcurrentTreeHashMap this_ = DoubleByteConcurrentTreeHashMap.this;
            //private final ReentrantLock lock = new ReentrantLock();
            private final ThreadLocal<ThreadState> tl = ThreadLocal.withInitial(() -> new ThreadState(this, this.this_));
            private final AtomicInteger sectorOffset = new AtomicInteger(0);
            ...
            @Override
            public synchronized DoubleByteTuple advance()   {
                if (!this.hasNext())    {
                    throw new IteratorCompleteException();
                }
                ThreadState state = this.tl.get();
                DoubleByteTuple current = state.tuple = new DoubleByteMutableTuple(this.next.k, this.next.v);
                state.entry = this.entry;
                this.findNextValue();
                return current;
            }
            ...
            class ThreadState   {
                public DoubleByteTuple tuple;
                public DoubleByteSubEntry entry;
                private final DoubleByteConcurrentIterator this_;
                private final DoubleByteConcurrentTreeHashMap this__;

                public ThreadState(DoubleByteConcurrentIterator this_, DoubleByteConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
     ...
     */
    public DoubleByteConcurrentIterator fastEntryIterator() {
        return new DoubleByteConcurrentIterator()  {
            private final DoubleByteConcurrentTreeHashMap this_ = DoubleByteConcurrentTreeHashMap.this;
            //private final ReentrantLock lock = new ReentrantLock();
            private final ThreadLocal<ThreadState> tl = ThreadLocal.withInitial(() -> new ThreadState(this, this.this_));
            ...
            private final AtomicReference<DoubleByteSubEntry.Sector> currSector = new AtomicReference(null);
            private final AtomicBoolean hasNextSub = new AtomicBoolean(true);
            private volatile DoubleByteMutableTuple next = new DoubleByteMutableTuple(0D, (byte) 0);
            private volatile boolean hasNext = true;
            private volatile int subEntryIndex = 0;
            private volatile DoubleByteSubEntry entry;
            ...
            @Override
            public DoubleByteMutableTuple get()   {
                ThreadState state = this.tl.get();
                if (state.entry == null)    {
                    throw new IllegalStateException();
                } else {
            ...
            @Override
            public synchronized DoubleByteMutableTuple advance()   {
                if (!this.hasNext())    {
                    throw new IteratorCompleteException();
                }
                ThreadState state = this.tl.get();
                DoubleByteMutableTuple a = state.tuple;
            ...
            class ThreadState   {
                public DoubleByteMutableTuple tuple = new DoubleByteMutableTuple(0D, (byte) 0);
                public DoubleByteSubEntry entry;
                private final DoubleByteConcurrentIterator this_;
                private final DoubleByteConcurrentTreeHashMap this__;
    ...
    private DoubleByteSubEntry getSubentry(int index, boolean create)    {
        assert index == (index & 0xFF);

        synchronized (this.entries) {
            DoubleByteSubEntry entry = (DoubleByteSubEntry) this.entries[index];
            if (entry == null && create)    {
                this.entries[index] = entry = new DoubleByteSubEntry(this);
            }
            return entry;
        }
    }
