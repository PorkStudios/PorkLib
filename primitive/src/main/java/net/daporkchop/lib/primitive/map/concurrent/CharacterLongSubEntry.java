...
/**
 * A single subentry for a {@link CharacterLongConcurrentTreeHashMap}.
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
class CharacterLongSubEntry    {
    public final AtomicInteger size = new AtomicInteger(0);
    public final CharacterLongConcurrentTreeHashMap parent;
    ...
    public CharacterLongSubEntry(CharacterLongConcurrentTreeHashMap parent)   {
        this.parent = parent;
        this.rootPointer = this.parent.subHashLength == 0 ? new CharacterLongSubEntry.ValueSector(null, -1) : new CharacterLongSubEntry.PointerSector(null, -1);
    }
    ...
    public void forEachKey(CharacterConsumer consumer)   {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof CharacterLongSubEntry.PointerSector)    {
    ...
    public char advanceKey(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext)   {
        ...
        for (int i = sector.states.nextSetBit(curr.get() + 1); i != -1; i = sector.states.nextSetBit(i + 1)) {
            curr.set(i);
            if (sector instanceof CharacterLongSubEntry.ValueSector)  {
    ...
    public void advanceEntry(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext, CharacterLongMutableTuple tuple)   {
        Sector sector = sectorRef.get();
        if (sector == null) {
            sectorRef.set(sector = this.rootPointer);
        }
    ...
    public Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.parent.subHashLength; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.parent.subHashLength ? new CharacterLongSubEntry.ValueSector(curr, off) : new CharacterLongSubEntry.PointerSector(curr, off);
    ...
    public abstract class Sector  {
        ...
        public long setEntry(int index, char key, long value)   {
            throw new UnsupportedOperationException();
        }
    ...
    public class ValueSector extends Sector   {
        ...
        public long setEntry(int index, char key, long value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                CharacterLongSubEntry.this.size.incrementAndGet();
                CharacterLongSubEntry.this.parent.size.incrementAndGet();
                return 0L;
            }
        }
        ...
        public long removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                CharacterLongSubEntry.this.size.decrementAndGet();
                CharacterLongSubEntry.this.parent.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
