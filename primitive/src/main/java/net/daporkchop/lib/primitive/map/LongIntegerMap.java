...
/**
 * A map, using a key type of long and a value type of int.
 * <p>
...
public interface LongIntegerMap   {
     ...
     */
    default void putAll(@NonNull long[] keys, @NonNull int[] values)    {
     ...
     */
    default void putAll(@NonNull LongIntegerMap map)    {
     ...
     * @param consumer the function to run
     */
    void forEachEntry(LongIntegerConsumer consumer);
     ...
     */
    LongIntegerIterator entryIterator();
     ...
     */
    default int computeIfAbsent(long key, @NonNull LongToIntegerFunction func) {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
