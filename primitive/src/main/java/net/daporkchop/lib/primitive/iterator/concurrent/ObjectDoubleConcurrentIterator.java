...
import net.daporkchop.lib.primitive.lambda.function.bi.ObjectDoubleToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of K => double mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface ObjectDoubleConcurrentIterator<K extends Object> extends ObjectDoubleIterator<K>   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectDoubleToBooleanFunction<K> function)    {
        try {
            while (this.hasNext())  {
                ObjectDoubleTuple<K> tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<ObjectDoubleTuple<K>> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<ObjectDoubleTuple<K>> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectDoubleConsumer<K> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<ObjectDoubleTuple<K>> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull ObjectDoubleToBooleanFunction<K> function)    {
        this.concurrentIterate(function);
    }
}
