 ...
 * Although this map 𝙒𝙊𝙍𝙆𝙎, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.ByteShortIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class ByteShortTreeMap implements ByteShortMap    {
    protected byte[] keys;
    ...
    @SuppressWarnings("unchecked")
    protected short remove(byte key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return (short) 0; //no mapping
    ...
    public ByteIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.ByteShortIndexedTreeMap} for an implementation of this method
        return null;
    }
