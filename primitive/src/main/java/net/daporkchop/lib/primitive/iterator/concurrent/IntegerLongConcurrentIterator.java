...
import net.daporkchop.lib.primitive.lambda.function.bi.IntegerLongToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of int => long mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface IntegerLongConcurrentIterator extends IntegerLongIterator   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull IntegerLongToBooleanFunction function)    {
        try {
            while (this.hasNext())  {
                IntegerLongTuple tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<IntegerLongTuple> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<IntegerLongTuple> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull IntegerLongConsumer consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<IntegerLongTuple> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull IntegerLongToBooleanFunction function)    {
        this.concurrentIterate(function);
    }
}
