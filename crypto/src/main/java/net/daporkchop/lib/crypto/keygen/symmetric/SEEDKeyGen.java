...
public class SEEDKeyGen {
    static {
        BouncyCastleInit.loadClass();
    }

    public static SEEDKey gen(byte[] data) {
        byte[] b = new byte[BlockCipherType.SEED.blockSize];
        int i = 0;
        do {
        ...
        data = b;
        return new SEEDKey(new SecretKeySpec(data, "aaa"), KeyRandom.getBytes(BlockCipherType.SEED.blockSize));
    }

    public static SEEDKey gen() {
        byte[] key = new byte[BlockCipherType.SEED.blockSize];
