...
import net.daporkchop.lib.primitive.lambda.function.bi.ObjectBooleanToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of K => boolean mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface ObjectBooleanConcurrentIterator<K extends Object> extends ObjectBooleanIterator<K>   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectBooleanToBooleanFunction<K> function)    {
        try {
            while (this.hasNext())  {
                ObjectBooleanTuple<K> tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<ObjectBooleanTuple<K>> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<ObjectBooleanTuple<K>> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectBooleanConsumer<K> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<ObjectBooleanTuple<K>> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull ObjectBooleanToBooleanFunction<K> function)    {
        this.concurrentIterate(function);
    }
}
