...
/**
 * A hash map, using a key type of char and a value type of float.
 ...
 * @author DaPorkchop_
 */
public class CharacterFloatHashMap implements CharacterFloatMap    {
    protected char[] keys;
    ...
    public CharacterFloatHashMap()    {
        this(16384);
    }

    public CharacterFloatHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public CharacterFloatHashMap(CharacterToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public CharacterFloatHashMap(int baseSize, CharacterToIntegerFunction keyHash)    {
    ...
    @Override
    public void forEachEntry(@NonNull CharacterFloatConsumer consumer)   {
    ...
    public CharacterFloatIterator entryIterator() {
    ...
    protected boolean reHash(float[] values, char[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
    ...
    private class KeyIterator implements CharacterIterator {
        ...
        public KeyIterator()    {
            this.size = CharacterFloatHashMap.this.size;
            this.next = CharacterFloatHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public char get()   {
            return CharacterFloatHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = CharacterFloatHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            CharacterFloatHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements FloatIterator {
        ...
        @SuppressWarnings("unchecked")
        public float get()   {
            return CharacterFloatHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements CharacterFloatIterator {
        private int size;
        private int index = -1;
        private int next;
        private CharacterFloatTuple tuple;
        ...
        @Override
        public CharacterFloatTuple advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new CharacterFloatImmutableTuple( CharacterFloatHashMap.this.keys[this.index],  CharacterFloatHashMap.this.values[this.index]);
        }
