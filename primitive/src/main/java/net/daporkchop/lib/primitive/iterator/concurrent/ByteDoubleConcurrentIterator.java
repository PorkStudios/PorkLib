...
import net.daporkchop.lib.primitive.lambda.function.bi.ByteDoubleToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of byte => double mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface ByteDoubleConcurrentIterator extends ByteDoubleIterator   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ByteDoubleToBooleanFunction function)    {
        try {
            while (this.hasNext())  {
                ByteDoubleTuple tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<ByteDoubleTuple> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<ByteDoubleTuple> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ByteDoubleConsumer consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<ByteDoubleTuple> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull ByteDoubleToBooleanFunction function)    {
        this.concurrentIterate(function);
    }
}
