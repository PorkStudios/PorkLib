...
/**
 * A single subentry for a {@link DoubleLongConcurrentTreeHashMap}.
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
class DoubleLongSubEntry    {
    public final AtomicInteger size = new AtomicInteger(0);
    public final DoubleLongConcurrentTreeHashMap parent;
    ...
    public DoubleLongSubEntry(DoubleLongConcurrentTreeHashMap parent)   {
        this.parent = parent;
        this.rootPointer = this.parent.subHashLength == 0 ? new DoubleLongSubEntry.ValueSector(null, -1) : new DoubleLongSubEntry.PointerSector(null, -1);
    }
    ...
    public void forEachKey(DoubleConsumer consumer)   {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof DoubleLongSubEntry.PointerSector)    {
    ...
    public double advanceKey(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext)   {
        ...
        for (int i = sector.states.nextSetBit(curr.get() + 1); i != -1; i = sector.states.nextSetBit(i + 1)) {
            curr.set(i);
            if (sector instanceof DoubleLongSubEntry.ValueSector)  {
    ...
    public void advanceEntry(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext, DoubleLongMutableTuple tuple)   {
        Sector sector = sectorRef.get();
        if (sector == null) {
            sectorRef.set(sector = this.rootPointer);
        }
    ...
    public Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.parent.subHashLength; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.parent.subHashLength ? new DoubleLongSubEntry.ValueSector(curr, off) : new DoubleLongSubEntry.PointerSector(curr, off);
    ...
    public abstract class Sector  {
        ...
        public long setEntry(int index, double key, long value)   {
            throw new UnsupportedOperationException();
        }
    ...
    public class ValueSector extends Sector   {
        ...
        public long setEntry(int index, double key, long value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                DoubleLongSubEntry.this.size.incrementAndGet();
                DoubleLongSubEntry.this.parent.size.incrementAndGet();
                return 0L;
            }
        }
        ...
        public long removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                DoubleLongSubEntry.this.size.decrementAndGet();
                DoubleLongSubEntry.this.parent.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
