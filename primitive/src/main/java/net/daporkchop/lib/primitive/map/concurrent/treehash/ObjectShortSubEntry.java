...
/**
 * A single subentry for a {@link ObjectShortConcurrentTreeHashMap}.
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
class ObjectShortSubEntry<K extends Object>    {
    public final AtomicInteger size = new AtomicInteger(0);
    public final ObjectShortConcurrentTreeHashMap parent;
    ...
    public ObjectShortSubEntry(ObjectShortConcurrentTreeHashMap parent)   {
        this.parent = parent;
        this.rootPointer = this.parent.subHashLength == 0 ? new ObjectShortSubEntry<K>.ValueSector(null, -1) : new ObjectShortSubEntry<K>.PointerSector(null, -1);
    }
    ...
    public void clear() {
        this.lock.lock();
        try {
            this.rootPointer = this.parent.subHashLength == 0 ? new ObjectShortSubEntry.ValueSector(null, -1) : new ObjectShortSubEntry.PointerSector(null, -1);
    ...
    public void forEachKey(ObjectConsumer<K> consumer)   {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof ObjectShortSubEntry.PointerSector)    {
    ...
    public K advanceKey(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext)   {
        ...
        for (int i = sector.states.nextSetBit(curr.get() + 1); i != -1; i = sector.states.nextSetBit(i + 1)) {
            curr.set(i);
            if (sector instanceof ObjectShortSubEntry.ValueSector)  {
    ...
    public void advanceEntry(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext, ObjectShortMutableTuple<K> tuple)   {
        Sector sector = sectorRef.get();
        if (sector == null) {
            sectorRef.set(sector = this.rootPointer);
        }
    ...
    public Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.parent.subHashLength; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.parent.subHashLength ? new ObjectShortSubEntry<K>.ValueSector(curr, off) : new ObjectShortSubEntry<K>.PointerSector(curr, off);
    ...
    public abstract class Sector  {
        ...
        public short setEntry(int index, Object key, short value)   {
            throw new UnsupportedOperationException();
        }
        ...
        public void forEachEntry(ObjectShortConsumer consumer)   {
            throw new UnsupportedOperationException();
        }
    ...
    public class ValueSector extends Sector   {
        ...
        public short setEntry(int index, Object key, short value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                ObjectShortSubEntry.this.size.incrementAndGet();
                ObjectShortSubEntry.this.parent.size.incrementAndGet();
                return (short) 0;
            }
        }
        ...
        public short removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                ObjectShortSubEntry.this.size.decrementAndGet();
                ObjectShortSubEntry.this.parent.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
