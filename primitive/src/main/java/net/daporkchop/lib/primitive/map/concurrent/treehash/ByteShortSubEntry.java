...
/**
 * A single subentry for a {@link ByteShortConcurrentTreeHashMap}.
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
class ByteShortSubEntry    {
    public final AtomicInteger size = new AtomicInteger(0);
    public final ByteShortConcurrentTreeHashMap parent;
    ...
    public ByteShortSubEntry(ByteShortConcurrentTreeHashMap parent)   {
        this.parent = parent;
        this.rootPointer = this.parent.subHashLength == 0 ? new ByteShortSubEntry.ValueSector(null, -1) : new ByteShortSubEntry.PointerSector(null, -1);
    }
    ...
    public void forEachKey(ByteConsumer consumer)   {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof ByteShortSubEntry.PointerSector)    {
    ...
    public byte advanceKey(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext)   {
        ...
        for (int i = sector.states.nextSetBit(curr.get() + 1); i != -1; i = sector.states.nextSetBit(i + 1)) {
            curr.set(i);
            if (sector instanceof ByteShortSubEntry.ValueSector)  {
    ...
    public void advanceEntry(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext, ByteShortMutableTuple tuple)   {
        Sector sector = sectorRef.get();
        if (sector == null) {
            sectorRef.set(sector = this.rootPointer);
        }
    ...
    public Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.parent.subHashLength; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.parent.subHashLength ? new ByteShortSubEntry.ValueSector(curr, off) : new ByteShortSubEntry.PointerSector(curr, off);
    ...
    public abstract class Sector  {
        ...
        public short setEntry(int index, byte key, short value)   {
            throw new UnsupportedOperationException();
        }
    ...
    public class ValueSector extends Sector   {
        ...
        public short setEntry(int index, byte key, short value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                ByteShortSubEntry.this.size.incrementAndGet();
                ByteShortSubEntry.this.parent.size.incrementAndGet();
                return (short) 0;
            }
        }
        ...
        public short removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                ByteShortSubEntry.this.size.decrementAndGet();
                ByteShortSubEntry.this.parent.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
