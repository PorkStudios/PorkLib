...
import net.daporkchop.lib.primitive.lambda.function.CharacterToDoubleFunction;
...
/**
 * A map, using a key type of char and a value type of double.
 * <p>
...
public interface CharacterDoubleMap   {
     ...
     */
    default void putAll(@NonNull char[] keys, @NonNull double[] values)    {
     ...
     */
    default void putAll(@NonNull CharacterDoubleMap map)    {
     ...
     * @param consumer the function to run
     */
    void forEachEntry(CharacterDoubleConsumer consumer);
     ...
     */
    CharacterDoubleIterator entryIterator();
     ...
     */
    default double getOrDefault(char key, double def)   {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
            this.put(key, def);
     ...
     */
    default double computeIfAbsent(char key, @NonNull CharacterToDoubleFunction func) {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
