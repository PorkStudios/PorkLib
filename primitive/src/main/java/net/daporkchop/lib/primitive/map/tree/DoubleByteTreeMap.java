...
/**
 * A tree map, using a key type of double and a value type of byte.
 ...
 * Although this map WORKS, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.DoubleByteIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class DoubleByteTreeMap implements DoubleByteMap    {
    ...
    @SuppressWarnings("unchecked")
    protected byte remove(double key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return (byte) 0; //no mapping
    ...
    public DoubleIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.DoubleByteIndexedTreeMap} for an implementation of this method
        return null;
    }
