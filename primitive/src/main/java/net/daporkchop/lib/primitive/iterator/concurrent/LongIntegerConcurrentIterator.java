...
import net.daporkchop.lib.primitive.lambda.function.bi.LongIntegerToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of long => int mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface LongIntegerConcurrentIterator extends LongIntegerIterator   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull LongIntegerToBooleanFunction function)    {
        try {
            while (this.hasNext())  {
                LongIntegerTuple tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<LongIntegerTuple> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<LongIntegerTuple> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull LongIntegerConsumer consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<LongIntegerTuple> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull LongIntegerToBooleanFunction function)    {
        this.concurrentIterate(function);
    }
}
