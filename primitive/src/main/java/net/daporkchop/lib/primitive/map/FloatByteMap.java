...
import net.daporkchop.lib.primitive.lambda.function.FloatToByteFunction;
...
/**
 * A map, using a key type of float and a value type of byte.
 * <p>
...
public interface FloatByteMap   {
     ...
     */
    default void putAll(@NonNull float[] keys, @NonNull byte[] values)    {
     ...
     */
    default void putAll(@NonNull FloatByteMap map)    {
     ...
     */
    default void removeAll(@NonNull Collection<Float> keys) {
     ...
     */
    void forEachKey(FloatConsumer consumer);
     ...
     * @param consumer the function to run
     */
    void forEachEntry(FloatByteConsumer consumer);
     ...
     */
    FloatByteIterator entryIterator();
     ...
     */
    default byte getOrDefault(float key, byte def)   {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
            this.put(key, def);
     ...
     */
    default byte computeIfAbsent(float key, @NonNull FloatToByteFunction func) {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
