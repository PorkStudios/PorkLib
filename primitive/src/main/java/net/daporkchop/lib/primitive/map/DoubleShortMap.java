...
import net.daporkchop.lib.primitive.lambda.function.DoubleToShortFunction;
...
/**
 * A map, using a key type of double and a value type of short.
 * <p>
...
public interface DoubleShortMap   {
     ...
     */
    default void putAll(@NonNull double[] keys, @NonNull short[] values)    {
     ...
     */
    default void putAll(@NonNull DoubleShortMap map)    {
     ...
     * @param consumer the function to run
     */
    void forEachEntry(DoubleShortConsumer consumer);
     ...
     */
    DoubleShortIterator entryIterator();
     ...
     */
    default short getOrDefault(double key, short def)   {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
            this.put(key, def);
     ...
     */
    default short computeIfAbsent(double key, @NonNull DoubleToShortFunction func) {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
