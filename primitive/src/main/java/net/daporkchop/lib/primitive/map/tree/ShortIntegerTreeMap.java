...
/**
 * A tree map, using a key type of short and a value type of int.
 ...
 * Although this map 𝙒𝙊𝙍𝙆𝙎, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.ShortIntegerIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class ShortIntegerTreeMap implements ShortIntegerMap    {
    ...
    @SuppressWarnings("unchecked")
    protected int remove(short key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return 0; //no mapping
    ...
    public ShortIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.ShortIntegerIndexedTreeMap} for an implementation of this method
        return null;
    }
