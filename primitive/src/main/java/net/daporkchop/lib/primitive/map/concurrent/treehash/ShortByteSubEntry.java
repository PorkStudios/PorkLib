...
/**
 * A single subentry for a {@link ShortByteConcurrentTreeHashMap}.
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
class ShortByteSubEntry    {
    public final AtomicInteger size = new AtomicInteger(0);
    public final ShortByteConcurrentTreeHashMap parent;
    ...
    public ShortByteSubEntry(ShortByteConcurrentTreeHashMap parent)   {
        this.parent = parent;
        this.rootPointer = this.parent.subHashLength == 0 ? new ShortByteSubEntry.ValueSector(null, -1) : new ShortByteSubEntry.PointerSector(null, -1);
    }
    ...
    public void forEachKey(ShortConsumer consumer)   {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof ShortByteSubEntry.PointerSector)    {
    ...
    public short advanceKey(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext)   {
        ...
        for (int i = sector.states.nextSetBit(curr.get() + 1); i != -1; i = sector.states.nextSetBit(i + 1)) {
            curr.set(i);
            if (sector instanceof ShortByteSubEntry.ValueSector)  {
    ...
    public void advanceEntry(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext, ShortByteMutableTuple tuple)   {
        Sector sector = sectorRef.get();
        if (sector == null) {
            sectorRef.set(sector = this.rootPointer);
        }
    ...
    public Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.parent.subHashLength; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.parent.subHashLength ? new ShortByteSubEntry.ValueSector(curr, off) : new ShortByteSubEntry.PointerSector(curr, off);
    ...
    public abstract class Sector  {
        ...
        public byte setEntry(int index, short key, byte value)   {
            throw new UnsupportedOperationException();
        }
    ...
    public class ValueSector extends Sector   {
        ...
        public byte setEntry(int index, short key, byte value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                ShortByteSubEntry.this.size.incrementAndGet();
                ShortByteSubEntry.this.parent.size.incrementAndGet();
                return (byte) 0;
            }
        }
        ...
        public byte removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                ShortByteSubEntry.this.size.decrementAndGet();
                ShortByteSubEntry.this.parent.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
