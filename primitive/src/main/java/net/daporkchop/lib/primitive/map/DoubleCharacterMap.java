...
import net.daporkchop.lib.primitive.lambda.function.DoubleToCharacterFunction;
...
/**
 * A map, using a key type of double and a value type of char.
 * <p>
...
public interface DoubleCharacterMap   {
     ...
     */
    default void putAll(@NonNull double[] keys, @NonNull char[] values)    {
     ...
     */
    default void putAll(@NonNull DoubleCharacterMap map)    {
     ...
     * @param consumer the function to run
     */
    void forEachEntry(DoubleCharacterConsumer consumer);
     ...
     */
    DoubleCharacterIterator entryIterator();
     ...
     */
    default char getOrDefault(double key, char def)   {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
            this.put(key, def);
     ...
     */
    default char computeIfAbsent(double key, @NonNull DoubleToCharacterFunction func) {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
