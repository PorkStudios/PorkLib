...
/**
 * A tree map, using a key type of double and a value type of short.
 ...
 * Although this map WORKS, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.DoubleShortIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class DoubleShortTreeMap implements DoubleShortMap    {
    ...
    @SuppressWarnings("unchecked")
    protected short remove(double key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return (short) 0; //no mapping
    ...
    public DoubleIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.DoubleShortIndexedTreeMap} for an implementation of this method
        return null;
    }
