 ...
 * Although this map 𝙒𝙊𝙍𝙆𝙎, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.ObjectFloatIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class ObjectFloatTreeMap<K extends Object> implements ObjectFloatMap<K>    {
    ...
    public float get(K key)    {
        ...
        if (!containsKeyHash(hash)) return 0F; //no mapping
    ...
    @SuppressWarnings("unchecked")
    protected float remove(K key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return 0F; //no mapping
        ...
        for (int w = 0; w < 256; w++)   {
            ...
            for (int x = 0; x < 256; x++)   {
                ...
                for (int y = 0; y < 256; y++)   {
                    ...
                    for (int z = 0; z < 256; z++)   {
                        int d = c[z];
                        if (d < 0) consumer.accept( (K) keys[d * -1 - 1]);
                    }
                }
            }
        }
            ...
            for (int x = 0; x < 256; x++)   {
                ...
                for (int y = 0; y < 256; y++)   {
                    ...
                    for (int z = 0; z < 256; z++)   {
                        ...
                        if (d < 0)  {
                            d = d * -1 - 1;
                            consumer.accept( (K) keys[d], values[d]);
                        }
                    }
                }
            }
    ...
    public ObjectIterator<K> keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.ObjectFloatIndexedTreeMap} for an implementation of this method
        return null;
    }
