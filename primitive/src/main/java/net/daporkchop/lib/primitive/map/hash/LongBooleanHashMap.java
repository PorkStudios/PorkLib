...
import net.daporkchop.lib.primitive.iterator.bi.LongBooleanIterator;
...
/**
 * A hash map, using a key type of long and a value type of boolean.
 ...
 * @author DaPorkchop_
 */
public class LongBooleanHashMap implements LongBooleanMap    {
    protected long[] keys;
    ...
    public LongBooleanHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public LongBooleanHashMap(LongToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public LongBooleanHashMap(int baseSize, LongToIntegerFunction keyHash)    {
    ...
    @Override
    public void forEachEntry(@NonNull LongBooleanConsumer consumer)   {
    ...
    public LongBooleanIterator entryIterator() {
    ...
    protected boolean reHash(boolean[] values, long[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
    ...
    private class KeyIterator implements LongIterator {
        ...
        public KeyIterator()    {
            this.size = LongBooleanHashMap.this.size;
            this.next = LongBooleanHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public long get()   {
            return LongBooleanHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = LongBooleanHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            LongBooleanHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements BooleanIterator {
        ...
        @SuppressWarnings("unchecked")
        public boolean get()   {
            return LongBooleanHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements LongBooleanIterator {
        private int size;
        private int index = -1;
        private int next;
        private LongBooleanTuple tuple;
        ...
        @Override
        public LongBooleanTuple advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new LongBooleanImmutableTuple( LongBooleanHashMap.this.keys[this.index],  LongBooleanHashMap.this.values[this.index]);
        }
