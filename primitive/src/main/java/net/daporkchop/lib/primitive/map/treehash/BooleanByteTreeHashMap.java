 ...
 * overhead, and can even be made concurrent with little extra work (see {@link net.daporkchop.lib.primitive.map.concurrent.BooleanByteConcurrentTreeHashMap})
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
public class BooleanByteTreeHashMap implements BooleanByteMap    {
    ...
    public BooleanByteTreeHashMap()    {
        this(null, 0);
    }

    public BooleanByteTreeHashMap(BooleanToLongFunction hashFunc, int hashLen)    {
        ...
        this.rootPointer = this.hashLen == 0 ? new BooleanByteTreeHashMap.ValueSector(null, -1) : new BooleanByteTreeHashMap.PointerSector(null, -1);
    }
    ...
    public void forEachKey(@NonNull BooleanConsumer consumer)  {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof BooleanByteTreeHashMap.PointerSector)    {
    ...
    private Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.hashLen; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.hashLen ? new BooleanByteTreeHashMap.ValueSector(curr, off) : new BooleanByteTreeHashMap.PointerSector(curr, off);
    ...
    private class ValueSector extends Sector   {
        ...
        public byte setEntry(int index, boolean key, byte value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                BooleanByteTreeHashMap.this.size.incrementAndGet();
                return (byte) 0;
            }
        }
        ...
        public byte removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                BooleanByteTreeHashMap.this.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
