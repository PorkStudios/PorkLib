...
import net.daporkchop.lib.primitive.map.tree.ByteLongTreeMap;
...
/**
 * A tree map, using a key type of byte and a value type of long.
 ...
 * @author DaPorkchop_
 */
public class ByteLongIndexedTreeMap extends ByteLongTreeMap    {
    protected boolean[] index;

    public ByteLongIndexedTreeMap()    {
        super();
    }
    ...
    private class KeyIterator implements ByteIterator   {
        ...
        private final ByteLongIndexedTreeMap this_ = ByteLongIndexedTreeMap.this;
    ...
    private class EntryIterator implements ByteLongIterator   {
        ...
        public ByteLongTuple advance()   {
            if (this.hasNext()) {
                this.findNext();
                return this.tuple = new ByteLongImmutableTuple( this_.keys[this.index],  this_.values[this.index]);
            } else {
                throw new ArrayIndexOutOfBoundsException();
            }
        }
