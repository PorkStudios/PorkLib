...
import net.daporkchop.lib.primitive.lambda.function.DoubleToFloatFunction;
...
/**
 * A map, using a key type of double and a value type of float.
 * <p>
...
public interface DoubleFloatMap   {
     ...
     */
    default void putAll(@NonNull double[] keys, @NonNull float[] values)    {
     ...
     */
    default void putAll(@NonNull DoubleFloatMap map)    {
     ...
     * @param consumer the function to run
     */
    void forEachEntry(DoubleFloatConsumer consumer);
     ...
     */
    DoubleFloatIterator entryIterator();
     ...
     */
    default float getOrDefault(double key, float def)   {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
            this.put(key, def);
     ...
     */
    default float computeIfAbsent(double key, @NonNull DoubleToFloatFunction func) {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
