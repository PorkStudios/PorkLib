...
public class TEAKeyGen {
    static {
        BouncyCastleInit.loadClass();
    }

    public static TEAKey gen(byte[] data) {
        byte[] b = new byte[16];
        int i = 0;
        do {
        ...
        data = b;
        return new TEAKey(new SecretKeySpec(data, "aaa"), KeyRandom.getBytes(BlockCipherType.TEA.blockSize));
    }

    public static TEAKey gen() {
