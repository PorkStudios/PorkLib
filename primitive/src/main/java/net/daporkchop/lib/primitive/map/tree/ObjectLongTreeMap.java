 ...
 * Although this map WORKS, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.ObjectLongIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class ObjectLongTreeMap<K extends Object> implements ObjectLongMap<K>    {
    ...
    @SuppressWarnings("unchecked")
    protected long remove(K key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return 0L; //no mapping
    ...
    public ObjectIterator<K> keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.ObjectLongIndexedTreeMap} for an implementation of this method
        return null;
    }
