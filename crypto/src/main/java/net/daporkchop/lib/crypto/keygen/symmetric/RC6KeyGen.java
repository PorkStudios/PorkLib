...
public class RC6KeyGen {
    static {
        BouncyCastleInit.loadClass();
    }

    public static RC6Key gen(byte[] data) {
        byte[] b = new byte[BlockCipherType.RC6.blockSize];
        int i = 0;
        do {
        ...
        data = b;
        return new RC6Key(new SecretKeySpec(data, "aaa"), KeyRandom.getBytes(BlockCipherType.RC6.blockSize));
    }

    public static RC6Key gen() {
        byte[] key = new byte[BlockCipherType.RC6.blockSize];
