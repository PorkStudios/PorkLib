 ...
 * Although this map 𝙒𝙊𝙍𝙆𝙎, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.LongIntegerIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class LongIntegerTreeMap implements LongIntegerMap    {
    protected long[] keys;
    ...
    @SuppressWarnings("unchecked")
    protected int remove(long key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return 0; //no mapping
    ...
    public LongIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.LongIntegerIndexedTreeMap} for an implementation of this method
        return null;
    }
