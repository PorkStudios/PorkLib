 ...
 * Although this map WORKS, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.IntegerFloatIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class IntegerFloatTreeMap implements IntegerFloatMap    {
    protected int[] keys;
    ...
    @SuppressWarnings("unchecked")
    protected float remove(int key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return 0F; //no mapping
    ...
    public IntegerIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.IntegerFloatIndexedTreeMap} for an implementation of this method
        return null;
    }
