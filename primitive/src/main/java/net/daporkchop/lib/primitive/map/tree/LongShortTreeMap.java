 ...
 * Although this map WORKS, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.LongShortIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class LongShortTreeMap implements LongShortMap    {
    protected long[] keys;
    ...
    @SuppressWarnings("unchecked")
    protected short remove(long key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return (short) 0; //no mapping
    ...
    public LongIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.LongShortIndexedTreeMap} for an implementation of this method
        return null;
    }
