 ...
 * @author DaPorkchop_
 */
public class CharacterBooleanConcurrentTreeHashMap implements CharacterBooleanMap    {
    final AtomicInteger size = new AtomicInteger(0);

    private final CharacterBooleanSubEntry[] entries = new CharacterBooleanSubEntry[256];
    private final CharacterToIntegerFunction keyHash;
    final CharacterToLongFunction subKeyHash;
    final int subHashLength;

    public CharacterBooleanConcurrentTreeHashMap()    {
        this(null, null);
    }

    public CharacterBooleanConcurrentTreeHashMap(CharacterToIntegerFunction keyHash, CharacterToLongFunction subKeyHash)    {
        this(keyHash, subKeyHash, 1);
    }

    public CharacterBooleanConcurrentTreeHashMap(CharacterToIntegerFunction keyHash, CharacterToLongFunction subKeyHash, int subHashLength)    {
    ...
    public boolean get(char key)   {
        CharacterBooleanSubEntry entry = this.getSubentry(this.keyHash.apply(key) & 0xFF, false);
        if (entry == null)  {
            return false;
        } else {
    ...
    public boolean put(char key, boolean value)   {
        CharacterBooleanSubEntry entry = this.getSubentry(this.keyHash.apply(key) & 0xFF, true);
    ...
    public boolean remove(char key)    {
        ...
        CharacterBooleanSubEntry entry = this.getSubentry(hash, false);
        if (entry == null)  {
            return false;
        } else {
    ...
    public void clear() {
        synchronized (this.entries) {
            for (int i = 0; i < 256; i++) {
                CharacterBooleanSubEntry entry = this.entries[i];
                if (entry != null)  {
                    entry.clear();
                    this.entries[i] = null;
                }
    ...
    public void forEachKey(CharacterConsumer consumer)   {
        for (int i = 0; i < 256; i++) {
            CharacterBooleanSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
    ...
    @Override
    public void forEachEntry(CharacterBooleanConsumer consumer) {
        for (int i = 0; i < 256; i++) {
            CharacterBooleanSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
    ...
    public void forEachEntry(CharacterBooleanConsumer consumer, VoidFunction complete) {
        for (int i = 0; i < 256; i++) {
            CharacterBooleanSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
                entry.forEachEntry(consumer);
    ...
    public CharacterConcurrentIterator keyIterator()   {
        return new CharacterConcurrentIterator()  {
            private final CharacterBooleanConcurrentTreeHashMap this_ = CharacterBooleanConcurrentTreeHashMap.this;
            ...
            private final AtomicReference<CharacterBooleanSubEntry.Sector> currSector = new AtomicReference(null);
            ...
            private volatile CharacterBooleanSubEntry entry;
            ...
            private void nextSub()   {
                ...
                while (this.subEntryIndex < 256)    {
                    CharacterBooleanSubEntry entry = getSubentry(this.subEntryIndex++, false);
                    if (entry != null)  {
                        this.entry = entry;
            ...
            class ThreadState   {
                public char key;
                public CharacterBooleanSubEntry entry;
                private final CharacterConcurrentIterator this_;
                private final CharacterBooleanConcurrentTreeHashMap this__;

                public ThreadState(CharacterConcurrentIterator this_, CharacterBooleanConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
    ...
    public BooleanConcurrentIterator valueIterator() {
        return new BooleanConcurrentIterator()  {
            ...
            private final AtomicReference<CharacterBooleanSubEntry.Sector> currSector = new AtomicReference(null);
            private final AtomicBoolean hasNextSub = new AtomicBoolean(true);
            private final CharacterBooleanMutableTuple next = new CharacterBooleanMutableTuple();
            private volatile boolean hasNext = true;
            private volatile int subEntryIndex = 0;
            private volatile CharacterBooleanSubEntry entry;
            ...
            class ThreadState   {
                ...
                public ThreadState(BooleanConcurrentIterator this_, CharacterBooleanConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
    ...
    @Override
    public CharacterBooleanConcurrentIterator entryIterator() {
        return new CharacterBooleanConcurrentIterator()  {
            private final CharacterBooleanConcurrentTreeHashMap this_ = CharacterBooleanConcurrentTreeHashMap.this;
            //private final ReentrantLock lock = new ReentrantLock();
            private final ThreadLocal<ThreadState> tl = ThreadLocal.withInitial(() -> new ThreadState(this, this.this_));
            private final AtomicInteger sectorOffset = new AtomicInteger(0);
            ...
            @Override
            public synchronized CharacterBooleanTuple advance()   {
                if (!this.hasNext())    {
                    throw new IteratorCompleteException();
                }
                ThreadState state = this.tl.get();
                CharacterBooleanTuple current = state.tuple = new CharacterBooleanMutableTuple(this.next.k, this.next.v);
                state.entry = this.entry;
                this.findNextValue();
                return current;
            }
            ...
            class ThreadState   {
                public CharacterBooleanTuple tuple;
                public CharacterBooleanSubEntry entry;
                private final CharacterBooleanConcurrentIterator this_;
                private final CharacterBooleanConcurrentTreeHashMap this__;

                public ThreadState(CharacterBooleanConcurrentIterator this_, CharacterBooleanConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
     ...
     */
    public CharacterBooleanConcurrentIterator fastEntryIterator() {
        return new CharacterBooleanConcurrentIterator()  {
            private final CharacterBooleanConcurrentTreeHashMap this_ = CharacterBooleanConcurrentTreeHashMap.this;
            //private final ReentrantLock lock = new ReentrantLock();
            private final ThreadLocal<ThreadState> tl = ThreadLocal.withInitial(() -> new ThreadState(this, this.this_));
            ...
            private final AtomicReference<CharacterBooleanSubEntry.Sector> currSector = new AtomicReference(null);
            private final AtomicBoolean hasNextSub = new AtomicBoolean(true);
            private volatile CharacterBooleanMutableTuple next = new CharacterBooleanMutableTuple((char) 0, false);
            private volatile boolean hasNext = true;
            private volatile int subEntryIndex = 0;
            private volatile CharacterBooleanSubEntry entry;
            ...
            @Override
            public CharacterBooleanMutableTuple get()   {
                ThreadState state = this.tl.get();
                if (state.entry == null)    {
                    throw new IllegalStateException();
                } else {
            ...
            @Override
            public synchronized CharacterBooleanMutableTuple advance()   {
                if (!this.hasNext())    {
                    throw new IteratorCompleteException();
                }
                ThreadState state = this.tl.get();
                CharacterBooleanMutableTuple a = state.tuple;
            ...
            class ThreadState   {
                public CharacterBooleanMutableTuple tuple = new CharacterBooleanMutableTuple((char) 0, false);
                public CharacterBooleanSubEntry entry;
                private final CharacterBooleanConcurrentIterator this_;
                private final CharacterBooleanConcurrentTreeHashMap this__;
    ...
    private CharacterBooleanSubEntry getSubentry(int index, boolean create)    {
        assert index == (index & 0xFF);

        synchronized (this.entries) {
            CharacterBooleanSubEntry entry = (CharacterBooleanSubEntry) this.entries[index];
            if (entry == null && create)    {
                this.entries[index] = entry = new CharacterBooleanSubEntry(this);
            }
            return entry;
        }
    }
