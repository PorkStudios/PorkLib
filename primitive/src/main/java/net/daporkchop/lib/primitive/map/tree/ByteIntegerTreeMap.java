 ...
 * Although this map 𝙒𝙊𝙍𝙆𝙎, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.ByteIntegerIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class ByteIntegerTreeMap implements ByteIntegerMap    {
    protected byte[] keys;
    ...
    @SuppressWarnings("unchecked")
    protected int remove(byte key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return 0; //no mapping
    ...
    public ByteIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.ByteIntegerIndexedTreeMap} for an implementation of this method
        return null;
    }
