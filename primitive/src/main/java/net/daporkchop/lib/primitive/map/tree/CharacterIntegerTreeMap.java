 ...
 * Although this map 𝙒𝙊𝙍𝙆𝙎, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.CharacterIntegerIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class CharacterIntegerTreeMap implements CharacterIntegerMap    {
    protected char[] keys;
    ...
    public CharacterIntegerTreeMap()    {
        this.clear();
    }
    ...
    @SuppressWarnings("unchecked")
    protected int remove(char key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return 0; //no mapping
    ...
    public CharacterIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.CharacterIntegerIndexedTreeMap} for an implementation of this method
        return null;
    }
