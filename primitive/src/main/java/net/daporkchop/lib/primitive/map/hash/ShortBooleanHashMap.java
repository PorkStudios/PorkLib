...
/**
 * A hash map, using a key type of short and a value type of boolean.
 ...
 * @author DaPorkchop_
 */
public class ShortBooleanHashMap implements ShortBooleanMap    {
    ...
    public ShortBooleanHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public ShortBooleanHashMap(ShortToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public ShortBooleanHashMap(int baseSize, ShortToIntegerFunction keyHash)    {
    ...
    protected boolean reHash(boolean[] values, short[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
    ...
    private class KeyIterator implements ShortIterator {
        ...
        public KeyIterator()    {
            this.size = ShortBooleanHashMap.this.size;
            this.next = ShortBooleanHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public short get()   {
            return ShortBooleanHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = ShortBooleanHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            ShortBooleanHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements BooleanIterator {
        ...
        @SuppressWarnings("unchecked")
        public boolean get()   {
            return ShortBooleanHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements ShortBooleanIterator {
        private int size;
        private int index = -1;
        private int next;
        private ShortBooleanTuple tuple;
        ...
        @Override
        public ShortBooleanTuple advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new ShortBooleanImmutableTuple( ShortBooleanHashMap.this.keys[this.index],  ShortBooleanHashMap.this.values[this.index]);
        }
