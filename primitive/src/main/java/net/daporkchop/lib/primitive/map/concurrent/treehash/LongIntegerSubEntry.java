...
/**
 * A single subentry for a {@link LongIntegerConcurrentTreeHashMap}.
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
class LongIntegerSubEntry    {
    public final AtomicInteger size = new AtomicInteger(0);
    public final LongIntegerConcurrentTreeHashMap parent;
    ...
    public LongIntegerSubEntry(LongIntegerConcurrentTreeHashMap parent)   {
        this.parent = parent;
        this.rootPointer = this.parent.subHashLength == 0 ? new LongIntegerSubEntry.ValueSector(null, -1) : new LongIntegerSubEntry.PointerSector(null, -1);
    }
    ...
    public void forEachKey(LongConsumer consumer)   {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof LongIntegerSubEntry.PointerSector)    {
    ...
    public long advanceKey(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext)   {
        ...
        for (int i = sector.states.nextSetBit(curr.get() + 1); i != -1; i = sector.states.nextSetBit(i + 1)) {
            curr.set(i);
            if (sector instanceof LongIntegerSubEntry.ValueSector)  {
    ...
    public void advanceEntry(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext, LongIntegerMutableTuple tuple)   {
        Sector sector = sectorRef.get();
        if (sector == null) {
            sectorRef.set(sector = this.rootPointer);
        }
    ...
    public Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.parent.subHashLength; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.parent.subHashLength ? new LongIntegerSubEntry.ValueSector(curr, off) : new LongIntegerSubEntry.PointerSector(curr, off);
    ...
    public abstract class Sector  {
        ...
        public int setEntry(int index, long key, int value)   {
            throw new UnsupportedOperationException();
        }
    ...
    public class ValueSector extends Sector   {
        ...
        public int setEntry(int index, long key, int value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                LongIntegerSubEntry.this.size.incrementAndGet();
                LongIntegerSubEntry.this.parent.size.incrementAndGet();
                return 0;
            }
        }
        ...
        public int removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                LongIntegerSubEntry.this.size.decrementAndGet();
                LongIntegerSubEntry.this.parent.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
