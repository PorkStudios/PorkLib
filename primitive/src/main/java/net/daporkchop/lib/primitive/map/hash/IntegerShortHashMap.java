...
/**
 * A hash map, using a key type of int and a value type of short.
 ...
 * @author DaPorkchop_
 */
public class IntegerShortHashMap implements IntegerShortMap    {
    protected int[] keys;
    ...
    public IntegerShortHashMap()    {
        this(16384);
    }

    public IntegerShortHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public IntegerShortHashMap(IntegerToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public IntegerShortHashMap(int baseSize, IntegerToIntegerFunction keyHash)    {
    ...
    protected boolean reHash(short[] values, int[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
            int key = this.keys[i];
            int j = getIndex(key);
            if (states.get(j))  {
    ...
    private class KeyIterator implements IntegerIterator {
        ...
        public KeyIterator()    {
            this.size = IntegerShortHashMap.this.size;
            this.next = IntegerShortHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public int get()   {
            return IntegerShortHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = IntegerShortHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            IntegerShortHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements ShortIterator {
        ...
        @SuppressWarnings("unchecked")
        public short get()   {
            return IntegerShortHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements IntegerShortIterator {
        private int size;
        private int index = -1;
        private int next;
        private IntegerShortTuple tuple;
        ...
        @Override
        public IntegerShortTuple advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new IntegerShortImmutableTuple( IntegerShortHashMap.this.keys[this.index],  IntegerShortHashMap.this.values[this.index]);
        }
