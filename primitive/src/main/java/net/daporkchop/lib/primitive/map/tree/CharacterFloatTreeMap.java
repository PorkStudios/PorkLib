...
/**
 * A tree map, using a key type of char and a value type of float.
 ...
 * Although this map 𝙒𝙊𝙍𝙆𝙎, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.CharacterFloatIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class CharacterFloatTreeMap implements CharacterFloatMap    {
    protected char[] keys;
    ...
    @SuppressWarnings("unchecked")
    protected float remove(char key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return 0F; //no mapping
    ...
    public CharacterIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.CharacterFloatIndexedTreeMap} for an implementation of this method
        return null;
    }
