 ...
 * Although this map 𝙒𝙊𝙍𝙆𝙎, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.ObjectDoubleIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class ObjectDoubleTreeMap<K extends Object> implements ObjectDoubleMap<K>    {
    ...
    @SuppressWarnings("unchecked")
    protected double remove(K key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return 0D; //no mapping
    ...
    public ObjectIterator<K> keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.ObjectDoubleIndexedTreeMap} for an implementation of this method
        return null;
    }
