...
import net.daporkchop.lib.primitive.iterator.bi.ObjectFloatIterator;
...
/**
 * A hash map, using a key type of K and a value type of float.
 ...
 * @author DaPorkchop_
 */
public class ObjectFloatHashMap<K extends Object> implements ObjectFloatMap<K>    {
    ...
    protected int mask;
    protected int size;
    protected final ObjectToIntegerFunction keyHash;

    public ObjectFloatHashMap()    {
        this(16384);
    }

    public ObjectFloatHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public ObjectFloatHashMap(ObjectToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public ObjectFloatHashMap(int baseSize, ObjectToIntegerFunction keyHash)    {
    ...
    public void forEachKey(@NonNull ObjectConsumer<K> consumer)   {
        this.forEach(i -> consumer.accept( (K) this.keys[i]));
    }
    ...
    @Override
    public void forEachEntry(@NonNull ObjectFloatConsumer<K> consumer)   {
        this.forEach(i -> consumer.accept( (K) this.keys[i],  this.values[i]));
    }
    ...
    public ObjectFloatIterator<K> entryIterator() {
    ...
    protected boolean reHash(float[] values, Object[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
            K key = (K) this.keys[i];
            int j = getIndex(key);
            if (states.get(j))  {
    ...
    private class KeyIterator implements ObjectIterator<K> {
        private int size;
        private int index = -1;
        private int next;

        public KeyIterator()    {
            this.size = ObjectFloatHashMap.this.size;
            this.next = ObjectFloatHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public K get()   {
            return (K) ObjectFloatHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = ObjectFloatHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            ObjectFloatHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements FloatIterator {
        ...
        @SuppressWarnings("unchecked")
        public float get()   {
            return ObjectFloatHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements ObjectFloatIterator<K> {
        private int size;
        private int index = -1;
        private int next;
        private ObjectFloatTuple<K> tuple;
        ...
        @Override
        public ObjectFloatTuple<K> advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new ObjectFloatImmutableTuple<K>( (K) ObjectFloatHashMap.this.keys[this.index],  ObjectFloatHashMap.this.values[this.index]);
        }
