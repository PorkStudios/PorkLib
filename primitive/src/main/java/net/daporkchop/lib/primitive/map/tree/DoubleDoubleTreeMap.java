 ...
 * Although this map WORKS, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.DoubleDoubleIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class DoubleDoubleTreeMap implements DoubleDoubleMap    {
    ...
    @SuppressWarnings("unchecked")
    protected double remove(double key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return 0D; //no mapping
    ...
    public DoubleIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.DoubleDoubleIndexedTreeMap} for an implementation of this method
        return null;
    }
