...
import net.daporkchop.lib.primitive.lambda.function.bi.DoubleShortToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of double => short mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface DoubleShortConcurrentIterator extends DoubleShortIterator   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull DoubleShortToBooleanFunction function)    {
        try {
            while (this.hasNext())  {
                DoubleShortTuple tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<DoubleShortTuple> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<DoubleShortTuple> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull DoubleShortConsumer consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<DoubleShortTuple> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull DoubleShortToBooleanFunction function)    {
        this.concurrentIterate(function);
    }
}
