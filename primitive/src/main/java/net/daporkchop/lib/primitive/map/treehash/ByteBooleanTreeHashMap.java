 ...
 * overhead, and can even be made concurrent with little extra work (see {@link net.daporkchop.lib.primitive.map.concurrent.ByteBooleanConcurrentTreeHashMap})
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
public class ByteBooleanTreeHashMap implements ByteBooleanMap    {
    ...
    public ByteBooleanTreeHashMap()    {
        this(null, 0);
    }

    public ByteBooleanTreeHashMap(ByteToLongFunction hashFunc, int hashLen)    {
        ...
        this.rootPointer = this.hashLen == 0 ? new ByteBooleanTreeHashMap.ValueSector(null, -1) : new ByteBooleanTreeHashMap.PointerSector(null, -1);
    }
    ...
    public void forEachKey(@NonNull ByteConsumer consumer)  {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof ByteBooleanTreeHashMap.PointerSector)    {
    ...
    private Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.hashLen; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.hashLen ? new ByteBooleanTreeHashMap.ValueSector(curr, off) : new ByteBooleanTreeHashMap.PointerSector(curr, off);
    ...
    private class ValueSector extends Sector   {
        ...
        public boolean setEntry(int index, byte key, boolean value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                ByteBooleanTreeHashMap.this.size.incrementAndGet();
                return false;
            }
        }
        ...
        public boolean removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                ByteBooleanTreeHashMap.this.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
