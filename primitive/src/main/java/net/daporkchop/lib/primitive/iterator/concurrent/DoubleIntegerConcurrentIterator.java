...
import net.daporkchop.lib.primitive.lambda.function.bi.DoubleIntegerToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of double => int mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface DoubleIntegerConcurrentIterator extends DoubleIntegerIterator   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull DoubleIntegerToBooleanFunction function)    {
        try {
            while (this.hasNext())  {
                DoubleIntegerTuple tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<DoubleIntegerTuple> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<DoubleIntegerTuple> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull DoubleIntegerConsumer consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<DoubleIntegerTuple> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull DoubleIntegerToBooleanFunction function)    {
        this.concurrentIterate(function);
    }
}
