...
/**
 * A hash map, using a key type of float and a value type of float.
 ...
 * @author DaPorkchop_
 */
public class FloatFloatHashMap implements FloatFloatMap    {
    ...
    public FloatFloatHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public FloatFloatHashMap(FloatToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public FloatFloatHashMap(int baseSize, FloatToIntegerFunction keyHash)    {
    ...
    protected boolean reHash(float[] values, float[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
    ...
    private class KeyIterator implements FloatIterator {
        ...
        public KeyIterator()    {
            this.size = FloatFloatHashMap.this.size;
            this.next = FloatFloatHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public float get()   {
            return FloatFloatHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = FloatFloatHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            FloatFloatHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements FloatIterator {
        ...
        @SuppressWarnings("unchecked")
        public float get()   {
            return FloatFloatHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements FloatFloatIterator {
        private int size;
        private int index = -1;
        private int next;
        private FloatFloatTuple tuple;
        ...
        @Override
        public FloatFloatTuple advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new FloatFloatImmutableTuple( FloatFloatHashMap.this.keys[this.index],  FloatFloatHashMap.this.values[this.index]);
        }
