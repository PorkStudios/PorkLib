...
/**
 * A tree map, using a key type of short and a value type of boolean.
 ...
 * Although this map 𝙒𝙊𝙍𝙆𝙎, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.ShortBooleanIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class ShortBooleanTreeMap implements ShortBooleanMap    {
    ...
    @SuppressWarnings("unchecked")
    protected boolean remove(short key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return false; //no mapping
    ...
    public ShortIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.ShortBooleanIndexedTreeMap} for an implementation of this method
        return null;
    }
