...
import net.daporkchop.lib.primitive.lambda.function.bi.IntegerByteToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of int => byte mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface IntegerByteConcurrentIterator extends IntegerByteIterator   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull IntegerByteToBooleanFunction function)    {
        try {
            while (this.hasNext())  {
                IntegerByteTuple tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<IntegerByteTuple> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<IntegerByteTuple> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull IntegerByteConsumer consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<IntegerByteTuple> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull IntegerByteToBooleanFunction function)    {
        this.concurrentIterate(function);
    }
}
