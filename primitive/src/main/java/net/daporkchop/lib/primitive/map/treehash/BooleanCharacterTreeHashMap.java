 ...
 * overhead, and can even be made concurrent with little extra work (see {@link net.daporkchop.lib.primitive.map.concurrent.BooleanCharacterConcurrentTreeHashMap})
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
public class BooleanCharacterTreeHashMap implements BooleanCharacterMap    {
    ...
    public BooleanCharacterTreeHashMap()    {
        this(null, 0);
    }

    public BooleanCharacterTreeHashMap(BooleanToLongFunction hashFunc, int hashLen)    {
        ...
        this.rootPointer = this.hashLen == 0 ? new BooleanCharacterTreeHashMap.ValueSector(null, -1) : new BooleanCharacterTreeHashMap.PointerSector(null, -1);
    }
    ...
    public void forEachKey(@NonNull BooleanConsumer consumer)  {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof BooleanCharacterTreeHashMap.PointerSector)    {
    ...
    private Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.hashLen; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.hashLen ? new BooleanCharacterTreeHashMap.ValueSector(curr, off) : new BooleanCharacterTreeHashMap.PointerSector(curr, off);
    ...
    private class ValueSector extends Sector   {
        ...
        public char setEntry(int index, boolean key, char value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                BooleanCharacterTreeHashMap.this.size.incrementAndGet();
                return (char) 0;
            }
        }
        ...
        public char removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                BooleanCharacterTreeHashMap.this.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
