...
import net.daporkchop.lib.primitive.lambda.function.LongToByteFunction;
...
/**
 * A map, using a key type of long and a value type of byte.
 * <p>
...
public interface LongByteMap   {
     ...
     */
    default void putAll(@NonNull long[] keys, @NonNull byte[] values)    {
     ...
     */
    default void putAll(@NonNull LongByteMap map)    {
     ...
     */
    default void removeAll(@NonNull Collection<Long> keys) {
     ...
     * @param consumer the function to run
     */
    void forEachEntry(LongByteConsumer consumer);
     ...
     */
    LongByteIterator entryIterator();
     ...
     */
    default long[] getKeys()  {
        long[] arr = new long[this.getSize()];
     ...
     */
    default byte getOrDefault(long key, byte def)   {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
            this.put(key, def);
     ...
     */
    default byte computeIfAbsent(long key, @NonNull LongToByteFunction func) {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
