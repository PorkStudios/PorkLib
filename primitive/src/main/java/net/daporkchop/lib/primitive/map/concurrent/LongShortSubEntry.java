...
/**
 * A single subentry for a {@link LongShortConcurrentTreeHashMap}.
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
class LongShortSubEntry    {
    public final AtomicInteger size = new AtomicInteger(0);
    public final LongShortConcurrentTreeHashMap parent;
    ...
    public LongShortSubEntry(LongShortConcurrentTreeHashMap parent)   {
        this.parent = parent;
        this.rootPointer = this.parent.subHashLength == 0 ? new LongShortSubEntry.ValueSector(null, -1) : new LongShortSubEntry.PointerSector(null, -1);
    }
    ...
    public void forEachKey(LongConsumer consumer)   {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof LongShortSubEntry.PointerSector)    {
    ...
    public long advanceKey(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext)   {
        ...
        for (int i = sector.states.nextSetBit(curr.get() + 1); i != -1; i = sector.states.nextSetBit(i + 1)) {
            curr.set(i);
            if (sector instanceof LongShortSubEntry.ValueSector)  {
    ...
    public void advanceEntry(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext, LongShortMutableTuple tuple)   {
        Sector sector = sectorRef.get();
        if (sector == null) {
            sectorRef.set(sector = this.rootPointer);
        }
    ...
    public Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.parent.subHashLength; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.parent.subHashLength ? new LongShortSubEntry.ValueSector(curr, off) : new LongShortSubEntry.PointerSector(curr, off);
    ...
    public class ValueSector extends Sector   {
        ...
        public short setEntry(int index, long key, short value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                LongShortSubEntry.this.size.incrementAndGet();
                LongShortSubEntry.this.parent.size.incrementAndGet();
                return (short) 0;
            }
        }
        ...
        public short removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                LongShortSubEntry.this.size.decrementAndGet();
                LongShortSubEntry.this.parent.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
