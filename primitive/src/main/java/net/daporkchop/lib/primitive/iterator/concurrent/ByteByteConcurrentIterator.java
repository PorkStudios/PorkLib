...
import net.daporkchop.lib.primitive.lambda.function.bi.ByteByteToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of byte => byte mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface ByteByteConcurrentIterator extends ByteByteIterator   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ByteByteToBooleanFunction function)    {
        try {
            while (this.hasNext())  {
                ByteByteTuple tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<ByteByteTuple> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<ByteByteTuple> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ByteByteConsumer consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<ByteByteTuple> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull ByteByteToBooleanFunction function)    {
        this.concurrentIterate(function);
    }
}
