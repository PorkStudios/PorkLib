...
import net.daporkchop.lib.primitive.lambda.function.bi.ObjectIntegerToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of K => int mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface ObjectIntegerConcurrentIterator<K extends Object> extends ObjectIntegerIterator<K>   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectIntegerToBooleanFunction<K> function)    {
        try {
            while (this.hasNext())  {
                ObjectIntegerTuple<K> tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<ObjectIntegerTuple<K>> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<ObjectIntegerTuple<K>> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectIntegerConsumer<K> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<ObjectIntegerTuple<K>> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull ObjectIntegerToBooleanFunction<K> function)    {
        this.concurrentIterate(function);
    }
}
