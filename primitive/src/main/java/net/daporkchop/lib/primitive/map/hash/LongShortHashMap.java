...
/**
 * A hash map, using a key type of long and a value type of short.
 ...
 * @author DaPorkchop_
 */
public class LongShortHashMap implements LongShortMap    {
    protected long[] keys;
    ...
    public LongShortHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public LongShortHashMap(LongToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public LongShortHashMap(int baseSize, LongToIntegerFunction keyHash)    {
    ...
    protected boolean reHash(short[] values, long[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
    ...
    private class KeyIterator implements LongIterator {
        ...
        public KeyIterator()    {
            this.size = LongShortHashMap.this.size;
            this.next = LongShortHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public long get()   {
            return LongShortHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = LongShortHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            LongShortHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements ShortIterator {
        ...
        @SuppressWarnings("unchecked")
        public short get()   {
            return LongShortHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements LongShortIterator {
        private int size;
        private int index = -1;
        private int next;
        private LongShortTuple tuple;
        ...
        @Override
        public LongShortTuple advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new LongShortImmutableTuple( LongShortHashMap.this.keys[this.index],  LongShortHashMap.this.values[this.index]);
        }
