 ...
 * overhead, and can even be made concurrent with little extra work (see {@link net.daporkchop.lib.primitive.map.concurrent.ObjectBooleanConcurrentTreeHashMap})
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
public class ObjectBooleanTreeHashMap<K extends Object> implements ObjectBooleanMap<K>    {
    ...
    public ObjectBooleanTreeHashMap()    {
        this(null, 0);
    }

    public ObjectBooleanTreeHashMap(ObjectToLongFunction hashFunc, int hashLen)    {
        ...
        this.rootPointer = this.hashLen == 0 ? new ObjectBooleanTreeHashMap<K>.ValueSector(null, -1) : new ObjectBooleanTreeHashMap<K>.PointerSector(null, -1);
    }
    ...
    @Override
    public void clear() {
        this.rootPointer = this.hashLen == 0 ? new ObjectBooleanTreeHashMap.ValueSector(null, -1) : new ObjectBooleanTreeHashMap.PointerSector(null, -1);
    }
    ...
    public void forEachKey(@NonNull ObjectConsumer<K> consumer)  {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof ObjectBooleanTreeHashMap.PointerSector)    {
    ...
    private Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.hashLen; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.hashLen ? new ObjectBooleanTreeHashMap<K>.ValueSector(curr, off) : new ObjectBooleanTreeHashMap<K>.PointerSector(curr, off);
    ...
    private class ValueSector extends Sector   {
        ...
        public boolean setEntry(int index, Object key, boolean value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                ObjectBooleanTreeHashMap.this.size.incrementAndGet();
                return false;
            }
        }
        ...
        public boolean removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                ObjectBooleanTreeHashMap.this.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
