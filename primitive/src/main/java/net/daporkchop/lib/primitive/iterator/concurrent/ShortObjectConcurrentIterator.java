...
import net.daporkchop.lib.primitive.lambda.function.bi.ShortObjectToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of short => V mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface ShortObjectConcurrentIterator<V extends Object> extends ShortObjectIterator<V>   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ShortObjectToBooleanFunction<V> function)    {
        try {
            while (this.hasNext())  {
                ShortObjectTuple<V> tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<ShortObjectTuple<V>> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<ShortObjectTuple<V>> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ShortObjectConsumer<V> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<ShortObjectTuple<V>> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull ShortObjectToBooleanFunction<V> function)    {
        this.concurrentIterate(function);
    }
}
