...
import net.daporkchop.lib.primitive.lambda.function.bi.CharacterDoubleToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of char => double mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface CharacterDoubleConcurrentIterator extends CharacterDoubleIterator   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull CharacterDoubleToBooleanFunction function)    {
        try {
            while (this.hasNext())  {
                CharacterDoubleTuple tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<CharacterDoubleTuple> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<CharacterDoubleTuple> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull CharacterDoubleConsumer consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<CharacterDoubleTuple> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull CharacterDoubleToBooleanFunction function)    {
        this.concurrentIterate(function);
    }
}
