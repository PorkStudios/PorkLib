...
import net.daporkchop.lib.primitive.iterator.concurrent.ShortObjectConcurrentIterator;
...
import net.daporkchop.lib.primitive.tuple.ShortObjectMutableTuple;
 ...
 * @author DaPorkchop_
 */
public class ShortObjectConcurrentTreeHashMap<V extends Object> implements ShortObjectMap<V>    {
    final AtomicInteger size = new AtomicInteger(0);

    private final ShortObjectSubEntry[] entries = new ShortObjectSubEntry[256];
    private final ShortToIntegerFunction keyHash;
    final ShortToLongFunction subKeyHash;
    final int subHashLength;

    public ShortObjectConcurrentTreeHashMap()    {
        this(null, null);
    }

    public ShortObjectConcurrentTreeHashMap(ShortToIntegerFunction keyHash, ShortToLongFunction subKeyHash)    {
        this(keyHash, subKeyHash, 1);
    }

    public ShortObjectConcurrentTreeHashMap(ShortToIntegerFunction keyHash, ShortToLongFunction subKeyHash, int subHashLength)    {
    ...
    public V get(short key)   {
        ShortObjectSubEntry<V> entry = this.getSubentry(this.keyHash.apply(key) & 0xFF, false);
        if (entry == null)  {
            return null;
        } else {
    ...
    public V put(short key, V value)   {
        ShortObjectSubEntry<V> entry = this.getSubentry(this.keyHash.apply(key) & 0xFF, true);
    ...
    public V remove(short key)    {
        ...
        ShortObjectSubEntry<V> entry = this.getSubentry(hash, false);
        if (entry == null)  {
            return null;
        } else {
    ...
    public void clear() {
        synchronized (this.entries) {
            for (int i = 0; i < 256; i++) {
                ShortObjectSubEntry<V> entry = this.entries[i];
                if (entry != null)  {
                    entry.clear();
                    this.entries[i] = null;
                }
    ...
    public void forEachKey(ShortConsumer consumer)   {
        for (int i = 0; i < 256; i++) {
            ShortObjectSubEntry<V> entry = this.getSubentry(i, false);
            if (entry != null)  {
    ...
    @Override
    public void forEachEntry(ShortObjectConsumer<V> consumer) {
        for (int i = 0; i < 256; i++) {
            ShortObjectSubEntry<V> entry = this.getSubentry(i, false);
            if (entry != null)  {
    ...
    public void forEachEntry(ShortObjectConsumer<V> consumer, VoidFunction complete) {
        for (int i = 0; i < 256; i++) {
            ShortObjectSubEntry<V> entry = this.getSubentry(i, false);
            if (entry != null)  {
                entry.forEachEntry(consumer);
    ...
    public ShortConcurrentIterator keyIterator()   {
        return new ShortConcurrentIterator()  {
            private final ShortObjectConcurrentTreeHashMap this_ = ShortObjectConcurrentTreeHashMap.this;
            ...
            private final AtomicReference<ShortObjectSubEntry<V>.Sector> currSector = new AtomicReference(null);
            ...
            private volatile ShortObjectSubEntry<V> entry;
            ...
            private void nextSub()   {
                ...
                while (this.subEntryIndex < 256)    {
                    ShortObjectSubEntry<V> entry = getSubentry(this.subEntryIndex++, false);
                    if (entry != null)  {
                        this.entry = entry;
            ...
            class ThreadState   {
                public short key;
                public ShortObjectSubEntry entry;
                private final ShortConcurrentIterator this_;
                private final ShortObjectConcurrentTreeHashMap this__;

                public ThreadState(ShortConcurrentIterator this_, ShortObjectConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
    ...
    public ObjectConcurrentIterator<V> valueIterator() {
        return new ObjectConcurrentIterator<V>()  {
            ...
            private final AtomicReference<ShortObjectSubEntry<V>.Sector> currSector = new AtomicReference(null);
            private final AtomicBoolean hasNextSub = new AtomicBoolean(true);
            private final ShortObjectMutableTuple<V> next = new ShortObjectMutableTuple<V>();
            private volatile boolean hasNext = true;
            private volatile int subEntryIndex = 0;
            private volatile ShortObjectSubEntry<V> entry;
            ...
            class ThreadState   {
                ...
                public ThreadState(ObjectConcurrentIterator<V> this_, ShortObjectConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
    ...
    @Override
    public ShortObjectConcurrentIterator<V> entryIterator() {
        return new ShortObjectConcurrentIterator<V>()  {
            private final ShortObjectConcurrentTreeHashMap this_ = ShortObjectConcurrentTreeHashMap.this;
            //private final ReentrantLock lock = new ReentrantLock();
            private final ThreadLocal<ThreadState> tl = ThreadLocal.withInitial(() -> new ThreadState(this, this.this_));
            private final AtomicInteger sectorOffset = new AtomicInteger(0);
            ...
            @Override
            public synchronized ShortObjectTuple<V> advance()   {
                if (!this.hasNext())    {
                    throw new IteratorCompleteException();
                }
                ThreadState state = this.tl.get();
                ShortObjectTuple<V> current = state.tuple = new ShortObjectMutableTuple<V>(this.next.k, this.next.v);
                state.entry = this.entry;
                this.findNextValue();
                return current;
            }
            ...
            class ThreadState   {
                public ShortObjectTuple<V> tuple;
                public ShortObjectSubEntry entry;
                private final ShortObjectConcurrentIterator<V> this_;
                private final ShortObjectConcurrentTreeHashMap this__;

                public ThreadState(ShortObjectConcurrentIterator<V> this_, ShortObjectConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
     ...
     */
    public ShortObjectConcurrentIterator<V> fastEntryIterator() {
        return new ShortObjectConcurrentIterator<V>()  {
            private final ShortObjectConcurrentTreeHashMap this_ = ShortObjectConcurrentTreeHashMap.this;
            //private final ReentrantLock lock = new ReentrantLock();
            private final ThreadLocal<ThreadState> tl = ThreadLocal.withInitial(() -> new ThreadState(this, this.this_));
            ...
            private final AtomicReference<ShortObjectSubEntry<V>.Sector> currSector = new AtomicReference(null);
            private final AtomicBoolean hasNextSub = new AtomicBoolean(true);
            private volatile ShortObjectMutableTuple<V> next = new ShortObjectMutableTuple<V>((short) 0, null);
            private volatile boolean hasNext = true;
            private volatile int subEntryIndex = 0;
            private volatile ShortObjectSubEntry<V> entry;
            ...
            @Override
            public ShortObjectMutableTuple<V> get()   {
                ThreadState state = this.tl.get();
                if (state.entry == null)    {
                    throw new IllegalStateException();
                } else {
            ...
            @Override
            public synchronized ShortObjectMutableTuple<V> advance()   {
                if (!this.hasNext())    {
                    throw new IteratorCompleteException();
                }
                ThreadState state = this.tl.get();
                ShortObjectMutableTuple<V> a = state.tuple;
            ...
            class ThreadState   {
                public ShortObjectMutableTuple<V> tuple = new ShortObjectMutableTuple<V>((short) 0, null);
                public ShortObjectSubEntry entry;
                private final ShortObjectConcurrentIterator<V> this_;
                private final ShortObjectConcurrentTreeHashMap this__;
    ...
    private ShortObjectSubEntry<V> getSubentry(int index, boolean create)    {
        assert index == (index & 0xFF);

        synchronized (this.entries) {
            ShortObjectSubEntry<V> entry = (ShortObjectSubEntry<V>) this.entries[index];
            if (entry == null && create)    {
                this.entries[index] = entry = new ShortObjectSubEntry<V>(this);
            }
            return entry;
        }
    }
