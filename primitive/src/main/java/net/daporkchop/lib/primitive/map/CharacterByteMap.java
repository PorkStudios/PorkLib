...
import net.daporkchop.lib.primitive.lambda.function.CharacterToByteFunction;
...
/**
 * A map, using a key type of char and a value type of byte.
 * <p>
...
public interface CharacterByteMap   {
     ...
     */
    default void putAll(@NonNull char[] keys, @NonNull byte[] values)    {
     ...
     */
    default void putAll(@NonNull CharacterByteMap map)    {
     ...
     * @param consumer the function to run
     */
    void forEachEntry(CharacterByteConsumer consumer);
     ...
     */
    CharacterByteIterator entryIterator();
     ...
     */
    default byte getOrDefault(char key, byte def)   {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
            this.put(key, def);
     ...
     */
    default byte computeIfAbsent(char key, @NonNull CharacterToByteFunction func) {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
