...
public class CAST6KeyGen {
    static {
        BouncyCastleInit.loadClass();
    }

    public static CAST6Key gen(byte[] data) {
        byte[] b = new byte[BlockCipherType.CAST6.blockSize];
        int i = 0;
        do {
        ...
        data = b;
        return new CAST6Key(new SecretKeySpec(data, "aaa"), KeyRandom.getBytes(BlockCipherType.CAST6.blockSize));
    }

    public static CAST6Key gen() {
        byte[] key = new byte[BlockCipherType.CAST6.blockSize];
