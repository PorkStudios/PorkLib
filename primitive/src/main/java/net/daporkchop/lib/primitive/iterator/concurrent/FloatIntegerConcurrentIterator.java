...
import net.daporkchop.lib.primitive.lambda.function.bi.FloatIntegerToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of float => int mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface FloatIntegerConcurrentIterator extends FloatIntegerIterator   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull FloatIntegerToBooleanFunction function)    {
        try {
            while (this.hasNext())  {
                FloatIntegerTuple tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<FloatIntegerTuple> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<FloatIntegerTuple> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull FloatIntegerConsumer consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<FloatIntegerTuple> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull FloatIntegerToBooleanFunction function)    {
        this.concurrentIterate(function);
    }
}
