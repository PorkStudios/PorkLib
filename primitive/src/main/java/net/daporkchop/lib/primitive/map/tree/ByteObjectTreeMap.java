 ...
 * Although this map WORKS, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.ByteObjectIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class ByteObjectTreeMap<V extends Object> implements ByteObjectMap<V>    {
    protected byte[] keys;
    ...
    @SuppressWarnings("unchecked")
    protected V remove(byte key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return null; //no mapping
    ...
    public ByteIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.ByteObjectIndexedTreeMap} for an implementation of this method
        return null;
    }
