...
/**
 * A single subentry for a {@link IntegerFloatConcurrentTreeHashMap}.
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
class IntegerFloatSubEntry    {
    public final AtomicInteger size = new AtomicInteger(0);
    public final IntegerFloatConcurrentTreeHashMap parent;
    ...
    public IntegerFloatSubEntry(IntegerFloatConcurrentTreeHashMap parent)   {
        this.parent = parent;
        this.rootPointer = this.parent.subHashLength == 0 ? new IntegerFloatSubEntry.ValueSector(null, -1) : new IntegerFloatSubEntry.PointerSector(null, -1);
    }
    ...
    public void forEachKey(IntegerConsumer consumer)   {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof IntegerFloatSubEntry.PointerSector)    {
    ...
    public int advanceKey(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext)   {
        ...
        for (int i = sector.states.nextSetBit(curr.get() + 1); i != -1; i = sector.states.nextSetBit(i + 1)) {
            curr.set(i);
            if (sector instanceof IntegerFloatSubEntry.ValueSector)  {
    ...
    public void advanceEntry(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext, IntegerFloatMutableTuple tuple)   {
        Sector sector = sectorRef.get();
        if (sector == null) {
            sectorRef.set(sector = this.rootPointer);
        }
    ...
    public Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.parent.subHashLength; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.parent.subHashLength ? new IntegerFloatSubEntry.ValueSector(curr, off) : new IntegerFloatSubEntry.PointerSector(curr, off);
    ...
    public class ValueSector extends Sector   {
        ...
        public float setEntry(int index, int key, float value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                IntegerFloatSubEntry.this.size.incrementAndGet();
                IntegerFloatSubEntry.this.parent.size.incrementAndGet();
                return 0F;
            }
        }
        ...
        public float removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                IntegerFloatSubEntry.this.size.decrementAndGet();
                IntegerFloatSubEntry.this.parent.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
