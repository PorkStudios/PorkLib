...
/**
 * A single subentry for a {@link FloatIntegerConcurrentTreeHashMap}.
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
class FloatIntegerSubEntry    {
    public final AtomicInteger size = new AtomicInteger(0);
    public final FloatIntegerConcurrentTreeHashMap parent;
    ...
    public FloatIntegerSubEntry(FloatIntegerConcurrentTreeHashMap parent)   {
        this.parent = parent;
        this.rootPointer = this.parent.subHashLength == 0 ? new FloatIntegerSubEntry.ValueSector(null, -1) : new FloatIntegerSubEntry.PointerSector(null, -1);
    }
    ...
    public void forEachKey(FloatConsumer consumer)   {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof FloatIntegerSubEntry.PointerSector)    {
    ...
    public float advanceKey(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext)   {
        ...
        for (int i = sector.states.nextSetBit(curr.get() + 1); i != -1; i = sector.states.nextSetBit(i + 1)) {
            curr.set(i);
            if (sector instanceof FloatIntegerSubEntry.ValueSector)  {
    ...
    public void advanceEntry(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext, FloatIntegerMutableTuple tuple)   {
        Sector sector = sectorRef.get();
        if (sector == null) {
            sectorRef.set(sector = this.rootPointer);
        }
    ...
    public Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.parent.subHashLength; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.parent.subHashLength ? new FloatIntegerSubEntry.ValueSector(curr, off) : new FloatIntegerSubEntry.PointerSector(curr, off);
    ...
    public abstract class Sector  {
        ...
        public int setEntry(int index, float key, int value)   {
            throw new UnsupportedOperationException();
        }
    ...
    public class ValueSector extends Sector   {
        ...
        public int setEntry(int index, float key, int value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                FloatIntegerSubEntry.this.size.incrementAndGet();
                FloatIntegerSubEntry.this.parent.size.incrementAndGet();
                return 0;
            }
        }
        ...
        public int removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                FloatIntegerSubEntry.this.size.decrementAndGet();
                FloatIntegerSubEntry.this.parent.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
