 ...
 * Although this map 𝙒𝙊𝙍𝙆𝙎, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.DoubleCharacterIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class DoubleCharacterTreeMap implements DoubleCharacterMap    {
    ...
    @SuppressWarnings("unchecked")
    protected char remove(double key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return (char) 0; //no mapping
    ...
    public DoubleIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.DoubleCharacterIndexedTreeMap} for an implementation of this method
        return null;
    }
