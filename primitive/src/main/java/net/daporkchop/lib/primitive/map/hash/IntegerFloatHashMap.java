...
/**
 * A hash map, using a key type of int and a value type of float.
 ...
 * @author DaPorkchop_
 */
public class IntegerFloatHashMap implements IntegerFloatMap    {
    protected int[] keys;
    ...
    public IntegerFloatHashMap()    {
        this(16384);
    }

    public IntegerFloatHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public IntegerFloatHashMap(IntegerToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public IntegerFloatHashMap(int baseSize, IntegerToIntegerFunction keyHash)    {
    ...
    protected boolean reHash(float[] values, int[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
            int key = this.keys[i];
            int j = getIndex(key);
            if (states.get(j))  {
    ...
    private class KeyIterator implements IntegerIterator {
        ...
        public KeyIterator()    {
            this.size = IntegerFloatHashMap.this.size;
            this.next = IntegerFloatHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public int get()   {
            return IntegerFloatHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = IntegerFloatHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            IntegerFloatHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements FloatIterator {
        ...
        @SuppressWarnings("unchecked")
        public float get()   {
            return IntegerFloatHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements IntegerFloatIterator {
        private int size;
        private int index = -1;
        private int next;
        private IntegerFloatTuple tuple;
        ...
        @Override
        public IntegerFloatTuple advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new IntegerFloatImmutableTuple( IntegerFloatHashMap.this.keys[this.index],  IntegerFloatHashMap.this.values[this.index]);
        }
