 ...
 * Although this map WORKS, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.CharacterObjectIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class CharacterObjectTreeMap<V extends Object> implements CharacterObjectMap<V>    {
    protected char[] keys;
    ...
    @SuppressWarnings("unchecked")
    protected V remove(char key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return null; //no mapping
    ...
    public CharacterIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.CharacterObjectIndexedTreeMap} for an implementation of this method
        return null;
    }
