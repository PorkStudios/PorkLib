...
import net.daporkchop.lib.primitive.lambda.function.bi.ShortIntegerToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of short => int mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface ShortIntegerConcurrentIterator extends ShortIntegerIterator   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ShortIntegerToBooleanFunction function)    {
        try {
            while (this.hasNext())  {
                ShortIntegerTuple tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<ShortIntegerTuple> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<ShortIntegerTuple> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ShortIntegerConsumer consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<ShortIntegerTuple> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull ShortIntegerToBooleanFunction function)    {
        this.concurrentIterate(function);
    }
}
