...
/**
 * A hash map, using a key type of long and a value type of int.
 ...
 * @author DaPorkchop_
 */
public class LongIntegerHashMap implements LongIntegerMap    {
    protected long[] keys;
    ...
    public LongIntegerHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public LongIntegerHashMap(LongToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public LongIntegerHashMap(int baseSize, LongToIntegerFunction keyHash)    {
    ...
    protected boolean reHash(int[] values, long[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
    ...
    private class KeyIterator implements LongIterator {
        ...
        public KeyIterator()    {
            this.size = LongIntegerHashMap.this.size;
            this.next = LongIntegerHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public long get()   {
            return LongIntegerHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = LongIntegerHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            LongIntegerHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements IntegerIterator {
        ...
        @SuppressWarnings("unchecked")
        public int get()   {
            return LongIntegerHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements LongIntegerIterator {
        ...
        public LongIntegerTuple advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new LongIntegerImmutableTuple( LongIntegerHashMap.this.keys[this.index],  LongIntegerHashMap.this.values[this.index]);
        }
