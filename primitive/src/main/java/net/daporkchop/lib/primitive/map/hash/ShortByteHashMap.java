...
import net.daporkchop.lib.primitive.iterator.bi.ShortByteIterator;
...
/**
 * A hash map, using a key type of short and a value type of byte.
 ...
 * @author DaPorkchop_
 */
public class ShortByteHashMap implements ShortByteMap    {
    ...
    public ShortByteHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public ShortByteHashMap(ShortToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public ShortByteHashMap(int baseSize, ShortToIntegerFunction keyHash)    {
    ...
    @Override
    public void forEachEntry(@NonNull ShortByteConsumer consumer)   {
    ...
    public ShortByteIterator entryIterator() {
    ...
    protected boolean reHash(byte[] values, short[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
    ...
    private class KeyIterator implements ShortIterator {
        ...
        public KeyIterator()    {
            this.size = ShortByteHashMap.this.size;
            this.next = ShortByteHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public short get()   {
            return ShortByteHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = ShortByteHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            ShortByteHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements ByteIterator {
        ...
        @SuppressWarnings("unchecked")
        public byte get()   {
            return ShortByteHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements ShortByteIterator {
        private int size;
        private int index = -1;
        private int next;
        private ShortByteTuple tuple;
        ...
        @Override
        public ShortByteTuple advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new ShortByteImmutableTuple( ShortByteHashMap.this.keys[this.index],  ShortByteHashMap.this.values[this.index]);
        }
