...
import net.daporkchop.lib.primitive.lambda.function.ShortToShortFunction;
...
/**
 * A map, using a key type of short and a value type of short.
 * <p>
...
public interface ShortShortMap   {
     ...
     */
    default void putAll(@NonNull short[] keys, @NonNull short[] values)    {
     ...
     */
    default void putAll(@NonNull ShortShortMap map)    {
     ...
     * @param consumer the function to run
     */
    void forEachEntry(ShortShortConsumer consumer);
     ...
     */
    ShortShortIterator entryIterator();
     ...
     */
    default short getOrDefault(short key, short def)   {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
            this.put(key, def);
     ...
     */
    default short computeIfAbsent(short key, @NonNull ShortToShortFunction func) {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
