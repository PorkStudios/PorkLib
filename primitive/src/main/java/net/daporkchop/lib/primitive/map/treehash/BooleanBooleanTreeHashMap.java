 ...
 * overhead, and can even be made concurrent with little extra work (see {@link net.daporkchop.lib.primitive.map.concurrent.BooleanBooleanConcurrentTreeHashMap})
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
public class BooleanBooleanTreeHashMap implements BooleanBooleanMap    {
    private final AtomicInteger size = new AtomicInteger(0);
    private final BooleanToLongFunction hashFunc;
    private final int hashLen;
    private volatile Sector rootPointer;
    private volatile ObjectConsumer<Sector> recursiveHack;

    public BooleanBooleanTreeHashMap()    {
        this(null, 0);
    }

    public BooleanBooleanTreeHashMap(BooleanToLongFunction hashFunc, int hashLen)    {
        ...
        this.rootPointer = this.hashLen == 0 ? new BooleanBooleanTreeHashMap.ValueSector(null, -1) : new BooleanBooleanTreeHashMap.PointerSector(null, -1);
    }
    ...
    public void forEachKey(@NonNull BooleanConsumer consumer)  {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof BooleanBooleanTreeHashMap.PointerSector)    {
    ...
    private Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.hashLen; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.hashLen ? new BooleanBooleanTreeHashMap.ValueSector(curr, off) : new BooleanBooleanTreeHashMap.PointerSector(curr, off);
    ...
    private abstract class Sector  {
        ...
        public boolean setEntry(int index, boolean key, boolean value)   {
            throw new UnsupportedOperationException();
        }
    ...
    private class ValueSector extends Sector   {
        ...
        public boolean setEntry(int index, boolean key, boolean value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                BooleanBooleanTreeHashMap.this.size.incrementAndGet();
                return false;
            }
        }
        ...
        public boolean removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                BooleanBooleanTreeHashMap.this.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
