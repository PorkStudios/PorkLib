...
/**
 * A hash map, using a key type of char and a value type of byte.
 ...
 * @author DaPorkchop_
 */
public class CharacterByteHashMap implements CharacterByteMap    {
    protected char[] keys;
    ...
    public CharacterByteHashMap()    {
        this(16384);
    }

    public CharacterByteHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public CharacterByteHashMap(CharacterToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public CharacterByteHashMap(int baseSize, CharacterToIntegerFunction keyHash)    {
    ...
    protected boolean reHash(byte[] values, char[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
    ...
    private class KeyIterator implements CharacterIterator {
        ...
        public KeyIterator()    {
            this.size = CharacterByteHashMap.this.size;
            this.next = CharacterByteHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public char get()   {
            return CharacterByteHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = CharacterByteHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            CharacterByteHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements ByteIterator {
        ...
        @SuppressWarnings("unchecked")
        public byte get()   {
            return CharacterByteHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements CharacterByteIterator {
        private int size;
        private int index = -1;
        private int next;
        private CharacterByteTuple tuple;
        ...
        @Override
        public CharacterByteTuple advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new CharacterByteImmutableTuple( CharacterByteHashMap.this.keys[this.index],  CharacterByteHashMap.this.values[this.index]);
        }
