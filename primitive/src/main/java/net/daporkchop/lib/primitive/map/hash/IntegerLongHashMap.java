...
/**
 * A hash map, using a key type of int and a value type of long.
 ...
 * @author DaPorkchop_
 */
public class IntegerLongHashMap implements IntegerLongMap    {
    protected int[] keys;
    ...
    public IntegerLongHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public IntegerLongHashMap(IntegerToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public IntegerLongHashMap(int baseSize, IntegerToIntegerFunction keyHash)    {
    ...
    @Override
    public void forEachEntry(@NonNull IntegerLongConsumer consumer)   {
    ...
    public IntegerLongIterator entryIterator() {
    ...
    protected boolean reHash(long[] values, int[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
            int key = this.keys[i];
            int j = getIndex(key);
            if (states.get(j))  {
    ...
    private class KeyIterator implements IntegerIterator {
        ...
        public KeyIterator()    {
            this.size = IntegerLongHashMap.this.size;
            this.next = IntegerLongHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public int get()   {
            return IntegerLongHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = IntegerLongHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            IntegerLongHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements LongIterator {
        ...
        @SuppressWarnings("unchecked")
        public long get()   {
            return IntegerLongHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements IntegerLongIterator {
        private int size;
        private int index = -1;
        private int next;
        private IntegerLongTuple tuple;
        ...
        @Override
        public IntegerLongTuple advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new IntegerLongImmutableTuple( IntegerLongHashMap.this.keys[this.index],  IntegerLongHashMap.this.values[this.index]);
        }
