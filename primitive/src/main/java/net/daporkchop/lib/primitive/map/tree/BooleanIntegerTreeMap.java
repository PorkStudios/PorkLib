 ...
 * Although this map 𝙒𝙊𝙍𝙆𝙎, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.BooleanIntegerIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class BooleanIntegerTreeMap implements BooleanIntegerMap    {
    ...
    @SuppressWarnings("unchecked")
    protected int remove(boolean key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return 0; //no mapping
    ...
    public BooleanIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.BooleanIntegerIndexedTreeMap} for an implementation of this method
        return null;
    }
