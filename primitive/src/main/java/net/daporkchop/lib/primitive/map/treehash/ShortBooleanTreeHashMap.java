 ...
 * overhead, and can even be made concurrent with little extra work (see {@link net.daporkchop.lib.primitive.map.concurrent.ShortBooleanConcurrentTreeHashMap})
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
public class ShortBooleanTreeHashMap implements ShortBooleanMap    {
    ...
    public ShortBooleanTreeHashMap()    {
        this(null, 0);
    }

    public ShortBooleanTreeHashMap(ShortToLongFunction hashFunc, int hashLen)    {
        ...
        this.rootPointer = this.hashLen == 0 ? new ShortBooleanTreeHashMap.ValueSector(null, -1) : new ShortBooleanTreeHashMap.PointerSector(null, -1);
    }
    ...
    public void forEachKey(@NonNull ShortConsumer consumer)  {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof ShortBooleanTreeHashMap.PointerSector)    {
    ...
    private Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.hashLen; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.hashLen ? new ShortBooleanTreeHashMap.ValueSector(curr, off) : new ShortBooleanTreeHashMap.PointerSector(curr, off);
    ...
    private class ValueSector extends Sector   {
        ...
        public boolean setEntry(int index, short key, boolean value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                ShortBooleanTreeHashMap.this.size.incrementAndGet();
                return false;
            }
        }
        ...
        public boolean removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                ShortBooleanTreeHashMap.this.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
