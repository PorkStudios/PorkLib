 ...
 * Although this map 𝙒𝙊𝙍𝙆𝙎, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.IntegerByteIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class IntegerByteTreeMap implements IntegerByteMap    {
    protected int[] keys;
    ...
    @SuppressWarnings("unchecked")
    protected byte remove(int key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return (byte) 0; //no mapping
    ...
    public IntegerIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.IntegerByteIndexedTreeMap} for an implementation of this method
        return null;
    }
