 ...
 * Although this map 𝙒𝙊𝙍𝙆𝙎, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.ObjectShortIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class ObjectShortTreeMap<K extends Object> implements ObjectShortMap<K>    {
    ...
    @SuppressWarnings("unchecked")
    protected short remove(K key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return (short) 0; //no mapping
    ...
    public ObjectIterator<K> keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.ObjectShortIndexedTreeMap} for an implementation of this method
        return null;
    }
