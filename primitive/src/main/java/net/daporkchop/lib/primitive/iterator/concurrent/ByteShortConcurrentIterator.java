...
import net.daporkchop.lib.primitive.lambda.function.bi.ByteShortToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of byte => short mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface ByteShortConcurrentIterator extends ByteShortIterator   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ByteShortToBooleanFunction function)    {
        try {
            while (this.hasNext())  {
                ByteShortTuple tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<ByteShortTuple> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<ByteShortTuple> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ByteShortConsumer consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<ByteShortTuple> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull ByteShortToBooleanFunction function)    {
        this.concurrentIterate(function);
    }
}
