...
import net.daporkchop.lib.primitive.iterator.bi.BooleanFloatIterator;
...
/**
 * A hash map, using a key type of boolean and a value type of float.
 ...
 * @author DaPorkchop_
 */
public class BooleanFloatHashMap implements BooleanFloatMap    {
    ...
    public BooleanFloatHashMap()    {
        this(16384);
    }

    public BooleanFloatHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public BooleanFloatHashMap(BooleanToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public BooleanFloatHashMap(int baseSize, BooleanToIntegerFunction keyHash)    {
    ...
    @Override
    public void forEachEntry(@NonNull BooleanFloatConsumer consumer)   {
    ...
    public BooleanFloatIterator entryIterator() {
    ...
    protected boolean reHash(float[] values, boolean[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
    ...
    private class KeyIterator implements BooleanIterator {
        ...
        public KeyIterator()    {
            this.size = BooleanFloatHashMap.this.size;
            this.next = BooleanFloatHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public boolean get()   {
            return BooleanFloatHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = BooleanFloatHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            BooleanFloatHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements FloatIterator {
        ...
        @SuppressWarnings("unchecked")
        public float get()   {
            return BooleanFloatHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements BooleanFloatIterator {
        private int size;
        private int index = -1;
        private int next;
        private BooleanFloatTuple tuple;
        ...
        @Override
        public BooleanFloatTuple advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new BooleanFloatImmutableTuple( BooleanFloatHashMap.this.keys[this.index],  BooleanFloatHashMap.this.values[this.index]);
        }
