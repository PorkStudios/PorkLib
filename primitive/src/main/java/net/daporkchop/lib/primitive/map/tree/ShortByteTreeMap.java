 ...
 * Although this map WORKS, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.ShortByteIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class ShortByteTreeMap implements ShortByteMap    {
    ...
    @SuppressWarnings("unchecked")
    protected byte remove(short key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return (byte) 0; //no mapping
    ...
    public ShortIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.ShortByteIndexedTreeMap} for an implementation of this method
        return null;
    }
