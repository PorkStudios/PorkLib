 ...
 * overhead, and can even be made concurrent with little extra work (see {@link net.daporkchop.lib.primitive.map.concurrent.ShortCharacterConcurrentTreeHashMap})
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
public class ShortCharacterTreeHashMap implements ShortCharacterMap    {
    ...
    public ShortCharacterTreeHashMap()    {
        this(null, 0);
    }

    public ShortCharacterTreeHashMap(ShortToLongFunction hashFunc, int hashLen)    {
        ...
        this.rootPointer = this.hashLen == 0 ? new ShortCharacterTreeHashMap.ValueSector(null, -1) : new ShortCharacterTreeHashMap.PointerSector(null, -1);
    }
    ...
    public void forEachKey(@NonNull ShortConsumer consumer)  {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof ShortCharacterTreeHashMap.PointerSector)    {
    ...
    private Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.hashLen; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.hashLen ? new ShortCharacterTreeHashMap.ValueSector(curr, off) : new ShortCharacterTreeHashMap.PointerSector(curr, off);
    ...
    private class ValueSector extends Sector   {
        ...
        public char setEntry(int index, short key, char value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                ShortCharacterTreeHashMap.this.size.incrementAndGet();
                return (char) 0;
            }
        }
        ...
        public char removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                ShortCharacterTreeHashMap.this.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
