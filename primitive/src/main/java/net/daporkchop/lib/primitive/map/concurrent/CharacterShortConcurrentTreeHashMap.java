...
import net.daporkchop.lib.primitive.iterator.concurrent.CharacterShortConcurrentIterator;
import net.daporkchop.lib.primitive.map.CharacterShortMap;
import net.daporkchop.lib.primitive.tuple.CharacterShortTuple;
import net.daporkchop.lib.primitive.tuple.CharacterShortImmutableTuple;
import net.daporkchop.lib.primitive.tuple.CharacterShortMutableTuple;
...
/**
 * A tree hash map, using a key type of char and a value type of short.
 ...
 * @author DaPorkchop_
 */
public class CharacterShortConcurrentTreeHashMap implements CharacterShortMap    {
    final AtomicInteger size = new AtomicInteger(0);

    private final CharacterShortSubEntry[] entries = new CharacterShortSubEntry[256];
    private final CharacterToIntegerFunction keyHash;
    final CharacterToLongFunction subKeyHash;
    final int subHashLength;

    public CharacterShortConcurrentTreeHashMap()    {
        this(null, null);
    }

    public CharacterShortConcurrentTreeHashMap(CharacterToIntegerFunction keyHash, CharacterToLongFunction subKeyHash)    {
        this(keyHash, subKeyHash, 1);
    }

    public CharacterShortConcurrentTreeHashMap(CharacterToIntegerFunction keyHash, CharacterToLongFunction subKeyHash, int subHashLength)    {
    ...
    public short get(char key)   {
        CharacterShortSubEntry entry = this.getSubentry(this.keyHash.apply(key) & 0xFF, false);
        if (entry == null)  {
            return (short) 0;
        } else {
    ...
    public short put(char key, short value)   {
        CharacterShortSubEntry entry = this.getSubentry(this.keyHash.apply(key) & 0xFF, true);
    ...
    public short remove(char key)    {
        ...
        CharacterShortSubEntry entry = this.getSubentry(hash, false);
        if (entry == null)  {
            return (short) 0;
        } else {
    ...
    public void clear() {
        synchronized (this.entries) {
            for (int i = 0; i < 256; i++) {
                CharacterShortSubEntry entry = this.entries[i];
                if (entry != null)  {
                    entry.clear();
                    this.entries[i] = null;
                }
    ...
    public void forEachKey(CharacterConsumer consumer)   {
        for (int i = 0; i < 256; i++) {
            CharacterShortSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
    ...
    @Override
    public void forEachEntry(CharacterShortConsumer consumer) {
        for (int i = 0; i < 256; i++) {
            CharacterShortSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
    ...
    public void forEachEntry(CharacterShortConsumer consumer, VoidFunction complete) {
        for (int i = 0; i < 256; i++) {
            CharacterShortSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
                entry.forEachEntry(consumer);
    ...
    public CharacterConcurrentIterator keyIterator()   {
        return new CharacterConcurrentIterator()  {
            private final CharacterShortConcurrentTreeHashMap this_ = CharacterShortConcurrentTreeHashMap.this;
            ...
            private final AtomicReference<CharacterShortSubEntry.Sector> currSector = new AtomicReference(null);
            ...
            private volatile CharacterShortSubEntry entry;
            ...
            private void nextSub()   {
                ...
                while (this.subEntryIndex < 256)    {
                    CharacterShortSubEntry entry = getSubentry(this.subEntryIndex++, false);
                    if (entry != null)  {
                        this.entry = entry;
            ...
            class ThreadState   {
                public char key;
                public CharacterShortSubEntry entry;
                private final CharacterConcurrentIterator this_;
                private final CharacterShortConcurrentTreeHashMap this__;

                public ThreadState(CharacterConcurrentIterator this_, CharacterShortConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
    ...
    public ShortConcurrentIterator valueIterator() {
        return new ShortConcurrentIterator()  {
            ...
            private final AtomicReference<CharacterShortSubEntry.Sector> currSector = new AtomicReference(null);
            private final AtomicBoolean hasNextSub = new AtomicBoolean(true);
            private final CharacterShortMutableTuple next = new CharacterShortMutableTuple();
            private volatile boolean hasNext = true;
            private volatile int subEntryIndex = 0;
            private volatile CharacterShortSubEntry entry;
            ...
            class ThreadState   {
                ...
                public ThreadState(ShortConcurrentIterator this_, CharacterShortConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
    ...
    @Override
    public CharacterShortConcurrentIterator entryIterator() {
        return new CharacterShortConcurrentIterator()  {
            private final CharacterShortConcurrentTreeHashMap this_ = CharacterShortConcurrentTreeHashMap.this;
            //private final ReentrantLock lock = new ReentrantLock();
            private final ThreadLocal<ThreadState> tl = ThreadLocal.withInitial(() -> new ThreadState(this, this.this_));
            private final AtomicInteger sectorOffset = new AtomicInteger(0);
            ...
            @Override
            public CharacterShortTuple get()   {
                ThreadState state = this.tl.get();
                if (state.entry == null)    {
                    throw new IllegalStateException();
                } else {
            ...
            @Override
            public synchronized CharacterShortTuple advance()   {
                if (!this.hasNext())    {
                    throw new IteratorCompleteException();
                }
                ThreadState state = this.tl.get();
                CharacterShortTuple current = state.tuple = new CharacterShortMutableTuple(this.next.k, this.next.v);
                state.entry = this.entry;
                this.findNextValue();
                return current;
            }
            ...
            class ThreadState   {
                public CharacterShortTuple tuple;
                public CharacterShortSubEntry entry;
                private final CharacterShortConcurrentIterator this_;
                private final CharacterShortConcurrentTreeHashMap this__;

                public ThreadState(CharacterShortConcurrentIterator this_, CharacterShortConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
     ...
     */
    public CharacterShortConcurrentIterator fastEntryIterator() {
        return new CharacterShortConcurrentIterator()  {
            private final CharacterShortConcurrentTreeHashMap this_ = CharacterShortConcurrentTreeHashMap.this;
            //private final ReentrantLock lock = new ReentrantLock();
            private final ThreadLocal<ThreadState> tl = ThreadLocal.withInitial(() -> new ThreadState(this, this.this_));
            ...
            private final AtomicReference<CharacterShortSubEntry.Sector> currSector = new AtomicReference(null);
            private final AtomicBoolean hasNextSub = new AtomicBoolean(true);
            private volatile CharacterShortMutableTuple next = new CharacterShortMutableTuple((char) 0, (short) 0);
            private volatile boolean hasNext = true;
            private volatile int subEntryIndex = 0;
            private volatile CharacterShortSubEntry entry;
            ...
            @Override
            public CharacterShortMutableTuple get()   {
                ThreadState state = this.tl.get();
                if (state.entry == null)    {
                    throw new IllegalStateException();
                } else {
            ...
            @Override
            public synchronized CharacterShortMutableTuple advance()   {
                if (!this.hasNext())    {
                    throw new IteratorCompleteException();
                }
                ThreadState state = this.tl.get();
                CharacterShortMutableTuple a = state.tuple;
            ...
            class ThreadState   {
                public CharacterShortMutableTuple tuple = new CharacterShortMutableTuple((char) 0, (short) 0);
                public CharacterShortSubEntry entry;
                private final CharacterShortConcurrentIterator this_;
                private final CharacterShortConcurrentTreeHashMap this__;
    ...
    private CharacterShortSubEntry getSubentry(int index, boolean create)    {
        assert index == (index & 0xFF);

        synchronized (this.entries) {
            CharacterShortSubEntry entry = (CharacterShortSubEntry) this.entries[index];
            if (entry == null && create)    {
                this.entries[index] = entry = new CharacterShortSubEntry(this);
            }
            return entry;
        }
    }
