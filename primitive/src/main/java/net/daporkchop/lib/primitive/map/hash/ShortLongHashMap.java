...
/**
 * A hash map, using a key type of short and a value type of long.
 ...
 * @author DaPorkchop_
 */
public class ShortLongHashMap implements ShortLongMap    {
    ...
    public ShortLongHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public ShortLongHashMap(ShortToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public ShortLongHashMap(int baseSize, ShortToIntegerFunction keyHash)    {
    ...
    protected boolean reHash(long[] values, short[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
    ...
    private class KeyIterator implements ShortIterator {
        ...
        public KeyIterator()    {
            this.size = ShortLongHashMap.this.size;
            this.next = ShortLongHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public short get()   {
            return ShortLongHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = ShortLongHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            ShortLongHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements LongIterator {
        ...
        @SuppressWarnings("unchecked")
        public long get()   {
            return ShortLongHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements ShortLongIterator {
        private int size;
        private int index = -1;
        private int next;
        private ShortLongTuple tuple;
        ...
        @Override
        public ShortLongTuple advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new ShortLongImmutableTuple( ShortLongHashMap.this.keys[this.index],  ShortLongHashMap.this.values[this.index]);
        }
