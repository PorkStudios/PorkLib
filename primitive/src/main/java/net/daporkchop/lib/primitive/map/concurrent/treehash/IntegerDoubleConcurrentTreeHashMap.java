...
import net.daporkchop.lib.primitive.iterator.concurrent.IntegerDoubleConcurrentIterator;
import net.daporkchop.lib.primitive.map.IntegerDoubleMap;
import net.daporkchop.lib.primitive.tuple.IntegerDoubleTuple;
import net.daporkchop.lib.primitive.tuple.IntegerDoubleImmutableTuple;
import net.daporkchop.lib.primitive.tuple.IntegerDoubleMutableTuple;
...
/**
 * A tree hash map, using a key type of int and a value type of double.
 ...
 * @author DaPorkchop_
 */
public class IntegerDoubleConcurrentTreeHashMap implements IntegerDoubleMap    {
    final AtomicInteger size = new AtomicInteger(0);

    private final IntegerDoubleSubEntry[] entries = new IntegerDoubleSubEntry[256];
    private final IntegerToIntegerFunction keyHash;
    final IntegerToLongFunction subKeyHash;
    final int subHashLength;

    public IntegerDoubleConcurrentTreeHashMap()    {
        this(null, null);
    }

    public IntegerDoubleConcurrentTreeHashMap(IntegerToIntegerFunction keyHash, IntegerToLongFunction subKeyHash)    {
        this(keyHash, subKeyHash, 3);
    }

    public IntegerDoubleConcurrentTreeHashMap(IntegerToIntegerFunction keyHash, IntegerToLongFunction subKeyHash, int subHashLength)    {
    ...
    public double get(int key)   {
        IntegerDoubleSubEntry entry = this.getSubentry(this.keyHash.apply(key) & 0xFF, false);
        if (entry == null)  {
            return 0D;
        } else {
    ...
    public double put(int key, double value)   {
        IntegerDoubleSubEntry entry = this.getSubentry(this.keyHash.apply(key) & 0xFF, true);
    ...
    public double remove(int key)    {
        ...
        IntegerDoubleSubEntry entry = this.getSubentry(hash, false);
        if (entry == null)  {
            return 0D;
        } else {
    ...
    public void clear() {
        synchronized (this.entries) {
            for (int i = 0; i < 256; i++) {
                IntegerDoubleSubEntry entry = this.entries[i];
                if (entry != null)  {
                    entry.clear();
                    this.entries[i] = null;
                }
    ...
    public void forEachKey(IntegerConsumer consumer)   {
        for (int i = 0; i < 256; i++) {
            IntegerDoubleSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
    ...
    @Override
    public void forEachEntry(IntegerDoubleConsumer consumer) {
        for (int i = 0; i < 256; i++) {
            IntegerDoubleSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
    ...
    public void forEachEntry(IntegerDoubleConsumer consumer, VoidFunction complete) {
        for (int i = 0; i < 256; i++) {
            IntegerDoubleSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
                entry.forEachEntry(consumer);
    ...
    public IntegerConcurrentIterator keyIterator()   {
        return new IntegerConcurrentIterator()  {
            private final IntegerDoubleConcurrentTreeHashMap this_ = IntegerDoubleConcurrentTreeHashMap.this;
            ...
            private final AtomicReference<IntegerDoubleSubEntry.Sector> currSector = new AtomicReference(null);
            private final AtomicBoolean hasNextSub = new AtomicBoolean(true);
            private volatile boolean hasNext = true;
            private volatile int subEntryIndex = 0;
            private volatile int next;
            private volatile IntegerDoubleSubEntry entry;
            ...
            private void nextSub()   {
                ...
                while (this.subEntryIndex < 256)    {
                    IntegerDoubleSubEntry entry = getSubentry(this.subEntryIndex++, false);
                    if (entry != null)  {
                        this.entry = entry;
            ...
            class ThreadState   {
                public int key;
                public IntegerDoubleSubEntry entry;
                private final IntegerConcurrentIterator this_;
                private final IntegerDoubleConcurrentTreeHashMap this__;

                public ThreadState(IntegerConcurrentIterator this_, IntegerDoubleConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
    ...
    public DoubleConcurrentIterator valueIterator() {
        return new DoubleConcurrentIterator()  {
            ...
            private final AtomicReference<IntegerDoubleSubEntry.Sector> currSector = new AtomicReference(null);
            private final AtomicBoolean hasNextSub = new AtomicBoolean(true);
            private final IntegerDoubleMutableTuple next = new IntegerDoubleMutableTuple();
            private volatile boolean hasNext = true;
            private volatile int subEntryIndex = 0;
            private volatile IntegerDoubleSubEntry entry;
            ...
            class ThreadState   {
                ...
                public ThreadState(DoubleConcurrentIterator this_, IntegerDoubleConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
    ...
    @Override
    public IntegerDoubleConcurrentIterator entryIterator() {
        return new IntegerDoubleConcurrentIterator()  {
            private final IntegerDoubleConcurrentTreeHashMap this_ = IntegerDoubleConcurrentTreeHashMap.this;
            //private final ReentrantLock lock = new ReentrantLock();
            private final ThreadLocal<ThreadState> tl = ThreadLocal.withInitial(() -> new ThreadState(this, this.this_));
            private final AtomicInteger sectorOffset = new AtomicInteger(0);
            ...
            @Override
            public synchronized IntegerDoubleTuple advance()   {
                if (!this.hasNext())    {
                    throw new IteratorCompleteException();
                }
                ThreadState state = this.tl.get();
                IntegerDoubleTuple current = state.tuple = new IntegerDoubleMutableTuple(this.next.k, this.next.v);
                state.entry = this.entry;
                this.findNextValue();
                return current;
            }
            ...
            class ThreadState   {
                public IntegerDoubleTuple tuple;
                public IntegerDoubleSubEntry entry;
                private final IntegerDoubleConcurrentIterator this_;
                private final IntegerDoubleConcurrentTreeHashMap this__;

                public ThreadState(IntegerDoubleConcurrentIterator this_, IntegerDoubleConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
     ...
     */
    public IntegerDoubleConcurrentIterator fastEntryIterator() {
        return new IntegerDoubleConcurrentIterator()  {
            private final IntegerDoubleConcurrentTreeHashMap this_ = IntegerDoubleConcurrentTreeHashMap.this;
            //private final ReentrantLock lock = new ReentrantLock();
            private final ThreadLocal<ThreadState> tl = ThreadLocal.withInitial(() -> new ThreadState(this, this.this_));
            ...
            private final AtomicReference<IntegerDoubleSubEntry.Sector> currSector = new AtomicReference(null);
            private final AtomicBoolean hasNextSub = new AtomicBoolean(true);
            private volatile IntegerDoubleMutableTuple next = new IntegerDoubleMutableTuple(0, 0D);
            private volatile boolean hasNext = true;
            private volatile int subEntryIndex = 0;
            private volatile IntegerDoubleSubEntry entry;
            ...
            @Override
            public IntegerDoubleMutableTuple get()   {
                ThreadState state = this.tl.get();
                if (state.entry == null)    {
                    throw new IllegalStateException();
                } else {
            ...
            @Override
            public synchronized IntegerDoubleMutableTuple advance()   {
                if (!this.hasNext())    {
                    throw new IteratorCompleteException();
                }
                ThreadState state = this.tl.get();
                IntegerDoubleMutableTuple a = state.tuple;
            ...
            class ThreadState   {
                public IntegerDoubleMutableTuple tuple = new IntegerDoubleMutableTuple(0, 0D);
                public IntegerDoubleSubEntry entry;
                private final IntegerDoubleConcurrentIterator this_;
                private final IntegerDoubleConcurrentTreeHashMap this__;
    ...
    private IntegerDoubleSubEntry getSubentry(int index, boolean create)    {
        assert index == (index & 0xFF);

        synchronized (this.entries) {
            IntegerDoubleSubEntry entry = (IntegerDoubleSubEntry) this.entries[index];
            if (entry == null && create)    {
                this.entries[index] = entry = new IntegerDoubleSubEntry(this);
            }
            return entry;
        }
    }
