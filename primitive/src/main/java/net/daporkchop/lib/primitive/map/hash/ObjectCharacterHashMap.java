...
import net.daporkchop.lib.primitive.tuple.ObjectCharacterImmutableTuple;
...
/**
 * A hash map, using a key type of K and a value type of char.
 ...
 * @author DaPorkchop_
 */
public class ObjectCharacterHashMap<K extends Object> implements ObjectCharacterMap<K>    {
    ...
    public ObjectCharacterHashMap()    {
        this(16384);
    }

    public ObjectCharacterHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public ObjectCharacterHashMap(ObjectToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public ObjectCharacterHashMap(int baseSize, ObjectToIntegerFunction keyHash)    {
    ...
    protected boolean reHash(char[] values, Object[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
            K key = (K) this.keys[i];
            int j = getIndex(key);
            if (states.get(j))  {
    ...
    private class KeyIterator implements ObjectIterator<K> {
        ...
        public KeyIterator()    {
            this.size = ObjectCharacterHashMap.this.size;
            this.next = ObjectCharacterHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public K get()   {
            return (K) ObjectCharacterHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = ObjectCharacterHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            ObjectCharacterHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements CharacterIterator {
        ...
        @SuppressWarnings("unchecked")
        public char get()   {
            return ObjectCharacterHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements ObjectCharacterIterator<K> {
        private int size;
        private int index = -1;
        private int next;
        private ObjectCharacterTuple<K> tuple;
        ...
        @Override
        public ObjectCharacterTuple<K> advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new ObjectCharacterImmutableTuple<K>( (K) ObjectCharacterHashMap.this.keys[this.index],  ObjectCharacterHashMap.this.values[this.index]);
        }
