 ...
 * Although this map 𝙒𝙊𝙍𝙆𝙎, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.IntegerDoubleIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class IntegerDoubleTreeMap implements IntegerDoubleMap    {
    protected int[] keys;
    ...
    @SuppressWarnings("unchecked")
    protected double remove(int key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return 0D; //no mapping
    ...
    public IntegerIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.IntegerDoubleIndexedTreeMap} for an implementation of this method
        return null;
    }
