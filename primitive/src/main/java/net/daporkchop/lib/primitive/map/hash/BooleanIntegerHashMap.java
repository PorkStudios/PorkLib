...
import net.daporkchop.lib.primitive.tuple.BooleanIntegerImmutableTuple;
...
/**
 * A hash map, using a key type of boolean and a value type of int.
 ...
 * @author DaPorkchop_
 */
public class BooleanIntegerHashMap implements BooleanIntegerMap    {
    ...
    public BooleanIntegerHashMap()    {
        this(16384);
    }

    public BooleanIntegerHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public BooleanIntegerHashMap(BooleanToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public BooleanIntegerHashMap(int baseSize, BooleanToIntegerFunction keyHash)    {
    ...
    protected boolean reHash(int[] values, boolean[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
    ...
    private class KeyIterator implements BooleanIterator {
        ...
        public KeyIterator()    {
            this.size = BooleanIntegerHashMap.this.size;
            this.next = BooleanIntegerHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public boolean get()   {
            return BooleanIntegerHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = BooleanIntegerHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            BooleanIntegerHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements IntegerIterator {
        ...
        @SuppressWarnings("unchecked")
        public int get()   {
            return BooleanIntegerHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements BooleanIntegerIterator {
        private int size;
        private int index = -1;
        private int next;
        private BooleanIntegerTuple tuple;
        ...
        @Override
        public BooleanIntegerTuple advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new BooleanIntegerImmutableTuple( BooleanIntegerHashMap.this.keys[this.index],  BooleanIntegerHashMap.this.values[this.index]);
        }
