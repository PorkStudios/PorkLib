...
public class SerpentKeyGen {
    static {
        BouncyCastleInit.loadClass();
    }

    public static SerpentKey gen(byte[] data) {
        byte[] b = new byte[BlockCipherType.SERPENT.blockSize];
        int i = 0;
        do {
        ...
        data = b;
        return new SerpentKey(new SecretKeySpec(data, "aaa"), KeyRandom.getBytes(BlockCipherType.SERPENT.blockSize));
    }

    public static SerpentKey gen() {
        byte[] key = new byte[BlockCipherType.SERPENT.blockSize];
