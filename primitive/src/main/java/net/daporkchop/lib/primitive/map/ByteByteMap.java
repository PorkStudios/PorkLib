...
import net.daporkchop.lib.primitive.lambda.function.ByteToByteFunction;
...
/**
 * A map, using a key type of byte and a value type of byte.
 * <p>
...
public interface ByteByteMap   {
     ...
     */
    default void putAll(@NonNull byte[] keys, @NonNull byte[] values)    {
     ...
     */
    default void putAll(@NonNull ByteByteMap map)    {
     ...
     */
    default void removeAll(@NonNull Collection<Byte> keys) {
     ...
     */
    void forEachKey(ByteConsumer consumer);
     ...
     * @param consumer the function to run
     */
    void forEachEntry(ByteByteConsumer consumer);
     ...
     */
    ByteByteIterator entryIterator();
     ...
     */
    default byte getOrDefault(byte key, byte def)   {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
            this.put(key, def);
     ...
     */
    default byte computeIfAbsent(byte key, @NonNull ByteToByteFunction func) {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
