...
import net.daporkchop.lib.primitive.iterator.concurrent.LongCharacterConcurrentIterator;
import net.daporkchop.lib.primitive.map.LongCharacterMap;
import net.daporkchop.lib.primitive.tuple.LongCharacterTuple;
import net.daporkchop.lib.primitive.tuple.LongCharacterImmutableTuple;
import net.daporkchop.lib.primitive.tuple.LongCharacterMutableTuple;
...
/**
 * A tree hash map, using a key type of long and a value type of char.
 ...
 * @author DaPorkchop_
 */
public class LongCharacterConcurrentTreeHashMap implements LongCharacterMap    {
    final AtomicInteger size = new AtomicInteger(0);

    private final LongCharacterSubEntry[] entries = new LongCharacterSubEntry[256];
    private final LongToIntegerFunction keyHash;
    final LongToLongFunction subKeyHash;
    final int subHashLength;

    public LongCharacterConcurrentTreeHashMap()    {
        this(null, null);
    }

    public LongCharacterConcurrentTreeHashMap(LongToIntegerFunction keyHash, LongToLongFunction subKeyHash)    {
        this(keyHash, subKeyHash, 7);
    }

    public LongCharacterConcurrentTreeHashMap(LongToIntegerFunction keyHash, LongToLongFunction subKeyHash, int subHashLength)    {
    ...
    public char get(long key)   {
        LongCharacterSubEntry entry = this.getSubentry(this.keyHash.apply(key) & 0xFF, false);
        if (entry == null)  {
            return (char) 0;
        } else {
    ...
    public char put(long key, char value)   {
        LongCharacterSubEntry entry = this.getSubentry(this.keyHash.apply(key) & 0xFF, true);
    ...
    public char remove(long key)    {
        ...
        LongCharacterSubEntry entry = this.getSubentry(hash, false);
        if (entry == null)  {
            return (char) 0;
        } else {
    ...
    public void clear() {
        synchronized (this.entries) {
            for (int i = 0; i < 256; i++) {
                LongCharacterSubEntry entry = this.entries[i];
                if (entry != null)  {
                    entry.clear();
                    this.entries[i] = null;
                }
    ...
    public void forEachKey(LongConsumer consumer)   {
        for (int i = 0; i < 256; i++) {
            LongCharacterSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
    ...
    @Override
    public void forEachEntry(LongCharacterConsumer consumer) {
        for (int i = 0; i < 256; i++) {
            LongCharacterSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
    ...
    public void forEachEntry(LongCharacterConsumer consumer, VoidFunction complete) {
        for (int i = 0; i < 256; i++) {
            LongCharacterSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
                entry.forEachEntry(consumer);
    ...
    public LongConcurrentIterator keyIterator()   {
        return new LongConcurrentIterator()  {
            private final LongCharacterConcurrentTreeHashMap this_ = LongCharacterConcurrentTreeHashMap.this;
            ...
            private final AtomicReference<LongCharacterSubEntry.Sector> currSector = new AtomicReference(null);
            ...
            private volatile LongCharacterSubEntry entry;
            ...
            private void nextSub()   {
                ...
                while (this.subEntryIndex < 256)    {
                    LongCharacterSubEntry entry = getSubentry(this.subEntryIndex++, false);
                    if (entry != null)  {
                        this.entry = entry;
            ...
            class ThreadState   {
                public long key;
                public LongCharacterSubEntry entry;
                private final LongConcurrentIterator this_;
                private final LongCharacterConcurrentTreeHashMap this__;

                public ThreadState(LongConcurrentIterator this_, LongCharacterConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
    ...
    public CharacterConcurrentIterator valueIterator() {
        return new CharacterConcurrentIterator()  {
            ...
            private final AtomicReference<LongCharacterSubEntry.Sector> currSector = new AtomicReference(null);
            private final AtomicBoolean hasNextSub = new AtomicBoolean(true);
            private final LongCharacterMutableTuple next = new LongCharacterMutableTuple();
            private volatile boolean hasNext = true;
            private volatile int subEntryIndex = 0;
            private volatile LongCharacterSubEntry entry;
            ...
            class ThreadState   {
                ...
                public ThreadState(CharacterConcurrentIterator this_, LongCharacterConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
    ...
    @Override
    public LongCharacterConcurrentIterator entryIterator() {
        return new LongCharacterConcurrentIterator()  {
            private final LongCharacterConcurrentTreeHashMap this_ = LongCharacterConcurrentTreeHashMap.this;
            //private final ReentrantLock lock = new ReentrantLock();
            private final ThreadLocal<ThreadState> tl = ThreadLocal.withInitial(() -> new ThreadState(this, this.this_));
            private final AtomicInteger sectorOffset = new AtomicInteger(0);
            ...
            @Override
            public synchronized LongCharacterTuple advance()   {
                if (!this.hasNext())    {
                    throw new IteratorCompleteException();
                }
                ThreadState state = this.tl.get();
                LongCharacterTuple current = state.tuple = new LongCharacterMutableTuple(this.next.k, this.next.v);
                state.entry = this.entry;
                this.findNextValue();
                return current;
            }
            ...
            class ThreadState   {
                public LongCharacterTuple tuple;
                public LongCharacterSubEntry entry;
                private final LongCharacterConcurrentIterator this_;
                private final LongCharacterConcurrentTreeHashMap this__;

                public ThreadState(LongCharacterConcurrentIterator this_, LongCharacterConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
     ...
     */
    public LongCharacterConcurrentIterator fastEntryIterator() {
        return new LongCharacterConcurrentIterator()  {
            private final LongCharacterConcurrentTreeHashMap this_ = LongCharacterConcurrentTreeHashMap.this;
            //private final ReentrantLock lock = new ReentrantLock();
            private final ThreadLocal<ThreadState> tl = ThreadLocal.withInitial(() -> new ThreadState(this, this.this_));
            ...
            private final AtomicReference<LongCharacterSubEntry.Sector> currSector = new AtomicReference(null);
            private final AtomicBoolean hasNextSub = new AtomicBoolean(true);
            private volatile LongCharacterMutableTuple next = new LongCharacterMutableTuple(0L, (char) 0);
            private volatile boolean hasNext = true;
            private volatile int subEntryIndex = 0;
            private volatile LongCharacterSubEntry entry;
            ...
            @Override
            public LongCharacterMutableTuple get()   {
                ThreadState state = this.tl.get();
                if (state.entry == null)    {
                    throw new IllegalStateException();
                } else {
            ...
            @Override
            public synchronized LongCharacterMutableTuple advance()   {
                if (!this.hasNext())    {
                    throw new IteratorCompleteException();
                }
                ThreadState state = this.tl.get();
                LongCharacterMutableTuple a = state.tuple;
            ...
            class ThreadState   {
                public LongCharacterMutableTuple tuple = new LongCharacterMutableTuple(0L, (char) 0);
                public LongCharacterSubEntry entry;
                private final LongCharacterConcurrentIterator this_;
                private final LongCharacterConcurrentTreeHashMap this__;
    ...
    private LongCharacterSubEntry getSubentry(int index, boolean create)    {
        assert index == (index & 0xFF);

        synchronized (this.entries) {
            LongCharacterSubEntry entry = (LongCharacterSubEntry) this.entries[index];
            if (entry == null && create)    {
                this.entries[index] = entry = new LongCharacterSubEntry(this);
            }
            return entry;
        }
    }
