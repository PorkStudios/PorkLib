...
/**
 * Response to {@link HandshakeEncodingPacket}
 *
 * @author DaPorkchop_
 */
@Data
@NoArgsConstructor
@AllArgsConstructor
public class HandshakeStartEncryptionPacket implements EncapsulatedPacket, EncapsulatedConstants {
    ...
    public void write(DataOut out, PacketProtocol protocol) throws IOException {
        out.writeBoolean(this.ecKey != null);
        if (this.ecKey != null) {
    ...
    @Override
    public byte getId() {
        return ID_HANDSHAKESTARTENCRYPTION;
    }

    public static class HandshakeStartEncryptionHandler implements PacketHandler<HandshakeStartEncryptionPacket, EncapsulatedSession> {
        @Override
        public void handle(HandshakeStartEncryptionPacket packet, EncapsulatedSession session) {
            ...
            if (packet.ecKey == null) {
                if (((Server) session.getEndpoint()).getCipherType() != BlockCipherType.NONE) {
                    throw new IllegalStateException("No ECDH keypair sent!");
                }
            } else {
                Server server = (Server) session.getEndpoint();
                CompletableFuture<BlockCipherHelper> future = AbstractEndpoint.getCipherHelper(packet.ecKey.getPublicKey(), AbstractEndpoint.getECKeyPairNow(server.getCurveType()).getPrivateKey(), server.getCipherType(), server.getCipherMode(), server.getCipherPadding());
                SessionData.CIPHER_HELPER.set(session.getSession(), future);
            }

            session.getSession().write(new HandshakeCompletePacket()).addListener(f -> ConnectionState.advance(session));
        }
    }
}
