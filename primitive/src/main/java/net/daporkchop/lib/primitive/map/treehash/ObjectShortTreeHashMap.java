 ...
 * overhead, and can even be made concurrent with little extra work (see {@link net.daporkchop.lib.primitive.map.concurrent.ObjectShortConcurrentTreeHashMap})
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
public class ObjectShortTreeHashMap<K extends Object> implements ObjectShortMap<K>    {
    ...
    public ObjectShortTreeHashMap()    {
        this(null, 0);
    }

    public ObjectShortTreeHashMap(ObjectToLongFunction hashFunc, int hashLen)    {
        ...
        this.rootPointer = this.hashLen == 0 ? new ObjectShortTreeHashMap<K>.ValueSector(null, -1) : new ObjectShortTreeHashMap<K>.PointerSector(null, -1);
    }
    ...
    @Override
    public void clear() {
        this.rootPointer = this.hashLen == 0 ? new ObjectShortTreeHashMap.ValueSector(null, -1) : new ObjectShortTreeHashMap.PointerSector(null, -1);
    }
    ...
    public void forEachKey(@NonNull ObjectConsumer<K> consumer)  {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof ObjectShortTreeHashMap.PointerSector)    {
    ...
    private Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.hashLen; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.hashLen ? new ObjectShortTreeHashMap<K>.ValueSector(curr, off) : new ObjectShortTreeHashMap<K>.PointerSector(curr, off);
    ...
    private class ValueSector extends Sector   {
        ...
        public short setEntry(int index, Object key, short value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                ObjectShortTreeHashMap.this.size.incrementAndGet();
                return (short) 0;
            }
        }
        ...
        public short removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                ObjectShortTreeHashMap.this.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
