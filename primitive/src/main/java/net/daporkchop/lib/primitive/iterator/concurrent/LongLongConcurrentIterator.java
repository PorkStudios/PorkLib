...
import net.daporkchop.lib.primitive.lambda.function.bi.LongLongToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of long => long mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface LongLongConcurrentIterator extends LongLongIterator   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull LongLongToBooleanFunction function)    {
        try {
            while (this.hasNext())  {
                LongLongTuple tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<LongLongTuple> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<LongLongTuple> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull LongLongConsumer consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<LongLongTuple> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull LongLongToBooleanFunction function)    {
        this.concurrentIterate(function);
    }
}
