...
/**
 * A hash map, using a key type of float and a value type of boolean.
 ...
 * @author DaPorkchop_
 */
public class FloatBooleanHashMap implements FloatBooleanMap    {
    ...
    public FloatBooleanHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public FloatBooleanHashMap(FloatToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public FloatBooleanHashMap(int baseSize, FloatToIntegerFunction keyHash)    {
    ...
    protected boolean reHash(boolean[] values, float[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
    ...
    private class KeyIterator implements FloatIterator {
        ...
        public KeyIterator()    {
            this.size = FloatBooleanHashMap.this.size;
            this.next = FloatBooleanHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public float get()   {
            return FloatBooleanHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = FloatBooleanHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            FloatBooleanHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements BooleanIterator {
        ...
        @SuppressWarnings("unchecked")
        public boolean get()   {
            return FloatBooleanHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements FloatBooleanIterator {
        private int size;
        private int index = -1;
        private int next;
        private FloatBooleanTuple tuple;
        ...
        @Override
        public FloatBooleanTuple advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new FloatBooleanImmutableTuple( FloatBooleanHashMap.this.keys[this.index],  FloatBooleanHashMap.this.values[this.index]);
        }
