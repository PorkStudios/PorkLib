...
/**
 * A tree map, using a key type of boolean and a value type of byte.
 ...
 * Although this map WORKS, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.BooleanByteIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class BooleanByteTreeMap implements BooleanByteMap    {
    ...
    @SuppressWarnings("unchecked")
    protected byte remove(boolean key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return (byte) 0; //no mapping
    ...
    public BooleanIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.BooleanByteIndexedTreeMap} for an implementation of this method
        return null;
    }
