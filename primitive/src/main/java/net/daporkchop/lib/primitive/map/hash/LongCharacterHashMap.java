...
/**
 * A hash map, using a key type of long and a value type of char.
 ...
 * @author DaPorkchop_
 */
public class LongCharacterHashMap implements LongCharacterMap    {
    protected long[] keys;
    ...
    public LongCharacterHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public LongCharacterHashMap(LongToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public LongCharacterHashMap(int baseSize, LongToIntegerFunction keyHash)    {
    ...
    protected boolean reHash(char[] values, long[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
    ...
    private class KeyIterator implements LongIterator {
        ...
        public KeyIterator()    {
            this.size = LongCharacterHashMap.this.size;
            this.next = LongCharacterHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public long get()   {
            return LongCharacterHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = LongCharacterHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            LongCharacterHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements CharacterIterator {
        ...
        @SuppressWarnings("unchecked")
        public char get()   {
            return LongCharacterHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements LongCharacterIterator {
        private int size;
        private int index = -1;
        private int next;
        private LongCharacterTuple tuple;
        ...
        @Override
        public LongCharacterTuple advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new LongCharacterImmutableTuple( LongCharacterHashMap.this.keys[this.index],  LongCharacterHashMap.this.values[this.index]);
        }
