 ...
 * overhead, and can even be made concurrent with little extra work (see {@link net.daporkchop.lib.primitive.map.concurrent.ByteLongConcurrentTreeHashMap})
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
public class ByteLongTreeHashMap implements ByteLongMap    {
    ...
    public ByteLongTreeHashMap()    {
        this(null, 0);
    }

    public ByteLongTreeHashMap(ByteToLongFunction hashFunc, int hashLen)    {
        ...
        this.rootPointer = this.hashLen == 0 ? new ByteLongTreeHashMap.ValueSector(null, -1) : new ByteLongTreeHashMap.PointerSector(null, -1);
    }
    ...
    public void forEachKey(@NonNull ByteConsumer consumer)  {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof ByteLongTreeHashMap.PointerSector)    {
    ...
    private Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.hashLen; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.hashLen ? new ByteLongTreeHashMap.ValueSector(curr, off) : new ByteLongTreeHashMap.PointerSector(curr, off);
    ...
    private class ValueSector extends Sector   {
        ...
        public long setEntry(int index, byte key, long value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                ByteLongTreeHashMap.this.size.incrementAndGet();
                return 0L;
            }
        }
        ...
        public long removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                ByteLongTreeHashMap.this.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
