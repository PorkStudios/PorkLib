...
/**
 * A tree map, using a key type of double and a value type of boolean.
 ...
 * Although this map WORKS, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.DoubleBooleanIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class DoubleBooleanTreeMap implements DoubleBooleanMap    {
    ...
    @SuppressWarnings("unchecked")
    protected boolean remove(double key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return false; //no mapping
    ...
    public DoubleIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.DoubleBooleanIndexedTreeMap} for an implementation of this method
        return null;
    }
