...
import net.daporkchop.lib.primitive.lambda.function.bi.LongDoubleToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of long => double mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface LongDoubleConcurrentIterator extends LongDoubleIterator   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull LongDoubleToBooleanFunction function)    {
        try {
            while (this.hasNext())  {
                LongDoubleTuple tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<LongDoubleTuple> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<LongDoubleTuple> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull LongDoubleConsumer consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<LongDoubleTuple> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull LongDoubleToBooleanFunction function)    {
        this.concurrentIterate(function);
    }
}
