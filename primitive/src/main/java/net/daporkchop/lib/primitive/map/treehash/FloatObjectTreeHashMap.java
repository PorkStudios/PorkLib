 ...
 * overhead, and can even be made concurrent with little extra work (see {@link net.daporkchop.lib.primitive.map.concurrent.FloatObjectConcurrentTreeHashMap})
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
public class FloatObjectTreeHashMap<V extends Object> implements FloatObjectMap<V>    {
    ...
    public FloatObjectTreeHashMap()    {
        this(null, 0);
    }

    public FloatObjectTreeHashMap(FloatToLongFunction hashFunc, int hashLen)    {
        ...
        this.rootPointer = this.hashLen == 0 ? new FloatObjectTreeHashMap<V>.ValueSector(null, -1) : new FloatObjectTreeHashMap<V>.PointerSector(null, -1);
    }
    ...
    @Override
    public void clear() {
        this.rootPointer = this.hashLen == 0 ? new FloatObjectTreeHashMap.ValueSector(null, -1) : new FloatObjectTreeHashMap.PointerSector(null, -1);
    }
    ...
    public void forEachKey(@NonNull FloatConsumer consumer)  {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof FloatObjectTreeHashMap.PointerSector)    {
    ...
    private Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.hashLen; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.hashLen ? new FloatObjectTreeHashMap<V>.ValueSector(curr, off) : new FloatObjectTreeHashMap<V>.PointerSector(curr, off);
    ...
    private class ValueSector extends Sector   {
        ...
        public Object setEntry(int index, float key, Object value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                FloatObjectTreeHashMap.this.size.incrementAndGet();
                return null;
            }
        }
        ...
        public Object removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                FloatObjectTreeHashMap.this.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
