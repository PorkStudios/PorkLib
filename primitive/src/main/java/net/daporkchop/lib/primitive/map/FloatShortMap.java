...
import net.daporkchop.lib.primitive.lambda.function.FloatToShortFunction;
...
/**
 * A map, using a key type of float and a value type of short.
 * <p>
...
public interface FloatShortMap   {
     ...
     */
    default void putAll(@NonNull float[] keys, @NonNull short[] values)    {
     ...
     */
    default void putAll(@NonNull FloatShortMap map)    {
     ...
     * @param consumer the function to run
     */
    void forEachEntry(FloatShortConsumer consumer);
     ...
     */
    FloatShortIterator entryIterator();
     ...
     */
    default short getOrDefault(float key, short def)   {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
            this.put(key, def);
     ...
     */
    default short computeIfAbsent(float key, @NonNull FloatToShortFunction func) {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
