...
import net.daporkchop.lib.primitive.iterator.ByteIterator;
import net.daporkchop.lib.primitive.iterator.bi.ByteByteIterator;
...
/**
 * A hash map, using a key type of byte and a value type of byte.
 ...
 * @author DaPorkchop_
 */
public class ByteByteHashMap implements ByteByteMap    {
    protected byte[] keys;
    ...
    public ByteByteHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public ByteByteHashMap(ByteToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public ByteByteHashMap(int baseSize, ByteToIntegerFunction keyHash)    {
    ...
    @Override
    public void forEachEntry(@NonNull ByteByteConsumer consumer)   {
    ...
    public ByteByteIterator entryIterator() {
    ...
    protected boolean reHash(byte[] values, byte[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
    ...
    private class KeyIterator implements ByteIterator {
        ...
        public KeyIterator()    {
            this.size = ByteByteHashMap.this.size;
            this.next = ByteByteHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public byte get()   {
            return ByteByteHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = ByteByteHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            ByteByteHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements ByteIterator {
        ...
        @SuppressWarnings("unchecked")
        public byte get()   {
            return ByteByteHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements ByteByteIterator {
        private int size;
        private int index = -1;
        private int next;
        private ByteByteTuple tuple;
        ...
        @Override
        public ByteByteTuple advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new ByteByteImmutableTuple( ByteByteHashMap.this.keys[this.index],  ByteByteHashMap.this.values[this.index]);
        }
