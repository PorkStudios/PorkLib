...
import net.daporkchop.lib.primitive.lambda.function.bi.ShortLongToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of short => long mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface ShortLongConcurrentIterator extends ShortLongIterator   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ShortLongToBooleanFunction function)    {
        try {
            while (this.hasNext())  {
                ShortLongTuple tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<ShortLongTuple> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<ShortLongTuple> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ShortLongConsumer consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<ShortLongTuple> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull ShortLongToBooleanFunction function)    {
        this.concurrentIterate(function);
    }
}
