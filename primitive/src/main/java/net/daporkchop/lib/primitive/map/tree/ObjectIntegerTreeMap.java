 ...
 * @author DaPorkchop_
 */
public class ObjectIntegerTreeMap<K extends Object> implements ObjectIntegerMap<K>    {
    ...
    @SuppressWarnings("unchecked")
    protected int remove(K key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return 0; //no mapping
    ...
    public ObjectIterator<K> keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.ObjectIntegerIndexedTreeMap} for an implementation of this method
        return null;
    }
