 ...
 * overhead, and can even be made concurrent with little extra work (see {@link net.daporkchop.lib.primitive.map.concurrent.FloatCharacterConcurrentTreeHashMap})
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
public class FloatCharacterTreeHashMap implements FloatCharacterMap    {
    ...
    public FloatCharacterTreeHashMap()    {
        this(null, 0);
    }

    public FloatCharacterTreeHashMap(FloatToLongFunction hashFunc, int hashLen)    {
        ...
        this.rootPointer = this.hashLen == 0 ? new FloatCharacterTreeHashMap.ValueSector(null, -1) : new FloatCharacterTreeHashMap.PointerSector(null, -1);
    }
    ...
    public void forEachKey(@NonNull FloatConsumer consumer)  {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof FloatCharacterTreeHashMap.PointerSector)    {
    ...
    private Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.hashLen; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.hashLen ? new FloatCharacterTreeHashMap.ValueSector(curr, off) : new FloatCharacterTreeHashMap.PointerSector(curr, off);
    ...
    private class ValueSector extends Sector   {
        ...
        public char setEntry(int index, float key, char value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                FloatCharacterTreeHashMap.this.size.incrementAndGet();
                return (char) 0;
            }
        }
        ...
        public char removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                FloatCharacterTreeHashMap.this.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
