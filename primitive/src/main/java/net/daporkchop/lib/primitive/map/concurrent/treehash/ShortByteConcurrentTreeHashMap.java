...
import net.daporkchop.lib.primitive.lambda.consumer.bi.ShortByteConsumer;
 ...
 * @author DaPorkchop_
 */
public class ShortByteConcurrentTreeHashMap implements ShortByteMap    {
    final AtomicInteger size = new AtomicInteger(0);

    private final ShortByteSubEntry[] entries = new ShortByteSubEntry[256];
    private final ShortToIntegerFunction keyHash;
    final ShortToLongFunction subKeyHash;
    final int subHashLength;

    public ShortByteConcurrentTreeHashMap()    {
        this(null, null);
    }

    public ShortByteConcurrentTreeHashMap(ShortToIntegerFunction keyHash, ShortToLongFunction subKeyHash)    {
        this(keyHash, subKeyHash, 1);
    }

    public ShortByteConcurrentTreeHashMap(ShortToIntegerFunction keyHash, ShortToLongFunction subKeyHash, int subHashLength)    {
    ...
    public byte get(short key)   {
        ShortByteSubEntry entry = this.getSubentry(this.keyHash.apply(key) & 0xFF, false);
        if (entry == null)  {
            return (byte) 0;
        } else {
    ...
    public byte put(short key, byte value)   {
        ShortByteSubEntry entry = this.getSubentry(this.keyHash.apply(key) & 0xFF, true);
    ...
    public byte remove(short key)    {
        ...
        ShortByteSubEntry entry = this.getSubentry(hash, false);
        if (entry == null)  {
            return (byte) 0;
        } else {
    ...
    public void clear() {
        synchronized (this.entries) {
            for (int i = 0; i < 256; i++) {
                ShortByteSubEntry entry = this.entries[i];
                if (entry != null)  {
                    entry.clear();
                    this.entries[i] = null;
                }
    ...
    public void forEachKey(ShortConsumer consumer)   {
        for (int i = 0; i < 256; i++) {
            ShortByteSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
    ...
    @Override
    public void forEachEntry(ShortByteConsumer consumer) {
        for (int i = 0; i < 256; i++) {
            ShortByteSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
    ...
    public void forEachEntry(ShortByteConsumer consumer, VoidFunction complete) {
        for (int i = 0; i < 256; i++) {
            ShortByteSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
                entry.forEachEntry(consumer);
    ...
    public ShortConcurrentIterator keyIterator()   {
        return new ShortConcurrentIterator()  {
            private final ShortByteConcurrentTreeHashMap this_ = ShortByteConcurrentTreeHashMap.this;
            ...
            private final AtomicReference<ShortByteSubEntry.Sector> currSector = new AtomicReference(null);
            ...
            private volatile ShortByteSubEntry entry;
            ...
            private void nextSub()   {
                ...
                while (this.subEntryIndex < 256)    {
                    ShortByteSubEntry entry = getSubentry(this.subEntryIndex++, false);
                    if (entry != null)  {
                        this.entry = entry;
            ...
            class ThreadState   {
                public short key;
                public ShortByteSubEntry entry;
                private final ShortConcurrentIterator this_;
                private final ShortByteConcurrentTreeHashMap this__;

                public ThreadState(ShortConcurrentIterator this_, ShortByteConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
    ...
    public ByteConcurrentIterator valueIterator() {
        return new ByteConcurrentIterator()  {
            ...
            private final AtomicReference<ShortByteSubEntry.Sector> currSector = new AtomicReference(null);
            private final AtomicBoolean hasNextSub = new AtomicBoolean(true);
            private final ShortByteMutableTuple next = new ShortByteMutableTuple();
            private volatile boolean hasNext = true;
            private volatile int subEntryIndex = 0;
            private volatile ShortByteSubEntry entry;
            ...
            class ThreadState   {
                ...
                public ThreadState(ByteConcurrentIterator this_, ShortByteConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
    ...
    @Override
    public ShortByteConcurrentIterator entryIterator() {
        return new ShortByteConcurrentIterator()  {
            private final ShortByteConcurrentTreeHashMap this_ = ShortByteConcurrentTreeHashMap.this;
            //private final ReentrantLock lock = new ReentrantLock();
            private final ThreadLocal<ThreadState> tl = ThreadLocal.withInitial(() -> new ThreadState(this, this.this_));
            private final AtomicInteger sectorOffset = new AtomicInteger(0);
            ...
            @Override
            public synchronized ShortByteTuple advance()   {
                if (!this.hasNext())    {
                    throw new IteratorCompleteException();
                }
                ThreadState state = this.tl.get();
                ShortByteTuple current = state.tuple = new ShortByteMutableTuple(this.next.k, this.next.v);
                state.entry = this.entry;
                this.findNextValue();
                return current;
            }
            ...
            class ThreadState   {
                public ShortByteTuple tuple;
                public ShortByteSubEntry entry;
                private final ShortByteConcurrentIterator this_;
                private final ShortByteConcurrentTreeHashMap this__;

                public ThreadState(ShortByteConcurrentIterator this_, ShortByteConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
     ...
     */
    public ShortByteConcurrentIterator fastEntryIterator() {
        return new ShortByteConcurrentIterator()  {
            private final ShortByteConcurrentTreeHashMap this_ = ShortByteConcurrentTreeHashMap.this;
            //private final ReentrantLock lock = new ReentrantLock();
            private final ThreadLocal<ThreadState> tl = ThreadLocal.withInitial(() -> new ThreadState(this, this.this_));
            ...
            private final AtomicReference<ShortByteSubEntry.Sector> currSector = new AtomicReference(null);
            private final AtomicBoolean hasNextSub = new AtomicBoolean(true);
            private volatile ShortByteMutableTuple next = new ShortByteMutableTuple((short) 0, (byte) 0);
            private volatile boolean hasNext = true;
            private volatile int subEntryIndex = 0;
            private volatile ShortByteSubEntry entry;
            ...
            @Override
            public ShortByteMutableTuple get()   {
                ThreadState state = this.tl.get();
                if (state.entry == null)    {
                    throw new IllegalStateException();
                } else {
            ...
            @Override
            public synchronized ShortByteMutableTuple advance()   {
                if (!this.hasNext())    {
                    throw new IteratorCompleteException();
                }
                ThreadState state = this.tl.get();
                ShortByteMutableTuple a = state.tuple;
            ...
            class ThreadState   {
                public ShortByteMutableTuple tuple = new ShortByteMutableTuple((short) 0, (byte) 0);
                public ShortByteSubEntry entry;
                private final ShortByteConcurrentIterator this_;
                private final ShortByteConcurrentTreeHashMap this__;
    ...
    private ShortByteSubEntry getSubentry(int index, boolean create)    {
        assert index == (index & 0xFF);

        synchronized (this.entries) {
            ShortByteSubEntry entry = (ShortByteSubEntry) this.entries[index];
            if (entry == null && create)    {
                this.entries[index] = entry = new ShortByteSubEntry(this);
            }
            return entry;
        }
    }
