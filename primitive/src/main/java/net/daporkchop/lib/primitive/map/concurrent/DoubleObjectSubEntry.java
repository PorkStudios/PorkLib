...
/**
 * A single subentry for a {@link DoubleObjectConcurrentTreeHashMap}.
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
class DoubleObjectSubEntry<V extends Object>    {
    public final AtomicInteger size = new AtomicInteger(0);
    public final DoubleObjectConcurrentTreeHashMap parent;
    ...
    public DoubleObjectSubEntry(DoubleObjectConcurrentTreeHashMap parent)   {
        this.parent = parent;
        this.rootPointer = this.parent.subHashLength == 0 ? new DoubleObjectSubEntry<V>.ValueSector(null, -1) : new DoubleObjectSubEntry<V>.PointerSector(null, -1);
    }
    ...
    public void clear() {
        this.lock.lock();
        try {
            this.rootPointer = this.parent.subHashLength == 0 ? new DoubleObjectSubEntry.ValueSector(null, -1) : new DoubleObjectSubEntry.PointerSector(null, -1);
    ...
    public void forEachKey(DoubleConsumer consumer)   {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof DoubleObjectSubEntry.PointerSector)    {
    ...
    public double advanceKey(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext)   {
        ...
        for (int i = sector.states.nextSetBit(curr.get() + 1); i != -1; i = sector.states.nextSetBit(i + 1)) {
            curr.set(i);
            if (sector instanceof DoubleObjectSubEntry.ValueSector)  {
    ...
    public void advanceEntry(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext, DoubleObjectMutableTuple<V> tuple)   {
        Sector sector = sectorRef.get();
        if (sector == null) {
            sectorRef.set(sector = this.rootPointer);
        }
    ...
    public Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.parent.subHashLength; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.parent.subHashLength ? new DoubleObjectSubEntry<V>.ValueSector(curr, off) : new DoubleObjectSubEntry<V>.PointerSector(curr, off);
    ...
    public class ValueSector extends Sector   {
        ...
        public Object setEntry(int index, double key, Object value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                DoubleObjectSubEntry.this.size.incrementAndGet();
                DoubleObjectSubEntry.this.parent.size.incrementAndGet();
                return null;
            }
        }
        ...
        public Object removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                DoubleObjectSubEntry.this.size.decrementAndGet();
                DoubleObjectSubEntry.this.parent.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
