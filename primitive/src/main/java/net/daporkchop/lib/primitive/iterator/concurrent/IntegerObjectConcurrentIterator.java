...
import net.daporkchop.lib.primitive.lambda.function.bi.IntegerObjectToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of int => V mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface IntegerObjectConcurrentIterator<V extends Object> extends IntegerObjectIterator<V>   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull IntegerObjectToBooleanFunction<V> function)    {
        try {
            while (this.hasNext())  {
                IntegerObjectTuple<V> tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<IntegerObjectTuple<V>> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<IntegerObjectTuple<V>> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull IntegerObjectConsumer<V> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<IntegerObjectTuple<V>> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull IntegerObjectToBooleanFunction<V> function)    {
        this.concurrentIterate(function);
    }
}
