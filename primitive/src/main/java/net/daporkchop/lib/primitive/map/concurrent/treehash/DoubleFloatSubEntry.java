...
/**
 * A single subentry for a {@link DoubleFloatConcurrentTreeHashMap}.
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
class DoubleFloatSubEntry    {
    public final AtomicInteger size = new AtomicInteger(0);
    public final DoubleFloatConcurrentTreeHashMap parent;
    ...
    public DoubleFloatSubEntry(DoubleFloatConcurrentTreeHashMap parent)   {
        this.parent = parent;
        this.rootPointer = this.parent.subHashLength == 0 ? new DoubleFloatSubEntry.ValueSector(null, -1) : new DoubleFloatSubEntry.PointerSector(null, -1);
    }
    ...
    public void forEachKey(DoubleConsumer consumer)   {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof DoubleFloatSubEntry.PointerSector)    {
    ...
    public double advanceKey(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext)   {
        ...
        for (int i = sector.states.nextSetBit(curr.get() + 1); i != -1; i = sector.states.nextSetBit(i + 1)) {
            curr.set(i);
            if (sector instanceof DoubleFloatSubEntry.ValueSector)  {
    ...
    public void advanceEntry(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext, DoubleFloatMutableTuple tuple)   {
        Sector sector = sectorRef.get();
        if (sector == null) {
            sectorRef.set(sector = this.rootPointer);
        }
    ...
    public Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.parent.subHashLength; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.parent.subHashLength ? new DoubleFloatSubEntry.ValueSector(curr, off) : new DoubleFloatSubEntry.PointerSector(curr, off);
    ...
    public abstract class Sector  {
        ...
        public float setEntry(int index, double key, float value)   {
            throw new UnsupportedOperationException();
        }
    ...
    public class ValueSector extends Sector   {
        ...
        public float setEntry(int index, double key, float value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                DoubleFloatSubEntry.this.size.incrementAndGet();
                DoubleFloatSubEntry.this.parent.size.incrementAndGet();
                return 0F;
            }
        }
        ...
        public float removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                DoubleFloatSubEntry.this.size.decrementAndGet();
                DoubleFloatSubEntry.this.parent.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
