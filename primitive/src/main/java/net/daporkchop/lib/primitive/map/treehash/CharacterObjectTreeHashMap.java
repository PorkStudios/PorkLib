 ...
 * overhead, and can even be made concurrent with little extra work (see {@link net.daporkchop.lib.primitive.map.concurrent.CharacterObjectConcurrentTreeHashMap})
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
public class CharacterObjectTreeHashMap<V extends Object> implements CharacterObjectMap<V>    {
    ...
    public CharacterObjectTreeHashMap()    {
        this(null, 0);
    }

    public CharacterObjectTreeHashMap(CharacterToLongFunction hashFunc, int hashLen)    {
        ...
        this.rootPointer = this.hashLen == 0 ? new CharacterObjectTreeHashMap<V>.ValueSector(null, -1) : new CharacterObjectTreeHashMap<V>.PointerSector(null, -1);
    }
    ...
    @Override
    public void clear() {
        this.rootPointer = this.hashLen == 0 ? new CharacterObjectTreeHashMap.ValueSector(null, -1) : new CharacterObjectTreeHashMap.PointerSector(null, -1);
    }
    ...
    public void forEachKey(@NonNull CharacterConsumer consumer)  {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof CharacterObjectTreeHashMap.PointerSector)    {
    ...
    private Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.hashLen; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.hashLen ? new CharacterObjectTreeHashMap<V>.ValueSector(curr, off) : new CharacterObjectTreeHashMap<V>.PointerSector(curr, off);
    ...
    private class ValueSector extends Sector   {
        ...
        public Object setEntry(int index, char key, Object value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                CharacterObjectTreeHashMap.this.size.incrementAndGet();
                return null;
            }
        }
        ...
        public Object removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                CharacterObjectTreeHashMap.this.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
