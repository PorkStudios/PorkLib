 ...
 * overhead, and can even be made concurrent with little extra work (see {@link net.daporkchop.lib.primitive.map.concurrent.LongCharacterConcurrentTreeHashMap})
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
public class LongCharacterTreeHashMap implements LongCharacterMap    {
    ...
    public LongCharacterTreeHashMap()    {
        this(null, 0);
    }

    public LongCharacterTreeHashMap(LongToLongFunction hashFunc, int hashLen)    {
        ...
        this.rootPointer = this.hashLen == 0 ? new LongCharacterTreeHashMap.ValueSector(null, -1) : new LongCharacterTreeHashMap.PointerSector(null, -1);
    }
    ...
    public void forEachKey(@NonNull LongConsumer consumer)  {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof LongCharacterTreeHashMap.PointerSector)    {
    ...
    private Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.hashLen; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.hashLen ? new LongCharacterTreeHashMap.ValueSector(curr, off) : new LongCharacterTreeHashMap.PointerSector(curr, off);
    ...
    private class ValueSector extends Sector   {
        ...
        public char setEntry(int index, long key, char value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                LongCharacterTreeHashMap.this.size.incrementAndGet();
                return (char) 0;
            }
        }
        ...
        public char removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                LongCharacterTreeHashMap.this.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
