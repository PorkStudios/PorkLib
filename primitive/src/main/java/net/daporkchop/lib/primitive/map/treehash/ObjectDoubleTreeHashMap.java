 ...
 * overhead, and can even be made concurrent with little extra work (see {@link net.daporkchop.lib.primitive.map.concurrent.ObjectDoubleConcurrentTreeHashMap})
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
public class ObjectDoubleTreeHashMap<K extends Object> implements ObjectDoubleMap<K>    {
    ...
    public ObjectDoubleTreeHashMap()    {
        this(null, 0);
    }

    public ObjectDoubleTreeHashMap(ObjectToLongFunction hashFunc, int hashLen)    {
        ...
        this.rootPointer = this.hashLen == 0 ? new ObjectDoubleTreeHashMap<K>.ValueSector(null, -1) : new ObjectDoubleTreeHashMap<K>.PointerSector(null, -1);
    }
    ...
    @Override
    public void clear() {
        this.rootPointer = this.hashLen == 0 ? new ObjectDoubleTreeHashMap.ValueSector(null, -1) : new ObjectDoubleTreeHashMap.PointerSector(null, -1);
    }
    ...
    public void forEachKey(@NonNull ObjectConsumer<K> consumer)  {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof ObjectDoubleTreeHashMap.PointerSector)    {
    ...
    private Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.hashLen; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.hashLen ? new ObjectDoubleTreeHashMap<K>.ValueSector(curr, off) : new ObjectDoubleTreeHashMap<K>.PointerSector(curr, off);
    ...
    private class ValueSector extends Sector   {
        ...
        public double setEntry(int index, Object key, double value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                ObjectDoubleTreeHashMap.this.size.incrementAndGet();
                return 0D;
            }
        }
        ...
        public double removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                ObjectDoubleTreeHashMap.this.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
