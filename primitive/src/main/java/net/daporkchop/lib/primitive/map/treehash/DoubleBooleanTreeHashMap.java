 ...
 * overhead, and can even be made concurrent with little extra work (see {@link net.daporkchop.lib.primitive.map.concurrent.DoubleBooleanConcurrentTreeHashMap})
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
public class DoubleBooleanTreeHashMap implements DoubleBooleanMap    {
    ...
    public DoubleBooleanTreeHashMap()    {
        this(null, 0);
    }

    public DoubleBooleanTreeHashMap(DoubleToLongFunction hashFunc, int hashLen)    {
        ...
        this.rootPointer = this.hashLen == 0 ? new DoubleBooleanTreeHashMap.ValueSector(null, -1) : new DoubleBooleanTreeHashMap.PointerSector(null, -1);
    }
    ...
    public void forEachKey(@NonNull DoubleConsumer consumer)  {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof DoubleBooleanTreeHashMap.PointerSector)    {
    ...
    private Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.hashLen; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.hashLen ? new DoubleBooleanTreeHashMap.ValueSector(curr, off) : new DoubleBooleanTreeHashMap.PointerSector(curr, off);
    ...
    private class ValueSector extends Sector   {
        ...
        public boolean setEntry(int index, double key, boolean value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                DoubleBooleanTreeHashMap.this.size.incrementAndGet();
                return false;
            }
        }
        ...
        public boolean removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                DoubleBooleanTreeHashMap.this.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
