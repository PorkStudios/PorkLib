...
/**
 * A tree map, using a key type of boolean and a value type of float.
 ...
 * Although this map 𝙒𝙊𝙍𝙆𝙎, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.BooleanFloatIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class BooleanFloatTreeMap implements BooleanFloatMap    {
    ...
    @SuppressWarnings("unchecked")
    protected float remove(boolean key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return 0F; //no mapping
    ...
    public BooleanIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.BooleanFloatIndexedTreeMap} for an implementation of this method
        return null;
    }
