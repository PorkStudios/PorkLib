...
/**
 * A single subentry for a {@link IntegerObjectConcurrentTreeHashMap}.
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
class IntegerObjectSubEntry<V extends Object>    {
    public final AtomicInteger size = new AtomicInteger(0);
    public final IntegerObjectConcurrentTreeHashMap parent;
    ...
    public IntegerObjectSubEntry(IntegerObjectConcurrentTreeHashMap parent)   {
        this.parent = parent;
        this.rootPointer = this.parent.subHashLength == 0 ? new IntegerObjectSubEntry<V>.ValueSector(null, -1) : new IntegerObjectSubEntry<V>.PointerSector(null, -1);
    }
    ...
    public void removeNonBlocking_BeReallyCarefulWithThis(int key)   {
        long hash = this.hashKey(key);
        Sector sector = this.getSectorFor(hash, false);
        if (sector != null) {
    ...
    public void clear() {
        this.lock.lock();
        try {
            this.rootPointer = this.parent.subHashLength == 0 ? new IntegerObjectSubEntry.ValueSector(null, -1) : new IntegerObjectSubEntry.PointerSector(null, -1);
    ...
    public void forEachKey(IntegerConsumer consumer)   {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof IntegerObjectSubEntry.PointerSector)    {
    ...
    public int advanceKey(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext)   {
        ...
        for (int i = sector.states.nextSetBit(curr.get() + 1); i != -1; i = sector.states.nextSetBit(i + 1)) {
            curr.set(i);
            if (sector instanceof IntegerObjectSubEntry.ValueSector)  {
    ...
    public void advanceEntry(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext, IntegerObjectMutableTuple<V> tuple)   {
        Sector sector = sectorRef.get();
        if (sector == null) {
            sectorRef.set(sector = this.rootPointer);
        }
    ...
    public Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.parent.subHashLength; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.parent.subHashLength ? new IntegerObjectSubEntry<V>.ValueSector(curr, off) : new IntegerObjectSubEntry<V>.PointerSector(curr, off);
    ...
    public abstract class Sector  {
        ...
        public Object setEntry(int index, int key, Object value)   {
            throw new UnsupportedOperationException();
        }
        ...
        public void forEachEntry(IntegerObjectConsumer consumer)   {
            throw new UnsupportedOperationException();
        }
    ...
    public class ValueSector extends Sector   {
        ...
        public Object setEntry(int index, int key, Object value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                IntegerObjectSubEntry.this.size.incrementAndGet();
                IntegerObjectSubEntry.this.parent.size.incrementAndGet();
                return null;
            }
        }
        ...
        public Object removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                IntegerObjectSubEntry.this.size.decrementAndGet();
                IntegerObjectSubEntry.this.parent.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
