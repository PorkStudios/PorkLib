...
import net.daporkchop.lib.primitive.iterator.concurrent.BooleanLongConcurrentIterator;
...
import net.daporkchop.lib.primitive.tuple.BooleanLongMutableTuple;
...
/**
 * A tree hash map, using a key type of boolean and a value type of long.
 ...
 * @author DaPorkchop_
 */
public class BooleanLongConcurrentTreeHashMap implements BooleanLongMap    {
    final AtomicInteger size = new AtomicInteger(0);

    private final BooleanLongSubEntry[] entries = new BooleanLongSubEntry[256];
    private final BooleanToIntegerFunction keyHash;
    final BooleanToLongFunction subKeyHash;
    final int subHashLength;

    public BooleanLongConcurrentTreeHashMap()    {
        this(null, null);
    }

    public BooleanLongConcurrentTreeHashMap(BooleanToIntegerFunction keyHash, BooleanToLongFunction subKeyHash)    {
        this(keyHash, subKeyHash, 0);
    }

    public BooleanLongConcurrentTreeHashMap(BooleanToIntegerFunction keyHash, BooleanToLongFunction subKeyHash, int subHashLength)    {
    ...
    public long get(boolean key)   {
        BooleanLongSubEntry entry = this.getSubentry(this.keyHash.apply(key) & 0xFF, false);
        if (entry == null)  {
            return 0L;
        } else {
    ...
    public long put(boolean key, long value)   {
        BooleanLongSubEntry entry = this.getSubentry(this.keyHash.apply(key) & 0xFF, true);
    ...
    public long remove(boolean key)    {
        ...
        BooleanLongSubEntry entry = this.getSubentry(hash, false);
        if (entry == null)  {
            return 0L;
        } else {
    ...
    public void clear() {
        synchronized (this.entries) {
            for (int i = 0; i < 256; i++) {
                BooleanLongSubEntry entry = this.entries[i];
                if (entry != null)  {
                    entry.clear();
                    this.entries[i] = null;
                }
    ...
    public void forEachKey(BooleanConsumer consumer)   {
        for (int i = 0; i < 256; i++) {
            BooleanLongSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
    ...
    @Override
    public void forEachEntry(BooleanLongConsumer consumer) {
        for (int i = 0; i < 256; i++) {
            BooleanLongSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
    ...
    public void forEachEntry(BooleanLongConsumer consumer, VoidFunction complete) {
        for (int i = 0; i < 256; i++) {
            BooleanLongSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
                entry.forEachEntry(consumer);
    ...
    public BooleanConcurrentIterator keyIterator()   {
        return new BooleanConcurrentIterator()  {
            private final BooleanLongConcurrentTreeHashMap this_ = BooleanLongConcurrentTreeHashMap.this;
            ...
            private final AtomicReference<BooleanLongSubEntry.Sector> currSector = new AtomicReference(null);
            ...
            private volatile BooleanLongSubEntry entry;
            ...
            private void nextSub()   {
                ...
                while (this.subEntryIndex < 256)    {
                    BooleanLongSubEntry entry = getSubentry(this.subEntryIndex++, false);
                    if (entry != null)  {
                        this.entry = entry;
            ...
            class ThreadState   {
                public boolean key;
                public BooleanLongSubEntry entry;
                private final BooleanConcurrentIterator this_;
                private final BooleanLongConcurrentTreeHashMap this__;

                public ThreadState(BooleanConcurrentIterator this_, BooleanLongConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
    ...
    public LongConcurrentIterator valueIterator() {
        return new LongConcurrentIterator()  {
            ...
            private final AtomicReference<BooleanLongSubEntry.Sector> currSector = new AtomicReference(null);
            private final AtomicBoolean hasNextSub = new AtomicBoolean(true);
            private final BooleanLongMutableTuple next = new BooleanLongMutableTuple();
            private volatile boolean hasNext = true;
            private volatile int subEntryIndex = 0;
            private volatile BooleanLongSubEntry entry;
            ...
            class ThreadState   {
                ...
                public ThreadState(LongConcurrentIterator this_, BooleanLongConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
    ...
    @Override
    public BooleanLongConcurrentIterator entryIterator() {
        return new BooleanLongConcurrentIterator()  {
            private final BooleanLongConcurrentTreeHashMap this_ = BooleanLongConcurrentTreeHashMap.this;
            //private final ReentrantLock lock = new ReentrantLock();
            private final ThreadLocal<ThreadState> tl = ThreadLocal.withInitial(() -> new ThreadState(this, this.this_));
            private final AtomicInteger sectorOffset = new AtomicInteger(0);
            ...
            @Override
            public synchronized BooleanLongTuple advance()   {
                if (!this.hasNext())    {
                    throw new IteratorCompleteException();
                }
                ThreadState state = this.tl.get();
                BooleanLongTuple current = state.tuple = new BooleanLongMutableTuple(this.next.k, this.next.v);
                state.entry = this.entry;
                this.findNextValue();
                return current;
            }
            ...
            class ThreadState   {
                public BooleanLongTuple tuple;
                public BooleanLongSubEntry entry;
                private final BooleanLongConcurrentIterator this_;
                private final BooleanLongConcurrentTreeHashMap this__;

                public ThreadState(BooleanLongConcurrentIterator this_, BooleanLongConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
     ...
     */
    public BooleanLongConcurrentIterator fastEntryIterator() {
        return new BooleanLongConcurrentIterator()  {
            private final BooleanLongConcurrentTreeHashMap this_ = BooleanLongConcurrentTreeHashMap.this;
            //private final ReentrantLock lock = new ReentrantLock();
            private final ThreadLocal<ThreadState> tl = ThreadLocal.withInitial(() -> new ThreadState(this, this.this_));
            ...
            private final AtomicReference<BooleanLongSubEntry.Sector> currSector = new AtomicReference(null);
            private final AtomicBoolean hasNextSub = new AtomicBoolean(true);
            private volatile BooleanLongMutableTuple next = new BooleanLongMutableTuple(false, 0L);
            private volatile boolean hasNext = true;
            private volatile int subEntryIndex = 0;
            private volatile BooleanLongSubEntry entry;
            ...
            @Override
            public BooleanLongMutableTuple get()   {
                ThreadState state = this.tl.get();
                if (state.entry == null)    {
                    throw new IllegalStateException();
                } else {
            ...
            @Override
            public synchronized BooleanLongMutableTuple advance()   {
                if (!this.hasNext())    {
                    throw new IteratorCompleteException();
                }
                ThreadState state = this.tl.get();
                BooleanLongMutableTuple a = state.tuple;
            ...
            class ThreadState   {
                public BooleanLongMutableTuple tuple = new BooleanLongMutableTuple(false, 0L);
                public BooleanLongSubEntry entry;
                private final BooleanLongConcurrentIterator this_;
                private final BooleanLongConcurrentTreeHashMap this__;
    ...
    private BooleanLongSubEntry getSubentry(int index, boolean create)    {
        assert index == (index & 0xFF);

        synchronized (this.entries) {
            BooleanLongSubEntry entry = (BooleanLongSubEntry) this.entries[index];
            if (entry == null && create)    {
                this.entries[index] = entry = new BooleanLongSubEntry(this);
            }
            return entry;
        }
    }
