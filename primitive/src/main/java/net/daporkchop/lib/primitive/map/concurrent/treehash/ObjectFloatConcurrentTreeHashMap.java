...
import net.daporkchop.lib.primitive.lambda.consumer.bi.ObjectFloatConsumer;
...
import net.daporkchop.lib.primitive.iterator.concurrent.ObjectFloatConcurrentIterator;
import net.daporkchop.lib.primitive.map.ObjectFloatMap;
import net.daporkchop.lib.primitive.tuple.ObjectFloatTuple;
import net.daporkchop.lib.primitive.tuple.ObjectFloatImmutableTuple;
import net.daporkchop.lib.primitive.tuple.ObjectFloatMutableTuple;
...
/**
 * A tree hash map, using a key type of K and a value type of float.
 ...
 * @author DaPorkchop_
 */
public class ObjectFloatConcurrentTreeHashMap<K extends Object> implements ObjectFloatMap<K>    {
    final AtomicInteger size = new AtomicInteger(0);

    private final ObjectFloatSubEntry[] entries = new ObjectFloatSubEntry[256];
    private final ObjectToIntegerFunction keyHash;
    final ObjectToLongFunction subKeyHash;
    final int subHashLength;

    public ObjectFloatConcurrentTreeHashMap()    {
        this(null, null);
    }

    public ObjectFloatConcurrentTreeHashMap(ObjectToIntegerFunction keyHash, ObjectToLongFunction subKeyHash)    {
        this(keyHash, subKeyHash, 3);
    }

    public ObjectFloatConcurrentTreeHashMap(ObjectToIntegerFunction keyHash, ObjectToLongFunction subKeyHash, int subHashLength)    {
    ...
    public float get(K key)   {
        ObjectFloatSubEntry<K> entry = this.getSubentry(this.keyHash.apply(key) & 0xFF, false);
        if (entry == null)  {
            return 0F;
        } else {
    ...
    public float put(K key, float value)   {
        ObjectFloatSubEntry<K> entry = this.getSubentry(this.keyHash.apply(key) & 0xFF, true);
    ...
    public float remove(K key)    {
        ...
        ObjectFloatSubEntry<K> entry = this.getSubentry(hash, false);
        if (entry == null)  {
            return 0F;
        } else {
    ...
    public void clear() {
        synchronized (this.entries) {
            for (int i = 0; i < 256; i++) {
                ObjectFloatSubEntry<K> entry = this.entries[i];
                if (entry != null)  {
                    entry.clear();
                    this.entries[i] = null;
                }
    ...
    public void forEachKey(ObjectConsumer<K> consumer)   {
        for (int i = 0; i < 256; i++) {
            ObjectFloatSubEntry<K> entry = this.getSubentry(i, false);
            if (entry != null)  {
    ...
    @Override
    public void forEachEntry(ObjectFloatConsumer<K> consumer) {
        for (int i = 0; i < 256; i++) {
            ObjectFloatSubEntry<K> entry = this.getSubentry(i, false);
            if (entry != null)  {
    ...
    public void forEachEntry(ObjectFloatConsumer<K> consumer, VoidFunction complete) {
        for (int i = 0; i < 256; i++) {
            ObjectFloatSubEntry<K> entry = this.getSubentry(i, false);
            if (entry != null)  {
                entry.forEachEntry(consumer);
    ...
    public ObjectConcurrentIterator<K> keyIterator()   {
        return new ObjectConcurrentIterator<K>()  {
            private final ObjectFloatConcurrentTreeHashMap this_ = ObjectFloatConcurrentTreeHashMap.this;
            ...
            private final AtomicReference<ObjectFloatSubEntry<K>.Sector> currSector = new AtomicReference(null);
            ...
            private volatile ObjectFloatSubEntry<K> entry;
            ...
            private void nextSub()   {
                ...
                while (this.subEntryIndex < 256)    {
                    ObjectFloatSubEntry<K> entry = getSubentry(this.subEntryIndex++, false);
                    if (entry != null)  {
                        this.entry = entry;
            ...
            class ThreadState   {
                public K key;
                public ObjectFloatSubEntry entry;
                private final ObjectConcurrentIterator<K> this_;
                private final ObjectFloatConcurrentTreeHashMap this__;

                public ThreadState(ObjectConcurrentIterator<K> this_, ObjectFloatConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
    ...
    public FloatConcurrentIterator valueIterator() {
        return new FloatConcurrentIterator()  {
            ...
            private final AtomicReference<ObjectFloatSubEntry<K>.Sector> currSector = new AtomicReference(null);
            private final AtomicBoolean hasNextSub = new AtomicBoolean(true);
            private final ObjectFloatMutableTuple<K> next = new ObjectFloatMutableTuple<K>();
            private volatile boolean hasNext = true;
            private volatile int subEntryIndex = 0;
            private volatile ObjectFloatSubEntry<K> entry;
            ...
            class ThreadState   {
                ...
                public ThreadState(FloatConcurrentIterator this_, ObjectFloatConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
    ...
    @Override
    public ObjectFloatConcurrentIterator<K> entryIterator() {
        return new ObjectFloatConcurrentIterator<K>()  {
            private final ObjectFloatConcurrentTreeHashMap this_ = ObjectFloatConcurrentTreeHashMap.this;
            //private final ReentrantLock lock = new ReentrantLock();
            private final ThreadLocal<ThreadState> tl = ThreadLocal.withInitial(() -> new ThreadState(this, this.this_));
            private final AtomicInteger sectorOffset = new AtomicInteger(0);
            ...
            @Override
            public synchronized ObjectFloatTuple<K> advance()   {
                if (!this.hasNext())    {
                    throw new IteratorCompleteException();
                }
                ThreadState state = this.tl.get();
                ObjectFloatTuple<K> current = state.tuple = new ObjectFloatMutableTuple<K>(this.next.k, this.next.v);
                state.entry = this.entry;
                this.findNextValue();
                return current;
            }
            ...
            class ThreadState   {
                public ObjectFloatTuple<K> tuple;
                public ObjectFloatSubEntry entry;
                private final ObjectFloatConcurrentIterator<K> this_;
                private final ObjectFloatConcurrentTreeHashMap this__;

                public ThreadState(ObjectFloatConcurrentIterator<K> this_, ObjectFloatConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
     ...
     */
    public ObjectFloatConcurrentIterator<K> fastEntryIterator() {
        return new ObjectFloatConcurrentIterator<K>()  {
            private final ObjectFloatConcurrentTreeHashMap this_ = ObjectFloatConcurrentTreeHashMap.this;
            //private final ReentrantLock lock = new ReentrantLock();
            private final ThreadLocal<ThreadState> tl = ThreadLocal.withInitial(() -> new ThreadState(this, this.this_));
            ...
            private final AtomicReference<ObjectFloatSubEntry<K>.Sector> currSector = new AtomicReference(null);
            private final AtomicBoolean hasNextSub = new AtomicBoolean(true);
            private volatile ObjectFloatMutableTuple<K> next = new ObjectFloatMutableTuple<K>(null, 0F);
            private volatile boolean hasNext = true;
            private volatile int subEntryIndex = 0;
            private volatile ObjectFloatSubEntry<K> entry;
            ...
            @Override
            public ObjectFloatMutableTuple<K> get()   {
                ThreadState state = this.tl.get();
                if (state.entry == null)    {
                    throw new IllegalStateException();
                } else {
            ...
            @Override
            public synchronized ObjectFloatMutableTuple<K> advance()   {
                if (!this.hasNext())    {
                    throw new IteratorCompleteException();
                }
                ThreadState state = this.tl.get();
                ObjectFloatMutableTuple<K> a = state.tuple;
            ...
            class ThreadState   {
                public ObjectFloatMutableTuple<K> tuple = new ObjectFloatMutableTuple<K>(null, 0F);
                public ObjectFloatSubEntry entry;
                private final ObjectFloatConcurrentIterator<K> this_;
                private final ObjectFloatConcurrentTreeHashMap this__;
    ...
    private ObjectFloatSubEntry<K> getSubentry(int index, boolean create)    {
        assert index == (index & 0xFF);

        synchronized (this.entries) {
            ObjectFloatSubEntry<K> entry = (ObjectFloatSubEntry<K>) this.entries[index];
            if (entry == null && create)    {
                this.entries[index] = entry = new ObjectFloatSubEntry<K>(this);
            }
            return entry;
        }
    }
