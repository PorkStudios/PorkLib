 ...
 * Although this map WORKS, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.ObjectCharacterIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class ObjectCharacterTreeMap<K extends Object> implements ObjectCharacterMap<K>    {
    ...
    @SuppressWarnings("unchecked")
    protected char remove(K key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return (char) 0; //no mapping
    ...
    public ObjectIterator<K> keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.ObjectCharacterIndexedTreeMap} for an implementation of this method
        return null;
    }
