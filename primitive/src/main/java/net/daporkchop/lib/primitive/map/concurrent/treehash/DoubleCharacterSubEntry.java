...
/**
 * A single subentry for a {@link DoubleCharacterConcurrentTreeHashMap}.
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
class DoubleCharacterSubEntry    {
    public final AtomicInteger size = new AtomicInteger(0);
    public final DoubleCharacterConcurrentTreeHashMap parent;
    ...
    public DoubleCharacterSubEntry(DoubleCharacterConcurrentTreeHashMap parent)   {
        this.parent = parent;
        this.rootPointer = this.parent.subHashLength == 0 ? new DoubleCharacterSubEntry.ValueSector(null, -1) : new DoubleCharacterSubEntry.PointerSector(null, -1);
    }
    ...
    public void forEachKey(DoubleConsumer consumer)   {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof DoubleCharacterSubEntry.PointerSector)    {
    ...
    public double advanceKey(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext)   {
        ...
        for (int i = sector.states.nextSetBit(curr.get() + 1); i != -1; i = sector.states.nextSetBit(i + 1)) {
            curr.set(i);
            if (sector instanceof DoubleCharacterSubEntry.ValueSector)  {
    ...
    public void advanceEntry(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext, DoubleCharacterMutableTuple tuple)   {
        Sector sector = sectorRef.get();
        if (sector == null) {
            sectorRef.set(sector = this.rootPointer);
        }
    ...
    public Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.parent.subHashLength; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.parent.subHashLength ? new DoubleCharacterSubEntry.ValueSector(curr, off) : new DoubleCharacterSubEntry.PointerSector(curr, off);
    ...
    public abstract class Sector  {
        ...
        public char setEntry(int index, double key, char value)   {
            throw new UnsupportedOperationException();
        }
    ...
    public class ValueSector extends Sector   {
        ...
        public char setEntry(int index, double key, char value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                DoubleCharacterSubEntry.this.size.incrementAndGet();
                DoubleCharacterSubEntry.this.parent.size.incrementAndGet();
                return (char) 0;
            }
        }
        ...
        public char removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                DoubleCharacterSubEntry.this.size.decrementAndGet();
                DoubleCharacterSubEntry.this.parent.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
