...
public class CharacterIntegerHashMap implements CharacterIntegerMap    {
    ...
    private class KeyIterator implements CharacterIterator {
        ...
        public KeyIterator()    {
            this.size = CharacterIntegerHashMap.this.size;
            this.next = CharacterIntegerHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public char get()   {
            return CharacterIntegerHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = CharacterIntegerHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            CharacterIntegerHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements IntegerIterator {
        ...
        @SuppressWarnings("unchecked")
        public int get()   {
            return CharacterIntegerHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements CharacterIntegerIterator {
        private int size;
        private int index = -1;
        private int next;
        private CharacterIntegerTuple tuple;
        ...
        @Override
        public CharacterIntegerTuple advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new CharacterIntegerImmutableTuple( CharacterIntegerHashMap.this.keys[this.index],  CharacterIntegerHashMap.this.values[this.index]);
        }
