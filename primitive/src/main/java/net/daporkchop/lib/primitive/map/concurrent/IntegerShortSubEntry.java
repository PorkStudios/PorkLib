...
/**
 * A single subentry for a {@link IntegerShortConcurrentTreeHashMap}.
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
class IntegerShortSubEntry    {
    public final AtomicInteger size = new AtomicInteger(0);
    public final IntegerShortConcurrentTreeHashMap parent;
    ...
    public IntegerShortSubEntry(IntegerShortConcurrentTreeHashMap parent)   {
        this.parent = parent;
        this.rootPointer = this.parent.subHashLength == 0 ? new IntegerShortSubEntry.ValueSector(null, -1) : new IntegerShortSubEntry.PointerSector(null, -1);
    }
    ...
    public void forEachKey(IntegerConsumer consumer)   {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof IntegerShortSubEntry.PointerSector)    {
    ...
    public int advanceKey(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext)   {
        ...
        for (int i = sector.states.nextSetBit(curr.get() + 1); i != -1; i = sector.states.nextSetBit(i + 1)) {
            curr.set(i);
            if (sector instanceof IntegerShortSubEntry.ValueSector)  {
    ...
    public void advanceEntry(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext, IntegerShortMutableTuple tuple)   {
        Sector sector = sectorRef.get();
        if (sector == null) {
            sectorRef.set(sector = this.rootPointer);
        }
    ...
    public Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.parent.subHashLength; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.parent.subHashLength ? new IntegerShortSubEntry.ValueSector(curr, off) : new IntegerShortSubEntry.PointerSector(curr, off);
    ...
    public abstract class Sector  {
        ...
        public short setEntry(int index, int key, short value)   {
            throw new UnsupportedOperationException();
        }
    ...
    public class ValueSector extends Sector   {
        ...
        public short setEntry(int index, int key, short value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                IntegerShortSubEntry.this.size.incrementAndGet();
                IntegerShortSubEntry.this.parent.size.incrementAndGet();
                return (short) 0;
            }
        }
        ...
        public short removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                IntegerShortSubEntry.this.size.decrementAndGet();
                IntegerShortSubEntry.this.parent.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
