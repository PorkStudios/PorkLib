...
import net.daporkchop.lib.primitive.lambda.function.bi.ShortFloatToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of short => float mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface ShortFloatConcurrentIterator extends ShortFloatIterator   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ShortFloatToBooleanFunction function)    {
        try {
            while (this.hasNext())  {
                ShortFloatTuple tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<ShortFloatTuple> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<ShortFloatTuple> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ShortFloatConsumer consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<ShortFloatTuple> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull ShortFloatToBooleanFunction function)    {
        this.concurrentIterate(function);
    }
}
