...
import net.daporkchop.lib.primitive.lambda.function.bi.LongObjectToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of long => V mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface LongObjectConcurrentIterator<V extends Object> extends LongObjectIterator<V>   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull LongObjectToBooleanFunction<V> function)    {
        try {
            while (this.hasNext())  {
                LongObjectTuple<V> tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<LongObjectTuple<V>> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<LongObjectTuple<V>> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull LongObjectConsumer<V> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<LongObjectTuple<V>> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull LongObjectToBooleanFunction<V> function)    {
        this.concurrentIterate(function);
    }
}
