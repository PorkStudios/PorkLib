...
import net.daporkchop.lib.primitive.lambda.function.bi.ByteFloatToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of byte => float mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface ByteFloatConcurrentIterator extends ByteFloatIterator   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ByteFloatToBooleanFunction function)    {
        try {
            while (this.hasNext())  {
                ByteFloatTuple tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<ByteFloatTuple> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<ByteFloatTuple> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ByteFloatConsumer consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<ByteFloatTuple> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull ByteFloatToBooleanFunction function)    {
        this.concurrentIterate(function);
    }
}
