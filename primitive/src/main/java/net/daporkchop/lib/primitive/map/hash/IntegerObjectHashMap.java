...
/**
 * A hash map, using a key type of int and a value type of V.
 ...
 * @author DaPorkchop_
 */
public class IntegerObjectHashMap<V extends Object> implements IntegerObjectMap<V>    {
    protected int[] keys;
    ...
    public IntegerObjectHashMap()    {
        this(16384);
    }

    public IntegerObjectHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public IntegerObjectHashMap(IntegerToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public IntegerObjectHashMap(int baseSize, IntegerToIntegerFunction keyHash)    {
    ...
    protected boolean reHash(Object[] values, int[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
            int key = this.keys[i];
            int j = getIndex(key);
            if (states.get(j))  {
    ...
    private class KeyIterator implements IntegerIterator {
        ...
        public KeyIterator()    {
            this.size = IntegerObjectHashMap.this.size;
            this.next = IntegerObjectHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public int get()   {
            return IntegerObjectHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = IntegerObjectHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            IntegerObjectHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements ObjectIterator<V> {
        ...
        @SuppressWarnings("unchecked")
        public V get()   {
            return (V) IntegerObjectHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements IntegerObjectIterator<V> {
        private int size;
        private int index = -1;
        private int next;
        private IntegerObjectTuple<V> tuple;
        ...
        @Override
        public IntegerObjectTuple<V> advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new IntegerObjectImmutableTuple<V>( IntegerObjectHashMap.this.keys[this.index],  (V) IntegerObjectHashMap.this.values[this.index]);
        }
