...
import net.daporkchop.lib.primitive.lambda.function.bi.DoubleLongToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of double => long mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface DoubleLongConcurrentIterator extends DoubleLongIterator   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull DoubleLongToBooleanFunction function)    {
        try {
            while (this.hasNext())  {
                DoubleLongTuple tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<DoubleLongTuple> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<DoubleLongTuple> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull DoubleLongConsumer consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<DoubleLongTuple> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull DoubleLongToBooleanFunction function)    {
        this.concurrentIterate(function);
    }
}
