...
/**
 * A hash map, using a key type of long and a value type of long.
...
public class LongLongHashMap implements LongLongMap    {
    ...
    public LongLongHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public LongLongHashMap(LongToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public LongLongHashMap(int baseSize, LongToIntegerFunction keyHash)    {
    ...
    protected boolean reHash(long[] values, long[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
    ...
    private class KeyIterator implements LongIterator {
        ...
        public KeyIterator()    {
            this.size = LongLongHashMap.this.size;
            this.next = LongLongHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public long get()   {
            return LongLongHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = LongLongHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            LongLongHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements LongIterator {
        ...
        @SuppressWarnings("unchecked")
        public long get()   {
            return LongLongHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements LongLongIterator {
        private int size;
        private int index = -1;
        private int next;
        private LongLongTuple tuple;
        ...
        @Override
        public LongLongTuple advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new LongLongImmutableTuple( LongLongHashMap.this.keys[this.index],  LongLongHashMap.this.values[this.index]);
        }
