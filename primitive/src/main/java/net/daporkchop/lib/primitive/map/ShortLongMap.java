...
/**
 * A map, using a key type of short and a value type of long.
 * <p>
...
public interface ShortLongMap   {
     ...
     * @return the element mapped to the given key, or 0L if no such mapping exists
     */
    long get(short key);
     ...
     * @return the element that was previously mapped to the given key, or 0L if no such mapping existed
     */
     ...
     */
    default void putAll(@NonNull short[] keys, @NonNull long[] values)    {
     ...
     */
    default void putAll(@NonNull ShortLongMap map)    {
     ...
     * @param consumer the function to run
     */
    void forEachEntry(ShortLongConsumer consumer);
     ...
     */
    ShortLongIterator entryIterator();
     ...
     *
     * @return the previous value (0L if not present previously)
     */
     ...
     */
    default long getOrDefault(short key, long def)   {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
            this.put(key, def);
     ...
     */
    default long computeIfAbsent(short key, @NonNull ShortToLongFunction func) {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
            long val = func.apply(key);
            this.put(key, val);
            return val;
        }
    }
