...
import net.daporkchop.lib.primitive.lambda.function.bi.CharacterLongToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of char => long mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface CharacterLongConcurrentIterator extends CharacterLongIterator   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull CharacterLongToBooleanFunction function)    {
        try {
            while (this.hasNext())  {
                CharacterLongTuple tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<CharacterLongTuple> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<CharacterLongTuple> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull CharacterLongConsumer consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<CharacterLongTuple> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull CharacterLongToBooleanFunction function)    {
        this.concurrentIterate(function);
    }
}
