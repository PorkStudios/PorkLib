...
/**
 * A tree map, using a key type of long and a value type of float.
 ...
 * Although this map 𝙒𝙊𝙍𝙆𝙎, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.LongFloatIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class LongFloatTreeMap implements LongFloatMap    {
    protected long[] keys;
    ...
    @SuppressWarnings("unchecked")
    protected float remove(long key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return 0F; //no mapping
    ...
    public LongIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.LongFloatIndexedTreeMap} for an implementation of this method
        return null;
    }
