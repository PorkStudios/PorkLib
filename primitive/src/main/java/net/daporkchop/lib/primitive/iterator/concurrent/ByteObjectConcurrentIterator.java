...
import net.daporkchop.lib.primitive.lambda.function.bi.ByteObjectToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of byte => V mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface ByteObjectConcurrentIterator<V extends Object> extends ByteObjectIterator<V>   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ByteObjectToBooleanFunction<V> function)    {
        try {
            while (this.hasNext())  {
                ByteObjectTuple<V> tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<ByteObjectTuple<V>> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<ByteObjectTuple<V>> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ByteObjectConsumer<V> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<ByteObjectTuple<V>> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull ByteObjectToBooleanFunction<V> function)    {
        this.concurrentIterate(function);
    }
}
