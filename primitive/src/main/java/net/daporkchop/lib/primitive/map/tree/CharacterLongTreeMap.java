...
/**
 * A tree map, using a key type of char and a value type of long.
 ...
 * Although this map WORKS, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.CharacterLongIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class CharacterLongTreeMap implements CharacterLongMap    {
    protected char[] keys;
    ...
    public long get(char key)    {
        ...
        if (!containsKeyHash(hash)) return 0L; //no mapping
    ...
    @SuppressWarnings("unchecked")
    protected long remove(char key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return 0L; //no mapping
    ...
    public CharacterIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.CharacterLongIndexedTreeMap} for an implementation of this method
        return null;
    }
