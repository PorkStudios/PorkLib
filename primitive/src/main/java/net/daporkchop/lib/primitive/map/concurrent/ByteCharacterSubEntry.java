...
/**
 * A single subentry for a {@link ByteCharacterConcurrentTreeHashMap}.
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
class ByteCharacterSubEntry    {
    public final AtomicInteger size = new AtomicInteger(0);
    public final ByteCharacterConcurrentTreeHashMap parent;
    ...
    public ByteCharacterSubEntry(ByteCharacterConcurrentTreeHashMap parent)   {
        this.parent = parent;
        this.rootPointer = this.parent.subHashLength == 0 ? new ByteCharacterSubEntry.ValueSector(null, -1) : new ByteCharacterSubEntry.PointerSector(null, -1);
    }
    ...
    public void forEachKey(ByteConsumer consumer)   {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof ByteCharacterSubEntry.PointerSector)    {
    ...
    public byte advanceKey(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext)   {
        ...
        for (int i = sector.states.nextSetBit(curr.get() + 1); i != -1; i = sector.states.nextSetBit(i + 1)) {
            curr.set(i);
            if (sector instanceof ByteCharacterSubEntry.ValueSector)  {
    ...
    public void advanceEntry(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext, ByteCharacterMutableTuple tuple)   {
        Sector sector = sectorRef.get();
        if (sector == null) {
            sectorRef.set(sector = this.rootPointer);
        }
    ...
    public Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.parent.subHashLength; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.parent.subHashLength ? new ByteCharacterSubEntry.ValueSector(curr, off) : new ByteCharacterSubEntry.PointerSector(curr, off);
    ...
    public class ValueSector extends Sector   {
        ...
        public char setEntry(int index, byte key, char value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                ByteCharacterSubEntry.this.size.incrementAndGet();
                ByteCharacterSubEntry.this.parent.size.incrementAndGet();
                return (char) 0;
            }
        }
        ...
        public char removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                ByteCharacterSubEntry.this.size.decrementAndGet();
                ByteCharacterSubEntry.this.parent.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
