...
/**
 * A single subentry for a {@link BooleanShortConcurrentTreeHashMap}.
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
class BooleanShortSubEntry    {
    public final AtomicInteger size = new AtomicInteger(0);
    public final BooleanShortConcurrentTreeHashMap parent;
    ...
    public BooleanShortSubEntry(BooleanShortConcurrentTreeHashMap parent)   {
        this.parent = parent;
        this.rootPointer = this.parent.subHashLength == 0 ? new BooleanShortSubEntry.ValueSector(null, -1) : new BooleanShortSubEntry.PointerSector(null, -1);
    }
    ...
    public void forEachKey(BooleanConsumer consumer)   {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof BooleanShortSubEntry.PointerSector)    {
    ...
    public boolean advanceKey(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext)   {
        ...
        for (int i = sector.states.nextSetBit(curr.get() + 1); i != -1; i = sector.states.nextSetBit(i + 1)) {
            curr.set(i);
            if (sector instanceof BooleanShortSubEntry.ValueSector)  {
    ...
    public void advanceEntry(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext, BooleanShortMutableTuple tuple)   {
        Sector sector = sectorRef.get();
        if (sector == null) {
            sectorRef.set(sector = this.rootPointer);
        }
    ...
    public Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.parent.subHashLength; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.parent.subHashLength ? new BooleanShortSubEntry.ValueSector(curr, off) : new BooleanShortSubEntry.PointerSector(curr, off);
    ...
    public abstract class Sector  {
        ...
        public short setEntry(int index, boolean key, short value)   {
            throw new UnsupportedOperationException();
        }
    ...
    public class ValueSector extends Sector   {
        ...
        public short setEntry(int index, boolean key, short value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                BooleanShortSubEntry.this.size.incrementAndGet();
                BooleanShortSubEntry.this.parent.size.incrementAndGet();
                return (short) 0;
            }
        }
        ...
        public short removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                BooleanShortSubEntry.this.size.decrementAndGet();
                BooleanShortSubEntry.this.parent.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
