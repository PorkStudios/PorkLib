...
import net.daporkchop.lib.primitive.map.tree.ObjectObjectTreeMap;
...
/**
 * A tree map, using a key type of K and a value type of V.
 ...
 * @author DaPorkchop_
 */
public class ObjectObjectIndexedTreeMap<K extends Object, V extends Object> extends ObjectObjectTreeMap<K, V>    {
    protected boolean[] index;

    public ObjectObjectIndexedTreeMap()    {
        super();
    }
    ...
    public void forEachEntry(@NonNull ObjectObjectConsumer<K, V> consumer)   {
        ...
        //scan through the index until -1 is reached
        for (int i = 0; i < index.length; i++)  {
            if (index[i]) consumer.accept( (K) keys[i], (V) values[i]);
        }
    }
    ...
    private class KeyIterator implements ObjectIterator<K>   {
        ...
        private final ObjectObjectIndexedTreeMap this_ = ObjectObjectIndexedTreeMap.this;
    ...
    private class EntryIterator implements ObjectObjectIterator<K, V>   {
        ...
        public ObjectObjectTuple<K, V> advance()   {
            if (this.hasNext()) {
                this.findNext();
                return this.tuple = new ObjectObjectImmutableTuple<K, V>( (K) this_.keys[this.index],  (V) this_.values[this.index]);
            } else {
                throw new ArrayIndexOutOfBoundsException();
            }
        }
