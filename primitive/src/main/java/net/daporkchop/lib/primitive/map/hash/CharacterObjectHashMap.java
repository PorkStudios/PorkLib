...
/**
 * A hash map, using a key type of char and a value type of V.
 ...
 * @author DaPorkchop_
 */
public class CharacterObjectHashMap<V extends Object> implements CharacterObjectMap<V>    {
    protected char[] keys;
    ...
    public CharacterObjectHashMap()    {
        this(16384);
    }

    public CharacterObjectHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public CharacterObjectHashMap(CharacterToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public CharacterObjectHashMap(int baseSize, CharacterToIntegerFunction keyHash)    {
    ...
    protected boolean reHash(Object[] values, char[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
    ...
    private class KeyIterator implements CharacterIterator {
        ...
        public KeyIterator()    {
            this.size = CharacterObjectHashMap.this.size;
            this.next = CharacterObjectHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public char get()   {
            return CharacterObjectHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = CharacterObjectHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            CharacterObjectHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements ObjectIterator<V> {
        ...
        @SuppressWarnings("unchecked")
        public V get()   {
            return (V) CharacterObjectHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements CharacterObjectIterator<V> {
        private int size;
        private int index = -1;
        private int next;
        private CharacterObjectTuple<V> tuple;
        ...
        @Override
        public CharacterObjectTuple<V> advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new CharacterObjectImmutableTuple<V>( CharacterObjectHashMap.this.keys[this.index],  (V) CharacterObjectHashMap.this.values[this.index]);
        }
