...
import net.daporkchop.lib.primitive.iterator.bi.ByteShortIterator;
import net.daporkchop.lib.primitive.map.ByteShortMap;
import net.daporkchop.lib.primitive.tuple.ByteShortTuple;
import net.daporkchop.lib.primitive.tuple.ByteShortImmutableTuple;
...
/**
 * A hash map, using a key type of byte and a value type of short.
 ...
 * @author DaPorkchop_
 */
public class ByteShortHashMap implements ByteShortMap    {
    protected byte[] keys;
    ...
    public ByteShortHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public ByteShortHashMap(ByteToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public ByteShortHashMap(int baseSize, ByteToIntegerFunction keyHash)    {
    ...
    @Override
    public void forEachEntry(@NonNull ByteShortConsumer consumer)   {
    ...
    public ByteShortIterator entryIterator() {
    ...
    protected boolean reHash(short[] values, byte[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
    ...
    private class KeyIterator implements ByteIterator {
        ...
        public KeyIterator()    {
            this.size = ByteShortHashMap.this.size;
            this.next = ByteShortHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public byte get()   {
            return ByteShortHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = ByteShortHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            ByteShortHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements ShortIterator {
        ...
        @SuppressWarnings("unchecked")
        public short get()   {
            return ByteShortHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements ByteShortIterator {
        private int size;
        private int index = -1;
        private int next;
        private ByteShortTuple tuple;
        ...
        @Override
        public ByteShortTuple advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new ByteShortImmutableTuple( ByteShortHashMap.this.keys[this.index],  ByteShortHashMap.this.values[this.index]);
        }
