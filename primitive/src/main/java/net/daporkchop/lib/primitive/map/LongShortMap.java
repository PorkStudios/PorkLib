...
import net.daporkchop.lib.primitive.lambda.function.LongToShortFunction;
...
/**
 * A map, using a key type of long and a value type of short.
 * <p>
...
public interface LongShortMap   {
     ...
     */
    default void putAll(@NonNull long[] keys, @NonNull short[] values)    {
     ...
     */
    default void putAll(@NonNull LongShortMap map)    {
     ...
     * @param consumer the function to run
     */
    void forEachEntry(LongShortConsumer consumer);
     ...
     */
    LongShortIterator entryIterator();
     ...
     */
    default short getOrDefault(long key, short def)   {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
            this.put(key, def);
     ...
     */
    default short computeIfAbsent(long key, @NonNull LongToShortFunction func) {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
