...
import net.daporkchop.lib.primitive.iterator.concurrent.LongShortConcurrentIterator;
import net.daporkchop.lib.primitive.map.LongShortMap;
import net.daporkchop.lib.primitive.tuple.LongShortTuple;
import net.daporkchop.lib.primitive.tuple.LongShortImmutableTuple;
import net.daporkchop.lib.primitive.tuple.LongShortMutableTuple;
 ...
 * @author DaPorkchop_
 */
public class LongShortConcurrentTreeHashMap implements LongShortMap    {
    final AtomicInteger size = new AtomicInteger(0);

    private final LongShortSubEntry[] entries = new LongShortSubEntry[256];
    private final LongToIntegerFunction keyHash;
    final LongToLongFunction subKeyHash;
    final int subHashLength;

    public LongShortConcurrentTreeHashMap()    {
        this(null, null);
    }

    public LongShortConcurrentTreeHashMap(LongToIntegerFunction keyHash, LongToLongFunction subKeyHash)    {
        this(keyHash, subKeyHash, 7);
    }

    public LongShortConcurrentTreeHashMap(LongToIntegerFunction keyHash, LongToLongFunction subKeyHash, int subHashLength)    {
    ...
    public short get(long key)   {
        LongShortSubEntry entry = this.getSubentry(this.keyHash.apply(key) & 0xFF, false);
        if (entry == null)  {
            return (short) 0;
        } else {
    ...
    public short put(long key, short value)   {
        LongShortSubEntry entry = this.getSubentry(this.keyHash.apply(key) & 0xFF, true);
    ...
    public short remove(long key)    {
        ...
        LongShortSubEntry entry = this.getSubentry(hash, false);
        if (entry == null)  {
            return (short) 0;
        } else {
    ...
    public void clear() {
        synchronized (this.entries) {
            for (int i = 0; i < 256; i++) {
                LongShortSubEntry entry = this.entries[i];
                if (entry != null)  {
                    entry.clear();
                    this.entries[i] = null;
                }
    ...
    public void forEachKey(LongConsumer consumer)   {
        for (int i = 0; i < 256; i++) {
            LongShortSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
    ...
    public void forEachEntry(LongShortConsumer consumer, VoidFunction complete) {
        for (int i = 0; i < 256; i++) {
            LongShortSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
                entry.forEachEntry(consumer);
    ...
    public LongConcurrentIterator keyIterator()   {
        return new LongConcurrentIterator()  {
            private final LongShortConcurrentTreeHashMap this_ = LongShortConcurrentTreeHashMap.this;
            ...
            private final AtomicReference<LongShortSubEntry.Sector> currSector = new AtomicReference(null);
            ...
            private volatile LongShortSubEntry entry;
            ...
            private void nextSub()   {
                ...
                while (this.subEntryIndex < 256)    {
                    LongShortSubEntry entry = getSubentry(this.subEntryIndex++, false);
                    if (entry != null)  {
                        this.entry = entry;
            ...
            class ThreadState   {
                public long key;
                public LongShortSubEntry entry;
                private final LongConcurrentIterator this_;
                private final LongShortConcurrentTreeHashMap this__;

                public ThreadState(LongConcurrentIterator this_, LongShortConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
    ...
    public ShortConcurrentIterator valueIterator() {
        return new ShortConcurrentIterator()  {
            ...
            private final AtomicReference<LongShortSubEntry.Sector> currSector = new AtomicReference(null);
            private final AtomicBoolean hasNextSub = new AtomicBoolean(true);
            private final LongShortMutableTuple next = new LongShortMutableTuple();
            private volatile boolean hasNext = true;
            private volatile int subEntryIndex = 0;
            private volatile LongShortSubEntry entry;
            ...
            class ThreadState   {
                ...
                public ThreadState(ShortConcurrentIterator this_, LongShortConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
    ...
    @Override
    public LongShortConcurrentIterator entryIterator() {
        return new LongShortConcurrentIterator()  {
            private final LongShortConcurrentTreeHashMap this_ = LongShortConcurrentTreeHashMap.this;
            //private final ReentrantLock lock = new ReentrantLock();
            private final ThreadLocal<ThreadState> tl = ThreadLocal.withInitial(() -> new ThreadState(this, this.this_));
            private final AtomicInteger sectorOffset = new AtomicInteger(0);
            ...
            @Override
            public synchronized LongShortTuple advance()   {
                if (!this.hasNext())    {
                    throw new IteratorCompleteException();
                }
                ThreadState state = this.tl.get();
                LongShortTuple current = state.tuple = new LongShortMutableTuple(this.next.k, this.next.v);
                state.entry = this.entry;
                this.findNextValue();
                return current;
            }
            ...
            class ThreadState   {
                public LongShortTuple tuple;
                public LongShortSubEntry entry;
                private final LongShortConcurrentIterator this_;
                private final LongShortConcurrentTreeHashMap this__;

                public ThreadState(LongShortConcurrentIterator this_, LongShortConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
     ...
     */
    public LongShortConcurrentIterator fastEntryIterator() {
        return new LongShortConcurrentIterator()  {
            private final LongShortConcurrentTreeHashMap this_ = LongShortConcurrentTreeHashMap.this;
            //private final ReentrantLock lock = new ReentrantLock();
            private final ThreadLocal<ThreadState> tl = ThreadLocal.withInitial(() -> new ThreadState(this, this.this_));
            ...
            private final AtomicReference<LongShortSubEntry.Sector> currSector = new AtomicReference(null);
            private final AtomicBoolean hasNextSub = new AtomicBoolean(true);
            private volatile LongShortMutableTuple next = new LongShortMutableTuple(0L, (short) 0);
            private volatile boolean hasNext = true;
            private volatile int subEntryIndex = 0;
            private volatile LongShortSubEntry entry;
            ...
            @Override
            public LongShortMutableTuple get()   {
                ThreadState state = this.tl.get();
                if (state.entry == null)    {
                    throw new IllegalStateException();
                } else {
            ...
            @Override
            public synchronized LongShortMutableTuple advance()   {
                if (!this.hasNext())    {
                    throw new IteratorCompleteException();
                }
                ThreadState state = this.tl.get();
                LongShortMutableTuple a = state.tuple;
            ...
            class ThreadState   {
                public LongShortMutableTuple tuple = new LongShortMutableTuple(0L, (short) 0);
                public LongShortSubEntry entry;
                private final LongShortConcurrentIterator this_;
                private final LongShortConcurrentTreeHashMap this__;
    ...
    private LongShortSubEntry getSubentry(int index, boolean create)    {
        assert index == (index & 0xFF);

        synchronized (this.entries) {
            LongShortSubEntry entry = (LongShortSubEntry) this.entries[index];
            if (entry == null && create)    {
                this.entries[index] = entry = new LongShortSubEntry(this);
            }
            return entry;
        }
    }
