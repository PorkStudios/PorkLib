...
/**
 * A map, using a key type of byte and a value type of int.
 * <p>
...
public interface ByteIntegerMap   {
     ...
     */
    default void putAll(@NonNull byte[] keys, @NonNull int[] values)    {
     ...
     */
    default void putAll(@NonNull ByteIntegerMap map)    {
     ...
     * @param consumer the function to run
     */
    void forEachEntry(ByteIntegerConsumer consumer);
     ...
     */
    ByteIntegerIterator entryIterator();
     ...
     */
    default int getOrDefault(byte key, int def)   {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
            this.put(key, def);
     ...
     */
    default int computeIfAbsent(byte key, @NonNull ByteToIntegerFunction func) {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
