...
import net.daporkchop.lib.primitive.lambda.function.bi.FloatObjectToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of float => V mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface FloatObjectConcurrentIterator<V extends Object> extends FloatObjectIterator<V>   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull FloatObjectToBooleanFunction<V> function)    {
        try {
            while (this.hasNext())  {
                FloatObjectTuple<V> tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<FloatObjectTuple<V>> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<FloatObjectTuple<V>> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull FloatObjectConsumer<V> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<FloatObjectTuple<V>> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull FloatObjectToBooleanFunction<V> function)    {
        this.concurrentIterate(function);
    }
}
