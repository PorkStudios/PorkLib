...
/**
 * A hash map, using a key type of byte and a value type of double.
 ...
 * @author DaPorkchop_
 */
public class ByteDoubleHashMap implements ByteDoubleMap    {
    protected byte[] keys;
    ...
    public ByteDoubleHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public ByteDoubleHashMap(ByteToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public ByteDoubleHashMap(int baseSize, ByteToIntegerFunction keyHash)    {
    ...
    protected boolean reHash(double[] values, byte[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
    ...
    private class KeyIterator implements ByteIterator {
        ...
        public KeyIterator()    {
            this.size = ByteDoubleHashMap.this.size;
            this.next = ByteDoubleHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public byte get()   {
            return ByteDoubleHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = ByteDoubleHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            ByteDoubleHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements DoubleIterator {
        ...
        @SuppressWarnings("unchecked")
        public double get()   {
            return ByteDoubleHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements ByteDoubleIterator {
        private int size;
        private int index = -1;
        private int next;
        private ByteDoubleTuple tuple;
        ...
        @Override
        public ByteDoubleTuple advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new ByteDoubleImmutableTuple( ByteDoubleHashMap.this.keys[this.index],  ByteDoubleHashMap.this.values[this.index]);
        }
