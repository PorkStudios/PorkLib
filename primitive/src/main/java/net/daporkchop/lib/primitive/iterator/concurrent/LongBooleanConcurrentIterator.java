...
import net.daporkchop.lib.primitive.lambda.function.bi.LongBooleanToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of long => boolean mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface LongBooleanConcurrentIterator extends LongBooleanIterator   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull LongBooleanToBooleanFunction function)    {
        try {
            while (this.hasNext())  {
                LongBooleanTuple tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<LongBooleanTuple> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<LongBooleanTuple> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull LongBooleanConsumer consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<LongBooleanTuple> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull LongBooleanToBooleanFunction function)    {
        this.concurrentIterate(function);
    }
}
