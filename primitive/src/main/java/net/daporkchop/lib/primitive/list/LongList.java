...
public interface LongList   {
     ...
     * @return the element at to the given index, or 0L if no such mapping exists
     */
    long get(int index);
     ...
     */
    default void addAll(@NonNull long[] values)    {
        for (long p : values)   {
            this.add(p);
        }
    }
     ...
     */
    default void addAll(@NonNull LongList list)    {
     ...
     */
    default void removeAll(@NonNull Collection<Long> values) {
