...
/**
 * A single subentry for a {@link CharacterByteConcurrentTreeHashMap}.
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
class CharacterByteSubEntry    {
    public final AtomicInteger size = new AtomicInteger(0);
    public final CharacterByteConcurrentTreeHashMap parent;
    ...
    public CharacterByteSubEntry(CharacterByteConcurrentTreeHashMap parent)   {
        this.parent = parent;
        this.rootPointer = this.parent.subHashLength == 0 ? new CharacterByteSubEntry.ValueSector(null, -1) : new CharacterByteSubEntry.PointerSector(null, -1);
    }
    ...
    public void forEachKey(CharacterConsumer consumer)   {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof CharacterByteSubEntry.PointerSector)    {
    ...
    public char advanceKey(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext)   {
        ...
        for (int i = sector.states.nextSetBit(curr.get() + 1); i != -1; i = sector.states.nextSetBit(i + 1)) {
            curr.set(i);
            if (sector instanceof CharacterByteSubEntry.ValueSector)  {
    ...
    public void advanceEntry(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext, CharacterByteMutableTuple tuple)   {
        Sector sector = sectorRef.get();
        if (sector == null) {
            sectorRef.set(sector = this.rootPointer);
        }
    ...
    public Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.parent.subHashLength; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.parent.subHashLength ? new CharacterByteSubEntry.ValueSector(curr, off) : new CharacterByteSubEntry.PointerSector(curr, off);
    ...
    public abstract class Sector  {
        ...
        public byte setEntry(int index, char key, byte value)   {
            throw new UnsupportedOperationException();
        }
    ...
    public class ValueSector extends Sector   {
        ...
        public byte setEntry(int index, char key, byte value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                CharacterByteSubEntry.this.size.incrementAndGet();
                CharacterByteSubEntry.this.parent.size.incrementAndGet();
                return (byte) 0;
            }
        }
        ...
        public byte removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                CharacterByteSubEntry.this.size.decrementAndGet();
                CharacterByteSubEntry.this.parent.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
