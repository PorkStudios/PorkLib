 ...
 * overhead, and can even be made concurrent with little extra work (see {@link net.daporkchop.lib.primitive.map.concurrent.ShortLongConcurrentTreeHashMap})
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
public class ShortLongTreeHashMap implements ShortLongMap    {
    ...
    public ShortLongTreeHashMap()    {
        this(null, 0);
    }

    public ShortLongTreeHashMap(ShortToLongFunction hashFunc, int hashLen)    {
        ...
        this.rootPointer = this.hashLen == 0 ? new ShortLongTreeHashMap.ValueSector(null, -1) : new ShortLongTreeHashMap.PointerSector(null, -1);
    }
    ...
    public void forEachKey(@NonNull ShortConsumer consumer)  {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof ShortLongTreeHashMap.PointerSector)    {
    ...
    private Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.hashLen; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.hashLen ? new ShortLongTreeHashMap.ValueSector(curr, off) : new ShortLongTreeHashMap.PointerSector(curr, off);
    ...
    private class ValueSector extends Sector   {
        ...
        public long setEntry(int index, short key, long value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                ShortLongTreeHashMap.this.size.incrementAndGet();
                return 0L;
            }
        }
        ...
        public long removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                ShortLongTreeHashMap.this.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
