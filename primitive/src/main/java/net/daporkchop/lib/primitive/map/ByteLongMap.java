...
/**
 * A map, using a key type of byte and a value type of long.
 * <p>
...
public interface ByteLongMap   {
     ...
     */
    default void putAll(@NonNull byte[] keys, @NonNull long[] values)    {
     ...
     */
    default void putAll(@NonNull ByteLongMap map)    {
     ...
     * @param consumer the function to run
     */
    void forEachEntry(ByteLongConsumer consumer);
     ...
     */
    ByteLongIterator entryIterator();
     ...
     */
    default long getOrDefault(byte key, long def)   {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
            this.put(key, def);
     ...
     */
    default long computeIfAbsent(byte key, @NonNull ByteToLongFunction func) {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
