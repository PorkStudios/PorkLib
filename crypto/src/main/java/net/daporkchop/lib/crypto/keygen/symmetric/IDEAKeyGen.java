...
public class IDEAKeyGen {
    ...
    public static IDEAKey gen(byte[] data) {
        byte[] b = new byte[BlockCipherType.IDEA.blockSize];
        int i = 0;
        do {
        ...
        data = b;
        return new IDEAKey(new SecretKeySpec(data, "aaa"), KeyRandom.getBytes(BlockCipherType.IDEA.blockSize));
    }

    public static IDEAKey gen() {
        byte[] key = new byte[BlockCipherType.IDEA.blockSize];
