 ...
 * overhead, and can even be made concurrent with little extra work (see {@link net.daporkchop.lib.primitive.map.concurrent.IntegerIntegerConcurrentTreeHashMap})
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
public class IntegerIntegerTreeHashMap implements IntegerIntegerMap    {
    ...
    public IntegerIntegerTreeHashMap()    {
        this(null, 0);
    }

    public IntegerIntegerTreeHashMap(IntegerToLongFunction hashFunc, int hashLen)    {
        ...
        this.rootPointer = this.hashLen == 0 ? new IntegerIntegerTreeHashMap.ValueSector(null, -1) : new IntegerIntegerTreeHashMap.PointerSector(null, -1);
    }
    ...
    public void forEachKey(@NonNull IntegerConsumer consumer)  {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof IntegerIntegerTreeHashMap.PointerSector)    {
    ...
    private Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.hashLen; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.hashLen ? new IntegerIntegerTreeHashMap.ValueSector(curr, off) : new IntegerIntegerTreeHashMap.PointerSector(curr, off);
    ...
    private class ValueSector extends Sector   {
        ...
        public int setEntry(int index, int key, int value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                IntegerIntegerTreeHashMap.this.size.incrementAndGet();
                return 0;
            }
        }
        ...
        public int removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                IntegerIntegerTreeHashMap.this.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
