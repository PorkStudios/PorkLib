...
/**
 * A hash map, using a key type of K and a value type of double.
 ...
 * @author DaPorkchop_
 */
public class ObjectDoubleHashMap<K extends Object> implements ObjectDoubleMap<K>    {
    ...
    public ObjectDoubleHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public ObjectDoubleHashMap(ObjectToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public ObjectDoubleHashMap(int baseSize, ObjectToIntegerFunction keyHash)    {
    ...
    protected boolean reHash(double[] values, Object[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
            K key = (K) this.keys[i];
            int j = getIndex(key);
            if (states.get(j))  {
    ...
    private class KeyIterator implements ObjectIterator<K> {
        ...
        public KeyIterator()    {
            this.size = ObjectDoubleHashMap.this.size;
            this.next = ObjectDoubleHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public K get()   {
            return (K) ObjectDoubleHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = ObjectDoubleHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            ObjectDoubleHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements DoubleIterator {
        ...
        @SuppressWarnings("unchecked")
        public double get()   {
            return ObjectDoubleHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements ObjectDoubleIterator<K> {
        private int size;
        private int index = -1;
        private int next;
        private ObjectDoubleTuple<K> tuple;
        ...
        @Override
        public ObjectDoubleTuple<K> advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new ObjectDoubleImmutableTuple<K>( (K) ObjectDoubleHashMap.this.keys[this.index],  ObjectDoubleHashMap.this.values[this.index]);
        }
