...
import net.daporkchop.lib.primitive.iterator.concurrent.BooleanShortConcurrentIterator;
import net.daporkchop.lib.primitive.map.BooleanShortMap;
import net.daporkchop.lib.primitive.tuple.BooleanShortTuple;
import net.daporkchop.lib.primitive.tuple.BooleanShortMutableTuple;
...
/**
 * A tree hash map, using a key type of boolean and a value type of short.
 ...
 * @author DaPorkchop_
 */
public class BooleanShortConcurrentTreeHashMap implements BooleanShortMap    {
    final AtomicInteger size = new AtomicInteger(0);

    private final BooleanShortSubEntry[] entries = new BooleanShortSubEntry[256];
    private final BooleanToIntegerFunction keyHash;
    final BooleanToLongFunction subKeyHash;
    final int subHashLength;

    public BooleanShortConcurrentTreeHashMap()    {
        this(null, null);
    }

    public BooleanShortConcurrentTreeHashMap(BooleanToIntegerFunction keyHash, BooleanToLongFunction subKeyHash)    {
        this(keyHash, subKeyHash, 0);
    }

    public BooleanShortConcurrentTreeHashMap(BooleanToIntegerFunction keyHash, BooleanToLongFunction subKeyHash, int subHashLength)    {
    ...
    public short get(boolean key)   {
        BooleanShortSubEntry entry = this.getSubentry(this.keyHash.apply(key) & 0xFF, false);
        if (entry == null)  {
            return (short) 0;
        } else {
    ...
    public short put(boolean key, short value)   {
        BooleanShortSubEntry entry = this.getSubentry(this.keyHash.apply(key) & 0xFF, true);
    ...
    public short remove(boolean key)    {
        ...
        BooleanShortSubEntry entry = this.getSubentry(hash, false);
        if (entry == null)  {
            return (short) 0;
        } else {
    ...
    public void clear() {
        synchronized (this.entries) {
            for (int i = 0; i < 256; i++) {
                BooleanShortSubEntry entry = this.entries[i];
                if (entry != null)  {
                    entry.clear();
                    this.entries[i] = null;
                }
    ...
    public void forEachKey(BooleanConsumer consumer)   {
        for (int i = 0; i < 256; i++) {
            BooleanShortSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
    ...
    @Override
    public void forEachEntry(BooleanShortConsumer consumer) {
        for (int i = 0; i < 256; i++) {
            BooleanShortSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
    ...
    public void forEachEntry(BooleanShortConsumer consumer, VoidFunction complete) {
        for (int i = 0; i < 256; i++) {
            BooleanShortSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
                entry.forEachEntry(consumer);
    ...
    public BooleanConcurrentIterator keyIterator()   {
        return new BooleanConcurrentIterator()  {
            private final BooleanShortConcurrentTreeHashMap this_ = BooleanShortConcurrentTreeHashMap.this;
            ...
            private final AtomicReference<BooleanShortSubEntry.Sector> currSector = new AtomicReference(null);
            ...
            private volatile BooleanShortSubEntry entry;
            ...
            private void nextSub()   {
                ...
                while (this.subEntryIndex < 256)    {
                    BooleanShortSubEntry entry = getSubentry(this.subEntryIndex++, false);
                    if (entry != null)  {
                        this.entry = entry;
            ...
            class ThreadState   {
                public boolean key;
                public BooleanShortSubEntry entry;
                private final BooleanConcurrentIterator this_;
                private final BooleanShortConcurrentTreeHashMap this__;

                public ThreadState(BooleanConcurrentIterator this_, BooleanShortConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
    ...
    public ShortConcurrentIterator valueIterator() {
        return new ShortConcurrentIterator()  {
            ...
            private final AtomicReference<BooleanShortSubEntry.Sector> currSector = new AtomicReference(null);
            private final AtomicBoolean hasNextSub = new AtomicBoolean(true);
            private final BooleanShortMutableTuple next = new BooleanShortMutableTuple();
            private volatile boolean hasNext = true;
            private volatile int subEntryIndex = 0;
            private volatile BooleanShortSubEntry entry;
            ...
            class ThreadState   {
                ...
                public ThreadState(ShortConcurrentIterator this_, BooleanShortConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
    ...
    @Override
    public BooleanShortConcurrentIterator entryIterator() {
        return new BooleanShortConcurrentIterator()  {
            private final BooleanShortConcurrentTreeHashMap this_ = BooleanShortConcurrentTreeHashMap.this;
            //private final ReentrantLock lock = new ReentrantLock();
            private final ThreadLocal<ThreadState> tl = ThreadLocal.withInitial(() -> new ThreadState(this, this.this_));
            private final AtomicInteger sectorOffset = new AtomicInteger(0);
            ...
            @Override
            public synchronized BooleanShortTuple advance()   {
                if (!this.hasNext())    {
                    throw new IteratorCompleteException();
                }
                ThreadState state = this.tl.get();
                BooleanShortTuple current = state.tuple = new BooleanShortMutableTuple(this.next.k, this.next.v);
                state.entry = this.entry;
                this.findNextValue();
                return current;
            }
            ...
            class ThreadState   {
                public BooleanShortTuple tuple;
                public BooleanShortSubEntry entry;
                private final BooleanShortConcurrentIterator this_;
                private final BooleanShortConcurrentTreeHashMap this__;

                public ThreadState(BooleanShortConcurrentIterator this_, BooleanShortConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
     ...
     */
    public BooleanShortConcurrentIterator fastEntryIterator() {
        return new BooleanShortConcurrentIterator()  {
            private final BooleanShortConcurrentTreeHashMap this_ = BooleanShortConcurrentTreeHashMap.this;
            //private final ReentrantLock lock = new ReentrantLock();
            private final ThreadLocal<ThreadState> tl = ThreadLocal.withInitial(() -> new ThreadState(this, this.this_));
            ...
            private final AtomicReference<BooleanShortSubEntry.Sector> currSector = new AtomicReference(null);
            private final AtomicBoolean hasNextSub = new AtomicBoolean(true);
            private volatile BooleanShortMutableTuple next = new BooleanShortMutableTuple(false, (short) 0);
            private volatile boolean hasNext = true;
            private volatile int subEntryIndex = 0;
            private volatile BooleanShortSubEntry entry;
            ...
            @Override
            public BooleanShortMutableTuple get()   {
                ThreadState state = this.tl.get();
                if (state.entry == null)    {
                    throw new IllegalStateException();
                } else {
            ...
            @Override
            public synchronized BooleanShortMutableTuple advance()   {
                if (!this.hasNext())    {
                    throw new IteratorCompleteException();
                }
                ThreadState state = this.tl.get();
                BooleanShortMutableTuple a = state.tuple;
            ...
            class ThreadState   {
                public BooleanShortMutableTuple tuple = new BooleanShortMutableTuple(false, (short) 0);
                public BooleanShortSubEntry entry;
                private final BooleanShortConcurrentIterator this_;
                private final BooleanShortConcurrentTreeHashMap this__;
    ...
    private BooleanShortSubEntry getSubentry(int index, boolean create)    {
        assert index == (index & 0xFF);

        synchronized (this.entries) {
            BooleanShortSubEntry entry = (BooleanShortSubEntry) this.entries[index];
            if (entry == null && create)    {
                this.entries[index] = entry = new BooleanShortSubEntry(this);
            }
            return entry;
        }
    }
