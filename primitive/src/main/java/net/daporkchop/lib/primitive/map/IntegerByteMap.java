...
import net.daporkchop.lib.primitive.lambda.function.IntegerToByteFunction;
...
/**
 * A map, using a key type of int and a value type of byte.
 * <p>
...
public interface IntegerByteMap   {
     ...
     */
    default void putAll(@NonNull int[] keys, @NonNull byte[] values)    {
     ...
     */
    default void putAll(@NonNull IntegerByteMap map)    {
     ...
     * @param consumer the function to run
     */
    void forEachEntry(IntegerByteConsumer consumer);
     ...
     */
    IntegerByteIterator entryIterator();
     ...
     */
    default byte getOrDefault(int key, byte def)   {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
            this.put(key, def);
     ...
     */
    default byte computeIfAbsent(int key, @NonNull IntegerToByteFunction func) {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
