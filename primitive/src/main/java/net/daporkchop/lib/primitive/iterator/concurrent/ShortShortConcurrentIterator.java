...
import net.daporkchop.lib.primitive.lambda.function.bi.ShortShortToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of short => short mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface ShortShortConcurrentIterator extends ShortShortIterator   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ShortShortToBooleanFunction function)    {
        try {
            while (this.hasNext())  {
                ShortShortTuple tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<ShortShortTuple> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<ShortShortTuple> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ShortShortConsumer consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<ShortShortTuple> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull ShortShortToBooleanFunction function)    {
        this.concurrentIterate(function);
    }
}
