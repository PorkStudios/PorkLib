...
/**
 * A tree map, using a key type of double and a value type of long.
 ...
 * Although this map 𝙒𝙊𝙍𝙆𝙎, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.DoubleLongIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class DoubleLongTreeMap implements DoubleLongMap    {
    ...
    @SuppressWarnings("unchecked")
    protected long remove(double key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return 0L; //no mapping
    ...
    public DoubleIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.DoubleLongIndexedTreeMap} for an implementation of this method
        return null;
    }
