...
import net.daporkchop.lib.primitive.lambda.function.ShortToFloatFunction;
...
/**
 * A map, using a key type of short and a value type of float.
 * <p>
...
public interface ShortFloatMap   {
     ...
     */
    default void putAll(@NonNull short[] keys, @NonNull float[] values)    {
     ...
     */
    default void putAll(@NonNull ShortFloatMap map)    {
     ...
     * @param consumer the function to run
     */
    void forEachEntry(ShortFloatConsumer consumer);
     ...
     */
    ShortFloatIterator entryIterator();
     ...
     */
    default float getOrDefault(short key, float def)   {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
            this.put(key, def);
     ...
     */
    default float computeIfAbsent(short key, @NonNull ShortToFloatFunction func) {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
