...
/**
 * A map, using a key type of float and a value type of int.
 * <p>
...
public interface FloatIntegerMap   {
     ...
     */
    default void putAll(@NonNull float[] keys, @NonNull int[] values)    {
     ...
     */
    default void putAll(@NonNull FloatIntegerMap map)    {
     ...
     * @param consumer the function to run
     */
    void forEachEntry(FloatIntegerConsumer consumer);
     ...
     */
    FloatIntegerIterator entryIterator();
     ...
     */
    default int getOrDefault(float key, int def)   {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
            this.put(key, def);
     ...
     */
    default int computeIfAbsent(float key, @NonNull FloatToIntegerFunction func) {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
