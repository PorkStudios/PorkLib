...
/**
 * A hash map, using a key type of int and a value type of byte.
 ...
 * @author DaPorkchop_
 */
public class IntegerByteHashMap implements IntegerByteMap    {
    protected int[] keys;
    ...
    public IntegerByteHashMap()    {
        this(16384);
    }

    public IntegerByteHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public IntegerByteHashMap(IntegerToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public IntegerByteHashMap(int baseSize, IntegerToIntegerFunction keyHash)    {
    ...
    protected boolean reHash(byte[] values, int[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
            int key = this.keys[i];
            int j = getIndex(key);
            if (states.get(j))  {
    ...
    private class KeyIterator implements IntegerIterator {
        ...
        public KeyIterator()    {
            this.size = IntegerByteHashMap.this.size;
            this.next = IntegerByteHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public int get()   {
            return IntegerByteHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = IntegerByteHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            IntegerByteHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements ByteIterator {
        ...
        @SuppressWarnings("unchecked")
        public byte get()   {
            return IntegerByteHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements IntegerByteIterator {
        private int size;
        private int index = -1;
        private int next;
        private IntegerByteTuple tuple;
        ...
        @Override
        public IntegerByteTuple advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new IntegerByteImmutableTuple( IntegerByteHashMap.this.keys[this.index],  IntegerByteHashMap.this.values[this.index]);
        }
