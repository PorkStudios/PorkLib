...
import net.daporkchop.lib.primitive.lambda.function.bi.CharacterCharacterToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of char => char mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface CharacterCharacterConcurrentIterator extends CharacterCharacterIterator   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull CharacterCharacterToBooleanFunction function)    {
        try {
            while (this.hasNext())  {
                CharacterCharacterTuple tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<CharacterCharacterTuple> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<CharacterCharacterTuple> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull CharacterCharacterConsumer consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<CharacterCharacterTuple> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull CharacterCharacterToBooleanFunction function)    {
        this.concurrentIterate(function);
    }
}
