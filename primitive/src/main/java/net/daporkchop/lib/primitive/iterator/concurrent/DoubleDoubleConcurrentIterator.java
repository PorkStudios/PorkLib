...
import net.daporkchop.lib.primitive.lambda.function.bi.DoubleDoubleToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of double => double mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface DoubleDoubleConcurrentIterator extends DoubleDoubleIterator   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull DoubleDoubleToBooleanFunction function)    {
        try {
            while (this.hasNext())  {
                DoubleDoubleTuple tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<DoubleDoubleTuple> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<DoubleDoubleTuple> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull DoubleDoubleConsumer consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<DoubleDoubleTuple> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull DoubleDoubleToBooleanFunction function)    {
        this.concurrentIterate(function);
    }
}
