...
import net.daporkchop.lib.primitive.tuple.ObjectLongImmutableTuple;
...
/**
 * A hash map, using a key type of K and a value type of long.
 ...
 * @author DaPorkchop_
 */
public class ObjectLongHashMap<K extends Object> implements ObjectLongMap<K>    {
    ...
    public ObjectLongHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public ObjectLongHashMap(ObjectToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public ObjectLongHashMap(int baseSize, ObjectToIntegerFunction keyHash)    {
    ...
    protected boolean reHash(long[] values, Object[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
            K key = (K) this.keys[i];
            int j = getIndex(key);
            if (states.get(j))  {
    ...
    private class KeyIterator implements ObjectIterator<K> {
        ...
        public KeyIterator()    {
            this.size = ObjectLongHashMap.this.size;
            this.next = ObjectLongHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public K get()   {
            return (K) ObjectLongHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = ObjectLongHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            ObjectLongHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements LongIterator {
        ...
        @SuppressWarnings("unchecked")
        public long get()   {
            return ObjectLongHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements ObjectLongIterator<K> {
        private int size;
        private int index = -1;
        private int next;
        private ObjectLongTuple<K> tuple;
        ...
        @Override
        public ObjectLongTuple<K> advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new ObjectLongImmutableTuple<K>( (K) ObjectLongHashMap.this.keys[this.index],  ObjectLongHashMap.this.values[this.index]);
        }
