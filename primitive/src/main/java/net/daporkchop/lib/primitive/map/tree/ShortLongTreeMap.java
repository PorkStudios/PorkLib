...
/**
 * A tree map, using a key type of short and a value type of long.
 ...
 * Although this map 𝙒𝙊𝙍𝙆𝙎, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.ShortLongIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class ShortLongTreeMap implements ShortLongMap    {
    ...
    @SuppressWarnings("unchecked")
    protected long remove(short key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return 0L; //no mapping
    ...
    public ShortIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.ShortLongIndexedTreeMap} for an implementation of this method
        return null;
    }
