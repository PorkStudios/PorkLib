...
import net.daporkchop.lib.primitive.lambda.consumer.bi.DoubleIntegerConsumer;
...
import net.daporkchop.lib.primitive.iterator.concurrent.DoubleIntegerConcurrentIterator;
import net.daporkchop.lib.primitive.map.DoubleIntegerMap;
import net.daporkchop.lib.primitive.tuple.DoubleIntegerTuple;
import net.daporkchop.lib.primitive.tuple.DoubleIntegerImmutableTuple;
import net.daporkchop.lib.primitive.tuple.DoubleIntegerMutableTuple;
...
/**
 * A tree hash map, using a key type of double and a value type of int.
 ...
 * @author DaPorkchop_
 */
public class DoubleIntegerConcurrentTreeHashMap implements DoubleIntegerMap    {
    final AtomicInteger size = new AtomicInteger(0);

    private final DoubleIntegerSubEntry[] entries = new DoubleIntegerSubEntry[256];
    private final DoubleToIntegerFunction keyHash;
    final DoubleToLongFunction subKeyHash;
    final int subHashLength;

    public DoubleIntegerConcurrentTreeHashMap()    {
        this(null, null);
    }

    public DoubleIntegerConcurrentTreeHashMap(DoubleToIntegerFunction keyHash, DoubleToLongFunction subKeyHash)    {
        this(keyHash, subKeyHash, 7);
    }

    public DoubleIntegerConcurrentTreeHashMap(DoubleToIntegerFunction keyHash, DoubleToLongFunction subKeyHash, int subHashLength)    {
    ...
    public int get(double key)   {
        DoubleIntegerSubEntry entry = this.getSubentry(this.keyHash.apply(key) & 0xFF, false);
        if (entry == null)  {
            return 0;
        } else {
    ...
    public int put(double key, int value)   {
        DoubleIntegerSubEntry entry = this.getSubentry(this.keyHash.apply(key) & 0xFF, true);
    ...
    public int remove(double key)    {
        ...
        DoubleIntegerSubEntry entry = this.getSubentry(hash, false);
        if (entry == null)  {
            return 0;
        } else {
    ...
    public void clear() {
        synchronized (this.entries) {
            for (int i = 0; i < 256; i++) {
                DoubleIntegerSubEntry entry = this.entries[i];
                if (entry != null)  {
                    entry.clear();
                    this.entries[i] = null;
                }
    ...
    public void forEachKey(DoubleConsumer consumer)   {
        for (int i = 0; i < 256; i++) {
            DoubleIntegerSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
    ...
    @Override
    public void forEachEntry(DoubleIntegerConsumer consumer) {
        for (int i = 0; i < 256; i++) {
            DoubleIntegerSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
    ...
    public void forEachEntry(DoubleIntegerConsumer consumer, VoidFunction complete) {
        for (int i = 0; i < 256; i++) {
            DoubleIntegerSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
                entry.forEachEntry(consumer);
    ...
    public DoubleConcurrentIterator keyIterator()   {
        return new DoubleConcurrentIterator()  {
            private final DoubleIntegerConcurrentTreeHashMap this_ = DoubleIntegerConcurrentTreeHashMap.this;
            ...
            private final AtomicReference<DoubleIntegerSubEntry.Sector> currSector = new AtomicReference(null);
            ...
            private volatile DoubleIntegerSubEntry entry;
            ...
            private void nextSub()   {
                ...
                while (this.subEntryIndex < 256)    {
                    DoubleIntegerSubEntry entry = getSubentry(this.subEntryIndex++, false);
                    if (entry != null)  {
                        this.entry = entry;
            ...
            class ThreadState   {
                public double key;
                public DoubleIntegerSubEntry entry;
                private final DoubleConcurrentIterator this_;
                private final DoubleIntegerConcurrentTreeHashMap this__;

                public ThreadState(DoubleConcurrentIterator this_, DoubleIntegerConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
    ...
    public IntegerConcurrentIterator valueIterator() {
        return new IntegerConcurrentIterator()  {
            ...
            private final AtomicReference<DoubleIntegerSubEntry.Sector> currSector = new AtomicReference(null);
            private final AtomicBoolean hasNextSub = new AtomicBoolean(true);
            private final DoubleIntegerMutableTuple next = new DoubleIntegerMutableTuple();
            private volatile boolean hasNext = true;
            private volatile int subEntryIndex = 0;
            private volatile DoubleIntegerSubEntry entry;
            ...
            class ThreadState   {
                ...
                public ThreadState(IntegerConcurrentIterator this_, DoubleIntegerConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
    ...
    @Override
    public DoubleIntegerConcurrentIterator entryIterator() {
        return new DoubleIntegerConcurrentIterator()  {
            private final DoubleIntegerConcurrentTreeHashMap this_ = DoubleIntegerConcurrentTreeHashMap.this;
            //private final ReentrantLock lock = new ReentrantLock();
            private final ThreadLocal<ThreadState> tl = ThreadLocal.withInitial(() -> new ThreadState(this, this.this_));
            private final AtomicInteger sectorOffset = new AtomicInteger(0);
            ...
            @Override
            public synchronized DoubleIntegerTuple advance()   {
                if (!this.hasNext())    {
                    throw new IteratorCompleteException();
                }
                ThreadState state = this.tl.get();
                DoubleIntegerTuple current = state.tuple = new DoubleIntegerMutableTuple(this.next.k, this.next.v);
                state.entry = this.entry;
                this.findNextValue();
                return current;
            }
            ...
            class ThreadState   {
                public DoubleIntegerTuple tuple;
                public DoubleIntegerSubEntry entry;
                private final DoubleIntegerConcurrentIterator this_;
                private final DoubleIntegerConcurrentTreeHashMap this__;

                public ThreadState(DoubleIntegerConcurrentIterator this_, DoubleIntegerConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
     ...
     */
    public DoubleIntegerConcurrentIterator fastEntryIterator() {
        return new DoubleIntegerConcurrentIterator()  {
            private final DoubleIntegerConcurrentTreeHashMap this_ = DoubleIntegerConcurrentTreeHashMap.this;
            //private final ReentrantLock lock = new ReentrantLock();
            private final ThreadLocal<ThreadState> tl = ThreadLocal.withInitial(() -> new ThreadState(this, this.this_));
            ...
            private final AtomicReference<DoubleIntegerSubEntry.Sector> currSector = new AtomicReference(null);
            private final AtomicBoolean hasNextSub = new AtomicBoolean(true);
            private volatile DoubleIntegerMutableTuple next = new DoubleIntegerMutableTuple(0D, 0);
            private volatile boolean hasNext = true;
            private volatile int subEntryIndex = 0;
            private volatile DoubleIntegerSubEntry entry;
            ...
            @Override
            public DoubleIntegerMutableTuple get()   {
                ThreadState state = this.tl.get();
                if (state.entry == null)    {
                    throw new IllegalStateException();
                } else {
            ...
            @Override
            public synchronized DoubleIntegerMutableTuple advance()   {
                if (!this.hasNext())    {
                    throw new IteratorCompleteException();
                }
                ThreadState state = this.tl.get();
                DoubleIntegerMutableTuple a = state.tuple;
            ...
            class ThreadState   {
                public DoubleIntegerMutableTuple tuple = new DoubleIntegerMutableTuple(0D, 0);
                public DoubleIntegerSubEntry entry;
                private final DoubleIntegerConcurrentIterator this_;
                private final DoubleIntegerConcurrentTreeHashMap this__;
    ...
    private DoubleIntegerSubEntry getSubentry(int index, boolean create)    {
        assert index == (index & 0xFF);

        synchronized (this.entries) {
            DoubleIntegerSubEntry entry = (DoubleIntegerSubEntry) this.entries[index];
            if (entry == null && create)    {
                this.entries[index] = entry = new DoubleIntegerSubEntry(this);
            }
            return entry;
        }
    }
