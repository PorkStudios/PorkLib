 ...
 * Although this map 𝙒𝙊𝙍𝙆𝙎, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.ByteByteIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class ByteByteTreeMap implements ByteByteMap    {
    protected byte[] keys;
    ...
    @SuppressWarnings("unchecked")
    protected byte remove(byte key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return (byte) 0; //no mapping
    ...
    public ByteIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.ByteByteIndexedTreeMap} for an implementation of this method
        return null;
    }
