 ...
 * Although this map 𝙒𝙊𝙍𝙆𝙎, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.FloatObjectIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class FloatObjectTreeMap<V extends Object> implements FloatObjectMap<V>    {
    ...
    @SuppressWarnings("unchecked")
    protected V remove(float key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return null; //no mapping
    ...
    public FloatIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.FloatObjectIndexedTreeMap} for an implementation of this method
        return null;
    }
