 ...
 * overhead, and can even be made concurrent with little extra work (see {@link net.daporkchop.lib.primitive.map.concurrent.LongObjectConcurrentTreeHashMap})
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
public class LongObjectTreeHashMap<V extends Object> implements LongObjectMap<V>    {
    ...
    public LongObjectTreeHashMap()    {
        this(null, 0);
    }

    public LongObjectTreeHashMap(LongToLongFunction hashFunc, int hashLen)    {
        ...
        this.rootPointer = this.hashLen == 0 ? new LongObjectTreeHashMap<V>.ValueSector(null, -1) : new LongObjectTreeHashMap<V>.PointerSector(null, -1);
    }
    ...
    @Override
    public void clear() {
        this.rootPointer = this.hashLen == 0 ? new LongObjectTreeHashMap.ValueSector(null, -1) : new LongObjectTreeHashMap.PointerSector(null, -1);
    }
    ...
    public void forEachKey(@NonNull LongConsumer consumer)  {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof LongObjectTreeHashMap.PointerSector)    {
    ...
    private Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.hashLen; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.hashLen ? new LongObjectTreeHashMap<V>.ValueSector(curr, off) : new LongObjectTreeHashMap<V>.PointerSector(curr, off);
    ...
    private class ValueSector extends Sector   {
        ...
        public Object setEntry(int index, long key, Object value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                LongObjectTreeHashMap.this.size.incrementAndGet();
                return null;
            }
        }
        ...
        public Object removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                LongObjectTreeHashMap.this.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
