...
/**
 * A single subentry for a {@link FloatBooleanConcurrentTreeHashMap}.
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
class FloatBooleanSubEntry    {
    public final AtomicInteger size = new AtomicInteger(0);
    public final FloatBooleanConcurrentTreeHashMap parent;
    ...
    public FloatBooleanSubEntry(FloatBooleanConcurrentTreeHashMap parent)   {
        this.parent = parent;
        this.rootPointer = this.parent.subHashLength == 0 ? new FloatBooleanSubEntry.ValueSector(null, -1) : new FloatBooleanSubEntry.PointerSector(null, -1);
    }
    ...
    public void forEachKey(FloatConsumer consumer)   {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof FloatBooleanSubEntry.PointerSector)    {
    ...
    public float advanceKey(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext)   {
        ...
        for (int i = sector.states.nextSetBit(curr.get() + 1); i != -1; i = sector.states.nextSetBit(i + 1)) {
            curr.set(i);
            if (sector instanceof FloatBooleanSubEntry.ValueSector)  {
    ...
    public void advanceEntry(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext, FloatBooleanMutableTuple tuple)   {
        Sector sector = sectorRef.get();
        if (sector == null) {
            sectorRef.set(sector = this.rootPointer);
        }
    ...
    public Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.parent.subHashLength; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.parent.subHashLength ? new FloatBooleanSubEntry.ValueSector(curr, off) : new FloatBooleanSubEntry.PointerSector(curr, off);
    ...
    public class ValueSector extends Sector   {
        ...
        public boolean setEntry(int index, float key, boolean value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                FloatBooleanSubEntry.this.size.incrementAndGet();
                FloatBooleanSubEntry.this.parent.size.incrementAndGet();
                return false;
            }
        }
        ...
        public boolean removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                FloatBooleanSubEntry.this.size.decrementAndGet();
                FloatBooleanSubEntry.this.parent.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
