...
import net.daporkchop.lib.primitive.iterator.bi.IntegerBooleanIterator;
...
/**
 * A hash map, using a key type of int and a value type of boolean.
 ...
 * @author DaPorkchop_
 */
public class IntegerBooleanHashMap implements IntegerBooleanMap    {
    protected int[] keys;
    ...
    public IntegerBooleanHashMap()    {
        this(16384);
    }

    public IntegerBooleanHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public IntegerBooleanHashMap(IntegerToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public IntegerBooleanHashMap(int baseSize, IntegerToIntegerFunction keyHash)    {
    ...
    @Override
    public void forEachEntry(@NonNull IntegerBooleanConsumer consumer)   {
    ...
    public IntegerBooleanIterator entryIterator() {
    ...
    protected boolean reHash(boolean[] values, int[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
            int key = this.keys[i];
            int j = getIndex(key);
            if (states.get(j))  {
    ...
    private class KeyIterator implements IntegerIterator {
        ...
        public KeyIterator()    {
            this.size = IntegerBooleanHashMap.this.size;
            this.next = IntegerBooleanHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public int get()   {
            return IntegerBooleanHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = IntegerBooleanHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            IntegerBooleanHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements BooleanIterator {
        ...
        @SuppressWarnings("unchecked")
        public boolean get()   {
            return IntegerBooleanHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements IntegerBooleanIterator {
        private int size;
        private int index = -1;
        private int next;
        private IntegerBooleanTuple tuple;
        ...
        @Override
        public IntegerBooleanTuple advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new IntegerBooleanImmutableTuple( IntegerBooleanHashMap.this.keys[this.index],  IntegerBooleanHashMap.this.values[this.index]);
        }
