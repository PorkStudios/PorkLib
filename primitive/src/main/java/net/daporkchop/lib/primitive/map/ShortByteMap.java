...
import net.daporkchop.lib.primitive.lambda.function.ShortToByteFunction;
...
/**
 * A map, using a key type of short and a value type of byte.
 * <p>
...
public interface ShortByteMap   {
     ...
     * @return the element mapped to the given key, or (byte) 0 if no such mapping exists
     */
    byte get(short key);
     ...
     * @return the element that was previously mapped to the given key, or (byte) 0 if no such mapping existed
     */
     ...
     */
    default void putAll(@NonNull short[] keys, @NonNull byte[] values)    {
     ...
     */
    default void putAll(@NonNull ShortByteMap map)    {
     ...
     */
    default void removeAll(@NonNull Collection<Short> keys) {
     ...
     */
    void forEachKey(ShortConsumer consumer);
     ...
     * @param consumer the function to run
     */
    void forEachValue(ByteConsumer consumer);
     ...
     * @param consumer the function to run
     */
    void forEachEntry(ShortByteConsumer consumer);
     ...
     */
    ShortByteIterator entryIterator();
     ...
     *
     * @return the previous value ((byte) 0 if not present previously)
     */
     ...
     */
    default byte getOrDefault(short key, byte def)   {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
            this.put(key, def);
     ...
     */
    default byte computeIfAbsent(short key, @NonNull ShortToByteFunction func) {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
            byte val = func.apply(key);
            this.put(key, val);
            return val;
        }
    }
