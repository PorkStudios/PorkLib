...
import net.daporkchop.lib.primitive.tuple.ByteObjectImmutableTuple;
...
/**
 * A hash map, using a key type of byte and a value type of V.
...
public class ByteObjectHashMap<V extends Object> implements ByteObjectMap<V>    {
    ...
    public ByteObjectHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public ByteObjectHashMap(ByteToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public ByteObjectHashMap(int baseSize, ByteToIntegerFunction keyHash)    {
    ...
    protected boolean reHash(Object[] values, byte[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
    ...
    private class KeyIterator implements ByteIterator {
        ...
        public KeyIterator()    {
            this.size = ByteObjectHashMap.this.size;
            this.next = ByteObjectHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public byte get()   {
            return ByteObjectHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = ByteObjectHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            ByteObjectHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements ObjectIterator<V> {
        ...
        @SuppressWarnings("unchecked")
        public V get()   {
            return (V) ByteObjectHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements ByteObjectIterator<V> {
        private int size;
        private int index = -1;
        private int next;
        private ByteObjectTuple<V> tuple;
        ...
        @Override
        public ByteObjectTuple<V> advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new ByteObjectImmutableTuple<V>( ByteObjectHashMap.this.keys[this.index],  (V) ByteObjectHashMap.this.values[this.index]);
        }
