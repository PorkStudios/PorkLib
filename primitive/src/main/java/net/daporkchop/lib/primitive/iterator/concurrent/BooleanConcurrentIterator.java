...
/**
 * Allows parallel iteration over an iterator of boolean values. Implementing classes should be thread safe
 * and concurrent (calling advance() should throw {@link IteratorCompleteException} instead of false,
 ...
 * @author DaPorkchop_
 */
public interface BooleanConcurrentIterator extends BooleanIterator   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull BooleanToBooleanFunction function)    {
        try {
            while (this.hasNext())  {
                boolean val = this.advance();
    ...
    @Override
    default void forEachRemaining(@NonNull BooleanConsumer consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull BooleanToBooleanFunction function)   {
