...
/**
 * A simple, fast map to map PorkLib's {@link Reliability} to the reliability enums/classes provided by other libraries
 *
 * @author DaPorkchop_
 */
public class ReliabilityMap<T> {
    ...
    public ReliabilityMap(@NonNull Class<T> clazz) {
        this.values = (T[]) Array.newInstance(clazz, Reliability.values().length);
        this.reverseLookup = new HashMap<>();
    }

    public ReliabilityMap<T> put(@NonNull Reliability pork, @NonNull T external) {
        this.values[pork.ordinal()] = external;
        return this;
    }

    public T get(@NonNull Reliability pork) {
        return this.values[pork.ordinal()];
    }

    public Reliability get(@NonNull T external) {
        return this.reverseLookup.get(external);
    }
}
