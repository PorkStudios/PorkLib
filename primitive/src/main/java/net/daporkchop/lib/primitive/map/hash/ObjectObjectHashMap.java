...
/**
 * A hash map, using a key type of K and a value type of V.
 ...
 * @author DaPorkchop_
 */
public class ObjectObjectHashMap<K extends Object, V extends Object> implements ObjectObjectMap<K, V>    {
    ...
    public ObjectObjectHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public ObjectObjectHashMap(ObjectToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public ObjectObjectHashMap(int baseSize, ObjectToIntegerFunction keyHash)    {
    ...
    @Override
    public void forEachEntry(@NonNull ObjectObjectConsumer<K, V> consumer)   {
        this.forEach(i -> consumer.accept( (K) this.keys[i],  (V) this.values[i]));
    }
    ...
    public ObjectObjectIterator<K, V> entryIterator() {
    ...
    protected boolean reHash(Object[] values, Object[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
            K key = (K) this.keys[i];
            int j = getIndex(key);
            if (states.get(j))  {
    ...
    private class KeyIterator implements ObjectIterator<K> {
        ...
        public KeyIterator()    {
            this.size = ObjectObjectHashMap.this.size;
            this.next = ObjectObjectHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public K get()   {
            return (K) ObjectObjectHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = ObjectObjectHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            ObjectObjectHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements ObjectIterator<V> {
        ...
        @SuppressWarnings("unchecked")
        public V get()   {
            return (V) ObjectObjectHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements ObjectObjectIterator<K, V> {
        private int size;
        private int index = -1;
        private int next;
        private ObjectObjectTuple<K, V> tuple;
        ...
        @Override
        public ObjectObjectTuple<K, V> advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new ObjectObjectImmutableTuple<K, V>( (K) ObjectObjectHashMap.this.keys[this.index],  (V) ObjectObjectHashMap.this.values[this.index]);
        }
