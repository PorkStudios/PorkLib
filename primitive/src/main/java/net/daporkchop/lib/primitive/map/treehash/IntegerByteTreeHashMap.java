 ...
 * overhead, and can even be made concurrent with little extra work (see {@link net.daporkchop.lib.primitive.map.concurrent.IntegerByteConcurrentTreeHashMap})
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
public class IntegerByteTreeHashMap implements IntegerByteMap    {
    ...
    public IntegerByteTreeHashMap()    {
        this(null, 0);
    }

    public IntegerByteTreeHashMap(IntegerToLongFunction hashFunc, int hashLen)    {
        ...
        this.rootPointer = this.hashLen == 0 ? new IntegerByteTreeHashMap.ValueSector(null, -1) : new IntegerByteTreeHashMap.PointerSector(null, -1);
    }
    ...
    public void forEachKey(@NonNull IntegerConsumer consumer)  {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof IntegerByteTreeHashMap.PointerSector)    {
    ...
    private Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.hashLen; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.hashLen ? new IntegerByteTreeHashMap.ValueSector(curr, off) : new IntegerByteTreeHashMap.PointerSector(curr, off);
    ...
    private class ValueSector extends Sector   {
        ...
        public byte setEntry(int index, int key, byte value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                IntegerByteTreeHashMap.this.size.incrementAndGet();
                return (byte) 0;
            }
        }
        ...
        public byte removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                IntegerByteTreeHashMap.this.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
