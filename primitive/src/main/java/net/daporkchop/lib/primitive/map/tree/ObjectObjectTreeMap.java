 ...
 * Although this map WORKS, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.ObjectObjectIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class ObjectObjectTreeMap<K extends Object, V extends Object> implements ObjectObjectMap<K, V>    {
    ...
    @SuppressWarnings("unchecked")
    protected V remove(K key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return null; //no mapping
            ...
            for (int x = 0; x < 256; x++)   {
                ...
                for (int y = 0; y < 256; y++)   {
                    ...
                    for (int z = 0; z < 256; z++)   {
                        ...
                        if (d < 0)  {
                            d = d * -1 - 1;
                            consumer.accept( (K) keys[d], (V) values[d]);
                        }
                    }
                }
            }
    ...
    public ObjectIterator<K> keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.ObjectObjectIndexedTreeMap} for an implementation of this method
        return null;
    }
