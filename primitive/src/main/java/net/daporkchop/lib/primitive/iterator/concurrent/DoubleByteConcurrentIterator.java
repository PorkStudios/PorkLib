...
import net.daporkchop.lib.primitive.lambda.function.bi.DoubleByteToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of double => byte mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface DoubleByteConcurrentIterator extends DoubleByteIterator   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull DoubleByteToBooleanFunction function)    {
        try {
            while (this.hasNext())  {
                DoubleByteTuple tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<DoubleByteTuple> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<DoubleByteTuple> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull DoubleByteConsumer consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<DoubleByteTuple> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull DoubleByteToBooleanFunction function)    {
        this.concurrentIterate(function);
    }
}
