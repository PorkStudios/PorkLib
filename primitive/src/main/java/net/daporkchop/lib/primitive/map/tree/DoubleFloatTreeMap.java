...
/**
 * A tree map, using a key type of double and a value type of float.
 ...
 * Although this map 𝙒𝙊𝙍𝙆𝙎, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.DoubleFloatIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class DoubleFloatTreeMap implements DoubleFloatMap    {
    ...
    @SuppressWarnings("unchecked")
    protected float remove(double key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return 0F; //no mapping
    ...
    public DoubleIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.DoubleFloatIndexedTreeMap} for an implementation of this method
        return null;
    }
