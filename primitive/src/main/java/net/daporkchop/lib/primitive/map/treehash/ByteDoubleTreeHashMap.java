 ...
 * overhead, and can even be made concurrent with little extra work (see {@link net.daporkchop.lib.primitive.map.concurrent.ByteDoubleConcurrentTreeHashMap})
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
public class ByteDoubleTreeHashMap implements ByteDoubleMap    {
    ...
    public ByteDoubleTreeHashMap()    {
        this(null, 0);
    }

    public ByteDoubleTreeHashMap(ByteToLongFunction hashFunc, int hashLen)    {
        ...
        this.rootPointer = this.hashLen == 0 ? new ByteDoubleTreeHashMap.ValueSector(null, -1) : new ByteDoubleTreeHashMap.PointerSector(null, -1);
    }
    ...
    public void forEachKey(@NonNull ByteConsumer consumer)  {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof ByteDoubleTreeHashMap.PointerSector)    {
    ...
    private Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.hashLen; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.hashLen ? new ByteDoubleTreeHashMap.ValueSector(curr, off) : new ByteDoubleTreeHashMap.PointerSector(curr, off);
    ...
    private class ValueSector extends Sector   {
        ...
        public double setEntry(int index, byte key, double value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                ByteDoubleTreeHashMap.this.size.incrementAndGet();
                return 0D;
            }
        }
        ...
        public double removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                ByteDoubleTreeHashMap.this.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
