 ...
 * Although this map WORKS, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.ShortCharacterIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class ShortCharacterTreeMap implements ShortCharacterMap    {
    ...
    @SuppressWarnings("unchecked")
    protected char remove(short key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return (char) 0; //no mapping
    ...
    public ShortIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.ShortCharacterIndexedTreeMap} for an implementation of this method
        return null;
    }
