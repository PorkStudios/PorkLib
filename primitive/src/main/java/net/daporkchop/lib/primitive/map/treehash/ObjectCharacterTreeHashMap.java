 ...
 * overhead, and can even be made concurrent with little extra work (see {@link net.daporkchop.lib.primitive.map.concurrent.ObjectCharacterConcurrentTreeHashMap})
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
public class ObjectCharacterTreeHashMap<K extends Object> implements ObjectCharacterMap<K>    {
    ...
    public ObjectCharacterTreeHashMap()    {
        this(null, 0);
    }

    public ObjectCharacterTreeHashMap(ObjectToLongFunction hashFunc, int hashLen)    {
        ...
        this.rootPointer = this.hashLen == 0 ? new ObjectCharacterTreeHashMap<K>.ValueSector(null, -1) : new ObjectCharacterTreeHashMap<K>.PointerSector(null, -1);
    }
    ...
    @Override
    public void clear() {
        this.rootPointer = this.hashLen == 0 ? new ObjectCharacterTreeHashMap.ValueSector(null, -1) : new ObjectCharacterTreeHashMap.PointerSector(null, -1);
    }
    ...
    public void forEachKey(@NonNull ObjectConsumer<K> consumer)  {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof ObjectCharacterTreeHashMap.PointerSector)    {
    ...
    private Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.hashLen; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.hashLen ? new ObjectCharacterTreeHashMap<K>.ValueSector(curr, off) : new ObjectCharacterTreeHashMap<K>.PointerSector(curr, off);
    ...
    private class ValueSector extends Sector   {
        ...
        public char setEntry(int index, Object key, char value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                ObjectCharacterTreeHashMap.this.size.incrementAndGet();
                return (char) 0;
            }
        }
        ...
        public char removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                ObjectCharacterTreeHashMap.this.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
