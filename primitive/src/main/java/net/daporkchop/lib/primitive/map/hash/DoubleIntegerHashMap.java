...
import net.daporkchop.lib.primitive.iterator.bi.DoubleIntegerIterator;
...
/**
 * A hash map, using a key type of double and a value type of int.
 ...
 * @author DaPorkchop_
 */
public class DoubleIntegerHashMap implements DoubleIntegerMap    {
    ...
    public DoubleIntegerHashMap()    {
        this(16384);
    }

    public DoubleIntegerHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public DoubleIntegerHashMap(DoubleToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public DoubleIntegerHashMap(int baseSize, DoubleToIntegerFunction keyHash)    {
    ...
    @Override
    public void forEachEntry(@NonNull DoubleIntegerConsumer consumer)   {
    ...
    public DoubleIntegerIterator entryIterator() {
    ...
    protected boolean reHash(int[] values, double[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
    ...
    private class KeyIterator implements DoubleIterator {
        ...
        public KeyIterator()    {
            this.size = DoubleIntegerHashMap.this.size;
            this.next = DoubleIntegerHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public double get()   {
            return DoubleIntegerHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = DoubleIntegerHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            DoubleIntegerHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements IntegerIterator {
        ...
        @SuppressWarnings("unchecked")
        public int get()   {
            return DoubleIntegerHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements DoubleIntegerIterator {
        private int size;
        private int index = -1;
        private int next;
        private DoubleIntegerTuple tuple;
        ...
        @Override
        public DoubleIntegerTuple advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new DoubleIntegerImmutableTuple( DoubleIntegerHashMap.this.keys[this.index],  DoubleIntegerHashMap.this.values[this.index]);
        }
