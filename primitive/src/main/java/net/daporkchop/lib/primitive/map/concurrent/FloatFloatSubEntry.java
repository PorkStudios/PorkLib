...
/**
 * A single subentry for a {@link FloatFloatConcurrentTreeHashMap}.
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
class FloatFloatSubEntry    {
    public final AtomicInteger size = new AtomicInteger(0);
    public final FloatFloatConcurrentTreeHashMap parent;
    ...
    public FloatFloatSubEntry(FloatFloatConcurrentTreeHashMap parent)   {
        this.parent = parent;
        this.rootPointer = this.parent.subHashLength == 0 ? new FloatFloatSubEntry.ValueSector(null, -1) : new FloatFloatSubEntry.PointerSector(null, -1);
    }
    ...
    public void forEachKey(FloatConsumer consumer)   {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof FloatFloatSubEntry.PointerSector)    {
    ...
    public float advanceKey(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext)   {
        ...
        for (int i = sector.states.nextSetBit(curr.get() + 1); i != -1; i = sector.states.nextSetBit(i + 1)) {
            curr.set(i);
            if (sector instanceof FloatFloatSubEntry.ValueSector)  {
    ...
    public void advanceEntry(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext, FloatFloatMutableTuple tuple)   {
        Sector sector = sectorRef.get();
        if (sector == null) {
            sectorRef.set(sector = this.rootPointer);
        }
    ...
    public Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.parent.subHashLength; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.parent.subHashLength ? new FloatFloatSubEntry.ValueSector(curr, off) : new FloatFloatSubEntry.PointerSector(curr, off);
    ...
    public abstract class Sector  {
        ...
        public float setEntry(int index, float key, float value)   {
            throw new UnsupportedOperationException();
        }
    ...
    public class ValueSector extends Sector   {
        ...
        public float setEntry(int index, float key, float value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                FloatFloatSubEntry.this.size.incrementAndGet();
                FloatFloatSubEntry.this.parent.size.incrementAndGet();
                return 0F;
            }
        }
        ...
        public float removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                FloatFloatSubEntry.this.size.decrementAndGet();
                FloatFloatSubEntry.this.parent.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
