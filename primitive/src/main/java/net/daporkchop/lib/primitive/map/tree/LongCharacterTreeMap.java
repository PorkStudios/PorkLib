 ...
 * Although this map 𝙒𝙊𝙍𝙆𝙎, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.LongCharacterIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class LongCharacterTreeMap implements LongCharacterMap    {
    protected long[] keys;
    ...
    @SuppressWarnings("unchecked")
    protected char remove(long key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return (char) 0; //no mapping
    ...
    public LongIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.LongCharacterIndexedTreeMap} for an implementation of this method
        return null;
    }
