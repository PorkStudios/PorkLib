...
namespace porklib::jni {
    ...
    [[gnu::cold, noreturn]] void throwAlreadyThrownJniException(JNIEnv*) {
        throw AlreadyThrownJniException{};
    }
    ...
    [[gnu::cold]] void handleCppExceptionTail(JNIEnv* env) noexcept {
        try {
            if (std::exception_ptr e = std::current_exception()) {
                ...
                //throwNewJniException(env, java_lang_AssertionError, "porklib::jni::handleCppExceptionTail() was called, but there is no current exception???");
            ...
            throwNewJniException(env, java_lang_OutOfMemoryError, e.what());
        } catch (const std::exception& e) {
            //throw an Exception
            throwNewJniException(env, java_lang_Exception, e.what());
