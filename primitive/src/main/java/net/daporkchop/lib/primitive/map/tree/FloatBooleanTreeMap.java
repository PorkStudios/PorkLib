...
/**
 * A tree map, using a key type of float and a value type of boolean.
 ...
 * Although this map WORKS, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.FloatBooleanIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class FloatBooleanTreeMap implements FloatBooleanMap    {
    ...
    @SuppressWarnings("unchecked")
    protected boolean remove(float key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return false; //no mapping
    ...
    public FloatIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.FloatBooleanIndexedTreeMap} for an implementation of this method
        return null;
    }
