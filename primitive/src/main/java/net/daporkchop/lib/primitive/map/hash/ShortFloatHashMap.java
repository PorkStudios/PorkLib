...
/**
 * A hash map, using a key type of short and a value type of float.
 ...
 * @author DaPorkchop_
 */
public class ShortFloatHashMap implements ShortFloatMap    {
    ...
    public ShortFloatHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public ShortFloatHashMap(ShortToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public ShortFloatHashMap(int baseSize, ShortToIntegerFunction keyHash)    {
    ...
    protected boolean reHash(float[] values, short[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
    ...
    private class KeyIterator implements ShortIterator {
        ...
        public KeyIterator()    {
            this.size = ShortFloatHashMap.this.size;
            this.next = ShortFloatHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public short get()   {
            return ShortFloatHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = ShortFloatHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            ShortFloatHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements FloatIterator {
        ...
        @SuppressWarnings("unchecked")
        public float get()   {
            return ShortFloatHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements ShortFloatIterator {
        private int size;
        private int index = -1;
        private int next;
        private ShortFloatTuple tuple;
        ...
        @Override
        public ShortFloatTuple advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new ShortFloatImmutableTuple( ShortFloatHashMap.this.keys[this.index],  ShortFloatHashMap.this.values[this.index]);
        }
