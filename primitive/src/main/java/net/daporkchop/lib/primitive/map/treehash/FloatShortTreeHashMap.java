 ...
 * overhead, and can even be made concurrent with little extra work (see {@link net.daporkchop.lib.primitive.map.concurrent.FloatShortConcurrentTreeHashMap})
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
public class FloatShortTreeHashMap implements FloatShortMap    {
    ...
    public FloatShortTreeHashMap()    {
        this(null, 0);
    }

    public FloatShortTreeHashMap(FloatToLongFunction hashFunc, int hashLen)    {
        ...
        this.rootPointer = this.hashLen == 0 ? new FloatShortTreeHashMap.ValueSector(null, -1) : new FloatShortTreeHashMap.PointerSector(null, -1);
    }
    ...
    public void forEachKey(@NonNull FloatConsumer consumer)  {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof FloatShortTreeHashMap.PointerSector)    {
    ...
    private Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.hashLen; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.hashLen ? new FloatShortTreeHashMap.ValueSector(curr, off) : new FloatShortTreeHashMap.PointerSector(curr, off);
    ...
    private class ValueSector extends Sector   {
        ...
        public short setEntry(int index, float key, short value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                FloatShortTreeHashMap.this.size.incrementAndGet();
                return (short) 0;
            }
        }
        ...
        public short removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                FloatShortTreeHashMap.this.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
