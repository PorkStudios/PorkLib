...
/**
 * A hash map, using a key type of float and a value type of V.
 ...
 * @author DaPorkchop_
 */
public class FloatObjectHashMap<V extends Object> implements FloatObjectMap<V>    {
    ...
    public FloatObjectHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public FloatObjectHashMap(FloatToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public FloatObjectHashMap(int baseSize, FloatToIntegerFunction keyHash)    {
    ...
    protected boolean reHash(Object[] values, float[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
    ...
    private class KeyIterator implements FloatIterator {
        ...
        public KeyIterator()    {
            this.size = FloatObjectHashMap.this.size;
            this.next = FloatObjectHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public float get()   {
            return FloatObjectHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = FloatObjectHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            FloatObjectHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements ObjectIterator<V> {
        ...
        @SuppressWarnings("unchecked")
        public V get()   {
            return (V) FloatObjectHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements FloatObjectIterator<V> {
        private int size;
        private int index = -1;
        private int next;
        private FloatObjectTuple<V> tuple;
        ...
        @Override
        public FloatObjectTuple<V> advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new FloatObjectImmutableTuple<V>( FloatObjectHashMap.this.keys[this.index],  (V) FloatObjectHashMap.this.values[this.index]);
        }
