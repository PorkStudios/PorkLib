...
/**
 * Allows parallel iteration over an iterator of char values. Implementing classes should be thread safe
 * and concurrent (calling advance() should throw {@link IteratorCompleteException} instead of (char) 0,
 ...
 * @author DaPorkchop_
 */
public interface CharacterConcurrentIterator extends CharacterIterator   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull CharacterToBooleanFunction function)    {
        try {
            while (this.hasNext())  {
                char val = this.advance();
    ...
    @Override
    default void forEachRemaining(@NonNull CharacterConsumer consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull CharacterToBooleanFunction function)   {
