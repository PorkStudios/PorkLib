...
public class XTEAKeyGen {
    static {
        BouncyCastleInit.loadClass();
    }

    public static XTEAKey gen(byte[] data) {
        byte[] b = new byte[16];
        int i = 0;
        do {
        ...
        data = b;
        return new XTEAKey(new SecretKeySpec(data, "aaa"), KeyRandom.getBytes(BlockCipherType.XTEA.blockSize));
    }

    public static XTEAKey gen() {
