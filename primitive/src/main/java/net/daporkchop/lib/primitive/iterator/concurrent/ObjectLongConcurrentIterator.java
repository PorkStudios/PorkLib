...
import net.daporkchop.lib.primitive.lambda.function.bi.ObjectLongToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of K => long mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface ObjectLongConcurrentIterator<K extends Object> extends ObjectLongIterator<K>   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectLongToBooleanFunction<K> function)    {
        try {
            while (this.hasNext())  {
                ObjectLongTuple<K> tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<ObjectLongTuple<K>> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<ObjectLongTuple<K>> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectLongConsumer<K> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<ObjectLongTuple<K>> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull ObjectLongToBooleanFunction<K> function)    {
        this.concurrentIterate(function);
    }
}
