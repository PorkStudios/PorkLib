...
import net.daporkchop.lib.primitive.iterator.ShortIterator;
import net.daporkchop.lib.primitive.iterator.bi.ShortShortIterator;
import net.daporkchop.lib.primitive.map.ShortShortMap;
import net.daporkchop.lib.primitive.tuple.ShortShortTuple;
import net.daporkchop.lib.primitive.tuple.ShortShortImmutableTuple;
...
/**
 * A hash map, using a key type of short and a value type of short.
...
public class ShortShortHashMap implements ShortShortMap    {
    ...
    public ShortShortHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public ShortShortHashMap(ShortToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public ShortShortHashMap(int baseSize, ShortToIntegerFunction keyHash)    {
    ...
    @Override
    public void forEachEntry(@NonNull ShortShortConsumer consumer)   {
    ...
    public ShortShortIterator entryIterator() {
    ...
    protected boolean reHash(short[] values, short[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
    ...
    private class KeyIterator implements ShortIterator {
        ...
        public KeyIterator()    {
            this.size = ShortShortHashMap.this.size;
            this.next = ShortShortHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public short get()   {
            return ShortShortHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = ShortShortHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            ShortShortHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements ShortIterator {
        ...
        @SuppressWarnings("unchecked")
        public short get()   {
            return ShortShortHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements ShortShortIterator {
        private int size;
        private int index = -1;
        private int next;
        private ShortShortTuple tuple;
        ...
        @Override
        public ShortShortTuple advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new ShortShortImmutableTuple( ShortShortHashMap.this.keys[this.index],  ShortShortHashMap.this.values[this.index]);
        }
