...
import net.daporkchop.lib.primitive.lambda.function.bi.CharacterByteToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of char => byte mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface CharacterByteConcurrentIterator extends CharacterByteIterator   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull CharacterByteToBooleanFunction function)    {
        try {
            while (this.hasNext())  {
                CharacterByteTuple tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<CharacterByteTuple> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<CharacterByteTuple> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull CharacterByteConsumer consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<CharacterByteTuple> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull CharacterByteToBooleanFunction function)    {
        this.concurrentIterate(function);
    }
}
