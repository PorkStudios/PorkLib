...
import net.daporkchop.lib.primitive.iterator.concurrent.DoubleShortConcurrentIterator;
...
import net.daporkchop.lib.primitive.tuple.DoubleShortMutableTuple;
...
/**
 * A tree hash map, using a key type of double and a value type of short.
 ...
 * @author DaPorkchop_
 */
public class DoubleShortConcurrentTreeHashMap implements DoubleShortMap    {
    final AtomicInteger size = new AtomicInteger(0);

    private final DoubleShortSubEntry[] entries = new DoubleShortSubEntry[256];
    private final DoubleToIntegerFunction keyHash;
    final DoubleToLongFunction subKeyHash;
    final int subHashLength;

    public DoubleShortConcurrentTreeHashMap()    {
        this(null, null);
    }

    public DoubleShortConcurrentTreeHashMap(DoubleToIntegerFunction keyHash, DoubleToLongFunction subKeyHash)    {
        this(keyHash, subKeyHash, 7);
    }

    public DoubleShortConcurrentTreeHashMap(DoubleToIntegerFunction keyHash, DoubleToLongFunction subKeyHash, int subHashLength)    {
    ...
    public short get(double key)   {
        DoubleShortSubEntry entry = this.getSubentry(this.keyHash.apply(key) & 0xFF, false);
        if (entry == null)  {
            return (short) 0;
        } else {
    ...
    public short put(double key, short value)   {
        DoubleShortSubEntry entry = this.getSubentry(this.keyHash.apply(key) & 0xFF, true);
    ...
    public short remove(double key)    {
        ...
        DoubleShortSubEntry entry = this.getSubentry(hash, false);
        if (entry == null)  {
            return (short) 0;
        } else {
    ...
    public void clear() {
        synchronized (this.entries) {
            for (int i = 0; i < 256; i++) {
                DoubleShortSubEntry entry = this.entries[i];
                if (entry != null)  {
                    entry.clear();
                    this.entries[i] = null;
                }
    ...
    public void forEachKey(DoubleConsumer consumer)   {
        for (int i = 0; i < 256; i++) {
            DoubleShortSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
    ...
    @Override
    public void forEachEntry(DoubleShortConsumer consumer) {
        for (int i = 0; i < 256; i++) {
            DoubleShortSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
    ...
    public void forEachEntry(DoubleShortConsumer consumer, VoidFunction complete) {
        for (int i = 0; i < 256; i++) {
            DoubleShortSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
                entry.forEachEntry(consumer);
    ...
    public DoubleConcurrentIterator keyIterator()   {
        return new DoubleConcurrentIterator()  {
            private final DoubleShortConcurrentTreeHashMap this_ = DoubleShortConcurrentTreeHashMap.this;
            ...
            private final AtomicReference<DoubleShortSubEntry.Sector> currSector = new AtomicReference(null);
            ...
            private volatile DoubleShortSubEntry entry;
            ...
            private void nextSub()   {
                ...
                while (this.subEntryIndex < 256)    {
                    DoubleShortSubEntry entry = getSubentry(this.subEntryIndex++, false);
                    if (entry != null)  {
                        this.entry = entry;
            ...
            class ThreadState   {
                public double key;
                public DoubleShortSubEntry entry;
                private final DoubleConcurrentIterator this_;
                private final DoubleShortConcurrentTreeHashMap this__;

                public ThreadState(DoubleConcurrentIterator this_, DoubleShortConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
    ...
    public ShortConcurrentIterator valueIterator() {
        return new ShortConcurrentIterator()  {
            ...
            private final AtomicReference<DoubleShortSubEntry.Sector> currSector = new AtomicReference(null);
            private final AtomicBoolean hasNextSub = new AtomicBoolean(true);
            private final DoubleShortMutableTuple next = new DoubleShortMutableTuple();
            private volatile boolean hasNext = true;
            private volatile int subEntryIndex = 0;
            private volatile DoubleShortSubEntry entry;
            ...
            class ThreadState   {
                ...
                public ThreadState(ShortConcurrentIterator this_, DoubleShortConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
    ...
    @Override
    public DoubleShortConcurrentIterator entryIterator() {
        return new DoubleShortConcurrentIterator()  {
            private final DoubleShortConcurrentTreeHashMap this_ = DoubleShortConcurrentTreeHashMap.this;
            //private final ReentrantLock lock = new ReentrantLock();
            private final ThreadLocal<ThreadState> tl = ThreadLocal.withInitial(() -> new ThreadState(this, this.this_));
            private final AtomicInteger sectorOffset = new AtomicInteger(0);
            ...
            @Override
            public synchronized DoubleShortTuple advance()   {
                if (!this.hasNext())    {
                    throw new IteratorCompleteException();
                }
                ThreadState state = this.tl.get();
                DoubleShortTuple current = state.tuple = new DoubleShortMutableTuple(this.next.k, this.next.v);
                state.entry = this.entry;
                this.findNextValue();
                return current;
            }
            ...
            class ThreadState   {
                public DoubleShortTuple tuple;
                public DoubleShortSubEntry entry;
                private final DoubleShortConcurrentIterator this_;
                private final DoubleShortConcurrentTreeHashMap this__;

                public ThreadState(DoubleShortConcurrentIterator this_, DoubleShortConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
     ...
     */
    public DoubleShortConcurrentIterator fastEntryIterator() {
        return new DoubleShortConcurrentIterator()  {
            private final DoubleShortConcurrentTreeHashMap this_ = DoubleShortConcurrentTreeHashMap.this;
            //private final ReentrantLock lock = new ReentrantLock();
            private final ThreadLocal<ThreadState> tl = ThreadLocal.withInitial(() -> new ThreadState(this, this.this_));
            ...
            private final AtomicReference<DoubleShortSubEntry.Sector> currSector = new AtomicReference(null);
            private final AtomicBoolean hasNextSub = new AtomicBoolean(true);
            private volatile DoubleShortMutableTuple next = new DoubleShortMutableTuple(0D, (short) 0);
            private volatile boolean hasNext = true;
            private volatile int subEntryIndex = 0;
            private volatile DoubleShortSubEntry entry;
            ...
            @Override
            public DoubleShortMutableTuple get()   {
                ThreadState state = this.tl.get();
                if (state.entry == null)    {
                    throw new IllegalStateException();
                } else {
            ...
            @Override
            public synchronized DoubleShortMutableTuple advance()   {
                if (!this.hasNext())    {
                    throw new IteratorCompleteException();
                }
                ThreadState state = this.tl.get();
                DoubleShortMutableTuple a = state.tuple;
            ...
            class ThreadState   {
                public DoubleShortMutableTuple tuple = new DoubleShortMutableTuple(0D, (short) 0);
                public DoubleShortSubEntry entry;
                private final DoubleShortConcurrentIterator this_;
                private final DoubleShortConcurrentTreeHashMap this__;
    ...
    private DoubleShortSubEntry getSubentry(int index, boolean create)    {
        assert index == (index & 0xFF);

        synchronized (this.entries) {
            DoubleShortSubEntry entry = (DoubleShortSubEntry) this.entries[index];
            if (entry == null && create)    {
                this.entries[index] = entry = new DoubleShortSubEntry(this);
            }
            return entry;
        }
    }
