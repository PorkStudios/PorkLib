 ...
 * Although this map WORKS, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.FloatByteIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class FloatByteTreeMap implements FloatByteMap    {
    ...
    @SuppressWarnings("unchecked")
    protected byte remove(float key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return (byte) 0; //no mapping
    ...
    public FloatIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.FloatByteIndexedTreeMap} for an implementation of this method
        return null;
    }
