...
/**
 * A tree map, using a key type of float and a value type of long.
 ...
 * Although this map 𝙒𝙊𝙍𝙆𝙎, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.FloatLongIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class FloatLongTreeMap implements FloatLongMap    {
    ...
    @SuppressWarnings("unchecked")
    protected long remove(float key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return 0L; //no mapping
    ...
    public FloatIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.FloatLongIndexedTreeMap} for an implementation of this method
        return null;
    }
