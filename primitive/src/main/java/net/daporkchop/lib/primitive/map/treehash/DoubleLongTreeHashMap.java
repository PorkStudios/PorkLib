 ...
 * overhead, and can even be made concurrent with little extra work (see {@link net.daporkchop.lib.primitive.map.concurrent.DoubleLongConcurrentTreeHashMap})
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
public class DoubleLongTreeHashMap implements DoubleLongMap    {
    ...
    public DoubleLongTreeHashMap()    {
        this(null, 0);
    }

    public DoubleLongTreeHashMap(DoubleToLongFunction hashFunc, int hashLen)    {
        ...
        this.rootPointer = this.hashLen == 0 ? new DoubleLongTreeHashMap.ValueSector(null, -1) : new DoubleLongTreeHashMap.PointerSector(null, -1);
    }
    ...
    public void forEachKey(@NonNull DoubleConsumer consumer)  {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof DoubleLongTreeHashMap.PointerSector)    {
    ...
    private Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.hashLen; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.hashLen ? new DoubleLongTreeHashMap.ValueSector(curr, off) : new DoubleLongTreeHashMap.PointerSector(curr, off);
    ...
    private class ValueSector extends Sector   {
        ...
        public long setEntry(int index, double key, long value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                DoubleLongTreeHashMap.this.size.incrementAndGet();
                return 0L;
            }
        }
        ...
        public long removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                DoubleLongTreeHashMap.this.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
