...
import net.daporkchop.lib.primitive.lambda.function.ByteToDoubleFunction;
...
/**
 * A map, using a key type of byte and a value type of double.
 * <p>
...
public interface ByteDoubleMap   {
     ...
     */
    default void putAll(@NonNull byte[] keys, @NonNull double[] values)    {
     ...
     */
    default void putAll(@NonNull ByteDoubleMap map)    {
     ...
     * @param consumer the function to run
     */
    void forEachEntry(ByteDoubleConsumer consumer);
     ...
     */
    ByteDoubleIterator entryIterator();
     ...
     */
    default double getOrDefault(byte key, double def)   {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
            this.put(key, def);
     ...
     */
    default double computeIfAbsent(byte key, @NonNull ByteToDoubleFunction func) {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
