...
import net.daporkchop.lib.primitive.iterator.concurrent.BooleanDoubleConcurrentIterator;
...
import net.daporkchop.lib.primitive.tuple.BooleanDoubleMutableTuple;
...
/**
 * A tree hash map, using a key type of boolean and a value type of double.
 ...
 * @author DaPorkchop_
 */
public class BooleanDoubleConcurrentTreeHashMap implements BooleanDoubleMap    {
    final AtomicInteger size = new AtomicInteger(0);

    private final BooleanDoubleSubEntry[] entries = new BooleanDoubleSubEntry[256];
    private final BooleanToIntegerFunction keyHash;
    final BooleanToLongFunction subKeyHash;
    final int subHashLength;

    public BooleanDoubleConcurrentTreeHashMap()    {
        this(null, null);
    }

    public BooleanDoubleConcurrentTreeHashMap(BooleanToIntegerFunction keyHash, BooleanToLongFunction subKeyHash)    {
        this(keyHash, subKeyHash, 0);
    }

    public BooleanDoubleConcurrentTreeHashMap(BooleanToIntegerFunction keyHash, BooleanToLongFunction subKeyHash, int subHashLength)    {
    ...
    public double get(boolean key)   {
        BooleanDoubleSubEntry entry = this.getSubentry(this.keyHash.apply(key) & 0xFF, false);
        if (entry == null)  {
            return 0D;
        } else {
    ...
    public double put(boolean key, double value)   {
        BooleanDoubleSubEntry entry = this.getSubentry(this.keyHash.apply(key) & 0xFF, true);
    ...
    public double remove(boolean key)    {
        ...
        BooleanDoubleSubEntry entry = this.getSubentry(hash, false);
        if (entry == null)  {
            return 0D;
        } else {
    ...
    public void clear() {
        synchronized (this.entries) {
            for (int i = 0; i < 256; i++) {
                BooleanDoubleSubEntry entry = this.entries[i];
                if (entry != null)  {
                    entry.clear();
                    this.entries[i] = null;
                }
    ...
    public void forEachKey(BooleanConsumer consumer)   {
        for (int i = 0; i < 256; i++) {
            BooleanDoubleSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
    ...
    @Override
    public void forEachEntry(BooleanDoubleConsumer consumer) {
        for (int i = 0; i < 256; i++) {
            BooleanDoubleSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
    ...
    public void forEachEntry(BooleanDoubleConsumer consumer, VoidFunction complete) {
        for (int i = 0; i < 256; i++) {
            BooleanDoubleSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
                entry.forEachEntry(consumer);
    ...
    public BooleanConcurrentIterator keyIterator()   {
        return new BooleanConcurrentIterator()  {
            private final BooleanDoubleConcurrentTreeHashMap this_ = BooleanDoubleConcurrentTreeHashMap.this;
            ...
            private final AtomicReference<BooleanDoubleSubEntry.Sector> currSector = new AtomicReference(null);
            ...
            private volatile BooleanDoubleSubEntry entry;
            ...
            private void nextSub()   {
                ...
                while (this.subEntryIndex < 256)    {
                    BooleanDoubleSubEntry entry = getSubentry(this.subEntryIndex++, false);
                    if (entry != null)  {
                        this.entry = entry;
            ...
            class ThreadState   {
                public boolean key;
                public BooleanDoubleSubEntry entry;
                private final BooleanConcurrentIterator this_;
                private final BooleanDoubleConcurrentTreeHashMap this__;

                public ThreadState(BooleanConcurrentIterator this_, BooleanDoubleConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
    ...
    public DoubleConcurrentIterator valueIterator() {
        return new DoubleConcurrentIterator()  {
            ...
            private final AtomicReference<BooleanDoubleSubEntry.Sector> currSector = new AtomicReference(null);
            private final AtomicBoolean hasNextSub = new AtomicBoolean(true);
            private final BooleanDoubleMutableTuple next = new BooleanDoubleMutableTuple();
            private volatile boolean hasNext = true;
            private volatile int subEntryIndex = 0;
            private volatile BooleanDoubleSubEntry entry;
            ...
            class ThreadState   {
                ...
                public ThreadState(DoubleConcurrentIterator this_, BooleanDoubleConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
    ...
    @Override
    public BooleanDoubleConcurrentIterator entryIterator() {
        return new BooleanDoubleConcurrentIterator()  {
            private final BooleanDoubleConcurrentTreeHashMap this_ = BooleanDoubleConcurrentTreeHashMap.this;
            //private final ReentrantLock lock = new ReentrantLock();
            private final ThreadLocal<ThreadState> tl = ThreadLocal.withInitial(() -> new ThreadState(this, this.this_));
            private final AtomicInteger sectorOffset = new AtomicInteger(0);
            ...
            @Override
            public synchronized BooleanDoubleTuple advance()   {
                if (!this.hasNext())    {
                    throw new IteratorCompleteException();
                }
                ThreadState state = this.tl.get();
                BooleanDoubleTuple current = state.tuple = new BooleanDoubleMutableTuple(this.next.k, this.next.v);
                state.entry = this.entry;
                this.findNextValue();
                return current;
            }
            ...
            class ThreadState   {
                public BooleanDoubleTuple tuple;
                public BooleanDoubleSubEntry entry;
                private final BooleanDoubleConcurrentIterator this_;
                private final BooleanDoubleConcurrentTreeHashMap this__;

                public ThreadState(BooleanDoubleConcurrentIterator this_, BooleanDoubleConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
     ...
     */
    public BooleanDoubleConcurrentIterator fastEntryIterator() {
        return new BooleanDoubleConcurrentIterator()  {
            private final BooleanDoubleConcurrentTreeHashMap this_ = BooleanDoubleConcurrentTreeHashMap.this;
            //private final ReentrantLock lock = new ReentrantLock();
            private final ThreadLocal<ThreadState> tl = ThreadLocal.withInitial(() -> new ThreadState(this, this.this_));
            ...
            private final AtomicReference<BooleanDoubleSubEntry.Sector> currSector = new AtomicReference(null);
            private final AtomicBoolean hasNextSub = new AtomicBoolean(true);
            private volatile BooleanDoubleMutableTuple next = new BooleanDoubleMutableTuple(false, 0D);
            private volatile boolean hasNext = true;
            private volatile int subEntryIndex = 0;
            private volatile BooleanDoubleSubEntry entry;
            ...
            @Override
            public BooleanDoubleMutableTuple get()   {
                ThreadState state = this.tl.get();
                if (state.entry == null)    {
                    throw new IllegalStateException();
                } else {
            ...
            @Override
            public synchronized BooleanDoubleMutableTuple advance()   {
                if (!this.hasNext())    {
                    throw new IteratorCompleteException();
                }
                ThreadState state = this.tl.get();
                BooleanDoubleMutableTuple a = state.tuple;
            ...
            class ThreadState   {
                public BooleanDoubleMutableTuple tuple = new BooleanDoubleMutableTuple(false, 0D);
                public BooleanDoubleSubEntry entry;
                private final BooleanDoubleConcurrentIterator this_;
                private final BooleanDoubleConcurrentTreeHashMap this__;
    ...
    private BooleanDoubleSubEntry getSubentry(int index, boolean create)    {
        assert index == (index & 0xFF);

        synchronized (this.entries) {
            BooleanDoubleSubEntry entry = (BooleanDoubleSubEntry) this.entries[index];
            if (entry == null && create)    {
                this.entries[index] = entry = new BooleanDoubleSubEntry(this);
            }
            return entry;
        }
    }
