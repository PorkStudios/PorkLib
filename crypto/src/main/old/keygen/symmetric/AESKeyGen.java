...
public class AESKeyGen {
    ...
    public static AESKey gen(byte[] data) {
        byte[] b = new byte[BlockCipherType.AES.blockSize];
        int i = 0;
        do {
        ...
        data = b;
        return new AESKey(new SecretKeySpec(data, "aaa"), KeyRandom.getBytes(BlockCipherType.AES.blockSize));
    }

    public static AESKey gen() {
        byte[] key = new byte[BlockCipherType.AES.blockSize];
