...
import net.daporkchop.lib.primitive.iterator.bi.IntegerCharacterIterator;
...
/**
 * A hash map, using a key type of int and a value type of char.
 ...
 * @author DaPorkchop_
 */
public class IntegerCharacterHashMap implements IntegerCharacterMap    {
    protected int[] keys;
    ...
    protected int mask;
    protected int size;
    protected final IntegerToIntegerFunction keyHash;

    public IntegerCharacterHashMap()    {
        this(16384);
    }

    public IntegerCharacterHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public IntegerCharacterHashMap(IntegerToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public IntegerCharacterHashMap(int baseSize, IntegerToIntegerFunction keyHash)    {
    ...
    @Override
    public void forEachEntry(@NonNull IntegerCharacterConsumer consumer)   {
    ...
    public IntegerCharacterIterator entryIterator() {
    ...
    protected boolean reHash(char[] values, int[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
            int key = this.keys[i];
            int j = getIndex(key);
            if (states.get(j))  {
    ...
    private class KeyIterator implements IntegerIterator {
        private int size;
        private int index = -1;
        private int next;

        public KeyIterator()    {
            this.size = IntegerCharacterHashMap.this.size;
            this.next = IntegerCharacterHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public int get()   {
            return IntegerCharacterHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = IntegerCharacterHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            IntegerCharacterHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements CharacterIterator {
        ...
        @SuppressWarnings("unchecked")
        public char get()   {
            return IntegerCharacterHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements IntegerCharacterIterator {
        private int size;
        private int index = -1;
        private int next;
        private IntegerCharacterTuple tuple;
        ...
        @Override
        public IntegerCharacterTuple advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new IntegerCharacterImmutableTuple( IntegerCharacterHashMap.this.keys[this.index],  IntegerCharacterHashMap.this.values[this.index]);
        }
