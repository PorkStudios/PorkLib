...
import net.daporkchop.lib.primitive.iterator.bi.CharacterLongIterator;
...
/**
 * A hash map, using a key type of char and a value type of long.
 ...
 * @author DaPorkchop_
 */
public class CharacterLongHashMap implements CharacterLongMap    {
    protected char[] keys;
    ...
    public CharacterLongHashMap()    {
        this(16384);
    }

    public CharacterLongHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public CharacterLongHashMap(CharacterToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public CharacterLongHashMap(int baseSize, CharacterToIntegerFunction keyHash)    {
    ...
    @Override
    public void forEachEntry(@NonNull CharacterLongConsumer consumer)   {
    ...
    public CharacterLongIterator entryIterator() {
    ...
    protected boolean reHash(long[] values, char[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
    ...
    private class KeyIterator implements CharacterIterator {
        ...
        public KeyIterator()    {
            this.size = CharacterLongHashMap.this.size;
            this.next = CharacterLongHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public char get()   {
            return CharacterLongHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = CharacterLongHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            CharacterLongHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements LongIterator {
        ...
        @SuppressWarnings("unchecked")
        public long get()   {
            return CharacterLongHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements CharacterLongIterator {
        private int size;
        private int index = -1;
        private int next;
        private CharacterLongTuple tuple;
        ...
        @Override
        public CharacterLongTuple advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new CharacterLongImmutableTuple( CharacterLongHashMap.this.keys[this.index],  CharacterLongHashMap.this.values[this.index]);
        }
