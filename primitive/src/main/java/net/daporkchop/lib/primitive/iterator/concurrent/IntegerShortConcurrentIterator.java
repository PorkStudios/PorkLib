...
import net.daporkchop.lib.primitive.lambda.function.bi.IntegerShortToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of int => short mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface IntegerShortConcurrentIterator extends IntegerShortIterator   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull IntegerShortToBooleanFunction function)    {
        try {
            while (this.hasNext())  {
                IntegerShortTuple tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<IntegerShortTuple> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<IntegerShortTuple> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull IntegerShortConsumer consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<IntegerShortTuple> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull IntegerShortToBooleanFunction function)    {
        this.concurrentIterate(function);
    }
}
