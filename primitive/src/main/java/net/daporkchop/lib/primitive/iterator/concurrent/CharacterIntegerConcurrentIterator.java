...
import net.daporkchop.lib.primitive.lambda.function.bi.CharacterIntegerToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of char => int mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface CharacterIntegerConcurrentIterator extends CharacterIntegerIterator   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull CharacterIntegerToBooleanFunction function)    {
        try {
            while (this.hasNext())  {
                CharacterIntegerTuple tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<CharacterIntegerTuple> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<CharacterIntegerTuple> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull CharacterIntegerConsumer consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<CharacterIntegerTuple> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull CharacterIntegerToBooleanFunction function)    {
        this.concurrentIterate(function);
    }
}
