 ...
 * Although this map 𝙒𝙊𝙍𝙆𝙎, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.CharacterShortIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class CharacterShortTreeMap implements CharacterShortMap    {
    protected char[] keys;
    ...
    @SuppressWarnings("unchecked")
    protected short remove(char key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return (short) 0; //no mapping
    ...
    public CharacterIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.CharacterShortIndexedTreeMap} for an implementation of this method
        return null;
    }
