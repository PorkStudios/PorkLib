...
import net.daporkchop.lib.primitive.lambda.function.DoubleToByteFunction;
...
/**
 * A map, using a key type of double and a value type of byte.
 * <p>
...
public interface DoubleByteMap   {
     ...
     */
    default void putAll(@NonNull double[] keys, @NonNull byte[] values)    {
     ...
     */
    default void putAll(@NonNull DoubleByteMap map)    {
     ...
     */
    default void removeAll(@NonNull Collection<Double> keys) {
     ...
     */
    void forEachKey(DoubleConsumer consumer);
     ...
     * @param consumer the function to run
     */
    void forEachEntry(DoubleByteConsumer consumer);
     ...
     */
    DoubleByteIterator entryIterator();
     ...
     */
    default byte getOrDefault(double key, byte def)   {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
            this.put(key, def);
     ...
     */
    default byte computeIfAbsent(double key, @NonNull DoubleToByteFunction func) {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
