 ...
 * @author DaPorkchop_
 */
public class PFutureImpl<R> implements PFuture<R> {
    protected final Object mutex = new Object[0];
    protected volatile R value;
    protected volatile Throwable t;
    protected volatile List<BiConsumer<R, Throwable>> listeners;
    ...
    @Override
    public R syncInterruptably() throws InterruptedException {
        synchronized (this.mutex)   {
            if (!this.complete)  {
                this.mutex.wait();
            }
    ...
    @Override
    public void complete(@NonNull R value) {
        synchronized (this.mutex)   {
            if (this.complete)  {
    ...
    @Override
    public void addListener(@NonNull BiConsumer<R, Throwable> callback) {
        synchronized (this.mutex) {
            if (this.complete) {
