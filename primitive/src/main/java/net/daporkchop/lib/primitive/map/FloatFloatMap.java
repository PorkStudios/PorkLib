...
import net.daporkchop.lib.primitive.lambda.function.FloatToFloatFunction;
...
/**
 * A map, using a key type of float and a value type of float.
 * <p>
...
public interface FloatFloatMap   {
     ...
     */
    default void putAll(@NonNull float[] keys, @NonNull float[] values)    {
     ...
     */
    default void putAll(@NonNull FloatFloatMap map)    {
     ...
     * @param consumer the function to run
     */
    void forEachEntry(FloatFloatConsumer consumer);
     ...
     */
    FloatFloatIterator entryIterator();
     ...
     */
    default float getOrDefault(float key, float def)   {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
            this.put(key, def);
     ...
     */
    default float computeIfAbsent(float key, @NonNull FloatToFloatFunction func) {
        if (this.containsKey(key))  {
            return this.get(key);
        } else {
