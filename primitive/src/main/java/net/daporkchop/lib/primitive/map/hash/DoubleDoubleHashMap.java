...
import net.daporkchop.lib.primitive.iterator.DoubleIterator;
import net.daporkchop.lib.primitive.iterator.bi.DoubleDoubleIterator;
...
/**
 * A hash map, using a key type of double and a value type of double.
 ...
 * @author DaPorkchop_
 */
public class DoubleDoubleHashMap implements DoubleDoubleMap    {
    ...
    public DoubleDoubleHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public DoubleDoubleHashMap(DoubleToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public DoubleDoubleHashMap(int baseSize, DoubleToIntegerFunction keyHash)    {
    ...
    @Override
    public void forEachEntry(@NonNull DoubleDoubleConsumer consumer)   {
    ...
    public DoubleDoubleIterator entryIterator() {
    ...
    protected boolean reHash(double[] values, double[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
    ...
    private class KeyIterator implements DoubleIterator {
        ...
        public KeyIterator()    {
            this.size = DoubleDoubleHashMap.this.size;
            this.next = DoubleDoubleHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public double get()   {
            return DoubleDoubleHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = DoubleDoubleHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            DoubleDoubleHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements DoubleIterator {
        ...
        @SuppressWarnings("unchecked")
        public double get()   {
            return DoubleDoubleHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements DoubleDoubleIterator {
        private int size;
        private int index = -1;
        private int next;
        private DoubleDoubleTuple tuple;
        ...
        @Override
        public DoubleDoubleTuple advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new DoubleDoubleImmutableTuple( DoubleDoubleHashMap.this.keys[this.index],  DoubleDoubleHashMap.this.values[this.index]);
        }
