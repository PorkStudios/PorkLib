...
import net.daporkchop.lib.primitive.lambda.function.bi.DoubleFloatToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of double => float mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface DoubleFloatConcurrentIterator extends DoubleFloatIterator   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull DoubleFloatToBooleanFunction function)    {
        try {
            while (this.hasNext())  {
                DoubleFloatTuple tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<DoubleFloatTuple> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<DoubleFloatTuple> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull DoubleFloatConsumer consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<DoubleFloatTuple> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull DoubleFloatToBooleanFunction function)    {
        this.concurrentIterate(function);
    }
}
