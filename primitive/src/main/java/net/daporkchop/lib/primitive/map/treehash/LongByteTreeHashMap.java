 ...
 * overhead, and can even be made concurrent with little extra work (see {@link net.daporkchop.lib.primitive.map.concurrent.LongByteConcurrentTreeHashMap})
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
public class LongByteTreeHashMap implements LongByteMap    {
    ...
    public LongByteTreeHashMap()    {
        this(null, 0);
    }

    public LongByteTreeHashMap(LongToLongFunction hashFunc, int hashLen)    {
        ...
        this.rootPointer = this.hashLen == 0 ? new LongByteTreeHashMap.ValueSector(null, -1) : new LongByteTreeHashMap.PointerSector(null, -1);
    }
    ...
    public void forEachKey(@NonNull LongConsumer consumer)  {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof LongByteTreeHashMap.PointerSector)    {
    ...
    private Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.hashLen; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.hashLen ? new LongByteTreeHashMap.ValueSector(curr, off) : new LongByteTreeHashMap.PointerSector(curr, off);
    ...
    private class ValueSector extends Sector   {
        ...
        public byte setEntry(int index, long key, byte value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                LongByteTreeHashMap.this.size.incrementAndGet();
                return (byte) 0;
            }
        }
        ...
        public byte removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                LongByteTreeHashMap.this.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
