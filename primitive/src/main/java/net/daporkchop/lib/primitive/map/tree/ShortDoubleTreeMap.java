...
/**
 * A tree map, using a key type of short and a value type of double.
 ...
 * Although this map 𝙒𝙊𝙍𝙆𝙎, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.ShortDoubleIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class ShortDoubleTreeMap implements ShortDoubleMap    {
    ...
    @SuppressWarnings("unchecked")
    protected double remove(short key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return 0D; //no mapping
    ...
    public ShortIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.ShortDoubleIndexedTreeMap} for an implementation of this method
        return null;
    }
