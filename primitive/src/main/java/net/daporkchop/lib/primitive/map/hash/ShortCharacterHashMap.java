...
import net.daporkchop.lib.primitive.iterator.bi.ShortCharacterIterator;
import net.daporkchop.lib.primitive.map.ShortCharacterMap;
import net.daporkchop.lib.primitive.tuple.ShortCharacterTuple;
import net.daporkchop.lib.primitive.tuple.ShortCharacterImmutableTuple;
...
/**
 * A hash map, using a key type of short and a value type of char.
 ...
 * @author DaPorkchop_
 */
public class ShortCharacterHashMap implements ShortCharacterMap    {
    ...
    public ShortCharacterHashMap()    {
        this(16384);
    }

    public ShortCharacterHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public ShortCharacterHashMap(ShortToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public ShortCharacterHashMap(int baseSize, ShortToIntegerFunction keyHash)    {
    ...
    @Override
    public void forEachEntry(@NonNull ShortCharacterConsumer consumer)   {
    ...
    public ShortCharacterIterator entryIterator() {
    ...
    protected boolean reHash(char[] values, short[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
    ...
    private class KeyIterator implements ShortIterator {
        ...
        public KeyIterator()    {
            this.size = ShortCharacterHashMap.this.size;
            this.next = ShortCharacterHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public short get()   {
            return ShortCharacterHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = ShortCharacterHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            ShortCharacterHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements CharacterIterator {
        ...
        @SuppressWarnings("unchecked")
        public char get()   {
            return ShortCharacterHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements ShortCharacterIterator {
        private int size;
        private int index = -1;
        private int next;
        private ShortCharacterTuple tuple;
        ...
        @Override
        public ShortCharacterTuple advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new ShortCharacterImmutableTuple( ShortCharacterHashMap.this.keys[this.index],  ShortCharacterHashMap.this.values[this.index]);
        }
