...
/**
 * A tree map, using a key type of long and a value type of long.
 ...
 * Although this map WORKS, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.LongLongIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class LongLongTreeMap implements LongLongMap    {
    protected long[] keys;
    ...
    @SuppressWarnings("unchecked")
    protected long remove(long key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return 0L; //no mapping
    ...
    public LongIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.LongLongIndexedTreeMap} for an implementation of this method
        return null;
    }
