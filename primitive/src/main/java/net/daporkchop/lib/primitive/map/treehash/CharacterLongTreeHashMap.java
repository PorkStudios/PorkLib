 ...
 * overhead, and can even be made concurrent with little extra work (see {@link net.daporkchop.lib.primitive.map.concurrent.CharacterLongConcurrentTreeHashMap})
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
public class CharacterLongTreeHashMap implements CharacterLongMap    {
    ...
    public CharacterLongTreeHashMap()    {
        this(null, 0);
    }

    public CharacterLongTreeHashMap(CharacterToLongFunction hashFunc, int hashLen)    {
        ...
        this.rootPointer = this.hashLen == 0 ? new CharacterLongTreeHashMap.ValueSector(null, -1) : new CharacterLongTreeHashMap.PointerSector(null, -1);
    }
    ...
    public void forEachKey(@NonNull CharacterConsumer consumer)  {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof CharacterLongTreeHashMap.PointerSector)    {
    ...
    private Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.hashLen; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.hashLen ? new CharacterLongTreeHashMap.ValueSector(curr, off) : new CharacterLongTreeHashMap.PointerSector(curr, off);
    ...
    private class ValueSector extends Sector   {
        ...
        public long setEntry(int index, char key, long value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                CharacterLongTreeHashMap.this.size.incrementAndGet();
                return 0L;
            }
        }
        ...
        public long removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                CharacterLongTreeHashMap.this.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
