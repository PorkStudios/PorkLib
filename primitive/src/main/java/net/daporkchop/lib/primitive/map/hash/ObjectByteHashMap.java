...
/**
 * A hash map, using a key type of K and a value type of byte.
 ...
 * @author DaPorkchop_
 */
public class ObjectByteHashMap<K extends Object> implements ObjectByteMap<K>    {
    ...
    public ObjectByteHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public ObjectByteHashMap(ObjectToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public ObjectByteHashMap(int baseSize, ObjectToIntegerFunction keyHash)    {
    ...
    protected boolean reHash(byte[] values, Object[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
            K key = (K) this.keys[i];
            int j = getIndex(key);
            if (states.get(j))  {
    ...
    private class KeyIterator implements ObjectIterator<K> {
        ...
        public KeyIterator()    {
            this.size = ObjectByteHashMap.this.size;
            this.next = ObjectByteHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public K get()   {
            return (K) ObjectByteHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = ObjectByteHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            ObjectByteHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements ByteIterator {
        ...
        @SuppressWarnings("unchecked")
        public byte get()   {
            return ObjectByteHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements ObjectByteIterator<K> {
        private int size;
        private int index = -1;
        private int next;
        private ObjectByteTuple<K> tuple;
        ...
        @Override
        public ObjectByteTuple<K> advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new ObjectByteImmutableTuple<K>( (K) ObjectByteHashMap.this.keys[this.index],  ObjectByteHashMap.this.values[this.index]);
        }
