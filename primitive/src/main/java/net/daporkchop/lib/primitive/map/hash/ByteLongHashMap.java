...
/**
 * A hash map, using a key type of byte and a value type of long.
 ...
 * @author DaPorkchop_
 */
public class ByteLongHashMap implements ByteLongMap    {
    protected byte[] keys;
    ...
    public ByteLongHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public ByteLongHashMap(ByteToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public ByteLongHashMap(int baseSize, ByteToIntegerFunction keyHash)    {
    ...
    protected boolean reHash(long[] values, byte[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
    ...
    private class KeyIterator implements ByteIterator {
        ...
        public KeyIterator()    {
            this.size = ByteLongHashMap.this.size;
            this.next = ByteLongHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public byte get()   {
            return ByteLongHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = ByteLongHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            ByteLongHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements LongIterator {
        ...
        @SuppressWarnings("unchecked")
        public long get()   {
            return ByteLongHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements ByteLongIterator {
        private int size;
        private int index = -1;
        private int next;
        private ByteLongTuple tuple;
        ...
        @Override
        public ByteLongTuple advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new ByteLongImmutableTuple( ByteLongHashMap.this.keys[this.index],  ByteLongHashMap.this.values[this.index]);
        }
