 ...
 * Although this map 𝙒𝙊𝙍𝙆𝙎, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.IntegerObjectIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class IntegerObjectTreeMap<V extends Object> implements IntegerObjectMap<V>    {
    protected int[] keys;
    ...
    @SuppressWarnings("unchecked")
    protected V remove(int key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return null; //no mapping
    ...
    public IntegerIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.IntegerObjectIndexedTreeMap} for an implementation of this method
        return null;
    }
