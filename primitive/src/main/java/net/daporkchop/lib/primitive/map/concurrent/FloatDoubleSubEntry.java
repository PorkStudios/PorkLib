...
/**
 * A single subentry for a {@link FloatDoubleConcurrentTreeHashMap}.
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
class FloatDoubleSubEntry    {
    public final AtomicInteger size = new AtomicInteger(0);
    public final FloatDoubleConcurrentTreeHashMap parent;
    ...
    public FloatDoubleSubEntry(FloatDoubleConcurrentTreeHashMap parent)   {
        this.parent = parent;
        this.rootPointer = this.parent.subHashLength == 0 ? new FloatDoubleSubEntry.ValueSector(null, -1) : new FloatDoubleSubEntry.PointerSector(null, -1);
    }
    ...
    public void forEachKey(FloatConsumer consumer)   {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof FloatDoubleSubEntry.PointerSector)    {
    ...
    public float advanceKey(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext)   {
        ...
        for (int i = sector.states.nextSetBit(curr.get() + 1); i != -1; i = sector.states.nextSetBit(i + 1)) {
            curr.set(i);
            if (sector instanceof FloatDoubleSubEntry.ValueSector)  {
    ...
    public void advanceEntry(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext, FloatDoubleMutableTuple tuple)   {
        Sector sector = sectorRef.get();
        if (sector == null) {
            sectorRef.set(sector = this.rootPointer);
        }
    ...
    public Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.parent.subHashLength; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.parent.subHashLength ? new FloatDoubleSubEntry.ValueSector(curr, off) : new FloatDoubleSubEntry.PointerSector(curr, off);
    ...
    public abstract class Sector  {
        ...
        public double setEntry(int index, float key, double value)   {
            throw new UnsupportedOperationException();
        }
    ...
    public class ValueSector extends Sector   {
        ...
        public double setEntry(int index, float key, double value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                FloatDoubleSubEntry.this.size.incrementAndGet();
                FloatDoubleSubEntry.this.parent.size.incrementAndGet();
                return 0D;
            }
        }
        ...
        public double removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                FloatDoubleSubEntry.this.size.decrementAndGet();
                FloatDoubleSubEntry.this.parent.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
