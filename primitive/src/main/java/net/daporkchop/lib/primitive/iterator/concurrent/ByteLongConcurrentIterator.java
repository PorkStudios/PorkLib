...
import net.daporkchop.lib.primitive.lambda.function.bi.ByteLongToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of byte => long mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface ByteLongConcurrentIterator extends ByteLongIterator   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ByteLongToBooleanFunction function)    {
        try {
            while (this.hasNext())  {
                ByteLongTuple tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<ByteLongTuple> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<ByteLongTuple> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ByteLongConsumer consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<ByteLongTuple> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull ByteLongToBooleanFunction function)    {
        this.concurrentIterate(function);
    }
}
