 ...
 * overhead, and can even be made concurrent with little extra work (see {@link net.daporkchop.lib.primitive.map.concurrent.IntegerShortConcurrentTreeHashMap})
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
public class IntegerShortTreeHashMap implements IntegerShortMap    {
    ...
    public IntegerShortTreeHashMap()    {
        this(null, 0);
    }

    public IntegerShortTreeHashMap(IntegerToLongFunction hashFunc, int hashLen)    {
        ...
        this.rootPointer = this.hashLen == 0 ? new IntegerShortTreeHashMap.ValueSector(null, -1) : new IntegerShortTreeHashMap.PointerSector(null, -1);
    }
    ...
    public void forEachKey(@NonNull IntegerConsumer consumer)  {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof IntegerShortTreeHashMap.PointerSector)    {
    ...
    private Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.hashLen; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.hashLen ? new IntegerShortTreeHashMap.ValueSector(curr, off) : new IntegerShortTreeHashMap.PointerSector(curr, off);
    ...
    private class ValueSector extends Sector   {
        ...
        public short setEntry(int index, int key, short value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                IntegerShortTreeHashMap.this.size.incrementAndGet();
                return (short) 0;
            }
        }
        ...
        public short removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                IntegerShortTreeHashMap.this.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
