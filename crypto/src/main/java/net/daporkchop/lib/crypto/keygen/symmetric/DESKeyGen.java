...
public class DESKeyGen {
    ...
    public static DESKey gen(byte[] data) {
        byte[] b = new byte[BlockCipherType.DES.blockSize];
        int i = 0;
        do {
        ...
        data = b;
        return new DESKey(new SecretKeySpec(data, "aaa"), KeyRandom.getBytes(BlockCipherType.DES.blockSize));
    }

    public static DESKey gen() {
        byte[] key = new byte[BlockCipherType.DES.blockSize];
