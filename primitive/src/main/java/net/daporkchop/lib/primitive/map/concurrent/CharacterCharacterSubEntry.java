...
/**
 * A single subentry for a {@link CharacterCharacterConcurrentTreeHashMap}.
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
class CharacterCharacterSubEntry    {
    public final AtomicInteger size = new AtomicInteger(0);
    public final CharacterCharacterConcurrentTreeHashMap parent;
    ...
    public CharacterCharacterSubEntry(CharacterCharacterConcurrentTreeHashMap parent)   {
        this.parent = parent;
        this.rootPointer = this.parent.subHashLength == 0 ? new CharacterCharacterSubEntry.ValueSector(null, -1) : new CharacterCharacterSubEntry.PointerSector(null, -1);
    }
    ...
    public void forEachKey(CharacterConsumer consumer)   {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof CharacterCharacterSubEntry.PointerSector)    {
    ...
    public char advanceKey(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext)   {
        ...
        for (int i = sector.states.nextSetBit(curr.get() + 1); i != -1; i = sector.states.nextSetBit(i + 1)) {
            curr.set(i);
            if (sector instanceof CharacterCharacterSubEntry.ValueSector)  {
    ...
    public void advanceEntry(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext, CharacterCharacterMutableTuple tuple)   {
        Sector sector = sectorRef.get();
        if (sector == null) {
            sectorRef.set(sector = this.rootPointer);
        }
    ...
    public Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.parent.subHashLength; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.parent.subHashLength ? new CharacterCharacterSubEntry.ValueSector(curr, off) : new CharacterCharacterSubEntry.PointerSector(curr, off);
    ...
    public abstract class Sector  {
        ...
        public char setEntry(int index, char key, char value)   {
            throw new UnsupportedOperationException();
        }
    ...
    public class ValueSector extends Sector   {
        ...
        public char setEntry(int index, char key, char value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                CharacterCharacterSubEntry.this.size.incrementAndGet();
                CharacterCharacterSubEntry.this.parent.size.incrementAndGet();
                return (char) 0;
            }
        }
        ...
        public char removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                CharacterCharacterSubEntry.this.size.decrementAndGet();
                CharacterCharacterSubEntry.this.parent.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
