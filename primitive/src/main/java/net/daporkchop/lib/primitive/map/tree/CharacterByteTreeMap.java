 ...
 * Although this map WORKS, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.CharacterByteIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class CharacterByteTreeMap implements CharacterByteMap    {
    protected char[] keys;
    ...
    @SuppressWarnings("unchecked")
    protected byte remove(char key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return (byte) 0; //no mapping
    ...
    public CharacterIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.CharacterByteIndexedTreeMap} for an implementation of this method
        return null;
    }
