...
import net.daporkchop.lib.primitive.lambda.consumer.bi.ByteIntegerConsumer;
...
import net.daporkchop.lib.primitive.iterator.concurrent.ByteIntegerConcurrentIterator;
import net.daporkchop.lib.primitive.map.ByteIntegerMap;
import net.daporkchop.lib.primitive.tuple.ByteIntegerTuple;
import net.daporkchop.lib.primitive.tuple.ByteIntegerImmutableTuple;
import net.daporkchop.lib.primitive.tuple.ByteIntegerMutableTuple;
...
/**
 * A tree hash map, using a key type of byte and a value type of int.
 ...
 * @author DaPorkchop_
 */
public class ByteIntegerConcurrentTreeHashMap implements ByteIntegerMap    {
    final AtomicInteger size = new AtomicInteger(0);

    private final ByteIntegerSubEntry[] entries = new ByteIntegerSubEntry[256];
    private final ByteToIntegerFunction keyHash;
    final ByteToLongFunction subKeyHash;
    final int subHashLength;

    public ByteIntegerConcurrentTreeHashMap()    {
        this(null, null);
    }

    public ByteIntegerConcurrentTreeHashMap(ByteToIntegerFunction keyHash, ByteToLongFunction subKeyHash)    {
        this(keyHash, subKeyHash, 0);
    }

    public ByteIntegerConcurrentTreeHashMap(ByteToIntegerFunction keyHash, ByteToLongFunction subKeyHash, int subHashLength)    {
    ...
    public int get(byte key)   {
        ByteIntegerSubEntry entry = this.getSubentry(this.keyHash.apply(key) & 0xFF, false);
        if (entry == null)  {
            return 0;
        } else {
    ...
    public int put(byte key, int value)   {
        ByteIntegerSubEntry entry = this.getSubentry(this.keyHash.apply(key) & 0xFF, true);
    ...
    public int remove(byte key)    {
        ...
        ByteIntegerSubEntry entry = this.getSubentry(hash, false);
        if (entry == null)  {
            return 0;
        } else {
    ...
    public void clear() {
        synchronized (this.entries) {
            for (int i = 0; i < 256; i++) {
                ByteIntegerSubEntry entry = this.entries[i];
                if (entry != null)  {
                    entry.clear();
                    this.entries[i] = null;
                }
    ...
    public void forEachKey(ByteConsumer consumer)   {
        for (int i = 0; i < 256; i++) {
            ByteIntegerSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
    ...
    @Override
    public void forEachEntry(ByteIntegerConsumer consumer) {
        for (int i = 0; i < 256; i++) {
            ByteIntegerSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
    ...
    public void forEachEntry(ByteIntegerConsumer consumer, VoidFunction complete) {
        for (int i = 0; i < 256; i++) {
            ByteIntegerSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
                entry.forEachEntry(consumer);
    ...
    public ByteConcurrentIterator keyIterator()   {
        return new ByteConcurrentIterator()  {
            private final ByteIntegerConcurrentTreeHashMap this_ = ByteIntegerConcurrentTreeHashMap.this;
            ...
            private final AtomicReference<ByteIntegerSubEntry.Sector> currSector = new AtomicReference(null);
            ...
            private volatile ByteIntegerSubEntry entry;
            ...
            private void nextSub()   {
                ...
                while (this.subEntryIndex < 256)    {
                    ByteIntegerSubEntry entry = getSubentry(this.subEntryIndex++, false);
                    if (entry != null)  {
                        this.entry = entry;
            ...
            class ThreadState   {
                public byte key;
                public ByteIntegerSubEntry entry;
                private final ByteConcurrentIterator this_;
                private final ByteIntegerConcurrentTreeHashMap this__;

                public ThreadState(ByteConcurrentIterator this_, ByteIntegerConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
    ...
    public IntegerConcurrentIterator valueIterator() {
        return new IntegerConcurrentIterator()  {
            ...
            private final AtomicReference<ByteIntegerSubEntry.Sector> currSector = new AtomicReference(null);
            private final AtomicBoolean hasNextSub = new AtomicBoolean(true);
            private final ByteIntegerMutableTuple next = new ByteIntegerMutableTuple();
            private volatile boolean hasNext = true;
            private volatile int subEntryIndex = 0;
            private volatile ByteIntegerSubEntry entry;
            ...
            class ThreadState   {
                ...
                public ThreadState(IntegerConcurrentIterator this_, ByteIntegerConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
    ...
    @Override
    public ByteIntegerConcurrentIterator entryIterator() {
        return new ByteIntegerConcurrentIterator()  {
            private final ByteIntegerConcurrentTreeHashMap this_ = ByteIntegerConcurrentTreeHashMap.this;
            //private final ReentrantLock lock = new ReentrantLock();
            private final ThreadLocal<ThreadState> tl = ThreadLocal.withInitial(() -> new ThreadState(this, this.this_));
            private final AtomicInteger sectorOffset = new AtomicInteger(0);
            ...
            @Override
            public synchronized ByteIntegerTuple advance()   {
                if (!this.hasNext())    {
                    throw new IteratorCompleteException();
                }
                ThreadState state = this.tl.get();
                ByteIntegerTuple current = state.tuple = new ByteIntegerMutableTuple(this.next.k, this.next.v);
                state.entry = this.entry;
                this.findNextValue();
                return current;
            }
            ...
            class ThreadState   {
                public ByteIntegerTuple tuple;
                public ByteIntegerSubEntry entry;
                private final ByteIntegerConcurrentIterator this_;
                private final ByteIntegerConcurrentTreeHashMap this__;

                public ThreadState(ByteIntegerConcurrentIterator this_, ByteIntegerConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
     ...
     */
    public ByteIntegerConcurrentIterator fastEntryIterator() {
        return new ByteIntegerConcurrentIterator()  {
            private final ByteIntegerConcurrentTreeHashMap this_ = ByteIntegerConcurrentTreeHashMap.this;
            //private final ReentrantLock lock = new ReentrantLock();
            private final ThreadLocal<ThreadState> tl = ThreadLocal.withInitial(() -> new ThreadState(this, this.this_));
            ...
            private final AtomicReference<ByteIntegerSubEntry.Sector> currSector = new AtomicReference(null);
            private final AtomicBoolean hasNextSub = new AtomicBoolean(true);
            private volatile ByteIntegerMutableTuple next = new ByteIntegerMutableTuple((byte) 0, 0);
            private volatile boolean hasNext = true;
            private volatile int subEntryIndex = 0;
            private volatile ByteIntegerSubEntry entry;
            ...
            @Override
            public ByteIntegerMutableTuple get()   {
                ThreadState state = this.tl.get();
                if (state.entry == null)    {
                    throw new IllegalStateException();
                } else {
            ...
            @Override
            public synchronized ByteIntegerMutableTuple advance()   {
                if (!this.hasNext())    {
                    throw new IteratorCompleteException();
                }
                ThreadState state = this.tl.get();
                ByteIntegerMutableTuple a = state.tuple;
            ...
            class ThreadState   {
                public ByteIntegerMutableTuple tuple = new ByteIntegerMutableTuple((byte) 0, 0);
                public ByteIntegerSubEntry entry;
                private final ByteIntegerConcurrentIterator this_;
                private final ByteIntegerConcurrentTreeHashMap this__;
    ...
    private ByteIntegerSubEntry getSubentry(int index, boolean create)    {
        assert index == (index & 0xFF);

        synchronized (this.entries) {
            ByteIntegerSubEntry entry = (ByteIntegerSubEntry) this.entries[index];
            if (entry == null && create)    {
                this.entries[index] = entry = new ByteIntegerSubEntry(this);
            }
            return entry;
        }
    }
