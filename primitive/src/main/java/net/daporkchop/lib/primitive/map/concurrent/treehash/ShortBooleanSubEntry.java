...
/**
 * A single subentry for a {@link ShortBooleanConcurrentTreeHashMap}.
 * <p>
 * DO NOT EDIT BY HAND! THIS FILE IS SCRIPT-GENERATED!
 *
 * @author DaPorkchop_
 */
class ShortBooleanSubEntry    {
    public final AtomicInteger size = new AtomicInteger(0);
    public final ShortBooleanConcurrentTreeHashMap parent;
    ...
    public ShortBooleanSubEntry(ShortBooleanConcurrentTreeHashMap parent)   {
        this.parent = parent;
        this.rootPointer = this.parent.subHashLength == 0 ? new ShortBooleanSubEntry.ValueSector(null, -1) : new ShortBooleanSubEntry.PointerSector(null, -1);
    }
    ...
    public void forEachKey(ShortConsumer consumer)   {
        ObjectConsumer<Sector> c = sector -> {
                    if (sector instanceof ShortBooleanSubEntry.PointerSector)    {
    ...
    public short advanceKey(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext)   {
        ...
        for (int i = sector.states.nextSetBit(curr.get() + 1); i != -1; i = sector.states.nextSetBit(i + 1)) {
            curr.set(i);
            if (sector instanceof ShortBooleanSubEntry.ValueSector)  {
    ...
    public void advanceEntry(AtomicReference<Sector> sectorRef, AtomicInteger curr, AtomicBoolean hasNext, ShortBooleanMutableTuple tuple)   {
        Sector sector = sectorRef.get();
        if (sector == null) {
            sectorRef.set(sector = this.rootPointer);
        }
    ...
    public Sector getSectorFor(long hash, boolean create)    {
        ...
        for (int i = 1; i < this.parent.subHashLength; i++) {
            ...
            if (next == null)    {
                if (create) {
                    next = i + 1 == this.parent.subHashLength ? new ShortBooleanSubEntry.ValueSector(curr, off) : new ShortBooleanSubEntry.PointerSector(curr, off);
    ...
    public abstract class Sector  {
        ...
        public boolean setEntry(int index, short key, boolean value)   {
            throw new UnsupportedOperationException();
        }
    ...
    public class ValueSector extends Sector   {
        ...
        public boolean setEntry(int index, short key, boolean value)   {
            ...
            if (this.states.get(index))    {
                ...
                this.states.set(index);
                this.refs++;
                ShortBooleanSubEntry.this.size.incrementAndGet();
                ShortBooleanSubEntry.this.parent.size.incrementAndGet();
                return false;
            }
        }
        ...
        public boolean removeEntry(int index)  {
            if (this.states.get(index)) {
                this.states.clear(index);
                ShortBooleanSubEntry.this.size.decrementAndGet();
                ShortBooleanSubEntry.this.parent.size.decrementAndGet();
                if (--this.refs == 0 && this.parent != null)   {
                    this.parent.removeSector(this.parentOffset);
                }
                return this.values[index];
