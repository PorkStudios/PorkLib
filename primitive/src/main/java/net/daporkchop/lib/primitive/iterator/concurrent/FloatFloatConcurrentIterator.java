...
import net.daporkchop.lib.primitive.lambda.function.bi.FloatFloatToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of float => float mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface FloatFloatConcurrentIterator extends FloatFloatIterator   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull FloatFloatToBooleanFunction function)    {
        try {
            while (this.hasNext())  {
                FloatFloatTuple tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<FloatFloatTuple> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<FloatFloatTuple> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull FloatFloatConsumer consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<FloatFloatTuple> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull FloatFloatToBooleanFunction function)    {
        this.concurrentIterate(function);
    }
}
