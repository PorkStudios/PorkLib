...
import net.daporkchop.lib.primitive.lambda.function.bi.CharacterShortToBooleanFunction;
...
/**
 * Allows parallel iteration over an iterator of char => short mappings. Implementing classes should be
 ...
 * @author DaPorkchop_
 */
public interface CharacterShortConcurrentIterator extends CharacterShortIterator   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull CharacterShortToBooleanFunction function)    {
        try {
            while (this.hasNext())  {
                CharacterShortTuple tuple = this.advance();
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull ObjectToBooleanFunction<CharacterShortTuple> function)    {
        try {
            while (this.hasNext())  {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectConsumer<CharacterShortTuple> consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull CharacterShortConsumer consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull ObjectToBooleanFunction<CharacterShortTuple> function)    {
        this.concurrentIterate(function);
    }

    @Override
    default void forEachRemaining(@NonNull CharacterShortToBooleanFunction function)    {
        this.concurrentIterate(function);
    }
}
