...
/**
 * A tree map, using a key type of short and a value type of float.
 ...
 * Although this map 𝙒𝙊𝙍𝙆𝙎, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.ShortFloatIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class ShortFloatTreeMap implements ShortFloatMap    {
    ...
    @SuppressWarnings("unchecked")
    protected float remove(short key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return 0F; //no mapping
    ...
    public ShortIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.ShortFloatIndexedTreeMap} for an implementation of this method
        return null;
    }
