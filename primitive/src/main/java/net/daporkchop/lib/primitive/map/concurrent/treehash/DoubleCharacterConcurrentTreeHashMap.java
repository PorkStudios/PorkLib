...
import net.daporkchop.lib.primitive.iterator.concurrent.DoubleCharacterConcurrentIterator;
import net.daporkchop.lib.primitive.map.DoubleCharacterMap;
import net.daporkchop.lib.primitive.tuple.DoubleCharacterTuple;
import net.daporkchop.lib.primitive.tuple.DoubleCharacterImmutableTuple;
import net.daporkchop.lib.primitive.tuple.DoubleCharacterMutableTuple;
...
/**
 * A tree hash map, using a key type of double and a value type of char.
 ...
 * @author DaPorkchop_
 */
public class DoubleCharacterConcurrentTreeHashMap implements DoubleCharacterMap    {
    final AtomicInteger size = new AtomicInteger(0);

    private final DoubleCharacterSubEntry[] entries = new DoubleCharacterSubEntry[256];
    private final DoubleToIntegerFunction keyHash;
    final DoubleToLongFunction subKeyHash;
    final int subHashLength;

    public DoubleCharacterConcurrentTreeHashMap()    {
        this(null, null);
    }

    public DoubleCharacterConcurrentTreeHashMap(DoubleToIntegerFunction keyHash, DoubleToLongFunction subKeyHash)    {
        this(keyHash, subKeyHash, 7);
    }

    public DoubleCharacterConcurrentTreeHashMap(DoubleToIntegerFunction keyHash, DoubleToLongFunction subKeyHash, int subHashLength)    {
    ...
    public char get(double key)   {
        DoubleCharacterSubEntry entry = this.getSubentry(this.keyHash.apply(key) & 0xFF, false);
        if (entry == null)  {
            return (char) 0;
        } else {
    ...
    public char put(double key, char value)   {
        DoubleCharacterSubEntry entry = this.getSubentry(this.keyHash.apply(key) & 0xFF, true);
    ...
    public char remove(double key)    {
        ...
        DoubleCharacterSubEntry entry = this.getSubentry(hash, false);
        if (entry == null)  {
            return (char) 0;
        } else {
    ...
    public void clear() {
        synchronized (this.entries) {
            for (int i = 0; i < 256; i++) {
                DoubleCharacterSubEntry entry = this.entries[i];
                if (entry != null)  {
                    entry.clear();
                    this.entries[i] = null;
                }
    ...
    public void forEachKey(DoubleConsumer consumer)   {
        for (int i = 0; i < 256; i++) {
            DoubleCharacterSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
    ...
    @Override
    public void forEachEntry(DoubleCharacterConsumer consumer) {
        for (int i = 0; i < 256; i++) {
            DoubleCharacterSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
    ...
    public void forEachEntry(DoubleCharacterConsumer consumer, VoidFunction complete) {
        for (int i = 0; i < 256; i++) {
            DoubleCharacterSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
                entry.forEachEntry(consumer);
    ...
    public DoubleConcurrentIterator keyIterator()   {
        return new DoubleConcurrentIterator()  {
            private final DoubleCharacterConcurrentTreeHashMap this_ = DoubleCharacterConcurrentTreeHashMap.this;
            ...
            private final AtomicReference<DoubleCharacterSubEntry.Sector> currSector = new AtomicReference(null);
            ...
            private volatile DoubleCharacterSubEntry entry;
            ...
            private void nextSub()   {
                ...
                while (this.subEntryIndex < 256)    {
                    DoubleCharacterSubEntry entry = getSubentry(this.subEntryIndex++, false);
                    if (entry != null)  {
                        this.entry = entry;
            ...
            class ThreadState   {
                public double key;
                public DoubleCharacterSubEntry entry;
                private final DoubleConcurrentIterator this_;
                private final DoubleCharacterConcurrentTreeHashMap this__;

                public ThreadState(DoubleConcurrentIterator this_, DoubleCharacterConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
    ...
    public CharacterConcurrentIterator valueIterator() {
        return new CharacterConcurrentIterator()  {
            ...
            private final AtomicReference<DoubleCharacterSubEntry.Sector> currSector = new AtomicReference(null);
            private final AtomicBoolean hasNextSub = new AtomicBoolean(true);
            private final DoubleCharacterMutableTuple next = new DoubleCharacterMutableTuple();
            private volatile boolean hasNext = true;
            private volatile int subEntryIndex = 0;
            private volatile DoubleCharacterSubEntry entry;
            ...
            class ThreadState   {
                ...
                public ThreadState(CharacterConcurrentIterator this_, DoubleCharacterConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
    ...
    @Override
    public DoubleCharacterConcurrentIterator entryIterator() {
        return new DoubleCharacterConcurrentIterator()  {
            private final DoubleCharacterConcurrentTreeHashMap this_ = DoubleCharacterConcurrentTreeHashMap.this;
            //private final ReentrantLock lock = new ReentrantLock();
            private final ThreadLocal<ThreadState> tl = ThreadLocal.withInitial(() -> new ThreadState(this, this.this_));
            private final AtomicInteger sectorOffset = new AtomicInteger(0);
            ...
            @Override
            public DoubleCharacterTuple get()   {
                ThreadState state = this.tl.get();
                if (state.entry == null)    {
                    throw new IllegalStateException();
                } else {
            ...
            @Override
            public synchronized DoubleCharacterTuple advance()   {
                if (!this.hasNext())    {
                    throw new IteratorCompleteException();
                }
                ThreadState state = this.tl.get();
                DoubleCharacterTuple current = state.tuple = new DoubleCharacterMutableTuple(this.next.k, this.next.v);
                state.entry = this.entry;
                this.findNextValue();
                return current;
            }
            ...
            class ThreadState   {
                public DoubleCharacterTuple tuple;
                public DoubleCharacterSubEntry entry;
                private final DoubleCharacterConcurrentIterator this_;
                private final DoubleCharacterConcurrentTreeHashMap this__;

                public ThreadState(DoubleCharacterConcurrentIterator this_, DoubleCharacterConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
     ...
     */
    public DoubleCharacterConcurrentIterator fastEntryIterator() {
        return new DoubleCharacterConcurrentIterator()  {
            private final DoubleCharacterConcurrentTreeHashMap this_ = DoubleCharacterConcurrentTreeHashMap.this;
            //private final ReentrantLock lock = new ReentrantLock();
            private final ThreadLocal<ThreadState> tl = ThreadLocal.withInitial(() -> new ThreadState(this, this.this_));
            ...
            private final AtomicReference<DoubleCharacterSubEntry.Sector> currSector = new AtomicReference(null);
            private final AtomicBoolean hasNextSub = new AtomicBoolean(true);
            private volatile DoubleCharacterMutableTuple next = new DoubleCharacterMutableTuple(0D, (char) 0);
            private volatile boolean hasNext = true;
            private volatile int subEntryIndex = 0;
            private volatile DoubleCharacterSubEntry entry;
            ...
            @Override
            public DoubleCharacterMutableTuple get()   {
                ThreadState state = this.tl.get();
                if (state.entry == null)    {
                    throw new IllegalStateException();
                } else {
            ...
            @Override
            public synchronized DoubleCharacterMutableTuple advance()   {
                if (!this.hasNext())    {
                    throw new IteratorCompleteException();
                }
                ThreadState state = this.tl.get();
                DoubleCharacterMutableTuple a = state.tuple;
            ...
            class ThreadState   {
                public DoubleCharacterMutableTuple tuple = new DoubleCharacterMutableTuple(0D, (char) 0);
                public DoubleCharacterSubEntry entry;
                private final DoubleCharacterConcurrentIterator this_;
                private final DoubleCharacterConcurrentTreeHashMap this__;
    ...
    private DoubleCharacterSubEntry getSubentry(int index, boolean create)    {
        assert index == (index & 0xFF);

        synchronized (this.entries) {
            DoubleCharacterSubEntry entry = (DoubleCharacterSubEntry) this.entries[index];
            if (entry == null && create)    {
                this.entries[index] = entry = new DoubleCharacterSubEntry(this);
            }
            return entry;
        }
    }
