...
import net.daporkchop.lib.primitive.tuple.BooleanCharacterImmutableTuple;
...
/**
 * A hash map, using a key type of boolean and a value type of char.
 ...
 * @author DaPorkchop_
 */
public class BooleanCharacterHashMap implements BooleanCharacterMap    {
    ...
    public BooleanCharacterHashMap()    {
        this(16384);
    }

    public BooleanCharacterHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public BooleanCharacterHashMap(BooleanToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public BooleanCharacterHashMap(int baseSize, BooleanToIntegerFunction keyHash)    {
    ...
    protected boolean reHash(char[] values, boolean[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
    ...
    private class KeyIterator implements BooleanIterator {
        ...
        public KeyIterator()    {
            this.size = BooleanCharacterHashMap.this.size;
            this.next = BooleanCharacterHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public boolean get()   {
            return BooleanCharacterHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = BooleanCharacterHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            BooleanCharacterHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements CharacterIterator {
        ...
        @SuppressWarnings("unchecked")
        public char get()   {
            return BooleanCharacterHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements BooleanCharacterIterator {
        private int size;
        private int index = -1;
        private int next;
        private BooleanCharacterTuple tuple;
        ...
        @Override
        public BooleanCharacterTuple advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new BooleanCharacterImmutableTuple( BooleanCharacterHashMap.this.keys[this.index],  BooleanCharacterHashMap.this.values[this.index]);
        }
