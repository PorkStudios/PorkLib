...
/**
 * Allows parallel iteration over an iterator of double values. Implementing classes should be thread safe
 * and concurrent (calling advance() should throw {@link IteratorCompleteException} instead of 0D,
 ...
 * @author DaPorkchop_
 */
public interface DoubleConcurrentIterator extends DoubleIterator   {
    /**
     ...
     *                 false otherwise.
     */
    default void concurrentIterate(@NonNull DoubleToBooleanFunction function)    {
        try {
            while (this.hasNext())  {
                double val = this.advance();
    ...
    @Override
    default void forEachRemaining(@NonNull DoubleConsumer consumer)    {
    ...
    @Override
    default void forEachRemaining(@NonNull DoubleToBooleanFunction function)   {
