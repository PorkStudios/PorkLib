 ...
 * Although this map 𝙒𝙊𝙍𝙆𝙎, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.IntegerLongIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class IntegerLongTreeMap implements IntegerLongMap    {
    protected int[] keys;
    ...
    @SuppressWarnings("unchecked")
    protected long remove(int key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return 0L; //no mapping
    ...
    public IntegerIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.IntegerLongIndexedTreeMap} for an implementation of this method
        return null;
    }
