...
import net.daporkchop.lib.primitive.tuple.ByteFloatImmutableTuple;
...
/**
 * A hash map, using a key type of byte and a value type of float.
 ...
 * @author DaPorkchop_
 */
public class ByteFloatHashMap implements ByteFloatMap    {
    protected byte[] keys;
    ...
    public ByteFloatHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public ByteFloatHashMap(ByteToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public ByteFloatHashMap(int baseSize, ByteToIntegerFunction keyHash)    {
    ...
    protected boolean reHash(float[] values, byte[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
    ...
    private class KeyIterator implements ByteIterator {
        ...
        public KeyIterator()    {
            this.size = ByteFloatHashMap.this.size;
            this.next = ByteFloatHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public byte get()   {
            return ByteFloatHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = ByteFloatHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            ByteFloatHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements FloatIterator {
        ...
        @SuppressWarnings("unchecked")
        public float get()   {
            return ByteFloatHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements ByteFloatIterator {
        private int size;
        private int index = -1;
        private int next;
        private ByteFloatTuple tuple;
        ...
        @Override
        public ByteFloatTuple advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new ByteFloatImmutableTuple( ByteFloatHashMap.this.keys[this.index],  ByteFloatHashMap.this.values[this.index]);
        }
