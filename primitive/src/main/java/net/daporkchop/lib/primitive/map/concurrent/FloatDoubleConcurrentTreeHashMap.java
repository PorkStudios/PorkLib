...
import net.daporkchop.lib.primitive.iterator.concurrent.FloatDoubleConcurrentIterator;
import net.daporkchop.lib.primitive.map.FloatDoubleMap;
import net.daporkchop.lib.primitive.tuple.FloatDoubleTuple;
import net.daporkchop.lib.primitive.tuple.FloatDoubleImmutableTuple;
import net.daporkchop.lib.primitive.tuple.FloatDoubleMutableTuple;
...
/**
 * A tree hash map, using a key type of float and a value type of double.
 ...
 * @author DaPorkchop_
 */
public class FloatDoubleConcurrentTreeHashMap implements FloatDoubleMap    {
    final AtomicInteger size = new AtomicInteger(0);

    private final FloatDoubleSubEntry[] entries = new FloatDoubleSubEntry[256];
    private final FloatToIntegerFunction keyHash;
    final FloatToLongFunction subKeyHash;
    final int subHashLength;

    public FloatDoubleConcurrentTreeHashMap()    {
        this(null, null);
    }

    public FloatDoubleConcurrentTreeHashMap(FloatToIntegerFunction keyHash, FloatToLongFunction subKeyHash)    {
        this(keyHash, subKeyHash, 3);
    }

    public FloatDoubleConcurrentTreeHashMap(FloatToIntegerFunction keyHash, FloatToLongFunction subKeyHash, int subHashLength)    {
    ...
    public double get(float key)   {
        FloatDoubleSubEntry entry = this.getSubentry(this.keyHash.apply(key) & 0xFF, false);
        if (entry == null)  {
            return 0D;
        } else {
    ...
    public double put(float key, double value)   {
        FloatDoubleSubEntry entry = this.getSubentry(this.keyHash.apply(key) & 0xFF, true);
    ...
    public double remove(float key)    {
        ...
        FloatDoubleSubEntry entry = this.getSubentry(hash, false);
        if (entry == null)  {
            return 0D;
        } else {
    ...
    public void clear() {
        synchronized (this.entries) {
            for (int i = 0; i < 256; i++) {
                FloatDoubleSubEntry entry = this.entries[i];
                if (entry != null)  {
                    entry.clear();
                    this.entries[i] = null;
                }
    ...
    public void forEachKey(FloatConsumer consumer)   {
        for (int i = 0; i < 256; i++) {
            FloatDoubleSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
    ...
    @Override
    public void forEachEntry(FloatDoubleConsumer consumer) {
        for (int i = 0; i < 256; i++) {
            FloatDoubleSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
    ...
    public void forEachEntry(FloatDoubleConsumer consumer, VoidFunction complete) {
        for (int i = 0; i < 256; i++) {
            FloatDoubleSubEntry entry = this.getSubentry(i, false);
            if (entry != null)  {
                entry.forEachEntry(consumer);
    ...
    public FloatConcurrentIterator keyIterator()   {
        return new FloatConcurrentIterator()  {
            private final FloatDoubleConcurrentTreeHashMap this_ = FloatDoubleConcurrentTreeHashMap.this;
            ...
            private final AtomicReference<FloatDoubleSubEntry.Sector> currSector = new AtomicReference(null);
            ...
            private volatile FloatDoubleSubEntry entry;
            ...
            private void nextSub()   {
                ...
                while (this.subEntryIndex < 256)    {
                    FloatDoubleSubEntry entry = getSubentry(this.subEntryIndex++, false);
                    if (entry != null)  {
                        this.entry = entry;
            ...
            class ThreadState   {
                public float key;
                public FloatDoubleSubEntry entry;
                private final FloatConcurrentIterator this_;
                private final FloatDoubleConcurrentTreeHashMap this__;

                public ThreadState(FloatConcurrentIterator this_, FloatDoubleConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
    ...
    public DoubleConcurrentIterator valueIterator() {
        return new DoubleConcurrentIterator()  {
            ...
            private final AtomicReference<FloatDoubleSubEntry.Sector> currSector = new AtomicReference(null);
            private final AtomicBoolean hasNextSub = new AtomicBoolean(true);
            private final FloatDoubleMutableTuple next = new FloatDoubleMutableTuple();
            private volatile boolean hasNext = true;
            private volatile int subEntryIndex = 0;
            private volatile FloatDoubleSubEntry entry;
            ...
            class ThreadState   {
                ...
                public ThreadState(DoubleConcurrentIterator this_, FloatDoubleConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
    ...
    @Override
    public FloatDoubleConcurrentIterator entryIterator() {
        return new FloatDoubleConcurrentIterator()  {
            private final FloatDoubleConcurrentTreeHashMap this_ = FloatDoubleConcurrentTreeHashMap.this;
            //private final ReentrantLock lock = new ReentrantLock();
            private final ThreadLocal<ThreadState> tl = ThreadLocal.withInitial(() -> new ThreadState(this, this.this_));
            private final AtomicInteger sectorOffset = new AtomicInteger(0);
            ...
            @Override
            public synchronized FloatDoubleTuple advance()   {
                if (!this.hasNext())    {
                    throw new IteratorCompleteException();
                }
                ThreadState state = this.tl.get();
                FloatDoubleTuple current = state.tuple = new FloatDoubleMutableTuple(this.next.k, this.next.v);
                state.entry = this.entry;
                this.findNextValue();
                return current;
            }
            ...
            class ThreadState   {
                public FloatDoubleTuple tuple;
                public FloatDoubleSubEntry entry;
                private final FloatDoubleConcurrentIterator this_;
                private final FloatDoubleConcurrentTreeHashMap this__;

                public ThreadState(FloatDoubleConcurrentIterator this_, FloatDoubleConcurrentTreeHashMap this__)  {
                    this.this_ = this_;
                    this.this__ = this__;
                }
     ...
     */
    public FloatDoubleConcurrentIterator fastEntryIterator() {
        return new FloatDoubleConcurrentIterator()  {
            private final FloatDoubleConcurrentTreeHashMap this_ = FloatDoubleConcurrentTreeHashMap.this;
            //private final ReentrantLock lock = new ReentrantLock();
            private final ThreadLocal<ThreadState> tl = ThreadLocal.withInitial(() -> new ThreadState(this, this.this_));
            ...
            private final AtomicReference<FloatDoubleSubEntry.Sector> currSector = new AtomicReference(null);
            private final AtomicBoolean hasNextSub = new AtomicBoolean(true);
            private volatile FloatDoubleMutableTuple next = new FloatDoubleMutableTuple(0F, 0D);
            private volatile boolean hasNext = true;
            private volatile int subEntryIndex = 0;
            private volatile FloatDoubleSubEntry entry;
            ...
            @Override
            public FloatDoubleMutableTuple get()   {
                ThreadState state = this.tl.get();
                if (state.entry == null)    {
                    throw new IllegalStateException();
                } else {
            ...
            @Override
            public synchronized FloatDoubleMutableTuple advance()   {
                if (!this.hasNext())    {
                    throw new IteratorCompleteException();
                }
                ThreadState state = this.tl.get();
                FloatDoubleMutableTuple a = state.tuple;
            ...
            class ThreadState   {
                public FloatDoubleMutableTuple tuple = new FloatDoubleMutableTuple(0F, 0D);
                public FloatDoubleSubEntry entry;
                private final FloatDoubleConcurrentIterator this_;
                private final FloatDoubleConcurrentTreeHashMap this__;
    ...
    private FloatDoubleSubEntry getSubentry(int index, boolean create)    {
        assert index == (index & 0xFF);

        synchronized (this.entries) {
            FloatDoubleSubEntry entry = (FloatDoubleSubEntry) this.entries[index];
            if (entry == null && create)    {
                this.entries[index] = entry = new FloatDoubleSubEntry(this);
            }
            return entry;
        }
    }
