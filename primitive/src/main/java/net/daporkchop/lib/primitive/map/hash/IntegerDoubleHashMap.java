...
/**
 * A hash map, using a key type of int and a value type of double.
 ...
 * @author DaPorkchop_
 */
public class IntegerDoubleHashMap implements IntegerDoubleMap    {
    protected int[] keys;
    ...
    public IntegerDoubleHashMap()    {
        this(16384);
    }

    public IntegerDoubleHashMap(int baseSize)    {
        this(baseSize, null);
    }

    public IntegerDoubleHashMap(IntegerToIntegerFunction keyHash) {
        this(16384, keyHash);
    }

    public IntegerDoubleHashMap(int baseSize, IntegerToIntegerFunction keyHash)    {
    ...
    protected boolean reHash(double[] values, int[] keys, BitSet states)    {
        return this.forEachBreaking(i -> {
            int key = this.keys[i];
            int j = getIndex(key);
            if (states.get(j))  {
    ...
    private class KeyIterator implements IntegerIterator {
        ...
        public KeyIterator()    {
            this.size = IntegerDoubleHashMap.this.size;
            this.next = IntegerDoubleHashMap.this.states.nextSetBit(0);
        }
        ...
        @SuppressWarnings("unchecked")
        public int get()   {
            return IntegerDoubleHashMap.this.keys[this.index];
        }
        ...
        protected void findNext()   {
            this.next = IntegerDoubleHashMap.this.states.nextSetBit((this.index = this.next) + 1);
        }

        @Override
        public void remove()    {
            IntegerDoubleHashMap.this.removeIndex(this.index);
            this.size--;
        }
    }
    ...
    private class ValueIterator implements DoubleIterator {
        ...
        @SuppressWarnings("unchecked")
        public double get()   {
            return IntegerDoubleHashMap.this.values[this.index];
        }
    ...
    private class EntryIterator implements IntegerDoubleIterator {
        private int size;
        private int index = -1;
        private int next;
        private IntegerDoubleTuple tuple;
        ...
        @Override
        public IntegerDoubleTuple advance()   {
            findNext();
            if (this.index == -1) throw new IllegalStateException("Reached end of array");
            return this.tuple = new IntegerDoubleImmutableTuple( IntegerDoubleHashMap.this.keys[this.index],  IntegerDoubleHashMap.this.values[this.index]);
        }
