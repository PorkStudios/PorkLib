...
/**
 * A tree map, using a key type of double and a value type of V.
 ...
 * Although this map 𝙒𝙊𝙍𝙆𝙎, it's not really particularly optimized. I'd advise using {@link net.daporkchop.lib.primitive.map.tree.index.DoubleObjectIndexedTreeMap},
 ...
 * @author DaPorkchop_
 */
public class DoubleObjectTreeMap<V extends Object> implements DoubleObjectMap<V>    {
    ...
    @SuppressWarnings("unchecked")
    protected V remove(double key, boolean shrink) {
        int hash = hashKey(key);
        if (!containsKeyHash(hash)) return null; //no mapping
    ...
    public DoubleIterator keyIterator()   {
        //see {@link net.daporkchop.lib.primitive.map.tree.index.DoubleObjectIndexedTreeMap} for an implementation of this method
        return null;
    }
